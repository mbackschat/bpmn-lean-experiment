...
The server adapts Node HTTP requests to ordered Fetch-compatible module routes, composes definition deployment, exact-version start, and exact-version Timer Start scheduling from published package entry points, and owns the lifecycle of one HTTP server, the definition and schedule SQLite owners, and one lazy reusable Temporal client connection. Startup reconciles durable nonterminal schedule rows before listening. Request authority comes only from the configured public origin, never from the untrusted Host header. Definition source limits remain owned by the definitions route and engine gateway; the transport adapter streams bodies without adding another size policy.
