...
| [`definition-correlated-message-service.ts`](src/definition-correlated-message-service.ts) and `definition-correlated-message-*.ts` | Exact-byte correlated Message capability reconstruction, target-free publication, public outcome projection, and HTTP boundary |
