...
| `sqlite-*.ts` and `in-memory-*.ts` under [`src/`](src/) | Durable and test repository implementations |
...
Tests under [`test/`](test/) mirror these owners by service, repository, route, or recovery contract.
