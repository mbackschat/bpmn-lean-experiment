...
Directories here are ownership boundaries, not deployment-service claims. M1 is implemented through the engine gateway, artifact store, public contracts, definition deployment and exact-version start workflows, public definition routes, Node server composition root, HTTP-only definition workspace, and the separate executable showcase. M2 additionally instantiates the `operate` module for an exact confirmed-start Process-instance index and search service. M3 checkpoint 1 adds the strict Work contract and durable confirmed-Process registration behind the mandatory review boundary; task aggregation, claims, audit, HTTP, and UI are not implemented yet. Direct dependencies belong in their owning package manifest, exact resolution belongs in the committed pnpm lockfile, pnpm reports the production closure, and [`license-policy.json`](license-policy.json) owns only the permitted licences and exact non-standard exceptions.
