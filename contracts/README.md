...
The checked BPMN graph and Semantic Process schemas freeze the artifact boundaries from [the Semantic Process IL spec](../docs/SEMANTIC-PROCESS-IL-SPEC.md). The bounded source compiler produces both artifacts after exact source/profile admission has mapped raw Service Task bindings to registered neutral effect identities, while the sequential, balanced-parallel, Intermediate Catch Timer, payload-free Service Task, CreateDocument data, and boundary-error execution paths consume only the Semantic Process program. The schemas validate transport shape; they do not establish raw source translation, lowering correspondence, or operational semantics.
