...
| Checked BPMN graph | Current `checkedProcess` contract; source-facing admitted graph with exact source/profile identity, identity-only Message and exact-duration Timer Start nodes, identity-only Terminate End, resolved closed `operationMessage \| directMessage` channels, profile-registered neutral effect descriptors, identity-only Exclusive Merge, exact Inclusive split/join pairing, exact divergent Event-Based Gateway identity, and one resolved Call Activity/called-Process identity, with no raw Camunda/A12 binding or runtime semantics |
