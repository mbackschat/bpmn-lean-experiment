...
| Checked BPMN graph | Current `checkedProcess` contract; source-facing admitted graph with exact source/profile identity, resolved closed `operationMessage \| directMessage` channels, profile-registered neutral effect descriptors, exact Inclusive split/join pairing, exact divergent Event-Based Gateway identity, and no raw Camunda/A12 binding or runtime semantics |
| Semantic Process program | Current `semanticProcess` contract; compiler/source/profile identity, typed control places and operations including the same closed Message channel, neutral effect descriptors, selected-branch split/join contracts, and one named Message/Timer race operation, with no mutable runtime state |
