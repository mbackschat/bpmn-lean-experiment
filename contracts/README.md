...
| Semantic Process program | Current `semanticProcess` contract; compiler/source/profile identity, typed control places and operations including channel-bound `initiateMessage`, duration-bound `initiateTimer`, no-output containing-scope `terminateScope`, and the structurally admitted but not execution-registered `awaitSequentialMultiInstanceUserTask`, the same closed Message channel, neutral effect descriptors, reusable nonempty `mergeExclusive` inputs, selected-branch split/join contracts, one named Message/Timer race operation, and one paired called-Process invocation/return, with no mutable runtime state |
