...
| BPM platform | M1 and M2 closed |
| Active work | M3 human-work checkpoint 1: engine Work operations, confirmed-start publication, gateway, and public contracts |
...
Today, the [Temporal engine runner](docs/RUNNABLE-TEMPORAL-MVP-SPEC.md) can admit exact BPMN XML, connect its Worker to an existing Temporal service, drive interactions published by a registered profile, and report final Process state. Its adapter is split by execution environment into protocol, client, Workflow, Worker, runner, and testkit packages, with no production umbrella package. Product 2 exposes deployment, admission diagnostics, definition lists, version lists, exact admitted source, exact-version start, one-shot Timer Start scheduling, one-target Message Start publication, and identity-only search over confirmed Product 2 starts through a bounded HTTP API backed by filesystem content storage and independent SQLite owners. Its React workspace uses only the public API to deploy and inspect exact definitions, start selected versions, manage one-shot schedules and Message Start receipts, and search exact Process-instance and definition identities without inferring lifecycle state or exposing Temporal facts. The registered M1 showcase preserves unseen-source deployment as an independent regression floor. The three M2 showcases cover real Schedule execution, direct Message Start ingress, and all three confirmed-start search producers through restart, exact-version binding, stable paging, private-identity exclusion, and headless-browser acceptance. The task inbox and identity system remain absent. The [complete differential/refinement pipeline](docs/TESTING-SPEC.md#complete-differentialrefinement-pipeline) exercises every registered scenario through its declared Lean, TypeScript, compatibility, durability, mutation, and replay lanes.
...
| M3 | In progress | Boolean Process-data and User Task metadata are closed; build the real inbox, typed form, claims, completion, and audit |
