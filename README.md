...
| Active work | M2 published Message Start ingress closure review, followed by instance search |
...
Today, the [Temporal engine runner](docs/RUNNABLE-TEMPORAL-MVP-SPEC.md) can admit exact BPMN XML, connect its Worker to an existing Temporal service, drive interactions published by a registered profile, and report final Process state. Its adapter is split by execution environment into protocol, client, Workflow, Worker, runner, and testkit packages, with no production umbrella package. Product 2 exposes deployment, admission diagnostics, definition lists, version lists, exact admitted source, exact-version start, one-shot Timer Start scheduling, and one-target Message Start publication through a bounded HTTP API backed by filesystem content storage and SQLite metadata. Scheduling binds an immutable definition version and Timer Start capability, while publication binds one immutable definition version and complete operation-addressed Message Start capability; both reconcile host response loss without exposing Temporal identity. Its React definition workspace uses only the public API to upload BPMN, display diagnostics, browse versions, render exact admitted source through the licensed viewer-only `bpmn-js` adapter, start the selected version, manage that version's schedule, and publish or refresh one Message Start receipt. The registered M1 showcase preserves unseen-source deployment as an independent regression floor. The M2 showcases cover real Schedule execution and direct Message Start ingress through platform restart, Worker absence and replacement, exact version-1 retention after version-2 deployment, replay, private-identity exclusion, and headless-browser acceptance. The task inbox and identity system remain absent. The [complete differential/refinement pipeline](docs/TESTING-SPEC.md#complete-differentialrefinement-pipeline) exercises every registered scenario through its declared Lean, TypeScript, compatibility, durability, mutation, and replay lanes.
...
| M2 | In progress | All five engine increments and exact-version definition scheduling are closure-reviewed; published Message Start ingress is implemented and evidence-green pending closure review, and instance search remains |
