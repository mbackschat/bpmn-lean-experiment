...
| BPM platform | M1 through M6 closed; functional MVP complete on the documented single-node deployment |
| Active work | remove Product 2's single-node scale boundary |
...
Today, the [Temporal engine runner](docs/RUNNABLE-TEMPORAL-MVP-SPEC.md) can admit exact BPMN XML, connect its Worker to an existing Temporal service, drive interactions published by a registered profile, and report final Process state. Its adapter is split by execution environment into protocol, client, Workflow, Worker, runner, and testkit packages, with no production umbrella package. Product 2 exposes deployment, admission diagnostics, definition lists, version lists, exact admitted source, exact-version start, one-shot Timer Start scheduling, one-target Message Start publication, identity-only Process-instance search, current human work, and current incident operations over confirmed Product 2 starts. The HTTP-only React workspace includes a priority-aware task inbox and definition-bound structured forms with regular scalar types, flat string lists, multiple resolution actions, and action-dependent input, plus an Operations workspace with current incidents, exact Retry and root-Process Cancel, semantic History and Diagram, and a per-instance canonical operator-audit download that keeps Work and incident actions separately ordered. Independent SQLite owners retain definition publication, Process identity, Work action state, incident action state, and audit delivery without turning any platform fact into BPMN meaning. M1 through M6 are independent regression floors; the M6 expense-exception journey exercises all structured field kinds and action routes at both desktop widths. Product 2 remains a documented single-node deployment with no production scalability or capacity claim; the approved [scalability roadmap](docs/TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md) makes its shared persistence and projection work the first post-MVP increment. The [complete differential/refinement pipeline](docs/TESTING-SPEC.md#complete-differentialrefinement-pipeline) exercises every registered scenario through its declared Lean, TypeScript, compatibility, durability, mutation, and replay lanes.
...
| M5 | Closed | Semantic History/Diagram/export, flow-node frequency/duration, and separately ordered operator history with canonical audit export are closure-reviewed |
| M6 | Closed | Definition-bound structured Human Work supports six field kinds, bounded server validation, multiple resolution actions, conditional input, canonical completion, and a retained expense-exception journey |

M1 remains the first proper platform demonstration and an independent regression floor. M3 adds the real task inbox and first typed form interaction. M6 keeps form schemas and computation in Product 2 while widening the generic engine value domain only for the values its retained model needs.

Beyond M6, the architecture leaves explicit seams for Work, Operate, Connect, Lifecycle, Intelligence, Agents, and Administration capabilities without creating empty production packages today. These are a growth horizon, not hidden M1 scope. See [the business-module map](docs/ARCHITECTURE.md#business-modules) and [competitive scope research](docs/research/BPM-PLATFORM-COMPETITIVE-SCOPE-RESEARCH.md#dependency-ordered-roadmap).
