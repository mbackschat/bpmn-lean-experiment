...
The project is currently building its fast full-pipeline walking skeleton, not a BPMN engine and not a compatibility result. It preserves the complete [architecture and assurance handoff](docs/ARCHITECTURE-AND-ASSURANCE-HANDOFF.md), ingests the official BPMN 2.0.2 sources through a local [reference corpus](docs/reference/bpmn-2.0.2/README.md), records the durable [Milestone 0 contract](docs/MILESTONE-0-FAST-PIPELINE.md), and compiles the first implementation-neutral Lean scenario and observation vocabulary.
...
| Lean | Pinned Lean 4.31.0 project with profile-independent outcomes, neutral scenario/trace/runner types, and executable contract locks |
...
Start with the [documentation registry](docs/README.md), then read the [BPMN conformance target](docs/BPMN-CONFORMANCE-TARGET.md), [Milestone 0 contract](docs/MILESTONE-0-FAST-PIPELINE.md), [implementation map](docs/IMPLEMENTATION-MAP.md), and [current plan](docs/PLAN.md).
