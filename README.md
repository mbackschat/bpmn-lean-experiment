...
The source contract, deterministic TypeScript and Lean lowerers, exact Lean definition binding, generic Lean relation/evaluator, independent TypeScript parallel evaluator, and focused Temporal parallel refinement/replay witnesses are implemented. The CIB and four-target differential parallel evidence remain open. The exact red/green sequence and resume point are in [PLAN.md](docs/PLAN.md).
