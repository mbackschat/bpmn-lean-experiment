...
async function runShowcaseHost(): Promise<void> {
  ...
  const identity = `bpmn-m2-process-search-${process.pid}`;
  ...
  try {
    ...
    const origin = await withDeadline(
      platform.listen(),
      operationDeadlineMs,
      "M2 Process-instance search platform listen",
    );
    process.stdout.write(`M2 Process-instance search showcase ready at ${origin}\n`);
