...
Treat **Focused gates green** as a commit-boundary trigger, not permission to accumulate completed lanes in one dirty worktree. After reviewing a lane, the root runs its complete affected package gate and commits that independently coherent slice before accepting another completed lane into the same uncommitted integration window or starting a long repository-wide gate. Defer that commit only when the lane depends on an explicitly named uncommitted shared contract or would leave `HEAD` uncompilable or semantically incomplete; record that dependency in the delegation contract. A broader semantic stage boundary does not authorize accumulating independently green slices.
