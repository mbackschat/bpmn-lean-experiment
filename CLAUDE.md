...
- Keep semantic `bpmn-moddle` and raw moddle objects inside `@bpmn-lean/bpmn-source`; Lean, the semantic core, and Temporal Workflow code consume only project-owned serializable contracts. If the reviewed [diagram presentation decision](docs/BPMN-DIAGRAM-PRESENTATION-DECISION.md) is implemented, its Product 2 presentation-only adapter is the sole permitted exception: it privately owns an exact parser graph, emits only validated DI bytes and closed provenance, never exports raw moddle values or generated non-DI XML, and has no admission or semantic authority.
