...
Keep the top-level `README.md` as a durable project front door. Do not put live status, evidence counts, current support inventories, or next-work narration there; link to [IMPLEMENTATION-MAP.md](docs/IMPLEMENTATION-MAP.md) and [PLAN.md](docs/PLAN.md) instead. The sole non-semantic exception is the marked repository-statistics block generated by `scripts/repository-statistics.ts`; never hand-edit its contents.
