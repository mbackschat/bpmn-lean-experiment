...
Keep the top-level `README.md` as a durable project front door. Do not put live status, evidence counts, current support inventories, or next-work narration there; link to [IMPLEMENTATION-MAP.md](docs/IMPLEMENTATION-MAP.md) and [PLAN.md](docs/PLAN.md) instead. The sole non-semantic exception is the four marked publication-statistics blocks generated by `scripts/publication-statistics.ts`; never hand-edit their contents. Tokei is a maintainer-machine publication prerequisite only, never contributor or CI infrastructure.
...
Lean's build parallelism is pinned, and [package.json](package.json)'s `config.leanBuildThreads` is its single owner. [The gate](scripts/verify.sh) and [the pnpm wrapper](scripts/pnpm.sh) both derive that value through [the pin reader](scripts/pinned-toolchain.sh) and export it as `LEAN_NUM_THREADS`, so changing the manifest field changes every Lean build and neither entry point holds a copy that can drift. An environment `LEAN_NUM_THREADS` overrides it for one run. The wrapper carries the pin as well as the gate because `test:semantic` runs `lake test`, which builds the proofs on a cold tree and reaches the same peak. The pin exists because this repository decides finite fixtures in the kernel and kernel reduction holds its terms in resident memory. Lake sizes its build pool from that variable or from the logical processor count and offers no `--jobs` option, so an unpinned build scales its peak with core count: on an 8-core host it ran four concurrent `lean` processes above 2 GB each and peaked at 7978 MB, against 2411 MB at one thread and 4699 MB at two, at roughly double the wall time. The default is the most conservative value on purpose, because the peak grows with the number of kernel-decided fixtures and that number grows with every capsule; a GitHub-hosted runner for a private repository has 7 GB and the lightweight tier 5 GB, so an unpinned build already exceeds them. Raising it is a per-machine choice for a host with spare RAM — export `LEAN_NUM_THREADS` and measure the peak rather than assuming it scales linearly. When invoking `lake` directly rather than through the gate, export the same pin.
