...
Before the first edit of an atomic change, read the owners it will grow and their remaining size headroom, which the source-hygiene gate reports as `SOURCE_HEADROOM` lines. Do not let the compiler discover the shape of the work: using type errors to enumerate change sites is efficient for finding them, but it means the plan is already mid-edit when a full owner forces an extraction. When an owner is nearly full, land the extraction as its own behavior-preserving commit before adding semantics, so the new work is not written under a size squeeze.
