...
| Create or push project tags | [`project-tags.ts`](project-tags.ts) | Use `tag:create` and `tag:push` so tag identity and preconditions stay centralized; phase and release creation fail closed unless the maintained publication statistics are current. |
