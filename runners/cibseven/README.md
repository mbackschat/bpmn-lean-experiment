...
Two test-only probes sit beside the retained sequential oracle. The generated-ID consistency probe checks the host-identity premise of `CIB-OP-0001`. The schema-valid parallel-gateway probe sends two arrivals through one incoming flow while the other branch remains open and records the resulting downstream activation as candidate deviation [`CIB-DEV-0001`](../../docs/CIB-BPMN-RELATION-REGISTER.md#cib-dev-0001--parallel-join-activates-from-duplicate-arrivals-through-one-incoming-flow). Neither probe expands the current canonical profile or production runner.
