...
The [profile artifact](profile.json) pins the same CIB Seven `2.2.0` environment used by the sequential draft so that balanced behavior can be observed reproducibly. CIB is an observation producer for this profile, not its compatibility authority. The selected meaning comes from BPMN 2.0.2 and the [parallel fork/join spec](../../docs/capsules/PARALLEL-FORK-JOIN-SPEC.md).

The artifact explicitly names `CIB-AGR-0002` and `CIB-OP-0001` for active User Task discovery, completion, and the generated-task-to-semantic-occurrence mapping used by the live-sibling stale witness. The witness keeps B live while a distinct stale A command is refused, so the refusal reaches the semantic core and remains separate from post-closure adapter classification.
