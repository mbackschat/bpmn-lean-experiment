...
The profile remains mutable because it is a calibration artifact rather than an approved immutable compatibility profile. Lean and TypeScript now independently implement its bounded lifecycle contract; that agreement does not promote the draft. Any later immutable profile must preserve or explicitly version the exact observable behavior and environment established by [M0.2](../../docs/MILESTONE-0-FAST-PIPELINE.md).
