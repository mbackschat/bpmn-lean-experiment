...
The shared Workflow-safe typed-tuple and command encoders recognize primitive Boolean and distinguish it from string and null identities. The registered Boolean Process-data profile reuses the existing User Task Update path: its dedicated live witness compiles the exact source, replaces the Worker at the active wait, retains tagged Boolean through Query, Update result, terminal receipt, Event History, and replay, and proves old-profile refusal remains state-preserving and usable. Test-owned stringification and outside-core binding mutations fail the durable relation. The witness adds no Signal, Timer, Activity, Child Workflow, cancellation, Search Attribute, Workflow type, or public fact, and effect transport remains exhaustive while profile admission rejects Boolean effect data.
