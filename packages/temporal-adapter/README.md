...
Message key correlation has two private hosting primitives: a canonical complete-address ingress with fixed configuration and start-or-recover echo validation, followed by content-bound candidate-registration prepare/finalize Updates. The registration owner keeps pending, active, and quarantined facts distinct; pending blocks its deterministic scan transition, an installed scan barrier makes prepare return `deferredByScan` without recording, exact retry retains one transaction, and finalize changes only that prepared fact to active. Count, canonical-byte, Activity-request, and future publication-result-envelope refusals are typed candidate-capacity outcomes. Live ensure response-loss/duplicate recovery and registration across Worker replacement both replay. Process-side staging, candidate fanout, global publication, target delivery, ingress continuation, public routing, and profile admission are not available.
