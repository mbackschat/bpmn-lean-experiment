...
Message key correlation has four private hosting primitives: canonical complete-address ingress, content-bound candidate registration, Process-side staged commit with an exact candidate Query, and barrier-linearized candidate fanout. Pending registration blocks scan begin; an installed barrier makes prepare return `deferredByScan` without recording; and only a complete ordered vector from running Process Workflows can reach exact finish. Missing, closed, malformed, or changed Query state fails the whole Activity and retains the barrier. Live ensure recovery, registration replacement, Process staging, successful fanout, closed-Workflow failure, and replay are green. Global publication, semantic matching, target delivery, ingress continuation, public routing, and profile admission are not available.
