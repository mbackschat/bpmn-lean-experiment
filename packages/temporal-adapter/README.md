...
Message key correlation has only its first hosting primitive: a canonical complete-address Workflow ID, one ensure-only ingress Workflow, a fixed address/protocol/capacity echo, and a shared start-or-recover client operation whose live lost-response and duplicate paths replay. Candidate registration, discovery, publication, routing, and profile admission are not available.
