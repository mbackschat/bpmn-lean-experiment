...
The protocol package owns the strict committed-execution publication page/result/export wire and its dedicated canonical UTF-8 JSON serializer. Its producer decoder binds redundant definition, Process, instance, operation, origin, owner, range, ordering, and current-position facts to the exact admitted Semantic Process program. A separate representation-free transport decoder binds the exact expected public definition, Process, instance, cursor, and limit while recursively checking closed shapes, enums, canonical order, range, batch, current, and position-fold equations; it deliberately does not claim that a public client can rederive private Program operation correspondence or control-place mappings. Structural JSON Schema validation does not replace either decoder. Publication accumulation, Workflow Query state, and Product 2 projection remain outside the protocol package.

The Workflow package owns the immutable revision-zero publication accumulator and unconditional cursor Query beside RuntimeState. Only a committed stable semantic step with a complete evaluator publication appends one numbered atomic batch; Query registration precedes the semantic loop, Query execution is pure, and command-result recording follows the append without an asynchronous boundary. The `execution-publication` client subpath validates the representation-free result at one private Workflow address. `@bpmn-lean/engine-api` interprets the existing opaque locator and exposes only public identity, cursor, and the closed observation result; no Program or host address reaches Product 2.
...
The registered Service Task incident-cancellation successor reuses the same one-attempt Activity/report policy as the incident-retry profile and adds one content-bound `bpmn-cancel-incident-process` Update. The runner copies the exact root Process and incident identity only from the committed enabled interaction, replaces the Worker while the incident is open, recovers the retained cancellation Update, and returns a typed cancelled receipt through ordinary Workflow completion. A distinct later command returns `processClosed` with that receipt. The live witness preserves the committed string Process variable, requires one technical Activity attempt, inspects a history with Update acceptance/completion and Workflow completion but no Workflow cancellation or termination family, replays it, and rejects test-owned native-termination and completed-receipt substitutions. The [specification](../../docs/capsules/SERVICE-TASK-INCIDENT-CANCELLATION-SPEC.md) owns the incident-gated semantic command and keeps arbitrary Process cancellation, native Temporal cancellation, compensation, and Product 2 operations outside this adapter lane.

The deterministic `bpmn-incident-operations` Query is registered for every profile and reads only the current committed semantic state. It returns `null` only before start, exact running incidents paired with Retry before optional Cancel, or an empty exact terminal result. Missing, duplicated, reordered, cross-incident, unsupported, or malformed state and interaction material fails closed; the Query never scans trace or Event History and never mutates state.

The dedicated live witness runs both graduated incident profiles against a real Temporal service. It requires Retry-only and Retry-before-Cancel snapshots while the incident is open, proves Query plus diagnostic trace reads append no history events, observes running zero incidents immediately after Retry while the retained diagnostic trace still contains the historical incident, corroborates completed and cancelled Query results with their exact terminal receipts through the Product 1 client, and replays both exact histories through the production Workflow bundle.

The committed-execution publication live witness uses the production Workflow and Product 1 observation client against a real Temporal service. It retains atomic start and completion batches through Worker replacement, proves cursor paging never splits a command batch, distinguishes inside-batch and ahead-of-head gaps, keeps repeated Queries history-neutral, and recovers the exact terminal page after replaying both a parallel Process and a cyclic Process. The cyclic witness publishes three distinct activations of the same Review element, while planted test-only alternatives prove that neither Temporal Event History counts nor RuntimeState differences can reconstruct the semantic revision sequence.
...
- the registered incident-cancellation successor publishes Retry before Cancel, derives the exact cancellation stimulus from the committed interaction, survives Worker replacement, retains the accepted cancellation Update, closes normally with a typed cancelled receipt, returns that receipt from a distinct late `processClosed`, preserves committed Process data, and replays without a Temporal cancellation or termination event;
- a test-owned native Workflow termination and a completed-receipt substitution each fail the cancellation relation rather than masquerading as semantic cancellation;
- the incident-operations Query exposes exact current Retry-only and Retry-before-Cancel snapshots, exposes zero current incidents after Retry despite historical trace retention, adds no Event History event, corroborates both terminal statuses through Product 1, and replays both live histories;
...
The adapter does not implement retained results beyond Temporal retention, a production canonical-observation API, evaluator Activities for Simple Boolean expressions, timer races or forms beyond the exact Intermediate Catch Timer capsule, Message payloads or key-based/global routing, modeled Message throw, effect incidents beyond the one literal-generation-1 incident family, a second semantic incident, arbitrary retry policy, general or native host cancellation beyond the exact incident-gated root command, Error propagation beyond one direct parent, Search Attributes, Continue-As-New, general Worker Versioning, general fault injection, a global task inbox, production authorization/forms, or BPMN beyond the admitted execution surfaces.
