...
Deployment-time code parses BPMN XML outside Workflow execution, checks semantic execution admission and the separate Temporal host-capability predicate, and passes an accepted Semantic Process program plus one explicit closed start stimulus to one generic Workflow. The start is either the existing manual `startProcess` with canonical string/null initial Process variables or the registered exact `triggerMessageStart` with resolved Process, Start Event, Message, Interface, and Interface Operation identity. Message Start uses direct Workflow creation, not Signal-With-Start or Update-With-Start. A wrong target is rejected before `client.start`, while an accepted-but-response-lost call remains an infrastructure ambiguity until a later product router owns a durable receipt. The live Message Start witness starts durably before a Worker polls, later exposes and completes the exact User Task through ordinary Update ingress, contains no Signal Event, and replays. Production start returns typed `started | rejected`; rejected input never calls Temporal. The Workflow lifetime derives from semantic terminal state and accepted-handler draining, never from future scenario stimuli. The `bpmn-open-user-tasks` Query invokes the core's current-state projection rather than scanning diagnostic trace history. The `bpmn-complete-user-task` Update delegates structural validation to the core, queues one completion, and returns the core-owned command outcome. Direct payload-free Message ingress uses the `bpmn-deliver-message` Signal plus a read-only result Query over a durable Workflow-local delivery ledger; malformed caller input and conflicting command identity remain adapter failures, while a wrong or stale well-formed delivery reaches the core and returns its semantic rejection. Handlers never advance semantic state directly; one Workflow loop alone calls the core.
...
Tests start clean in-memory Temporal servers, execute the retained User Task, Message Start, resumption-bounded User Task cycle, parallel, timer, Timer/User Task composition, Intermediate Catch Message, effect, mapping, boundary-error, Simple Boolean Exclusive Gateway, ordinary embedded Sub-Process, and Sub-Process Error-propagation witnesses, fetch their live histories, replay those histories through the Workflow bundle, and shut the servers down. The Message Start witness proves service acceptance while no Worker polls, later User Task progress, no Signal ingress, exact replay, and rejection of a test-owned Signal-With-Start history. The finite cycle witness crosses both back-edges, replaces its Worker between activations, refuses a stale occurrence, exits through the default route, checks Temporal's actual Continue-As-New suggestion stays false, and records the service-reported history count and size without treating either as BPMN state. No Event History fixture, legacy IR reader, patch branch, or migration path is committed while contracts are still changing freely.
