...
The shared Workflow-safe typed-tuple and command encoders recognize primitive Boolean and distinguish it from string and null identities. The Boolean Process-data profile is not registered or live-hosted at the semantic checkpoint; effect transport remains exhaustive while profile admission rejects Boolean effect data.
