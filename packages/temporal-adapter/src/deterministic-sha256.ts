/**
 * Dependency-free SHA-256 for the deterministic Workflow sandbox.
 *
 * Node crypto is intentionally unavailable because replay recomputes content-bound commands from
 * history-backed semantic state.
 */
export function deterministicSha256Hex(value: string): string {
  if (!isWellFormedWireString(value)) {
    throw new TypeError(
      "SHA-256 input must be a well-formed Unicode scalar string",
    );
  }
