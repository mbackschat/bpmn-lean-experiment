...
| [internal-transition-batch.ts](src/internal-transition-batch.ts) | Complete mixed ordinary/composed-data arming and local-control frontiers, all-pairs dependency checks, unique publication alternatives, and retained-patch dispatch; [mixed batch tests](test/internal-transition-local-control-batch.test.ts) bind accepted projections, prefix invariants, and rollback |
