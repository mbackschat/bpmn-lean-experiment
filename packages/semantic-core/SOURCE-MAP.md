...
The [local-control preparation tests](test/internal-transition-local-control-preparation.test.ts) compare retained patches and instantiated publications with actual single operations, exercise sixteen helper-level opposite-order pairs, and reject stale preparations and unsafe output multiplicity. These helpers do not enable production batches; selected-join preparation-frame completeness is excluded.
