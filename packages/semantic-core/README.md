...
- [Compensation trigger and handler proposal](../../docs/capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md) owns the strict Program declaration, hidden RuntimeState, public failed-Process result, and pure TypeScript transition account. The core constructs one root-owned trigger, consumes exact retained sources, restores frozen Event Sub-Process context, starts the complete maximal reverse-dependency frontier, completes or fail-fast cancels handlers, preserves succeeded tombstones under the retained-count bound, and rejects another active trigger before source selection. The proved Lean transition account remains the next semantic-checkpoint work; source/profile admission, live Temporal hosting, and public Compensation capability remain excluded.
