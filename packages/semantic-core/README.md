...
The execution boundary validates Semantic Process graph structure independently from exact profile-selected operation cardinality. It supports the reviewed sequential User Task, balanced two-branch parallel fork/join, exact `PT1S` Timer, finite acyclic Timer/User Task and Message/User Task compositions, one resumption-bounded User Task cycle, operation-addressed Intermediate Catch Message and direct-Message Receive Task subscriptions, exact top-level Message and Timer Start profiles, payload-free effect, data/error, Simple Boolean conditional choice, structured Inclusive Gateway, bounded Event-Based Gateway, ordinary embedded Sub-Process, direct-parent Sub-Process Error propagation, bounded called-Process Call Activity, the registered nested Terminate End profile, and one unregistered configured Task checkpoint without selecting among complete program topology predicates or adding an operation family. The configured checkpoint keeps `configuredTask` distinct in checked source and specializes it to existing Activity/Probe `awaitEffect` with empty mappings and no Error route; runtime, stimulus, state, observation, and Temporal contracts remain unchanged. The registered Message Start profile admits one exact `triggerMessageStart`; the registered Timer Start profile admits one exact `triggerTimerStart`. Each creates a fresh root occurrence and closes through its distinct initiation operation to the existing User Task wait without installing a running-instance subscription or Timer. Semantic Process programs come from the separate [source-ingestion package](../bpmn-source/README.md), and results are checked independently against retained CIB evidence where declared and the generic [Lean Semantic Process interpreter](../../BpmnSemantics/SemanticProcess.lean). The cycle evaluator executes `mergeExclusive` only for a unique offered occurrence and retains the general per-offered-occurrence relation in Lean; the selected profile proves that concurrent offers are unreachable. The parallel evaluator executes `duplicate` and per-incoming-flow `synchronize`; the timer evaluator executes `awaitTimer`, owns occurrence identity and logical deadlines, admits only exact-deadline firing, and projects `openTimers`; the Message evaluator executes `awaitMessage`, requires the complete active subscription identity and exact closed definition channel, consumes it once, and projects `openMessageSubscriptions`; `invokeProcess` and `returnProcess` create and close one distinct hidden called-instance association; `throwError` atomically cancels one exact child occurrence subtree and emits its resolved parent boundary continuation; and `terminateScope` clears one exact containing-scope occurrence while retaining it for unchanged scope completion. Canonical CIB and focused Temporal evidence are checked outside this package.
...
| [checked-process-profile-shape.ts](src/checked-process-profile-shape.ts) | Exact checked-node multisets selected by registered and checkpoint profiles |
| [semantic-program-profile-shape.ts](src/semantic-program-profile-shape.ts) | Exact operation multisets selected by registered and checkpoint profiles |
| [semantic-profile-catalog.ts](src/semantic-profile-catalog.ts) | Separate registered and unregistered checkpoint profile identities |
