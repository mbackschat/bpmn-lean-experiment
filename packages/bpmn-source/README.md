...
The bounded compiler accepts the executable `None Start Event → User Task → None End Event` Process, the exact approved balanced two-branch Parallel Gateway topology, and the exact `None Start Event → PT1S Intermediate Catch Timer Event → None End Event` Process. The checked graph preserves BPMN node and Sequence Flow identities and retains the timer's exact `PT1S` literal; the lowerer emits typed `initiate`, `awaitUserTask`, `awaitTimer`, `duplicate`, `synchronize`, and `terminate` operations with one control place per admitted Sequence Flow. Optional User Task names preserve omission as `null`. Every parser warning blocks admission, and unsupported elements or properties are rejected instead of discarded. Raw moddle objects never cross the package boundary.
