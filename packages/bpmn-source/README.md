...
The bounded compiler applies profile-selected mechanism/cardinality capability to a reusable checked-graph validator for references, node arity, reachability, co-reachability, and either whole-graph or resumption-cut acyclicity. It accepts the reviewed sequential User Task, balanced two-branch Parallel Gateway, exact `PT1S` Timer, finite acyclic Timer/User Task and Message/User Task compositions, one exact resumption-bounded User Task cycle, the exact direct-Message Receive Task semantic checkpoint, one top-level payload-free Message Start Event, one registered top-level exact-`PT1S` Timer Start Event, the unregistered nested Terminate End checkpoint, payload-free and bounded mapped Service Tasks, Simple Boolean Exclusive Gateway, structured Inclusive Gateway, bounded Event-Based Gateway, ordinary embedded Sub-Process, direct-parent Sub-Process Error propagation, and one exact namespace-qualified in-document called Process. The checked graph preserves BPMN node and Sequence Flow identities, exact conditions and mappings, definition-scope ownership, explicit boundary/Error/Terminate End identity and attachment or ownership, exact Timer literals, the complete MessageEventDefinition → Interface Operation → input Message reference chain for both passive catch and Message Start, the Receive Task's resolved direct Message ID, the Call Activity's resolved called Process ID, and nullable source-overlay identity. The start lowerers derive canonical distinct outputs only from validated Sequence Flow endpoints and preserve either the complete Message channel or the exact Timer duration. Terminate End lowering derives its consumed input and containing definition scope only from validated endpoints and ownership and emits no continuation output. The cycle profile classifies its converging Exclusive Gateway as identity-only checked source and derives the canonical nonempty `mergeExclusive` inputs solely from validated Sequence Flow endpoints. Raw namespace/QName validation remains source-only. The required root Message, Interface, and Operation names are validated but deliberately do not cross the checked-source boundary. Repeated bounded singleton containments reject before moddle can overwrite them, and the Terminate checkpoint additionally binds its exact inline definition count before raw XML is discarded. The lowerer emits the current typed Semantic Process operations with one control place per ordinary admitted Sequence Flow plus the reviewed virtual completion edges. Optional User Task names preserve omission as `null`. Every parser warning blocks admission, and unsupported elements or properties are rejected instead of discarded. Raw moddle objects never cross the package boundary.
