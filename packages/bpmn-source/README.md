...
The current bounded compiler accepts the executable `None Start Event → User Task → None End Event` Process and the exact approved balanced two-branch Parallel Gateway topology. The checked graph preserves BPMN node and Sequence Flow identities; the lowerer emits typed `initiate`, `awaitUserTask`, `duplicate`, `synchronize`, and `terminate` operations with one control place per admitted Sequence Flow. Optional User Task names preserve omission as `null`. Every parser warning blocks admission, and unsupported elements or properties are rejected instead of discarded. Raw moddle objects never cross the package boundary.
