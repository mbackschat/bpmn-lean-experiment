...
- **Runtime wait identity:** closed. `waitIdentitiesUnique` supplies cardinality one over each wait family's occurrence key, and `key_absent_after_erase` turns it into withdrawal finality for any reflexive key matcher, so the Timer stale-identity laws hold quantified rather than as finite witnesses. What remains unsupported is narrower and stated where it is used: the caller's program cannot decide whether a wait of a *called* instance is declared, because a called Process is a separate definition that `RuntimeState` does not carry.
