...
The green Compensation Event Sub-Process snapshot checkpoint is awaiting its mandatory independent review. Lean and the independently written TypeScript core validate canonical parent/handler declarations and hidden provisional/promoted context records, reserve on root/child entry, promote from the deciding pre-completion context, purge unsuccessful or unselected-root records, retain selected-root terminal ownership, and refuse count/byte overflow with exact pre-stimulus state and no trace or publication. Closure uses a focused three-arm attempt path with quantified exact delegation for declaration-free Programs; Error, bounded-Timer, root-close, replay, atomicity, and snapshot-specific commutation footprint witnesses are green under the unchanged 3 GiB proof ceiling. Source/profile admission, trigger/restoration, handler execution and order, public projection, dedicated Temporal refinement, Multi-Instance Sub-Process meaning, Transactions, and Cancel Events remain absent.
...
- **Compensation and Transactions:** the first hidden completed-work register and its selected ordinary/sequential/parallel producers are independently approved. The complete first parent-snapshot semantic checkpoint is green and review-pending with current root/child entry, success, Error, Timer, root-lifetime, capacity, replay, rollback, and commutation evidence. Source provenance and the trigger/restoration/handler order and cancellation risk band remain next after approval; Multi-Instance Sub-Process snapshots, Transaction Subprocess, and Cancel Event admission remain later and unselected.
