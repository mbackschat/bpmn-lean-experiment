...
The Message payload catch checkpoint adds the distinct checked/IL representations and payload-delivery stimulus structure to TypeScript, with content-bound stimulus equality and structural program admission. Every execution-bearing consumer remains fail-closed: the new operation cannot arm a wait or publish a transition, both delivery arms preserve state, profile execution and scenario sequencing are unregistered, and Lean and JSON Schemas remain unchanged until the semantic-checkpoint review approves the boundary.
...
- admission-only Message payload catch contracts with an operation-addressed channel, exact Event-owned direct-output identities, content-bound payload stimulus equality, a well-formed source-lowered program, and explicit no-arming/no-delivery/no-publication behavior at the checkpoint boundary
...
- **Unsupported across families:** arbitrary serial composition, arbitrary graph progress, repeated or nested scope activation, cycles outside the exact registered root-scope User Task profile, concurrent Multi-Merge execution, Standard Loop Characteristics, Multi-Instance beyond the exact registered sequential and parallel slices, Message payload execution beyond the admission-only catch-Event checkpoint, Message key and global correlation, compensation, and general Event semantics.
