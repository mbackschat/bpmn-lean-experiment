...
The Message payload catch checkpoint is approved, and both semantic accounts execute it. Lean adds the declarative delivery relation, evaluator-soundness bridge, exact routed/refusal witnesses, and quantified `runtimeStateWellFormed` preservation; strict Lean JSON decoders and encoders lock the additive checked-node, IL-operation, stimulus, interaction, and publication-kind arms. Shared JSON Schema, differential, corpus, and Temporal lanes remain open.
