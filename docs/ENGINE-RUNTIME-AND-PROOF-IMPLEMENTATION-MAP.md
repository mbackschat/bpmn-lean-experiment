...
The owner-approved Compensation trigger/handler checkpoint has completed its Program and runtime/public representation slice. Lean, TypeScript, and the shared schema strictly admit one optional root-owned declaration, a dormant `triggerCompensation` operation, zero or more canonical subjects with exact retention/snapshot coverage, the closed single-effect handler descriptor and input arms, canonical acyclic dependencies, and positive bounded limits. RuntimeState now has optional trigger and handler-effect-wait collections; deferred and active Event Sub-Process handlers preserve their exact frozen parent context; declaration-bound validation closes lifecycle, join, identity, order, failure, and exact canonical-capacity invariants.

A failed Process has a distinct terminal control, observation, and v1 receipt arm, while old Program bytes remain unchanged. Strict continuation carries all four Compensation collections, Product 2 rejects the new terminal arm, every existing profile rejects the dormant operation, and Temporal host admission returns the typed `compensationSchedulerUnavailable` refusal. No trigger construction, handler frontier, restoration transition, failure production, cancellation transition, source/profile producer, live host scheduler, or Compensation capability exists yet.

Error, bounded-Timer, root-close, replay, atomicity, and snapshot-specific commutation footprint witnesses are green under the unchanged 3 GiB proof ceiling and fail-closed cgroup acceptance rule. Source/profile admission, trigger/restoration, handler execution and order, public projection, dedicated Temporal refinement, Multi-Instance Sub-Process meaning, Transactions, and Cancel Events remain absent.
...
- strict Compensation trigger/handler Program and RuntimeState decoding and admission for the exact root/trigger ownership, zero-subject case, retention/snapshot coverage, single-effect body, identity separation, dependency DAG, limits, optional trigger/effect-wait collections, pending/active restored context, and failed Process result, with explicit dormant dispositions across every closed Lean operation reader and no transition constructor
...
- strict Compensation trigger/handler Program and representation admission with exact root/trigger ownership, zero or more canonically ordered subjects, complete retention/snapshot coverage, closed single-effect bodies, disjoint identities, canonical acyclic dependencies, bounded limits, optional trigger/effect-wait RuntimeState collections, frozen deferred/active Event Sub-Process context, declaration-bound exact validity/capacity, failed Process observation/receipt, and no producing transition
...
- implemented bounded control and Event mechanisms cover per-incoming-flow parallel synchronization, one Simple Boolean Exclusive Gateway, one structured Inclusive split and selected-subset join, one exact operation-addressed Message-versus-`PT1S` Event-Based Gateway race with bounded durable refinement, one exact Intermediate Catch Timer, operation-addressed payload-free and scalar-payload Intermediate Catch Message subscriptions, the semantic checkpoint for one context-backed correlated-payload subscription, and one interrupting payload-free Message Boundary Event on a User Task
...
- executable Event Sub-Processes, Call Activities beyond the exact two-Process empty-data normal-return slice, Transactions, Compensation trigger construction/restoration/handler frontier/order beyond the strict Program and Runtime representation plus the independently approved hidden retention and parent-snapshot checkpoints, Multi-Instance Sub-Process snapshots, general cancellation beyond the exact reviewed regional slices, and exceptional propagation beyond one direct-parent exact-code Error handler
...
- **Compensation and Transactions:** the first hidden completed-work register and its selected ordinary/sequential/parallel producers are independently approved. The complete first parent-snapshot semantic checkpoint is independently approved with current root/child entry, success, Error, Timer, root-lifetime, capacity, replay, rollback, and commutation evidence. The strict trigger/handler Program declaration, optional RuntimeState execution collections, deferred/active restored context, declaration-bound validity and capacity, failed Process observation/receipt, strict continuation, downstream terminal propagation, Product 2 rejection, and host refusal are implemented without enabling execution. Independent TypeScript and Lean trigger/restoration/frontier, completion/failure/cancellation, stale-result, and capacity transitions are next. Source provenance follows that review; Multi-Instance Sub-Process snapshots, Transaction Subprocess, and Cancel Event admission remain later and unselected.
