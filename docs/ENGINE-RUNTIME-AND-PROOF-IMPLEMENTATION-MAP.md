...
The Message key-correlation slice has reached its mandatory pre-host semantic checkpoint. Lean and the independently written TypeScript core implement the exact scalar selectors, complete-address finite matcher, current candidate projection, correlated wait arming, and target-revalidating per-instance delivery. Exact zero/unique/ambiguous matching, candidate-order irrelevance, positive ingress ordinal, stale/current-value refusal, no payload write, outgoing-token production, non-target preservation, and old direct-profile exclusion are green. The correlated arm is integrated into the complete internal-commutation census and prepared footprint through its Process-property read and complete candidate-publication atom. Independent checkpoint review, profile registration, global Temporal discovery/ingress, differential scenarios, corpus, and closure remain ahead.
