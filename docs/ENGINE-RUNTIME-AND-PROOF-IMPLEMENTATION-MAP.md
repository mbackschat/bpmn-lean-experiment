...
- **absent:** live production Temporal refinement witnesses and execution registration. The private exact-version capacity owner is measured and green, while quantified well-formedness preservation across the four arms stays a deliberately open lane with a decided instance in its place, and Lean's canonical-byte measure does not model JSON escapes, so it undercounts against the core and is recorded as an open cross-target lane rather than as agreement
