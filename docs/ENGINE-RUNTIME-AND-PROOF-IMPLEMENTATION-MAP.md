...
- four controller-specific conjuncts join the well-formedness predicate in both languages: exact Activity-record ownership, exact program/operation/owner/User-Task/one-Timer binding, controller-identity uniqueness, and non-exhaustion. Program-binding negatives include a live child-scope body and a changed declared task, and the body-turnover preservation theorem carries the profile-specific binding premise explicitly
...
- Lean decodes and validates the `awaitSequentialMultiInstanceUserTask` Program operation, rewrites it into one definition-fact arm, states entry, nonfinal iteration, final completion, and interruption as declarative relations, and proves the three evaluators produce only transitions those relations permit. Laws and kernel-decided facts cover a three-item run, interruption, empty collection, program-aware binding refusals, and finite-snapshot conditional closure under explicit accepted-completion-or-Timer progress without claiming human or host fairness
