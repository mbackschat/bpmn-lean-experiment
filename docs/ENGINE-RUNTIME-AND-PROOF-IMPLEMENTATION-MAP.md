...
The owner-approved Compensation trigger/handler checkpoint has completed its first Program-contract slice. Lean, TypeScript, and the shared schema strictly admit one optional root-owned declaration, a dormant `triggerCompensation` operation, zero or more canonical subjects with exact retention/snapshot coverage, the closed single-effect handler descriptor and input arms, canonical acyclic dependencies, and positive bounded limits. Every existing profile rejects the new operation, every runtime/publication dispatcher classifies it as unavailable, and Temporal host admission returns the typed `compensationSchedulerUnavailable` refusal. No trigger transition, RuntimeState execution collection, handler frontier, restoration, failure, cancellation, source/profile producer, public surface, or hosting refinement exists yet.
...
- strict Compensation trigger/handler Program-contract decoding and admission for the exact root/trigger ownership, zero-subject case, retention/snapshot coverage, single-effect body, identity separation, dependency DAG, and limits, with explicit dormant dispositions across every closed Lean operation reader and no transition constructor
...
- strict Program-only Compensation trigger/handler admission with exact root/trigger ownership, zero or more canonically ordered subjects, complete retention/snapshot coverage, closed single-effect bodies, disjoint identities, canonical acyclic dependencies, bounded limits, and explicit dormant classification across every exhaustive runtime/publication reader
...
- executable Event Sub-Processes, Call Activities beyond the exact two-Process empty-data normal-return slice, Transactions, Compensation trigger execution/restoration/handler frontier/order beyond the strict Program-only declaration and the independently approved hidden retention and parent-snapshot checkpoints, Multi-Instance Sub-Process snapshots, general cancellation beyond the exact reviewed regional slices, and exceptional propagation beyond one direct-parent exact-code Error handler
