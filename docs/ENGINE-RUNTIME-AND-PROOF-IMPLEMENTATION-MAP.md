...
Every Lean law consuming `waitIdentitiesUnique` therefore assumes it. Three rules are also implemented narrower than stated and are marked in their own rows by the owning specification, which owns rule-level scope.
