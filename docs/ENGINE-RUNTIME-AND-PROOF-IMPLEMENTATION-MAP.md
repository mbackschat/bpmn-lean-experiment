...
The sequential Multi-Instance checked node, Semantic Process operation, runtime controller, four state transitions, public progress, E2 occurrence accounting, and Temporal managed-deadline host class are implemented, and the profile is still not execution-registered.
...
- **absent:** live Temporal refinement witnesses, the measured capacity budget owner, and execution registration. Quantified well-formedness preservation across the four arms stays a deliberately open lane with a decided instance in its place, and Lean's canonical-byte measure does not model JSON escapes, so it undercounts against the core and is recorded as an open cross-target lane rather than as agreement
