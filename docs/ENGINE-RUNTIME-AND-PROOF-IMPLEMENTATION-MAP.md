...
The Message key-correlation slice has reached its mandatory pre-host semantic checkpoint. Lean and the independently written TypeScript core implement the exact scalar selectors, complete-address finite matcher, current candidate projection, correlated wait arming, and target-revalidating per-instance delivery. Exact zero/unique/ambiguous matching, candidate-order irrelevance, positive ingress ordinal, stale/current-value refusal, no payload write, outgoing-token production, non-target preservation, and old direct-profile exclusion are green. The correlated arm is integrated into the complete internal-commutation census and prepared footprint through its Process-property read and complete candidate-publication atom. Independent checkpoint review approved correction target `1ce28ed5`; profile registration, global Temporal discovery/ingress, differential scenarios, corpus, and closure remain ahead.
...
The independently approved Compensation trigger/handler proposal admits one optional root declaration, canonical subjects covering exact retention/snapshot sources, closed single-effect handlers, acyclic dependencies, and bounded limits. Optional trigger/effect-wait state retains deferred or active frozen context, while validation enforces exact identity, lifecycle, join, ordering, one-active-trigger, one failed trigger, retained-count, and byte invariants.

The independent TypeScript core atomically consumes exact sources while preserving retention chronology, restores frozen context, and activates the complete maximal reverse-dependency frontier. Success starts newly complete predecessors or one final continuation; failure commits a typed terminal state and removes same-trigger live regions. Both capacity checks precede consumption, succeeded tombstones remain charged, a zero-subject throw creates no record, and active-trigger refusal precedes empty-source selection.

Private anchors publish only existing lifecycle arms. Strict continuation carries all four Compensation collections and anchors; Product 2 rejects failed ingress, and registered profiles and Temporal reject the operation.

One checkpoint-only source identity lowers the travel-booking definition. Both implementations derive its required String start binding from the Program, prospectively construct the production promoted snapshot and B/C first frontier, and reject either inherited canonical-byte overflow before state creation. Both source orders reach that frontier with the frozen restored argument; an exhaustive exact-source guard proves it exceeds every later frontier in the three legal B/A/C, B/C/A, and C/B/A success orders.

Product 1 rejects malformed and over-cap starts as semantic-process unsupported and returns `compensationSchedulerUnavailable` for the exact valid start, always before Workflow creation. Lean proves the bounded trigger, frontier, completion, failure, cancellation, stale-result, retained-tombstone, zero-subject, capacity, start-admission, soundness, and trace/replay account.
...
- Message key-correlation checkpoint proofs for exact scalar-path decoding/evaluation, complete-address isolation, finite-matcher permutation invariance and exact cardinality, candidate projection from committed Process data, per-instance evaluator soundness, full target revalidation, subscription withdrawal, no payload write, outgoing-token production, refusal and non-target preservation, strict codecs, and the correlated prepared-patch/footprint laws
- boundary Compensation retention checkpoint proofs and kernel-decided producer witnesses for declaration admission, exact completion-family matching, ordinary and zero/one/many Multi-Instance all-success production, interruption and actual-early non-retention, append chronology, duplicate/count/byte refusal before completion mutation, exact pre-state preservation, root-register lifetime, zero-item outer identity issuance, strict decoding, and canonical escaped-string byte measurement
- independently approved Compensation Event Sub-Process snapshot proofs for declaration-derived dormant-handler admission, record census and root ownership, exact reservation/promotion/purge and capacity laws, aggregate-valid applied child-entry, bounded-child-entry, and completion successors, focused three-arm attempt/trace closure, whole-stimulus rejection with no lifecycle or publication, emitted-trace replay and result erasure, declaration-free evaluator delegation, Error/Timer/root discriminators, and snapshot-specific commutation atoms
- strict Compensation Program/RuntimeState and checkpoint-only checked-source decoding and admission, including exact root/trigger ownership, retention/snapshot coverage, dormant-scope validation, bodies, dependencies, limits, hidden collections, restored context, and failed results; independent lowering and definition binding preserve old omission, while proved trigger, frontier, completion, failure, cancellation, stale-result, tombstone, capacity, soundness, and trace/replay witnesses retain the existing runtime account
...
- checkpoint-only Message key-correlation execution with the exact context-specific scalar paths, complete immutable definition address, malformed-evidence refusal, pure finite no/unique/ambiguous matching without an order winner, candidate projection from committed Process data, global interaction without a caller target, positive ingress ordinal, full per-instance revalidation, atomic subscription withdrawal and output-token production, no payload write, and exact-state refusal
- checkpoint-only boundary Compensation retention with an optional flat-root declaration and hidden register, exact ordinary/outer-Multi-Instance completion producers, immutable append-only records, duplicate/count/byte refusal before completion mutation, zero-item outer identity issuance, start/normal-close lifetime, owner-keyed zero-entry commutation footprints, and no quiescence or public-observation effect
- independently approved Compensation Event Sub-Process snapshots with an optional declaration and hidden provisional/promoted record collection, exact context capture, entry/completion/Error/Timer/root integration, three-arm closure refusal precedence, whole-stimulus rollback, declaration-free behavior preservation, and snapshot-aware commutation footprints without public projection or an emitted profile
- strict Compensation checked/Program/runtime representation and source-to-Program binding with exact root/trigger ownership, canonical subjects, complete retention/snapshot coverage, dormant handler, closed bodies/restored binding, disjoint identities, acyclic dependencies, fixed limits, optional trigger/effect-wait collections, one-active/exact-failed validity, failed Process observation/receipt, strict continuation, one Program-derived exact String checkpoint start binding with prospective production-capacity admission, and old checked-field omission
- pure TypeScript Compensation execution with atomic source consumption, simultaneous maximal reverse-dependency frontiers, exact restored arguments, success and fail-fast failure/cancellation, final continuation, stale-result and pre-mutation capacity refusal, retained-tombstone accounting, zero-subject execution, and private throw/handler/body flow-node occurrence lifecycle
...
- implemented bounded control mechanisms cover per-incoming-flow parallel synchronization, one Simple Boolean Exclusive Gateway, one structured Inclusive split and selected-subset join, and one exact operation-addressed Message-versus-`PT1S` Event-Based Gateway race with bounded durable refinement
- implemented bounded Event mechanisms cover one exact Intermediate Catch Timer, operation-addressed payload-free and scalar-payload Intermediate Catch Message subscriptions, the semantic checkpoint for one context-backed correlated-payload subscription, and one interrupting payload-free Message Boundary Event on a User Task
- implemented bounded Activity and scope mechanisms cover sequential User Task lifecycle/refusal, one direct-Message payload-free Receive Task, bounded successful Service Task execution, one exact configured external-effect Task extension, bounded string/null input/output mapping, one exact-code attached interrupting Service Task Error route, ordinary one-level embedded Sub-Process completion, one direct-parent exact-code Error End propagation with regional cancellation, and one exact nested Terminate End that cancels only its containing Sub-Process occurrence before ordinary parent continuation
...
- **Event subscriptions and scope handlers:** the exact one-key correlation semantic checkpoint now establishes source-to-IL identity, finite matching, per-instance consumption, and commutation classification, but global discovery/ingress, end-to-end refinement, profile registration, and closure remain its next gate. Broader reusable subscription lifetime, payload and correlation identity, modeled throw/consume behavior, cancellation, and deterministic races must precede wider boundary handlers and interrupting then non-interrupting Event Subprocesses. Message, Timer, Signal, Error, and Escalation triggers remain distinct propositions even when they reuse one kernel.
- **Compensation and Transactions:** approved completed-work/snapshot accounts cover ordinary/Multi-Instance producers plus failure, capacity, replay, rollback, and commutation. Implemented: strict Program/Runtime admission, restored context, Process failure/continuation, Product 2 rejection, source provenance/lowering, and host refusal. TypeScript and Lean implement atomic source consumption, reverse-dependency frontiers, fail-fast failure/cancellation, capacity/refusal, retained triggers, private lifecycle, and Program-derived capacity-total start admission. Exact-source starts are green pending checkpoint review. Three long-String Lean reductions hit the fixed 3 GiB ceiling. The [start-data proposal](capsules/COMPENSATION-DURABILITY-START-DATA-REPAIR-PROPOSAL.md#lean-assurance-correction-after-the-hard-ceiling) retains the same claim through a universal implication, short kernel boundaries, and independent TypeScript inherited-limit discriminators; neither a higher limit nor `native_decide` is selected. Concurrent same-root triggers, sibling-trigger terminal lifecycle, Multi-Instance Sub-Process snapshots, Transaction Subprocess, and Cancel Event admission remain unselected.
...
- **Unsupported across families:** arbitrary serial composition, arbitrary graph progress, repeated or nested scope activation, cycles outside the exact registered root-scope User Task profile, concurrent Multi-Merge execution, Standard Loop Characteristics, Multi-Instance beyond the exact registered sequential and parallel slices, Message payloads beyond the exact single-output catch-Event profile, public Message key/global correlation, Compensation beyond the exact source and private semantic checkpoints, and general Event semantics.
