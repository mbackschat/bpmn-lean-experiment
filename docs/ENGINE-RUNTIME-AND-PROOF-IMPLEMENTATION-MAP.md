...
- boundary Compensation retention checkpoint proofs for declaration admission, exact completion-family matching, zero/one/many all-success discrimination, interruption non-retention, append chronology, duplicate/count/byte refusal, exact pre-state preservation, root-register lifetime, zero-item outer identity issuance, strict decoding, and canonical escaped-string byte measurement
...
- checkpoint-only boundary Compensation retention with an optional flat-root declaration and hidden register, exact ordinary/outer-Multi-Instance completion facts, immutable append-only records, duplicate/count/byte refusal before mutation, start/normal-close lifetime, no quiescence or public-observation effect, and no integration into existing completion producers
...
- **Compensation and Transactions:** the first hidden completed-work register is implemented but not producer-integrated. The next high-risk slice must stage retention before every selected ordinary/sequential/parallel completion mutation and lock zero-item identity plus early/interrupted non-retention; Compensation Event Sub-Process parent snapshots follow before handler selection, dependency/reverse ordering, cancellation, failure, replay, Transaction Subprocess, or Cancel Event admission.
