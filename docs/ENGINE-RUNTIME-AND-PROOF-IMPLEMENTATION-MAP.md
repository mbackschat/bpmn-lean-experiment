...
The Activity data-input profile carries no DataOutput, output mediation, other data-bearing Task host, collection, transformation, expression runtime, or later data-ingress command. A Process waiting on an absent source stays durably Running; that is a recorded liveness limitation, not a hidden retry.
...
- **Data and executable Tasks:** the nearest unsupported claim is one DataOutput and DataOutputAssociation copying a completed Activity value back to Process scope. In separate prerequisite-ordered capsules, broaden standard DataInput, DataOutput, Data Association, scoped mapping, and Activity data lifetime before selecting Script Task, Business Rule Task, or Send Task execution profiles demanded by independent whole-model families. Language runtimes, DMN, outbound transport, and CIB bindings remain explicit overlays or host effects; none may silently enter the pure semantic core.
...
- **Unsupported across families:** arbitrary serial composition, arbitrary graph progress, repeated or nested scope activation, cycles outside the exact registered root-scope User Task profile, concurrent Multi-Merge execution, Standard Loop Characteristics, Multi-Instance beyond the exact registered sequential and parallel slices, Message payload, key, and global correlation, compensation, and general Event semantics.
