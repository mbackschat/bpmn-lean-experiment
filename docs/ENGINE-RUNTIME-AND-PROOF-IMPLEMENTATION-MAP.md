...
Private anchors publish only the existing start/completed/cancelled lifecycle. Strict continuation carries all four Compensation collections and those anchors; Product 2 rejects failed ingress, existing profiles reject the operation, and Temporal host admission returns `compensationSchedulerUnavailable`. The proved Lean transition account remains open. Source/profile admission, dedicated Temporal refinement, live scheduling, public capability, Multi-Instance Sub-Process meaning, Transactions, and Cancel Events remain absent; earlier proof evidence remains green under the unchanged 3 GiB ceiling.
...
- strict Compensation trigger/handler Program and representation with exact root/trigger ownership, zero or more canonically ordered subjects, complete retention/snapshot coverage, closed single-effect bodies, disjoint identities, canonical acyclic dependencies, optional trigger/effect-wait collections, frozen deferred/active Event Sub-Process context, one-active-trigger and exact failed-trigger validity, failed Process observation/receipt, and strict continuation
...
- **Compensation and Transactions:** the hidden completed-work register and its ordinary and current Multi-Instance producers are independently approved. The parent-snapshot checkpoint is independently approved with root/child lifecycle, failure, capacity, replay, rollback, and commutation evidence. Strict trigger/handler Program and RuntimeState admission, restored context, failed Process propagation, continuation, Product 2 rejection, and host refusal are implemented. TypeScript additionally implements atomic source consumption, reverse-dependency frontiers, success, fail-fast failure/cancellation, stale-result refusal, retained-count and zero-subject capacity, and private occurrence lifecycle. The proved Lean transitions are next. Source provenance follows checkpoint review; concurrent same-root triggers, sibling-trigger terminal lifecycle, Multi-Instance Sub-Process snapshots, Transaction Subprocess, and Cancel Event admission remain unselected.
