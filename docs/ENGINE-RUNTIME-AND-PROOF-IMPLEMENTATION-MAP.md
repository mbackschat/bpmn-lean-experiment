...
- a whole-state replacement in both languages that withdraws the outgoing body wait, arms the incoming one from the body element's own counter family, advances that counter, and rewrites the record in one step. Both refuse a record this state does not hold and a record whose body does not resolve to exactly one live wait, over the same identity comparison in each language, so an unowned replacement and an ambiguous body are both undefined rather than repaired
