...
- first-green body-claim admission and writer preservation; supporting review and Parallel Multi-Instance closing remain open
...
- Lean decodes and validates the `awaitSequentialMultiInstanceUserTask` Program operation, rewrites it into one definition-fact arm, states entry, nonfinal iteration, final completion, and interruption as declarative relations, proves the three evaluators produce only transitions those relations permit, and preserves `controllersOwnLiveActivity` across all five rewrites. Laws and kernel-decided facts cover a three-item run, interruption, empty collection, bidirectional program-aware binding refusals, and finite-snapshot conditional closure whose target-indexed actual relation proves close-or-decrease under explicit accepted-event progress without claiming human or host fairness
...
- **Parallel Multi-Instance:** checkpoint, entry preservation, and first-green [body-claim support](ACTIVITY-BODY-CLAIM-UNIQUENESS-PROPOSAL.md#first-green-implementation-checkpoint) exist; review and all closing/family evidence remain open.
