...
**The per-element claim is bounded to classification, and the boundary is stated rather than left to a reader.** An element is rejected by *classification*: its type, an own property at the `Definitions`, `Process`, or selected executed flow-element locus, or a foreign attribute the profile does not consume. A refusal stated over the whole document or over the checked graph, including encoding, the root-definition multiset, identity distinctness, connectivity, arity, and profile cardinality, rejects the source rather than an element and carries no element, because naming one there would be a location the compiler cannot justify. Both kinds are the same typed record, and the nullable element is what makes the difference visible instead of guessable.

**Executed flow-element keys have one owner.** [The closed profile/type inventory](../packages/bpmn-source/src/projected-flow-element-keys.ts) supplies the pre-projection classifier and every top-level flow-element projector predicate in the generic compiler and the three selected-shape readers. The inventory owns consumed keys, including default-aware keys such as the A12 boundary Error reader's `cancelActivity`; it does not own value, identity, nested-child, topology, or cardinality rules. An unsupported own property therefore gets a located `unsupportedProperty`, while `cancelActivity="false"`, an unqualified Call Activity target, or a malformed nested mapping keeps its existing document-level refusal.

Classification order is part of this contract. Each selected executed flow element is classified before an inventory-backed projector may reject its shape. The finite denominator is the generic compiler, the A12 CreateDocument reader, the A12 boundary Error reader, the Call Activity reader, and the generic recursive embedded Sub-Process gate. Recursive scope collection may decide containment, identity, `name`, `triggeredByEvent`, and listed values, but it never decides the flow-element key inventory.

[The inventory guard](../packages/bpmn-source/test/projected-flow-element-keys.test.ts) scans every production TypeScript source, compares every inventory consumer with a closed profile/type consumer table in both directions, and must fail when either a projector-local key list or a pre-classification Embedded Sub-Process inventory call is restored. [The public-result test](../packages/bpmn-source/test/per-element-admission-diagnostics.test.ts) compares four complete accepted results and four representative non-property refusals with the immutable fixture bound to pre-change target `8746bc6`; it also requires exact located own-property diagnostics across all four dispatch paths and the recursive Embedded Sub-Process gate. These are preservation oracles for this diagnostic increment, not evidence for a wider admission claim.
...
| An unsupported own property on the first executed Start Event in each of the four dispatch paths | reject with the exact located `unsupportedProperty`, rejected subject, and `PreserveProperty` capability |
| The same own property on an embedded Sub-Process | classify and locate it before recursive scope projection can return a document-level shape refusal |
| An unperturbed source in each dispatch path | preserve the complete accepted result from the immutable pre-change fixture |
