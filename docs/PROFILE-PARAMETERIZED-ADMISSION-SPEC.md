...
**Implemented current pre-release contract.** The registered capability catalog remains unchanged while the independently reviewable Message Start semantic checkpoint uses one separate unregistered capability.
...
The separate unregistered Message Start checkpoint capability is deliberately outside that product catalog. It admits one root scope with one `initiateMessage`, one `awaitUserTask`, one `reachNoneEnd`, and one `completeScope`; the reusable operation accepts canonical nonempty outputs while this capability requires exactly one. Independent checkpoint approval is required before this capability can move into the registered table with its profile, scenario, product example, differential case, and live Temporal evidence.
