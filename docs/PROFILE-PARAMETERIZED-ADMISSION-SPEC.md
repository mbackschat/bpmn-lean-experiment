...
The closest unsupported claim is arbitrary serial composition. Admission does not infer an unbounded grammar, repeated Timer or User Task mechanisms, cycles beyond the exact pending resumption-bounded profile, Standard Loop Characteristics, Multi-Instance, arbitrary graph cardinalities, or general BPMN Process Execution Conformance.
...
The checkpoint-only capability `bpmn-2.0.2-user-task-cycle-draft` is intentionally outside the registered table. It requires one `initiate`, one `mergeExclusive` with exactly three inputs, one `awaitUserTask`, one `choose`, one `reachNoneEnd`, one `completeScope`, and one definition scope. Its later profile artifact, scenario, and product example must register atomically.
