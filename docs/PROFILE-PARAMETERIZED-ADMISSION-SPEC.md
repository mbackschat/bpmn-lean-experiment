...
3. topology-independent graph checks establishing a canonical acyclic definition-scope forest and exact node/flow or operation/place ownership: checked source has exactly one entry root plus, only for the bounded Call profile, one distinct parentless called root, and validates reachability and co-reachability independently inside every scope; the Semantic Process program has exactly one Process initiation operation owned by the entry root, one completion strategy per scope, and global operation reachability from that initiation and co-reachability to entry-root completion, including the virtual called-End-to-`returnProcess` edge, under the current producer/consumer discipline; every profile except the registered User Task cycle requires whole-graph acyclicity, while that one profile applies its closed resumption cut before acyclicity;
...
[semantic-profile-catalog.ts](../packages/semantic-core/src/semantic-profile-catalog.ts) owns the registered profile identifiers and is the executable source read by the documentation-reviewability guard. [checked-process-profile-shape.ts](../packages/semantic-core/src/checked-process-profile-shape.ts) and [semantic-program-profile-shape.ts](../packages/semantic-core/src/semantic-program-profile-shape.ts) own the exact checked and IL multisets. [semantic-process-profile.ts](../packages/semantic-core/src/semantic-process-profile.ts) coordinates those shapes with profile payload and cardinality validation. The guard requires this summary to contain exactly one row for every registered identifier.

The registered Message Start and Timer Start capabilities each admit one root scope, one `awaitUserTask`, one `reachNoneEnd`, and one `completeScope`. Message Start uses one `initiateMessage`; Timer Start uses one `initiateTimer` with exact normalized duration `1000`. Both reusable initiation operations accept canonical nonempty outputs while their registered capabilities require exactly one.

Profile capability does not replace operation-payload validation. Exact Timer duration, effect descriptor and mapping, boundary route, gateway condition, source-language, origin, and arity restrictions remain checked by their existing owners. Generic `mergeExclusive` admits a canonical nonempty input collection; only the registered cycle profile requires exactly three inputs.
...
The checked-source validator operates on project-owned BPMN nodes and Sequence Flows after XML parsing and source-shape projection. It requires distinct resolved identities, a canonical acyclic definition-scope forest, exact node and Sequence Flow ownership, profile-permitted node arities and conditions, one profile-selected Start Event and at least one admitted exit Event in every scope, scope-local reference closure, reachability of every node from that scope's Start Event, co-reachability of every node to one of that scope's exits, and the selected finite graph policy inside every scope. Registered profiles use None Start except for the exact Message Start and Timer Start profiles, each of which uses its matching top-level Start Event with no incoming and one conditionless outgoing Sequence Flow. Every profile except the resumption-bounded User Task cycle requires whole-graph acyclicity. The cycle profile removes outgoing edges from its exact User Task before saturation-certified acyclicity, without weakening full-graph reachability or co-reachability. Every registered profile except the bounded Call profile has exactly one parentless entry root whose origin identifies `processId`; the Call profile has that same unique entry root plus one distinct parentless called root whose origin identifies the Call Activity's resolved `calledProcessId`. An admitted exit is a None End Event or Error End Event. A boundary Error remains a parent-scope node and receives one checked-graph-only exceptional reachability edge from its attached Sub-Process; that edge is never a Sequence Flow and never crosses definition scopes. Every non-root definition scope corresponds to exactly one ordinary embedded Sub-Process in its parent; Event Sub-Processes are rejected.

The Semantic Process validator operates independently on control places and typed operations. It requires the same canonical forest and unique entry root, exact operation and control-place ownership, exact one-producer/one-consumer control-place shape, and exactly one profile-selected Process initiation operation owned by that entry root. Registered profiles use `initiate` except for Message Start, which uses `initiateMessage`, and Timer Start, which uses `initiateTimer`. The first start stimulus must match that choice. Every nested scope has one `enterScope` and one `completeScope`; the entry root has one root `completeScope`; and each parentless non-entry called root has no `enterScope` or `completeScope` and instead has one paired `returnProcess`. The global operation graph includes the virtual edge from the called root's unique `reachNoneEnd` to its return, then requires every operation to be reachable from the unique initiation and co-reachable to the entry-root completion. Every profile except the registered User Task cycle requires whole-graph finite acyclicity. The cycle profile removes only the exact `awaitUserTask` continuation edges selected by its closed graph policy before saturation-certified acyclicity, while full-graph reachability and co-reachability remain mandatory. TypeScript and Lean each implement this check; neither calls the other.
...
The [answer-free resumption-bounded User Task cycle](../scenarios/user-task-cycle/README.md) is likewise consumed under one exact source/profile identity by Lean, the independently implemented TypeScript semantic core, and Temporal, with CIB explicitly absent. Its schedule traverses both conditional back-edges and then the default exit, while the differential activation mutation, semantic-core candidate-output swap, and test-only Temporal element-ID-cache and reset-to-one Workflows prove that branch binding and complete occurrence identity remain observable. The live witness preserves activations `1`, `2`, and `3` across Worker replacement, refuses a stale occurrence without changing the Query, records exact finite Event History structure, and replays.

The [answer-free Message Start Event scenario](../scenarios/message-start-event/README.md) is consumed under one exact source/profile identity by Lean, the independently implemented TypeScript semantic core, and Temporal, with CIB explicitly absent. Its exact trigger preserves Process, Start Event, Message, Interface, and Interface Operation identity. The live witness proves service acceptance while no Worker polls, later User Task completion, terminal receipt, no Signal ingress, and replay; an Interface Operation mutation and a test-owned Signal-With-Start history separate the selected channel and hosting path.
