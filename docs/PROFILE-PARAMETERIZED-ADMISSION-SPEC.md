...
| BPMN resumption-bounded User Task cycle (`bpmn-2.0.2-user-task-cycle-draft`) | 1 | one `initiate`, one `mergeExclusive`, one `awaitUserTask`, one `choose`, one `reachNoneEnd`, one `completeScope` |
