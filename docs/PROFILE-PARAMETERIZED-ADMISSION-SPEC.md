...
The registered Message Start capability admits one root scope with one `initiateMessage`, one `awaitUserTask`, one `reachNoneEnd`, and one `completeScope`; the reusable operation accepts canonical nonempty outputs while this capability requires exactly one.

Outside the registered table, the unregistered `bpmn-2.0.2-timer-start-event-draft` checkpoint capability admits the analogous root scope with one `initiateTimer`, one `awaitUserTask`, one `reachNoneEnd`, and one `completeScope`; its reusable operation accepts canonical nonempty outputs while the checkpoint capability requires one output and exact normalized duration `1000`. It does not enter product profile enumeration until checkpoint review closes.
