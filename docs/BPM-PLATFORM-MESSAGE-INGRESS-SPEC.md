...
The maintained acceptance boundary is the complete repository gate, the M2 live and browser showcase gates, the stable rule matrix above, and the exact implemented and absent status in the [`implementation-status-owner:BPM-PLATFORM`](BPM-PLATFORM-IMPLEMENTATION-MAP.md). The closure evidence must retain an exact production-derived version-2 fanout mutation and a field-specific browser oracle for the distinct Process-instance identity.
