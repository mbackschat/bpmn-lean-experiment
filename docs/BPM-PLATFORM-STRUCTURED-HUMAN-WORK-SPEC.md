# BPM platform structured Human Work specification

## Status

**Implemented, independently closure-reviewed, and evidence-closed.** M6 provides regular field types, flat arrays, multiple resolution actions, and action-dependent input. Form schemas, validation, and computations belong to Product 2 and may grow toward JSON Schema-scale complexity or beyond through Zod. Human Task execution semantics follow BPMN 2.0.2 rather than importing CIB Seven form behavior into Lean, the Semantic Process IL, the semantic core, or Temporal Workflow state. The Product 1 semantic checkpoint and Product 2 catalog, validation, completion, retained-model, responsive browser, and bundle-boundary lanes are green. [The receipt](#independent-cold-review-receipt) records the approved proposal, checkpoint, closure, and correction targets.

This specification supersedes only the bounded first-form exclusions in [the platform proposal](BPM-PLATFORM-PROPOSAL.md#task-interaction) and [the existing Human Work specification](BPM-PLATFORM-HUMAN-WORK-SPEC.md#required-optional-and-excluded-functionality); that specification remains authoritative for claim, release, exact completion transport, authorization, and Work audit. [PLAN.md](PLAN.md) owns sequencing, the [`implementation-status-owner:BPM-PLATFORM`](BPM-PLATFORM-IMPLEMENTATION-MAP.md) owns the implemented boundary, [the Human Tasks and forms research](research/HUMAN-TASKS-AND-FORMS-RESEARCH.md) owns source-grounded findings, and [the diagram presentation decision](BPMN-DIAGRAM-PRESENTATION-DECISION.md) owns the Product 2 BPMN parser exception generalized here.

The implemented [shared-persistence addendum](BPM-PLATFORM-SHARED-PERSISTENCE-AND-PROJECTION-PROPOSAL.md) stores the same canonical exact-source-bound Human Task catalog in Definitions-owned PostgreSQL rows. Shared Work generation recovery joins that catalog through the exact deployed source identity, and the instance-scoped current-task read still precedes structured validation and completion. No form, value, action, or Product 1 contract changes.
...
One authoritative Product 2 Zod schema factory per catalog version, server-authoritative execution, client-safe descriptions, and mutation tests at every boundary keep form rules out of BPMN import and the semantic core. Catalog-order canonicalization remains distinct from generic semantic list order and multiplicity.

Reopen this specification before adding nested objects or heterogeneous arrays to an admitted semantic value, arbitrary field-to-field or scripted computation, I/O during form computation, Workflow-side form validation, a mutable or remotely resolved catalog, `latest` binding, draft state, attachments, timers, multiple assignment identities, a new completion command family, form facts in semantic observations, standard DataInput/DataOutput execution, or a CIB form construct.
