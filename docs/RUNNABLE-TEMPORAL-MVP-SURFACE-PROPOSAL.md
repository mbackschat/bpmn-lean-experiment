...
The current host-capability predicate admits all fifteen registered profiles: it rejects only a token split combined with a timer or effect wait, or a managed event race other than the one exact conforming Message/`PT1S` race, and no registered profile combines a split with a host-driven wait. The stop condition therefore guards future widening rather than a live unknown — if a later profile does need a rejected wait-set shape, that is an owner capability decision, not something the driver may work around.
