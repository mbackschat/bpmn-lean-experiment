...
| Platform web runtime graph | The exact 23-package reachable graph contains 20 MIT identities, 1 Apache-2.0 identity, 1 ISC identity, and `bpmn-js@18.22.1` under its identity-scoped `LicenseRef-bpmn.io` text | React 19.2.8, React DOM 19.2.8, and the viewer-only `bpmn-js` adapter ship in the static client; the exact bpmn.io notice ships beside it and the supplied visible attribution remains unchanged and linked |
