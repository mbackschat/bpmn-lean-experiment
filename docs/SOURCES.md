...
| Product 2 PostgreSQL runtime and migrations | Direct [`pg@8.22.0`](https://www.npmjs.com/package/pg/v/8.22.0), [`@types/pg@8.20.0`](https://www.npmjs.com/package/@types/pg/v/8.20.0), and [`node-pg-migrate@9.0.0`](https://www.npmjs.com/package/node-pg-migrate/v/9.0.0) are MIT; exact resolution and integrity are fixed by `pnpm-lock.yaml` | `pg` owns low-level runtime transport and `node-pg-migrate` executes only the forward checksum-bound administrative migration command. Its production closure adds `glob@13.0.6`, `lru-cache@11.5.2`, `minimatch@10.2.6`, `minipass@7.1.3`, and `path-scurry@2.0.2` under [BlueOak-1.0.0](https://blueoakcouncil.org/license/1.0.0); their distributed package licence files preserve the required notice. No driver or migration dependency enters Product 1, Lean, the semantic core, or Temporal Workflow code. |
