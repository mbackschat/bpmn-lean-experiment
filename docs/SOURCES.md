...
Hosted verification also executes external GitHub Actions. Their exact revisions are recorded where they are used, as a commit pin plus a version comment on each `uses:` line in [the workflow directory](../.github/workflows/); those files own them, and this document deliberately keeps no second copy to drift from them.

Local external reference trees live by default under repository sibling `../oss`; `BPMN_EXTERNAL_ROOT` selects another shared root. The machine-readable [`external-sources.lock`](../scripts/external-sources.lock) binds every registered top-level Git tree and Git submodule below to its canonical GitHub remote, full revision, immutable commit/exact-tag classification, relative path, material kind, and provisioning scope. Branch and release names in this document provide human provenance context only; the 40-character commit is the immutable identity, and an exact tag is additionally checked only when it resolves to that commit. The [contributor setup guide](CONTRIBUTOR-SETUP-GUIDE.md) owns clean-machine commands, complete dependency/cache inventory, diagnosis, and the decision to use an exact lock rather than engine-repository submodules. The repositories and revisions recorded here remain provenance authority, and the checkouts remain read-only research or optional adoption-evidence inputs unless an explicitly named experiment follows the instrumentation policy.

Run `./scripts/setup-external-sources.sh verify` for the normative corpus required by the default MIT engine gate, `./scripts/setup-external-sources.sh research` before source-grounded research, or `./scripts/setup-external-sources.sh adoption` before the optional exact-A12-source lane. The matching `doctor.sh` scope is the read-only preflight. `all` deliberately selects every scope. A selected evidence lane fails on absent or mismatched input; a scope not selected makes no claim and is reported as such rather than silently degraded.
...
| [`a12/a12-widgets`](../../oss/a12/a12-widgets) | `mgm-tp/a12-widgets`, branch `main`, revision `f924a855b4c2b11b85647da83d3c9615d7b36102` | `EUPL-1.2 OR LicenseRef-commercial`; A12's own React styled component kit. Registered because the finding that it is React-only and dual-licensed is what justifies the owner's 2026-08-07 decision to drop platform reuse levels 2 and 3, recorded in [the platform stack research](research/BPM-PLATFORM-STACK-RESEARCH.md) |
...
Source and executable CIB revisions must not diverge inside one evidence claim. If the selected source checkout, published engine artifact, retained evidence, or declared profile names a different revision, the lane fails closed until each fact is routed to its exact revision-specific owner; a source inspection against one revision cannot validate execution against another.
...
| Node and pnpm | Node `24.18.0` and pnpm `11.20.0`; Homebrew formulae report MIT | External runtime and package manager; both pins are owned by `package.json` and derived by every wrapper, doctor, and CI consumer; exact nvm/asdf pins and a non-mutating Homebrew fallback are tracked; the project wrapper disables pnpm's automatic project-driven CLI switching after performing its own exact version check so offline and sandboxed runs cannot stall in a hidden tool download |
...
| BPMN definition-projection graph | Direct `bpmn-auto-layout@1.3.0`, `bpmn-moddle@10.0.0`, `min-dash@5.1.0`, and `saxen@11.1.0`, plus locked `moddle@8.2.0` and `moddle-xml@12.1.0`, are MIT | Adopted only in the Product 2 `@bpmn-lean/platform-bpmn-definition-projection` worker and private parser boundary; it emits closed diagram and Human Task catalog projections, and no dependency enters Product 1, Lean, the semantic core, or Temporal |
| Product 2 structured-form validation | Direct [`zod@4.4.3`](https://www.npmjs.com/package/zod/v/4.4.3) is MIT and has no runtime dependencies | Pinned only in Product 2 platform contracts and Work; it strictly decodes the public catalog and validates/computes structured submissions, while form schemas and Zod stay out of Product 1, Lean, the semantic core, and Temporal Workflow code |
| Platform production dependency graph | Direct production dependencies are declared by their owning workspace packages and the complete resolved graph is fixed only by `pnpm-lock.yaml`; reachable licences must be 0BSD, MIT, Apache-2.0, BlueOak-1.0.0, BSD-3-Clause, ISC, or the exact `bpmn-js@18.22.1` identity-scoped `LicenseRef-bpmn.io` text | React 19.2.8, React DOM 19.2.8, the viewer-only `bpmn-js` adapter, the concrete Temporal client, and the PostgreSQL runtime ship in their respective Product 2 runtime closures; the exact bpmn.io notice ships beside the web distribution and its supplied visible attribution remains unchanged and linked |
| Product 2 PostgreSQL runtime and migrations | Direct [`pg@8.22.0`](https://www.npmjs.com/package/pg/v/8.22.0), [`@types/pg@8.20.0`](https://www.npmjs.com/package/@types/pg/v/8.20.0), and [`node-pg-migrate@9.0.0`](https://www.npmjs.com/package/node-pg-migrate/v/9.0.0) are MIT; exact resolution and integrity are fixed by `pnpm-lock.yaml`. The isolated hosted witness and evaluation distribution use the official [`postgres:18.4-bookworm`](https://hub.docker.com/_/postgres) image at digest `sha256:882236b897e39051d2368c5ccc6cda944904723506b2dfc97f2a8f5bc9afa382`. | `pg` owns low-level runtime transport and the isolated per-package test-database driver; `node-pg-migrate` executes only the forward checksum-bound administrative migration command. The PostgreSQL image is pulled as an external evaluation runtime and is not copied into a project-built image. The production closure adds `glob@13.0.6`, `lru-cache@11.5.2`, `minimatch@10.2.6`, `minipass@7.1.3`, and `path-scurry@2.0.2` under [BlueOak-1.0.0](https://blueoakcouncil.org/license/1.0.0); their distributed package licence files preserve the required notice. No driver or migration dependency enters Product 1, Lean, the semantic core, or Temporal Workflow code. |
| Platform browser-test graph | Direct `@playwright/test@1.62.1`, transitive `playwright@1.62.1` and `playwright-core@1.62.1` are Apache-2.0; optional macOS `fsevents@2.3.2` is MIT | Development and CI only. Playwright downloads its pinned Chromium revision into an external user cache; no Playwright package or browser binary enters the static distribution or reachable production graph |
| Platform web build graph | Direct development roots `vite@7.3.6`, `@types/react@19.2.18`, and `@types/react-dom@19.2.4` are MIT; their locked build-only graph is MIT, BSD-2-Clause, or BSD-3-Clause | Development and CI only; no build package enters the static distribution or reachable production graph, and pnpm denies `esbuild@0.28.2` its optional install script because the locked native binary is sufficient |
...
| Temporal CLI `v1.8.1` | MIT; downloaded from the official Temporal distribution endpoint and Git-ignored. The evaluation distribution pulls the official [`temporalio/temporal:1.8.1`](https://hub.docker.com/r/temporalio/temporal) image at digest `sha256:59561b9ef060eaeb1f46cb6a1842d6cbdd8a393eb3b6d315ecef5fe2f0b1d7a6`. | Full local development server for M0.5 integration tests and the evaluation Compose topology. The cached host binary is not redistributed, and the official image is referenced directly rather than copied into a project-built image. |
| Evaluation Node base image | Node is MIT; the evaluation Dockerfile pins official [`node:24.18.0-bookworm-slim`](https://hub.docker.com/_/node) at digest `sha256:6f7b03f7c2c8e2e784dcf9295400527b9b1270fd37b7e9a7285cf83b6951452d`. | Builder and four unprivileged project runtime stages only. Production closures are installed from the frozen workspace lock; repository research, Lean, Java, showcases, and tests are absent from final stages. |
...
M0.5 separately pins the released packages `@temporalio/client@1.21.0`, `@temporalio/testing@1.21.0`, `@temporalio/worker@1.21.0`, and `@temporalio/workflow@1.21.0`. They are isolated by execution environment: `@bpmn-lean/temporal-client` owns the client SDK, `@bpmn-lean/temporal-workflow` owns the Workflow SDK, `@bpmn-lean/temporal-worker` owns the Worker SDK, and `@bpmn-lean/temporal-testkit` owns test-server and combined test reachability. The semantic core and `@bpmn-lean/temporal-protocol` remain SDK-free. The SDK requires Node `>=20.3.0`, satisfied by the project’s pinned Node `24.18.0`.

The resolved pnpm graph is locked. Direct Temporal versions live only in the owning package manifests and the frozen lockfile owns the complete resolution; the Temporal package-boundary guard enforces execution-environment reachability without restating those versions. pnpm’s supply-chain guard explicitly denies install scripts for `@swc/core@1.15.46` and `protobufjs@7.6.5`; the installed native SWC binding was verified without its optional fallback script, and protobufjs’s postinstall is not required for runtime behavior. Temporal’s published declarations currently fail library checking under TypeScript `7.0.2`, so only the adapter sets `skipLibCheck: true`; project source remains strict and the semantic core continues to check dependency-free with library checking enabled.
...
The adopted Product 2 presentation dependency is the separately published [`bpmn-auto-layout@1.3.0`](https://github.com/bpmn-io/bpmn-auto-layout/tree/v1.3.0). Its registry tarball integrity is `sha512-ufjMy1MaaZxkQF7f2wY6EL6TnRMSFr/r9eBhp0QvS41UHXW77aQVt65+dn2MmVMlHdyvZyOMNYXO2qYpgX6bQA==`; the exact resolved graph is `bpmn-auto-layout@1.3.0`, `bpmn-moddle@10.0.0`, `moddle@8.2.0`, `moddle-xml@12.1.0`, `min-dash@5.1.0`, and `saxen@11.1.0`, all MIT. The published manifest declares a `prepare` lifecycle script for source/package preparation; the committed pnpm registry installation did not execute it. The dependency is confined to [`@bpmn-lean/platform-bpmn-definition-projection`](../platform/foundation/bpmn-definition-projection/README.md), which runs layout in a killable worker, discards generated non-DI XML, and exposes only validated DI plus closed provenance or the separately decoded exact-source-bound Human Task catalog.
...
The project-authored [BPM platform UI/UX and information-architecture research](research/BPM-PLATFORM-UI-UX-INFORMATION-ARCHITECTURE-RESEARCH.md) inspected official documentation and documentation screenshots on 2026-08-13. Current documentation is preferred; the exact CIB Seven 1.0 dashboard page is retained only as labeled historical interaction evidence. These sources establish product interaction precedents only. They do not select an implementation stack, define BPMN meaning, prove accessibility or responsive quality beyond each product's documented claims, or authorize copying proprietary visual design.

| Product surface | Primary source | Research role |
|---|---|---|
| CIB Seven Tasklist and Cockpit | [Current 2.2 Tasklist](https://docs.cibseven.org/manual/2.2/webapps/tasklist/), [current 2.2 focus and shortcut configuration](https://docs.cibseven.org/manual/2.2/webapps/configuration-options/), [current User Task forms](https://docs.cibseven.org/manual/latest/user-guide/task-forms/), [current 2.2 Process Definition View](https://docs.cibseven.org/manual/2.2/webapps/cockpit/bpmn/process-definition-view/), [current 2.2 Process Instance View](https://docs.cibseven.org/manual/2.2/webapps/cockpit/bpmn/process-instance-view/), and [historical 1.0 dashboard](https://docs.cibseven.org/manual/1.0/webapps/tasklist/dashboard/) | Current list, focus-region, assignment, Process-start, form, running-position, incident, diagram-navigation, and instance-detail precedent; historical Form/Diagram sibling-tab evidence only |
...
| `CONSOLIDATED-BPMN-LEAN-RECOMMENDATIONS.md` | `a16820e2cd71533edd9650ef7797ede9277df2b748fda2b1babbdcfad97fe21a` |
...
The third document was inspected on 2026-08-04 under the same standing, and only that one of its sibling extracts was read. Two of its statements about this project were independently re-established from project source and are recorded as this project's findings by their owners: one ordered-work item contradicted the same document's own resume point, now corrected and bound by [an executable guard](../scripts/plan-status-consistency.test.ts), and the runtime uniqueness invariant the boundary-timer capsule carries as a hypothesis has a settled shape in two existing state predicates rather than being an open design question, now recorded in the [`implementation-status-owner:ENGINE-RUNTIME-PROOF`](ENGINE-RUNTIME-AND-PROOF-IMPLEMENTATION-MAP.md) and deferred with its coupling in [PLAN.md](PLAN.md). Nothing else from it is adopted, and neither finding carries its authority.

The second document was independently checked against project source on 2026-07-31 under the same standing. The defect it named in this project was narrower than stated: Lean's `closeSupported` refused an unresolved multiple-enabled state rather than selecting the enabled-list head. The later committed-execution publication checkpoint closed that TypeScript divergence through complete-enabled-set classification, one exact checked independent User Task pair, and fail-closed refusal of every other multiple-enabled shape, as recorded in the [`implementation-status-owner:ENGINE-RUNTIME-PROOF`](ENGINE-RUNTIME-AND-PROOF-IMPLEMENTATION-MAP.md). Its remaining recommendations are not adopted, because their deciding facts are unmade product decisions rather than named consumers or standing obligations. Separating enabled-operation computation from selection inside the TypeScript core did land, but under the [profile-parameterized admission gate](PROFILE-PARAMETERIZED-ADMISSION-SPEC.md) and forced by that gate's need for a checkable enabledness predicate; it is this project's finding and carries none of the document's authority.
