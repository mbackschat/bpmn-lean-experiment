...
| Proposed BPMN presentation graph | Selected candidate `bpmn-auto-layout@1.3.0` plus its intended `bpmn-moddle@10.0.0` and `min-dash@5.1.0` graph are MIT | Not yet adopted at this review target. Implementation must add the exact Product 2 manifest and lock atomically, then advance this row to current evidence |
...
The selected Product 2 presentation candidate is the separately published [`bpmn-auto-layout@1.3.0`](https://github.com/bpmn-io/bpmn-auto-layout/tree/v1.3.0). Its registry tarball integrity is `sha512-ufjMy1MaaZxkQF7f2wY6EL6TnRMSFr/r9eBhp0QvS41UHXW77aQVt65+dn2MmVMlHdyvZyOMNYXO2qYpgX6bQA==`; the intended resolved graph is `bpmn-auto-layout@1.3.0`, `bpmn-moddle@10.0.0`, `moddle@8.2.0`, `moddle-xml@12.1.0`, `min-dash@5.1.0`, and `saxen@11.1.0`, all MIT. The published manifest declares a `prepare` lifecycle script for source/package preparation; pnpm's locked registry installation must not execute it, and implementation evidence must verify that fact. The package is not adopted at this review target. Its implementation must confine it to `platform/foundation/bpmn-presentation`, discard generated non-DI XML, and expose only validated DI plus closed provenance.
