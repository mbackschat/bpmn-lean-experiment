...
| BPMN presentation graph | Direct `bpmn-auto-layout@1.3.0` plus its exact locked `bpmn-moddle@10.0.0` and `min-dash@5.1.0` graph are MIT | Product 2 presentation-only generation in `platform/foundation/bpmn-presentation`; no raw moddle value, generated non-DI XML, parser type, or layout dependency enters Product 1, admitted source identity, the semantic core, or Temporal |
...
The Product 2 presentation dependency is the separately published [`bpmn-auto-layout@1.3.0`](https://github.com/bpmn-io/bpmn-auto-layout/tree/v1.3.0). Its registry tarball integrity is `sha512-ufjMy1MaaZxkQF7f2wY6EL6TnRMSFr/r9eBhp0QvS41UHXW77aQVt65+dn2MmVMlHdyvZyOMNYXO2qYpgX6bQA==`; its exact locked runtime graph is `bpmn-auto-layout@1.3.0`, `bpmn-moddle@10.0.0`, `moddle@8.2.0`, `moddle-xml@12.1.0`, `min-dash@5.1.0`, and `saxen@11.1.0`, all MIT. The published manifest has no registry lifecycle script; its source-only `prepare` script does not execute during pnpm's locked registry installation. The package is confined to `platform/foundation/bpmn-presentation`, where generated non-DI XML is discarded and only validated DI plus closed provenance can cross the package boundary.
