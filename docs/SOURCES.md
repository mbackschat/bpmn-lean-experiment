...
| Platform production dependency graph | Direct production dependencies are declared by their owning workspace packages and the complete resolved graph is fixed only by `pnpm-lock.yaml`; reachable licences must be 0BSD, MIT, Apache-2.0, BSD-3-Clause, ISC, or the exact `bpmn-js@18.22.1` identity-scoped `LicenseRef-bpmn.io` text | React 19.2.8, React DOM 19.2.8, the viewer-only `bpmn-js` adapter, and the concrete Temporal client ship in their respective Product 2 runtime closures; the exact bpmn.io notice ships beside the web distribution and its supplied visible attribution remains unchanged and linked |
