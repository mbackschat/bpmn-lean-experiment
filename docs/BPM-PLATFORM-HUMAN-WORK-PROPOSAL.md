...
**Redesigned draft for a new independent context-cold proposal review.** Target `58aa25e` received `APPROVE WITH REQUIRED EDITS`; correction `5fa5925` closed four findings but materially replaced the direct-start producer failure boundary with a durable prepare/start/describe lifecycle, so it was ineligible for warm approval. This target makes that replacement exact, preserves the M2 evidence boundary while declaring the prospective M3 supersession, and completes the owner/evidence routing. It still selects the same current inbox, platform claim and authorization policy, one typed form field, retry-safe completion, distinct platform audit, and CSS-Modules UI scope. Implementation remains paused until the [User Task assignment and form metadata proposal](capsules/USER-TASK-ASSIGNMENT-FORM-METADATA-PROPOSAL.md) receives closure approval and this proposal receives owner approval.
