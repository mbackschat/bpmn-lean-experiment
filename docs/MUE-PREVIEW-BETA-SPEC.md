...
The platform-web component test and UI-quality browser test require the exact seven-row order, text classifications, checkpoint-only absences, capability-catalog separation, and responsive containment. They and the cross-owner integration guard consume one shared test-only oracle that is independent of the production catalog. The oracle neither owns nor generates any artifact it checks: PLAN owns the denominator, this specification owns the reviewed evidence matrix, and Product 2 owns its presentation catalog. The integration guard also requires the build-once release command and the relevant documentation owners to agree.
...
The closure reviewer found that both presentation tests derived row identities from the production checkpoint catalog and asserted labels only globally, so a label-to-row swap could pass. Correction `8bcd8746` gave both the component test and the two-width browser test independent seven-row oracles and scoped each exact title, evidence label, Product 2 surface, and combined boundary/limit assertion to its content ID. The later shared test-only fixture removes the duplicate test copies without importing or generating from the production catalog, so the independence established by the correction remains intact. A planted evidence-label swap failed at `SEQUENTIAL-MULTI-INSTANCE`; after reversion, the same reviewer approved the correction with no remaining findings.

The retained gate digests above bind closure target `361911b8`, while the original local tag object `6a56a28489afae649b1dede5f4ce6180290e008a` targeted `1c3243e8`; they therefore did not establish an exact-HEAD gate result after correction `8bcd8746`. This is an evidence-record gap rather than a retrospective semantic-failure claim. The owner-directed pre-publication correction verifies that the remote reference is absent, reruns both applicable gates into exact-target receipts, and recreates the local tag through the guarded project-tag command; no published reference moves, and the replacement tag object is the canonical target record. Tag creation now requires successful command receipts bound to the exact target commit, preventing the same gap at future phase and release boundaries.
