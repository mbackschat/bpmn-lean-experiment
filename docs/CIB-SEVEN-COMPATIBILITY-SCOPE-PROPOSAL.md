...
CIB Seven `2.2.0` nevertheless has a distinct planning role: its executable BPMN Process breadth is the primary baseline for ordering which standards mechanisms the project closes after the M0 engine-runner floor. That priority does not make CIB normative, promote all CIB extensions into scope, require a CIB target for every standards capsule, or combine BPMN and CIB coverage measures. The durable rule is owned by [PROJECT-DESIGN.md](PROJECT-DESIGN.md#cib-seven-220-breadth-ordering).
...
| Exact User Task completion with canonical string/null Process-variable patch | **Implemented evidence-closed draft** | First concrete consumer is the engine runner's simulated actor; selected pinned `TaskService.complete(taskId, variables)` behavior is classified and implemented under the [completion-data specification](capsules/USER-TASK-COMPLETION-DATA-SPEC.md) |
...
| Form metadata/rendering, assignees, candidates, dates and identity extensions | **Deferred to later User Task profiles** | The engine runner simulates values without rendering a form or selecting human-resource extensions; reopen only with those exact consumers |
...
The selected CIB direction remains read-only JUEL under the [JUEL evaluation architecture decision](JUEL-EVALUATION-ARCHITECTURE-DECISION.md), but it is not the first production condition language. The implemented [Exclusive Gateway condition specification](capsules/EXCLUSIVE-GATEWAY-CONDITION-SPEC.md) uses the project-owned Simple Boolean profile to establish BPMN structure without a Java Activity. When JUEL is reopened, its complete Process-scope context preserves presence versus explicit null and reuses the canonical tagged String/Null variable contract; only after strict boundary validation does the Java evaluator convert those values to Java `String` or `null`. The context contains no `execution`, bean, function, method, Process Engine service, or arbitrary Java object. One batched request returns language results while the semantic core selects the outgoing Sequence Flow.
