...
The selected CIB direction remains read-only JUEL under the [JUEL evaluation architecture decision](JUEL-EVALUATION-ARCHITECTURE-DECISION.md), but it is no longer the first production condition language. The active [Exclusive Gateway condition proposal](capsules/EXCLUSIVE-GATEWAY-CONDITION-PROPOSAL.md) uses the project-owned Simple Boolean profile to establish BPMN structure without a Java Activity. When JUEL is reopened, its complete Process-scope context preserves presence versus explicit null and reuses the canonical tagged String/Null variable contract; only after strict boundary validation does the Java evaluator convert those values to Java `String` or `null`. The context contains no `execution`, bean, function, method, Process Engine service, or arbitrary Java object. One batched request returns language results while the semantic core selects the outgoing Sequence Flow.
