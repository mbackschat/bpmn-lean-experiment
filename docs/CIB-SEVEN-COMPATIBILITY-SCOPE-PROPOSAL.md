...
| Exact User Task completion with canonical string/null Process-variable patch | **Implemented evidence-closed draft** | First concrete consumer is the dummy actor in the runnable MVP; selected pinned `TaskService.complete(taskId, variables)` behavior is classified and implemented under the [completion-data specification](capsules/USER-TASK-COMPLETION-DATA-SPEC.md) |
