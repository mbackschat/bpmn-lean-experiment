...
**Owner-approved on 2026-08-03; partially implemented and not evidence-closed.** The implemented and absent scope is recorded in [IMPLEMENTATION-MAP.md](../IMPLEMENTATION-MAP.md#current-claim). The Temporal lane exists: the host arms the deadline durably and refuses only a completion and deadline sharing one Workflow activation. A closure review rejected an earlier target because required evidence this capsule itself specifies is still absent — the declarative victory relation with its evaluator bridge and quantified laws, the registered pre-due-firing scenario, the Worker-replacement history, and a witness for the preflight's durable-Update obligation. Those absences are recorded rather than closed. The owner approved all four decisions below as recommended, after the independent cold proposal review returned `approve-with-required-edits` and both correction rounds passed the same reviewer's audit. Nothing in this document is a coverage, conformance, or CIB compatibility claim, and approval authorizes exactly the scope recorded here.
