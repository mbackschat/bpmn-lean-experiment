...
- **Lean:** `ABTIMER-ARM-01` only. The arming relation and its evaluator-soundness bridge exist, plus one quantified refusal theorem. **Absent:** the declarative two-constructor victory relation, the soundness bridge over victory transitions, and the quantified exclusivity, interruption-with-counter-preservation, and state-preservation laws. Both victories are executable functions checked by `by decide` over one concrete fixture, which does not discharge a quantified law.
