...
- **Temporal refinement:** the two victory histories exist and replay. **Absent:** the Worker-replacement-across-the-deadline history, and any witness for the preflight's obligation that a coalesced in-flight Update is durably resolved rather than stranded — a named stop condition that has therefore never been evaluated against evidence.
- **Negative witness or mutation:** two of five are seeded — the retained withdrawn deadline and the misrouted deadline victory. The partial-arm and pre-due mutations are **absent**; boundary-Flow identity erasure is covered instead by the positive lowering lock in `packages/bpmn-source/test/activity-boundary-timer-source.test.ts`.
