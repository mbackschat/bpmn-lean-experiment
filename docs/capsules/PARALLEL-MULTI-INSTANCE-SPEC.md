# Parallel Multi-Instance User Task specification

## Status

Implemented, independently closure-reviewed, and evidence-closed for the exact bounded standards-only profile. Exact implemented and absent status is maintained by [`implementation-status-delegation:ENGINE-SEMANTIC-FAMILY`](../ENGINE-SEMANTIC-FAMILY-IMPLEMENTATION-MAP.md#parallel-multi-instance-user-task), and the governed review history is retained below.

## Question and bounded outcome

The exact bounded standards-only profile is implemented across source admission and lowering, Lean, the independently written TypeScript semantic core, canonical progress, E1/E2 publication, the differential pipeline, the production Temporal Workflow chain, the retained whole-model corpus, and Product 2's strict additive copied-contract boundary. Three registered schedules cover all-complete aggregation, first-complete sibling termination, and outer-Timer interruption. The real-service refinement witness additionally covers zero and one-item cases, pre-arming Continue-As-New, Worker replacement, accepted-result recovery, both task/Timer orders, exact terminal receipts, paired publication traversal, and replay of every Run. Reproducible cost and the clean immutable closure gate are recorded, and both correction-audit rounds are complete.
...
Final publication requires the assembled result collection to satisfy the same item-count, per-item UTF-8, and canonical JSON collection-byte limits as the input snapshot. Failure rejects the completing command with its predecessor state and no committed events; the last child and Timer remain available. Individually admitted partial results are not an output collection and do not acquire an aggregate-publication bound before final completion. The [Lean completion evaluator](../../BpmnSemantics/SemanticProcess/ParallelMultiInstanceTransition.lean) enforces this separately from structural runtime-state preservation.
...
Entry, progress, final, early, and Timer routes preserve the complete production runtime-state invariant through their declarative shared steps and executable evaluators. The closing proof exposed a pre-existing cross-family gap: production well-formedness admitted a distinct unrelated Activity record whose body claimed one live child removed by the selected route. The implemented [`AOO-CLAIM-01`](../ACTIVITY-OCCURRENCE-OWNERSHIP-SPEC.md#stable-semantic-rules) rule rejects both User Task and child-scope aliases, derives lookup and turnover consequences, and is preserved across the guarded writer set. The Parallel proofs consume that general rule without adding a family-specific premise.
...
The real-service refinement witness covers zero items; one item under `first`; three items completed out of index order under `all`; Timer interruption with multiple active children; task-first and Timer-first schedules; Worker replacement; accepted-result recovery; any permitted pre-arming Continue-As-New; terminal receipt equality; complete E1/E2 publication; and replay of every Run. The direct-VM witness covers coalesced readiness with the selected typed failure and durable Update resolution. The retained capacity witness covers multi-child first completion at the selected bound and replay.
...
Established are the applicable normative clauses and artifacts, exact source and profile boundary, direct-index aggregation, no-partial-output choice, explicit accepted-stimulus ordering, runtime ownership, proved Lean transition and preservation account, independent TypeScript realization, E1/E2 publication, bounded durable refinement, retained corpus reach, CIB non-selection, and governed closure approval.

The [closure-cost ledger](../CAPSULE-COST-LEDGER.md) records `25dd98cd..17e23590` as `+21035/-593` nonblank code lines and `+454/-148` documentation lines. Against the nearest same-mechanism Sequential Multi-Instance comparator, code additions fell by 5,825 and documentation additions by 617 while this slice added genuine concurrency, completion-condition cancellation, commutation, a schedule-sensitive non-law, coalesced host refusal, and Product 2 contract compatibility. The range conservatively retains the supporting body-claim invariant and later proof and verification corrections; both addition measures fell, so no further process-weight removal is manufactured.
...
The 2026-09-20 proof/evidence correction at `2aee015b38c7643afc38a12f3a79c9fb5e5870ef`, relative to `8220c4c4b97980bdda746dfc05df63de93bb67fa`, received semantic-checkpoint `approve` from `/root/review_claims_semantic_checkpoint` with `fork-turns-none` and no model or effort override. Packet `d4ff4fb4879d89490ac8ee72db814088b9270842fb85c32f96cd5b40f93fa564` binds the slot-order proof, shared-evidence limits, lowering comparison, and embedded-scope evidence classification. Its complete Lean and infrastructure gates passed; this was not a closure approval. The subsequent repository-wide run failed a Temporal publication availability assertion after successful terminal execution and replay; the unchanged focused refinement test passed on rerun, leaving complete-gate confirmation required.

That reviewer separately exposed missing TypeScript aggregate-output admission. The correction shares the input collection predicate with final output selection before retention or closure. [Retained core regressions](../../packages/semantic-core/test/parallel-multi-instance-preservation.test.ts) reject 8,241-byte array-overhead and 9,226-byte escaped outputs, distinguish 8,191/8,192/8,193 bytes, retain exact state and empty events on refusal, permit a corrected completion, and preserve Timer interruption after oversized unpublished partial results. The five complete-output cases also agree exactly through the existing Lean/core scenario pipeline. This correction preserves the existing profile limits and Lean account.

Output-capacity target `61faeee720d54528b3bcf814953ea0914c51fb25`, relative to `2aee015b38c7643afc38a12f3a79c9fb5e5870ef`, received semantic-checkpoint `approve` without required findings from `/root/parallel_output_capacity_checkpoint_review`, with `fork-turns-none` and no model or effort override. Packet `9a67518a9ff3e908f5844c7351b352371c8078a5c7df96246cb35d818b8402b8` binds that review. Complete core and infrastructure gates, the five Lean/core comparisons, and the reviewer-selected focused tests passed. Its repository-wide run encountered a Node/V8 native crash in the Temporal deadline test file, whose unchanged focused rerun passed all four tests; that rerun did not replace the failed complete run. The macOS report at 2026-09-20 19:10:22 +0200 records `SIGSEGV` at address `0xE` in `ClearStaleLeftTrimmedPointerVisitor::VisitRootPointers` during mark/compact collection, matching the signal and top-frame failure already recorded in [earlier production verification](../CAPSULE-COST-LEDGER.md#mixed-local-control-production-verification). No VM flag, concurrency limit, deadline, or dependency pin changed for this correction, and no V8 fix is claimed.

The subsequent complete clean-implementation gate passed at `83dcd3e3bf9f3f32768848d9c592585a7a64d8c7`; the [review-follow-up cost/reflection record](../CAPSULE-COST-LEDGER.md#architecture-and-workflow-review-follow-up-2026-09-20) binds its command, receipt, timing, and output digest separately from those failures. That record compares the immutable repair and complete review-follow-up ranges, identifies removed duplication, and leaves the regional trial pending outside this correction.

Warm closure at `83dcd3e3bf9f3f32768848d9c592585a7a64d8c7` received `approve-with-required-edits` from the exact checkpoint reviewer `/root/parallel_output_capacity_checkpoint_review`. Packet `161e145ef94cef3a1a95e46bc7cdeff08c3eb7a17170050eecae3dd5ca4052c3` binds the target and gates; manifest `4a3fba6a097a22d59416012df9d606f548589c7ea1ac0cdd7238c032bfdc5b26` covers all thirteen substantive account, contract, exclusion, and evidence sections, whose completeness the reviewer independently confirmed. There were no implementation findings or advisories. The sole required edit was to commit current full-gate evidence and repair cost/reflection while preserving failed-run history and the pending regional trial. The same reviewer approved correction audit round one at `8e014d9d4e7b512ec32513b6b5683b79f997cb0a`, closing that finding with no remaining findings or advisories. Correction packet `169a413bbfc47b4f71ed6e23bb11bd4962750e6d75aab529187356461e8380ce` and complete thirteen-section continuity manifest `406c89b54b5997999957c0c04b44c96fd933cc9faae12fe5efec45d3ad044ed2` bind the documentary correction and successful infrastructure gate. This closes the PMI capacity and proof/evidence correction; neither the regional trial nor broader Compensation registration is included.
