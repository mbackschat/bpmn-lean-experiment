...
Lean declares operation-step constructors separately from executable dispatch, but the scope-entry, User Task arming, None-End, and scope-completion arms take their corresponding evaluator helper's result as a premise. For these arms, `fire_sound` checks dispatch/constructor consistency, not independently specified state transformation. The concrete conformance theorems below check their stated outcomes, and `completeScopeState_refuses_nonquiescent` quantifies over refusal while owned runtime work remains. The [targeted preservation gate](#targeted-preservation-gate) supplies executable core-side evidence over the selected prefixes; no general Lean preservation theorem follows from the dispatcher bridge.
...
|---|---|---|---|---|---|---|
| `SUBPROC-ENTER-01` | Clause 13.3.4 and exact source profile | `start_enters_one_child_scope_and_opens_both_tasks` checks the concrete outcome; `OperationStep.enterScope`/`fire_sound` check dispatch only | `CIB-AGR-0007` initial public tasks | Scoped lowering and runtime entry tests | Start trace in the four-target pipeline and restart witness | Event Sub-Process and cross-scope-flow rejection |
| `SUBPROC-WAIT-01` | Exact child topology and generic scoped graph | Concrete two-wait observation; operation soundness checks dispatch only | Both public child tasks in all four retained artifacts | Source declaration permutation and runtime prefix gates | Two passive User Task Updates with zero host-driven history | Premature outer-task mutation |
