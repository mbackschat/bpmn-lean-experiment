...
The next parallel capsule must add explicit semantic choice where more than one transition order is permitted rather than encode the evaluator’s branch order as semantics.
...
|---|---|---|---|---|
| `enteringStart`, `enteringUserTask`, `leavingUserTask`, and `enteringEnd` | Project-owned synthetic semantic control positions derived from the admitted sequential topology | State the internal flow-taking and activity-entry/exit steps without pretending they are BPMN XML nodes | Hidden from stable canonical observations; their transitions may appear only in diagnostic `MicroEvent` traces | Internal closure preserves logical time and must either reach a stable state or report harness-bound exhaustion |
| `waitingUserTask(instanceId, activation)` | Semantic runtime occurrence derived when the admitted User Task activates | Distinguish the shared User Task definition from one active occurrence | Projected as one active wait and one structured `OpenUserTask`; the raw constructor is not exposed | Exact Process instance, element, and activation identity is preserved until exact completion consumes the occurrence |
| `MicroEvent` | Project-owned diagnostic evidence derived from command admission and internal closure | Explain internal flow, wait, and completion transitions | Excluded from the canonical comparison trace in this capsule | Never substitutes for committed observation or command outcome |
