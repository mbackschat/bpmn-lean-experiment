...
| Semantic checkpoint | `ef4edd4` | `not-recorded` | `pending` | `not-applicable` |
...
The cold semantic-checkpoint review of `ef4edd4` ran under `fork-turns-none` and returned `approve-with-required-edits` across six required and five advisory findings. The row stays `pending` until the same reviewer's correction audit closes, because [the receipt guard](../../scripts/independent-review-policy.test.ts) requires that verdict to name a resolved audit target.
...
| [bounded-wait admission](../../packages/semantic-core/src/bounded-wait-admission.ts) | 457 | **Created by this change**, and it replaces the deleted `bounded-task-admission.ts`: the bounded Activity deadline and the bounded scope deadline are one admission concern, and keeping two near-duplicate owners was what would have let a managed race beside a bounded wait admit. |
...
| [Lean bounded scope](../../BpmnSemantics/SemanticProcess/BoundedScope.lean) | 535 | **Created by this change** as the narrow module beside `BoundedTask.lean` that the `Execution.lean` row promised. Carries arming today; the victory relation and laws land here. |
...
| [Lean runtime state](../../BpmnSemantics/SemanticProcess/RuntimeState.lean) | 48 | Near the re-expiry threshold with no new collection planned; adding one would force an extraction first. |
...
| [adapter typed contracts](../../packages/temporal-adapter/src/contracts.ts) | 346 | Sufficient; carries the new refusal identity. |
