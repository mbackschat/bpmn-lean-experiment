...
| [Lean bounded scope](../../BpmnSemantics/SemanticProcess/BoundedScope.lean) | 157 | **Created by this change** as the narrow module beside `BoundedTask.lean` that the `Execution.lean` row promised. Carries arming, both victory arms, `BoundedScopeVictoryStep`, both evaluator-soundness bridges, the withdrawal law, the counter-and-history preservation law, and the logical-time separation law. The capsule's required Lean lane is complete. |
