...
The checkpoint review left three obligations for closure. `ADINPUT-SCOPE-01`'s single-binding half is enforced at no boundary in either account: runtime well-formedness constrains the local-scope collection only by ordering, the projections answer absence for every cardinality but one, and the property holds because the single writer inserts one binding. Closure states that as an issuing-discipline consequence or adds the conjunct. The uniqueness half of the same rule likewise rests on `fresh` as an undischarged hypothesis rather than on a stated invariant. The family also owes the adversarial negatives its account already names: duplicate and absent local owner refusing completion, a wrong completion identity against a live wait, and the source mutations for two `sourceRef`s, a foreign-owner Property, a second `InputSet`, a `Transformation`, and `loopCharacteristics`.
