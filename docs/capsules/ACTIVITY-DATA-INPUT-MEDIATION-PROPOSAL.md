...
The closed internal-commutation account classifies `awaitDataInputUserTask` as `compositeWaitAndActivityArming`, not ordinary wait arming. Its TypeScript footprint reads `RuntimeControl`, the containing `ScopeOccurrence`, the incoming `ControlToken`, the exact `ProcessVariable`, both User Task and Activity `Activation` atoms, the absence-sensitive `Wait`, `OpenWaitAnchor`, `ActivityAssociation`, `ActivityVariableScope`, and target `ActivityVariable`, plus `LogicalTime` for the publication. Its one atomic successor writes the consumed `ControlToken`, both incremented `Activation` atoms, the new `Wait`, `OpenWaitAnchor`, `ActivityAssociation`, `ActivityVariableScope`, and `ActivityVariable`. Its publications are the `CommittedTransition`, started `FlowNodeLifecycle`, and their `PublicationPair`. It reads but does not write the Process binding and has no Timer, effect, incident, event-race, call, controller, occurrence-region, end-count, or unrelated variable atom. The Lean census receives the same composite family, and adversarial overlap evidence must reject an apparently disjoint candidate that shares the Process binding, incoming token, task or Activity activation, wait anchor, Activity association, local scope, local binding, or publication identity.
...
| Product 2 compatibility | Strict optional-input decoding and copied-contract tests only; no presentation or browser claim |
