...
BPMN 2.0.2 Clause 10.4.1 states that a Property's lifetime is tied to its parent Flow Element and that a Process-owned Property is accessible to the Process's contained elements. Clauses 10.4.1 and 10.4.2 plus Tables 10.58, 10.59, 10.63, and 10.64 state that Tasks may define data requirements through `InputOutputSpecification`, that every such specification has at least one `InputSet` and `OutputSet`, and that a direct Data Association with no transformation has exactly one source whose contents are copied to the target.
...
The public `OpenUserTask` contract gains an optional `inputs` field whose present shape is exactly one canonical `VariableBinding`. A data-bearing User Task must publish the field; every existing profile must omit it. This is an engine observation of selected BPMN DataInput state, not a form schema or authorization contract. General Activity-local variables remain private, and the Process-variable observation continues to expose only Process scope.

The existing `startProcess.initialVariables` ingress seeds the selected Process Property by exact Property ID. Duplicate names, unsupported value arms, and noncanonical order retain the existing admission refusal. No new data-update command is added.
...
- `ADINPUT-OBSERVE-01`: the active task publishes exactly its selected input binding, while no other Activity-local state enters public observation.

The smallest positive starts `Property_ReviewContext` with string `invoice-4711`, reaches one open task whose sole input is `DataInput_ReviewContext = invoice-4711`, completes it with no submitted value, observes the Process Property unchanged, and reaches the None End.
...
The ownership negative inserts an effect-owned local scope with the same structural three-field identity as the new Activity occurrence. It must neither satisfy the Activity input lookup nor be removed by task completion. This separates the proposed discriminated owner from an unsafe structural alias.
...
The pure semantic core alone decides whether the source Property is present, whether the User Task becomes active, which exact value is copied, and whether completion removes the local scope. Temporal stores the complete committed Program and RuntimeState and schedules the already-published task interaction. A Worker or Product 2 form may display the published input but may not fill a missing source, reinterpret null, or choose an InputSet.
...
Query and Product 1 publication derive the optional task input only from committed semantic state. Product 2 may copy that published field through its strict contract but may not reconstruct it from start payloads or definition XML. Visibility and Temporal Event History remain non-authoritative.

The smallest live-history refinement witness starts the invoice-review model with a non-null summary, reaches one task with the exact input, replaces the Worker, completes the task, obtains the terminal receipt, and replays every Run. A second direct-VM witness starts explicit null and distinguishes it from an absent source. The nearest adapter counterexample schedules a task from the source XML before the semantic core copies the binding, exposing an open task with no input; exact Query and replay evidence must reject it.
...
|---|---|
| Normative input availability and direct copy | BPMN 2.0.2 Clauses 10.4.1 and 10.4.2, Tables 10.58, 10.59, 10.63, and 10.64, and the pinned CMOF/XSD anchors; no CIB semantic vote |
...
| TypeScript realization | Separately written readiness, copy, ownership, completion, and observation logic plus focused state-preservation and negative tests |
...
| Durable refinement | Real-service Worker-replacement, terminal receipt, exact Query input, and every-Run replay plus a direct-VM absence/null discriminator |
...
| Activity-local input scope | Direct copy from the selected Process Property into the target DataInput | Only its one selected task binding appears as `OpenUserTask.inputs` | Created with the Activity and removed atomically with it |
...
The BPMN layer owns readiness, direct copy, local lifetime, and completion. The profile layer owns the exact source graph, Property-ID binding, value subset, and absence/null representation. Lean and TypeScript independently realize the same reviewed account. Temporal owns durability and delivery only. Product 2 consumes the published input and adds no semantic rule.

The invoice-review source is a retained business-purpose model and Product 2 adoption witness. It is not a model-specific semantic implementation. The reusable mechanism is the direct Activity input contract and its occurrence-owned runtime lifecycle.
...
|---|---|---|---|---|
| Proposal | `498a5bba` | `not-recorded` | `pending` | `not-applicable` |
