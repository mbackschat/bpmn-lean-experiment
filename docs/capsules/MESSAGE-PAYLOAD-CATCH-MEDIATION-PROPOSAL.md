...
The reviewed requirement ID is `BPMN-MESSAGE-PAYLOAD-CATCH-01`. Its requirement-ledger disposition would become `supported` only for this exact bounded slice; the broad `BPMN-MECH-EVENT-01` and `BPMN-MECH-DATA-01` families remain `unsupported`, the latter because its scope is exactly the `ItemDefinition`, `DataOutput`, and `DataAssociation` breadth this slice takes one point of. `BPMN-MESSAGE-CATCH-01` stays `supported` for its own payload-free slice, which this capsule does not reinterpret.
