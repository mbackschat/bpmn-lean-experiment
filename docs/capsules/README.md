...
| [Bounded called-Process Call Activity](CALL-ACTIVITY-PROPOSAL.md) | Owner-approved standards-only proposal at its required-edits semantic checkpoint for one namespace-qualified in-document called Process, distinct semantic Process-instance identity, quiescent exactly-once return, empty data boundary, and planned single-Workflow passive-Update refinement without a Temporal Child Workflow or CIB Call-Activity claim; artifact, differential, and durable Temporal lanes remain blocked |
