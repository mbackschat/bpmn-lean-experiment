...
| [Inclusive Gateway selected-branch synchronization](INCLUSIVE-GATEWAY-PROPOSAL.md) | Draft, not owner-approved proposal for one structured two-condition-plus-default split and paired selected-subset join using Simple Boolean v1, generic multi-selection/synchronization operations, hidden occurrence-owned selection state, and standards-first Lean/TypeScript/Temporal evidence |
