...
| [Bounded called-Process Call Activity](CALL-ACTIVITY-PROPOSAL.md) | Unapproved standards-only proposal for one namespace-qualified in-document called Process, distinct semantic Process-instance identity, quiescent exactly-once return, empty data boundary, and single-Workflow passive-Update refinement without a Temporal Child Workflow or CIB Call-Activity claim |
