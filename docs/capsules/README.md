...
| [Inclusive Gateway selected-branch synchronization](INCLUSIVE-GATEWAY-PROPOSAL.md) | Owner-approved implemented closure candidate for one structured two-condition-plus-default split and paired selected-subset join using Simple Boolean v1, generic multi-selection/synchronization operations, hidden occurrence-owned selection state, four standards-only differential cases, and Temporal replacement/replay evidence; closure review remains pending |
