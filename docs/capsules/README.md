...
| [User Task completion data](USER-TASK-COMPLETION-DATA-PROPOSAL.md) | Owner-approved unimplemented CIB Seven `2.2.0` completion-patch proposal for selected Process-variable input and string/null simulated form submission; phase-zero CIB evidence remains required |
...
| [Embedded Sub-Process Error propagation](SUBPROCESS-ERROR-PROPAGATION-PROPOSAL.md) | Selected bounded proposal for one error-only embedded Sub-Process, exact-code Error End propagation to its direct interrupting boundary handler, regional cancellation of a live sibling User Task, and outer recovery; implementation is deferred behind the runnable MVP and CIB `2.2.0` breadth schedule |
