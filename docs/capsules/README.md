...
| [Event-Based Gateway Message/Timer race](EVENT-BASED-GATEWAY-PROPOSAL.md) | Owner-approved, implemented standards-only proposal for one non-instantiating Exclusive Event-Based Gateway that atomically arms one operation-addressed Message catch and one exact `PT1S` Timer catch, commits one explicitly ordered winner, withdraws the loser, and refines separate readiness through bounded durable scheduling/replay without a CIB Event-Gateway claim; cold closure review is pending |
