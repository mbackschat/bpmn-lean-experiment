...
| [CreateDocument data and mapping](CREATE-DOCUMENT-DATA-PROPOSAL.md) | Owner-approved A12 target capsule; the distinct CIB Seven `2.0.0` profile, exact unchanged-source admission, hostile-source controls, typed checked mappings, and TypeScript/Lean lowering are implemented, while runtime data semantics and CIB/Temporal evidence remain pending |
