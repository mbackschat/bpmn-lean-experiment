...
15. atomic versioning consequences across every producer and consumer, including the executable guards and test oracles the implementation must change **or must satisfy** and not only runtime code, each named as a Markdown link to its path so ordinary link resolution proves the referent exists, plus an explicit statement of the applicable pre-release or durable-history policy. A guard that constrains the work without changing counts: module-size headroom in the owners the implementation will grow is part of this content, not a detail discovered while editing. Derive both lists mechanically with `node scripts/what-binds.ts <path>...` rather than from recall; [the reviewability guard](../../scripts/document-reviewability.test.ts) requires every capsule proposal to name at least one resolvable guard and one resolvable source owner here, because a constraint that is only remembered is the one that surfaces mid-implementation. Every structural claim in this inventory states the condition under which it stops applying, never a bare instruction: "extract first" is true only under a measurement, so record the measurement and the threshold. The same guard recomputes each stated headroom figure, so an owner that changes size fails the gate rather than leaving a requirement that reads as permanent after its premise expired;
...
| [Event-Based Gateway Message/Timer race](EVENT-BASED-GATEWAY-SPEC.md) | Implemented evidence-closed specification for one non-instantiating Exclusive Event-Based Gateway that atomically arms one operation-addressed Message catch and one exact `PT1S` Timer catch, commits one explicitly ordered winner, withdraws the loser, and refines separate readiness through bounded durable scheduling/replay without a CIB Event-Gateway claim |
| [Bounded called-Process Call Activity](CALL-ACTIVITY-SPEC.md) | Implemented and evidence-closed standards-only profile for one namespace-qualified in-document called Process, distinct semantic Process-instance identity, quiescent exactly-once return, empty data boundary, registered Lean/core/Temporal differential case, and single-Workflow passive-Update refinement with Worker replacement, mutations, and replay but no Temporal Child Workflow or CIB Call-Activity claim |
...
| [Interrupting Sub-Process boundary Timer](SUBPROCESS-BOUNDARY-TIMER-SPEC.md) | Implemented and evidence-closed, graduated on the closed closure review: one interrupting exact-`PT1S` Timer Boundary Event on one embedded Sub-Process, giving a *scope* a deadline that terminates its live child region, withdrawn by the child's own quiescent completion, standards-only with one recorded normative conflict in Clause 13.3.2's nested-termination wording |
| [Interrupting Activity boundary Timer](ACTIVITY-BOUNDARY-TIMER-SPEC.md) | Implemented and evidence-closed: one interrupting exact-`PT1S` Timer Boundary Event on one User Task, one new Activity-owned wait operation instead of a widened event race or sibling waits, a distinct published follow-on User Task per route as the separating boundary, standards-only with no CIB relationship, and the Update-arm coalescing premise witness |
