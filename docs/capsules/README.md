...
| [Embedded Sub-Process Error propagation](SUBPROCESS-ERROR-PROPAGATION-PROPOSAL.md) | Owner-approved proposal for exact-code Error End propagation, direct interrupting handling, regional cancellation with monotonic counter preservation, unreachable normal continuation, outer recovery, and post-throw Worker replacement; the selected CIB Seven `2.2.0` phase-zero lane is green under `CIB-AGR-0008`, while semantic implementation has not started |
