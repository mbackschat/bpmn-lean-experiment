...
| [Bounded called-Process Call Activity](CALL-ACTIVITY-PROPOSAL.md) | Owner-approved standards-only closure candidate for one namespace-qualified in-document called Process, distinct semantic Process-instance identity, quiescent exactly-once return, empty data boundary, registered Lean/core/Temporal differential case, and single-Workflow passive-Update refinement with Worker replacement, mutations, and replay but no Temporal Child Workflow or CIB Call-Activity claim |
