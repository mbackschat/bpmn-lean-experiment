...
| [Interrupting Activity boundary Timer](ACTIVITY-BOUNDARY-TIMER-PROPOSAL.md) | Proposed and unapproved: one interrupting exact-`PT1S` Timer Boundary Event on one User Task, one new Activity-owned wait operation instead of a widened event race or sibling waits, two distinct End Events as the separating boundary, standards-only with no CIB relationship, and the Update-arm coalescing risk named for review |
