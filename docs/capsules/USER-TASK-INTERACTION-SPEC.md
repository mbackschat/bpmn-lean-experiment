...
| Closure bound and ambiguity flags | Harness-only diagnostics derived by complete enabled-set closure classification | Keep nontermination and undeclared scheduling choice separate from semantic outcomes | Excluded from canonical observations and command outcomes | Lean and TypeScript both reach a stable state, report bound exhaustion, or refuse an unsupported multiple-enabled set without publishing. TypeScript retains ambiguity only on the internal `StimulusEvaluationResult`, not the public command result. The exact independent two-User-Task pair is the sole multiple-enabled shape that advances, with both activation orders checked to have the same stable public observation |
