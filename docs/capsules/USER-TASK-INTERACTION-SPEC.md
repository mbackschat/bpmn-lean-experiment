...
|---|---|---|---|---|
| `ProcessControl` plus `initiationPending` | Project-owned semantic lifecycle derived from an accepted start and the generic `initiate`, `reachNoneEnd`, and quiescent `completeScope` operations | Separate external start admission from internal initiation and completion without a topology-specific control-state enum | Projected only as the canonical Process status | One Process instance moves from not started to running; it becomes completed only after `reachNoneEnd` records the End occurrence and `completeScope` finds the root occurrence quiescent |
...
| Closure bound and ambiguity flags | Harness-only diagnostics derived by unique-only internal closure | Keep nontermination and undeclared scheduling choice separate from semantic outcomes | Excluded from canonical observations and command outcomes | Lean's closure reaches a stable state, reports bound exhaustion, or reports more than one enabled successor without choosing by collection order. The shipped TypeScript core diverges: it has no ambiguity outcome and advances the lowest canonical operation ID among enabled successors. The two accounts coincide only because current admission keeps every admitted state at one enabled successor or the checked disjoint two-task pair, where sorted-operation admission makes the lowest ID and Lean's program-order head the same choice |
