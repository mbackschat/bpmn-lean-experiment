...
The exact profile guard admits nine operations and seven control places arranged as one initiation, one choice, three branch waits, three end-reaching operations, and one root completion. Starting the Process takes exactly three internal transitions: `initiate`, `choose`, then `awaitUserTask`. Completing the accepted branch takes exactly two internal transitions: `reachNoneEnd`, then quiescent `completeScope`. The runtime-bound guard exposes exhaustion at two start steps; every admitted command path remains below `semanticProcessClosureLimit = 8`.
