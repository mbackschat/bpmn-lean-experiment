...
The profile ID is `cibseven-2.2.0-parallel-user-task-assignment-form-metadata-draft`. The CIB prefix records that the admitted source includes the selected CIB assignment/form extensions. Parallel synchronization remains the standards-owned account, with the bounded positive CIB calibration under `CIB-AGR-0003` and candidate deviation `CIB-DEV-0001` still prominent.

The exact composed CIB observation must show both live tasks and the exact public identity-link and form-field facts for each task. Existing `CIB-EXT-0011` and `CIB-EXT-0012` may be reused only if that combined observation preserves their existing per-task meaning. If simultaneous publication changes the proposition, evidence shape, or fidelity boundary, stop and register a separately classified relationship before implementation.
...
7. Keep the CIB claims separated: balanced parallel behavior remains `CIB-AGR-0003`, task identity mapping remains `CIB-OP-0001`, metadata remains `CIB-EXT-0011/0012`, and `CIB-DEV-0001` remains unresolved. Add one combined raw-service observation instead of inferring composition from separate fixtures.
...
| `PARMETA-HOST-01` | Existing Update ingress, committed semantic state, Query, Worker replacement, replay, and terminal receipt host the composed wait set without a new Temporal primitive or host-owned meaning. | Temporal adapter |
...
The host-capability gate must accept the exact pair of independent metadata-bearing User Task waits before Workflow start. It must still reject every unreviewed wait-set shape. Duplicate recovery returns the retained result and creates no second semantic completion. Worker replacement between the two completions and exact history replay must preserve both metadata values, the remaining sibling, and the final receipt.
...
| `PARMETA-DATA-01` | distinct declared fields in exact model | disjoint-key hypothesis | accepted-order and disjoint-commutation tests | raw variables are calibration only | exact command order | exact form submissions |
| `PARMETA-CIB-01` | classified relationships only | not a CIB theorem | raw-to-canonical comparison | combined public-service observation | not a host fact | no CIB IDs exposed |
...
- proved Lean composition, independent TypeScript tests, combined pinned-CIB evidence, four-target differential comparison, live Temporal replacement/replay, and the complete Chromium journey;
...
- arbitrary parallel graphs, more branches, repeated task elements, loops, nested scopes, mixed waits, conditions, data-based routing, or multiple-enabled shapes beyond the approved independent User Task pair;
...
The profile and evidence artifacts must satisfy the strict [semantic profile schema](../../contracts/schemas/semantic-profile.schema.json), [CIB evidence schema](../../contracts/schemas/cibseven-evidence.schema.json), [contract artifact registry](../../scripts/contract-artifact-cases.ts), and the generated catalog and corpus guards. The measured bindings for the proposal are 32 executable guards and two registries; the principal implementation owners each bind 27 or more guards plus their package registry. Re-run `node scripts/what-binds.ts` at implementation start because these measurements are valid only for the current worktree.

### Guards and registries

The atomic change must satisfy the existing [admission composition oracle](../../packages/semantic-core/test/admission-composition.test.ts), [foreign-attribute admission oracle](../../packages/bpmn-source/test/foreign-attribute-admission.test.ts), [profile artifact guard](../../scripts/contract-definition-artifacts.test.ts), [runnable profile registry guard](../../packages/temporal-adapter/testkit/test/product-example-configs.test.ts), [corpus policy guard](../../model-corpus/test/executable-model-corpus.test.ts), [document reviewability guard](../../scripts/document-reviewability.test.ts), and every additional binding reported by `node scripts/what-binds.ts`. Update the human registries in [profiles](../../profiles/README.md), [scenarios](../../scenarios/README.md), [semantic core](../../packages/semantic-core/README.md), [source compiler](../../packages/bpmn-source/README.md), [differential harness](../../packages/differential/README.md), [Temporal adapter](../../packages/temporal-adapter/README.md), and [shared contracts](../../contracts/README.md) in the same change.
