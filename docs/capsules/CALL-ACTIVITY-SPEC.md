...
The reverse-uniqueness repair was independently approved on 2026-09-19 over `36cce36d..6aa195bf` by a `fork-turns-none` reviewer inheriting the root model and effort, with no required semantic corrections. Semantic-checkpoint packet digest: `c2a58287f933ba6f7a098a111292bd685f3a7e9315284a748908a65eb28bb086`. The reviewer verified both balanced many-to-one counterexamples, unchanged scope-separation theorem hypotheses and conclusion, and all 422 archived Lean/build inputs against the target. Reviewer-selected pairing/association TypeScript checks passed all 11 tests; the narrow Lean pairing and admission-facts build completed 160 jobs with exit zero. The [fixed-3-GiB calibration](../CAPSULE-COST-LEDGER.md#call-pairing-calibration) records an actual fixture rebuild separately from that warmed native check. Historical red-run output is unavailable; no new historical-red claim is made. The sole advisory, a separated cost-table row, is corrected in this receipt update.

The clean complete path-selected gate `./scripts/pnpm.sh run test:pre-push` subsequently completed at `6aa195bf8e304aa99e73d6a68d66ab74ef3b3f40` with exit zero, separately asserted through `scripts/assert-command-receipt.ts`. Its retained output SHA-256 is `c6c75fc03145b2861ed22d9209d28607168f81a771b63bb77a96cae0f6f6ef0d`. This accepts the bounded pairing correction; it neither broadens the original Call profile nor closes scope-creation lifecycle publication or whole internal commutation.
...
The standalone pairing predicates in Lean and TypeScript require both directions of uniqueness independently of graph admission: every invoke resolves exactly one return, and every return is referenced by exactly one invoke. Equal population counts alone are insufficient. The [Lean pairing witnesses](../../BpmnSemantics/CallActivityPairingConformance.lean) and [independent TypeScript tests](../../packages/semantic-core/test/call-activity-pairing.test.ts) reject balanced populations containing two invokes that share one return and a second unclaimed return, with either duplicated or distinct invoke identities. Both constructed populations also fail full Lean Program admission; these witnesses establish the pairing predicate's local contract, not an admitted-program exploit or broader Call capability.
...
The live witness starts the caller Workflow, pins the called-task Query and derived semantic ID, rejects a caller-owned substitute with the called wait preserved, commits called completion, stops the Worker, recovers an exact retry plus the caller-only Query under a replacement Worker, completes the caller task, checks the terminal receipt, and replays the disposable history. The exact retry adds no accepted Update; the history contains three distinct accepted/completed Updates and zero Child Workflow, Signal, Timer, Activity, effect, or cancellation events.
...
The durable witness starts one Workflow under the caller/root address, queries the called-owned task, rejects a caller-owned substitute while preserving the called wait, and submits the called completion through the existing API that takes the Workflow address and semantic task-instance address separately. It stops the Worker after that completion, then requires a replacement Worker to recover the exact committed retry and the identical caller-only Query. Caller completion returns the ordinary terminal receipt. The retry adds no accepted Update; the history contains exactly three distinct accepted/completed Updates, no Signal, Timer, Activity, Child Workflow, effect, or cancellation event, and replays in the same disposable gate.
