...
The capsule-local preservation gate executes Trigger-first and Sibling-first schedules and establishes that every stable state reached by those two checked schedules is terminal or has at least one explicit User Task resumption surface; no checked stable running state retains a token or active scope without an interaction. Within those schedules, closure never chooses between multiple enabled internal operations by program list order, and the child's existing `completeScope` operation never competes with or precedes the enabled Error throw.
...
| `SUBERR-ADMIT-01` | selected narrowing | program rejection | source/program rejection | typed pre-start rejection | non-interrupting, misattached, unresolved-reference, and missing-code source rejection plus handler-output and attached-scope program mutations | exact fixture deployment in phase zero |
