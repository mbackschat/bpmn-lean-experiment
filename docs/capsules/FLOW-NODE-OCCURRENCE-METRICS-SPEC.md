...
2. Publish the selected start-to-terminal occurrence projection for every currently admitted flow-node family: start, then either completed or cancelled. The data-input User Task's [Ready interval remains unpublished](ACTIVITY-DATA-INPUT-MEDIATION-SPEC.md#runtime-command-stable-state-and-observation-contract). A Boundary Event starts only when it catches, while Event-Based Gateway candidate Catch Events start when the race arms them and later complete or cancel.
