...
The additive Activity-family issuing discipline required by the public outer identity is implemented and independently checkpoint-approved under [its governed receipt](../RUNTIME-STATE-ACTIVITY-ISSUING-DISCIPLINE-PROPOSAL.md#independent-cold-review-receipt). The Temporal host class is implemented at the admission, controller-join, durable-deadline, and rollover-policy boundary. Production E1 now validates the selected optional `openMultiInstances` projection only for a declaring program, while independent E2 completeness and Program binding admit the exact zero/nonzero entry, task-turnover, final-completion, and lifetime-Timer interruption shapes without licensing the synthetic outer/controller occurrence. The private capacity probe may therefore measure those production envelopes without execution registration. Execution registration remains blocked by the measured capacity owner and then by the live refinement witnesses, complete gates, and closure review below.
