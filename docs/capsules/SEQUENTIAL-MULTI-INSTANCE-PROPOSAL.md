...
The budget owner's inputs come from one private testkit-only Temporal capacity probe, not from invented constants and not from an SMI semantic-execution bypass. The probe uses the real Temporal service and production serializers, validators, continuation envelopes, recovery envelopes, and publication envelopes to run the maximal natural and interrupted host-command topologies described above: pre-arming Continue-As-New, one lifetime Timer, sixteen content-bound completion Updates with their Workflow Tasks and recovery results, the largest aligned E1/E2 publications, and terminal closure. Its payloads are the exact maximal SMI state and publication envelopes produced through the pure semantic transition and projection boundaries, but the probe does not call command admission, execute BPMN, publish a supported profile, or define semantic meaning. It records service-reported Event count and History size, the largest activation payload, and closed event-family counts for each topology. The pure owner cites those measured maxima and proves the project trigger and reserve inequalities, exact-16 fit, and exact-17 refusal. The later production witness must remain within every corresponding bound and reproduce the required event families; a mismatch reopens the owner rather than being excused by the probe.
