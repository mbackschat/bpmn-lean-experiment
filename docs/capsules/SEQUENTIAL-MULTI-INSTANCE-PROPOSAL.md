...
The additive Activity-family issuing discipline required by the public outer identity is implemented and independently checkpoint-approved under [its governed receipt](../RUNTIME-STATE-ACTIVITY-ISSUING-DISCIPLINE-PROPOSAL.md#independent-cold-review-receipt). The Temporal host class is implemented at the admission, controller-join, durable-deadline, and rollover-policy boundary. The private capacity probe exposed two already-required production publication prerequisites before it could reach the Temporal service: E1 validation still rejects the selected optional `openMultiInstances` projection, and E2 completeness still carries the deliberate fail-closed arm from before the runtime transition existed. Implement those two reviewed validators without execution registration, then measure and close the capacity owner. Execution registration remains blocked by that owner and then by the live refinement witnesses, complete gates, and closure review below.
...
The first private-probe Red then stopped before Temporal startup at two production publication validators that the implementation checklist above already requires. The exact SMI `StateObservation` includes `openMultiInstances`, but the protocol E1 validator still admits only the pre-SMI key set; after diagnostic omission of that field, the E2 completeness relation rejects the internal `awaitSequentialMultiInstanceUserTask` lifecycle through the placeholder fail-closed arm recorded while the operation was source-only. The production evaluator remains correctly unavailable before registration, so neither validator may be bypassed with a fabricated committed checkpoint. Implementing the reviewed E1 field validation and E2 lifecycle completeness before the private probe changes no selected account, public contract, exclusion, capacity strategy, or registration claim and does not invalidate the approved checkpoint.
