...
The additive Activity-family issuing discipline required by the public outer identity is implemented and independently checkpoint-approved under [its governed receipt](../RUNTIME-STATE-ACTIVITY-ISSUING-DISCIPLINE-PROPOSAL.md#independent-cold-review-receipt). The Temporal host class is implemented at the admission, controller-join, durable-deadline, and rollover-policy boundary. Production E1 validates the selected optional `openMultiInstances` projection only for a declaring program, while independent E2 completeness and Program binding admit the exact zero/nonzero entry, task-turnover, final-completion, and lifetime-Timer interruption shapes without licensing the synthetic outer/controller occurrence. The private exact-version capacity probe and pure capacity owner are now measured and green without execution registration. Execution registration remains blocked by the live production refinement witnesses, complete gates, and closure review below.
...
The budget owner's inputs come from one private testkit-only Temporal capacity probe, not from invented constants and not from an SMI semantic-execution bypass. The probe uses the real Temporal service and production serializers, validators, continuation envelopes, recovery envelopes, and publication envelopes to run the maximal natural and interrupted host-command topologies described above: pre-arming Continue-As-New, one lifetime Timer, sixteen content-bound completion Updates with their Workflow Tasks and recovery results, the largest aligned E1/E2 publications, and terminal closure. Its payloads are the exact jointly admissible maximal SMI state and publication envelopes produced through the pure semantic transition and projection boundaries, but the probe does not call command admission, execute BPMN, publish a supported profile, or define semantic meaning. It records service-reported Event count and History size, the largest activation payload, and closed event-family counts for each topology. The pinned CLI `1.8.1`, Server `1.31.2`, and SDK `1.21.0` measurement replays all five Runs and retains exact maxima of 87 Events, 259,636 conservative History bytes, 10 activation Events, and 246,799 canonical activation payload bytes. The pure owner proves those values remain below the 8,000-Event, 8 MiB, 2,240-Event, and 2 MiB boundaries, and separately executes exact-16 fit plus a count-only exact-17 refusal with unchanged state. The later production witness must remain within every corresponding bound and reproduce the required event families; a mismatch reopens the owner rather than being excused by the probe.

This ordering is required by the execution boundary. Both initial Workflow entry and incoming continuation validate `supportsSemanticProcessExecution`, so an unregistered profile cannot produce either the direct-VM witness or the production histories from which the earlier ordering expected capacity constants. Constructing a committed checkpoint manually would bypass the boundary being evidenced, while registering early would reverse the stop condition. Implementation order is therefore: measure the private host-capacity probe; close the pure budget owner with its exact-16 fit and exact-17 refusal; atomically add execution registration together with the direct-VM and production live witnesses; then compare the production histories with the pre-registration bounds before making any support claim. If the selected sixteen-item limit does not fit, if the probe cannot reproduce the named production host topology without a semantic bypass, or if a live production history exceeds its bound, implementation stops and this proposal is reopened.
...
| [history-capacity owner](../../packages/temporal-adapter/workflow/src/sequential-multi-instance-history-capacity.ts) | 26 |
| [history-capacity owner tests](../../packages/temporal-adapter/workflow/test/sequential-multi-instance-history-capacity.test.ts) | 213 |
| [capacity source fixture](../../packages/temporal-adapter/testkit/test/sequential-multi-instance-history-capacity-fixture.ts) | 219 |
| [capacity fixture oracle](../../packages/temporal-adapter/testkit/test/sequential-multi-instance-history-capacity-fixture.test.ts) | 569 |
...
| [capacity topology builders](../../packages/temporal-adapter/testkit/test/sequential-multi-instance-history-capacity-topologies.ts) | 222 |
| [capacity private Workflow](../../packages/temporal-adapter/testkit/test/sequential-multi-instance-history-capacity-workflows.ts) | 205 |
| [capacity real-service probe](../../packages/temporal-adapter/testkit/test/sequential-multi-instance-history-capacity.temporal-test.ts) | 194 |
