...
- `cancelActivity` omitted as in the target fixture, with lexical `true` tolerated as redundant because it is the attribute's declared default, and every lexeme naming *false*, which is `false` and `0`, rejected. `1` is refused before parsing and under its own diagnostic, as the one valid lexeme the parser inverts;
...
The checked source retains the mapping bodies, Boundary Event ID and optional name, attachment, Error Event Definition ID and reference, root Error ID, optional name, exact code, boundary Sequence Flow, and exact BPMN element provenance. The target's `BoundaryEvent.name="Error Event"` and `Error.name="Link Limit Reached"` are source metadata, not matching keys. A name change changes exact source identity but not runtime matching. Admission rejects a missing or unresolved reference, a different code, a `cancelActivity` naming *false*, a second handler, a catch-all definition, an unattached event, the wrong expression sigil, or foreign executable content.
