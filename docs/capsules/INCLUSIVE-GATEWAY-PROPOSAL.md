...
All nodes and Sequence Flows belong to the root definition scope. Every reference resolves, every branch runs directly from the split through its User Task to the join, and the graph is acyclic, reachable, and co-reachable. There is no alternate entry, exit, cross-branch Flow, nested scope, boundary Event, loop characteristic, or additional incoming Flow at either Gateway. For each Gateway, `gatewayDirection` may be omitted, use the exact XSD literal `Unspecified`, or use the exact `Diverging` or `Converging` literal matching the direction derived from this admitted arity; an opposite direction and the exact `Mixed` literal are rejected because the admitted arity is not mixed. Other casing is invalid BPMN XML and is rejected rather than normalized. This follows the CMOF default and the implemented Parallel and Exclusive Gateway admission convention while retaining topology-derived direction as the authority. Source admission tests must distinguish omitted, exact `Unspecified`, matching, opposite, exact `Mixed`, and case-invalid forms rather than proving only the representative omission.
...
Semantic admission and Temporal host-capability admission remain separate. This exact profile passes host capability because it contains no Timer or effect wait, but `selectMany` is a token-splitting operation. The implemented exhaustive operation-kind classification recognizes both `duplicate` and `selectMany` and fails compilation when another operation kind is added without a decision. Negative programs combining `selectMany` with an `awaitTimer` or `awaitEffect` are rejected before Workflow start as `concurrentHostDrivenWaits`; a mutation omitting `selectMany` from split classification proves that class guard is load-bearing even though those combinations remain outside this capsule.
...
The exact established claim is that one admitted acyclic Inclusive split evaluates both Simple Boolean conditions from the same committed bindings, activates the complete true subset or the default, records that occurrence's corresponding join inputs, and releases the paired join only after every selected input arrives. The closest unsupported claim remains general Inclusive Gateway reachability for unstructured, nested, repeated, externally entered, or otherwise nonlocal graphs.

The main common-mode risk is a shared wrong branch-to-join mapping across source lowering, Lean, TypeScript, and Temporal. Branch-local checked-definition binding, a same-set wrong-pairing negative, independent Lean lowering equality, source-order permutations, and the one-true/both-true/default cases constrain that class. A separate `selectMany` selection-bypass Workflow and case-specific differential mutations prove that agreement is not merely inherited from one shared expected result.
