# Message key correlation specification
...
The reviewed requirement is `BPMN-MESSAGE-CORRELATION-01`. Its disposition is `supported` only for this exact bounded slice. Existing `BPMN-MESSAGE-CATCH-01`, `BPMN-MESSAGE-PAYLOAD-CATCH-01`, `BPMN-BOUNDARY-MESSAGE-01`, and `BPMN-RECEIVE-TASK-01` meanings remain unchanged.
...
| Whole-model and product reach | One credible project-owned settlement-confirmation model, exact pipeline binding, generated corpus map, canonical capability row, Product 2 About-page disclosure, and a production-browser/real-Temporal target-free publication journey covering unique, zero-match, ambiguous, response-loss retry, privacy, and only-target advancement |
...
No size exception was required. The implementation extracted correlation-specific source reading, scalar-path handling, the pure matcher and per-instance transition, Lean relations and laws, population evidence, the definition-scoped API, protocol and codecs, the public client, ingress Workflow state machines, Process registration and Query adaptation, and Worker Activities into bounded owners. Existing contract, dispatch, package-index, Workflow-composition, and continuation files retain only the closed arms and composition needed to connect those owners.

The implementation updated this specification, the [shared wire-contract evolution policy](../../contracts/README.md#evolution-policy), the [requirement ledger](../BPMN-REQUIREMENT-LEDGER.md), [Semantic Process IL specification](../SEMANTIC-PROCESS-IL-SPEC.md), the [internal-commutation proposal](../INTERNAL-COMMUTATION-PROPOSAL.md), the [Temporal Process lifecycle specification](../TEMPORAL-PROCESS-LIFECYCLE-SPEC.md), applicable detail maps routed by [`implementation-status-router`](../IMPLEMENTATION-MAP.md), the engine API README, semantic-core and Temporal package source maps including the [Temporal adapter source map](../../packages/temporal-adapter/SOURCE-MAP.md), Lean module graph, scenario/profile/corpus registries, generated corpus map, canonical capability catalog, Product 2 About-page disclosure, capsule cost ledger, and [PLAN](../PLAN.md).

## Epistemic closure and reopen conditions

Established: the normative key/context account and exact machine-readable shape; direct payload extraction and Process-Property writing; direct Message subscription identity/lifetime; per-instance payload delivery and Temporal Signal recovery; CIB's modeled-correlation limitation; exact source admission and checked/IL lowering for the bounded graph; strict scalar-path decoding; complete-address finite matching; target-revalidating per-instance delivery; correlated candidate projection; internal-commutation census and footprint integration; the SDK boundary requiring client-backed Activities for cross-Workflow Queries and Updates; durable engine-owned ingress identity and registration; barrier-linearized complete fanout; bounded publication reservation and FIFO settlement; same-target delivery and response-loss recovery; target-contradiction quarantine; independently reviewed bounded ingress continuation carrying the combined registration/publication state, assigned-before-barrier pending phase, installed barrier, selected target, queued work, and settled results without Event History or platform authority; the target-free Product 1 publication operation; the registered profile and answer-free engine-population contract; exact Lean/core population agreement; and public Temporal unique, zero, ambiguous, and cross-definition refinement through Worker replacement, same-target response-loss recovery, and replay.

Established beyond the population-refinement boundary: retained whole-model corpus coverage through a protected engine-population route, generated corpus-map coverage, canonical capability disclosure, Product 2 About-page disclosure, and a production-browser/real-Temporal Product 2 journey that reconstructs the definition-scoped capability, registers four Process candidates through the ordinary Product 1 path, publishes without a Process or host locator, distinguishes unique, zero-match, and ambiguous outcomes, retries identical content after a dropped response, and advances only the uniquely matched Process.

Independent closure review approved this complete evidence boundary with one required evidence correction: the production Process-start guard now derives every direct and scheduled constructor from the Temporal client TypeScript graph, verifies its exact Workflow-chain host argument and content-bound producer, and proves that deleting any one of the four current host arguments fails the guard. The semantic disposition is supported only for the bounded source, profile, runtime, hosting, and observation contract above.
...
The nearest realistic counterexample is a lexically first candidate with the same local Process, Message, key, and subscription ids as the intended candidate but a different immutable definition identity, combined with a retry after the intended Process has already committed. The cross-definition and permutation schedules prove that local-id order cannot select that candidate, while the live response-loss witness proves that retry resolves only against the retained target and cannot rematch or retarget.
...
| Closure | `a4acf21d` | `fork-turns-none` | `approve-with-required-edits` | `21697824` |
