...
BPMN 2.0.2 Clauses 10.7.2 and 13.5.5 define Compensation catch/throw use, global untargeted compensation, compensation handlers, and reverse dependency order. Tables 10.87, 10.91, and 10.92 settle three source traps: Compensation Start `isInterrupting` is inapplicable, Compensation Boundary `cancelActivity` is inapplicable, and Association direction has no execution meaning. The official Travel Booking machine-readable example confirms that an explicit true Compensation Start attribute cannot select interruption behavior.
...
**Event Sub-Process handler:** `ArrangeGroundTravel` contains exactly one Event Sub-Process with `triggeredByEvent=true`. Its body is `Compensation Start -> Service Task -> None End`; the Service Task carries the exact implementation literal. It has one required scalar `DataInput`, one `InputSet` containing exactly that input, one empty `OutputSet`, and one direct `DataInputAssociation` from the root Process Property to that DataInput. The Property and DataInput `itemSubjectRef` values resolve to the same ItemDefinition by parser-object identity. No Sub-Process-local Property, DataObject, DataObjectReference, or other binding is present, so the approved complete-parent-snapshot premise stays closed.

**Optional:** human-readable `name` values, BPMN DI, inline versus referenced effective Compensation definitions, and any schema-valid values of the semantically inapplicable attributes above. Optional material is preserved in exact source bytes or validated DI and never becomes semantic identity.
...
The checked graph contains three definition scopes: the root Process, the ordinary embedded `ArrangeGroundTravel` scope, and its dormant Compensation Event Sub-Process scope. The dormant scope has no ordinary checked nodes or Sequence Flows; its complete body is owned by the checked Compensation subject. Checked admission derives this sole empty-scope exception from that exact subject and rejects any undeclared, active, multiply claimed, or nonempty dormant scope.

`triggerElementId` must identify the one global synchronous throw node, which prevents later cardinality widening from making the declaration-to-trigger join implicit. Subjects sort canonically by their source subject element id; dependencies sort by predecessor then successor id. The checkpoint profile requires exactly two boundary subjects, one Event Sub-Process subject, one dependency from the first boundary subject to the Event Sub-Process parent, and no other pairing. Every id is a resolved source identity, never a display name.

The fixed limits reserve room below the existing per-collection 65,536-byte ceiling and match the only reachable cardinalities. Implementation evidence must measure a positive full fixture, exact-boundary acceptance, and one-byte-over refusal for each collection, and prove that an accepted 8 KiB snapshot can fit the 16 KiB trigger representation. A measured valid fixture that cannot fit these values reopens this proposal; the compiler may not silently raise them.
...
For the restored body, `sourcePropertyId` becomes the existing Program `sourceName` and `targetDataInputId` becomes `argumentName`. Those fields are runtime binding keys and intentionally carry resolved BPMN ids; XML display names have no authority. The DataInputAssociation id and its internal flow ids remain exact-source evidence because the Program has no field for them; claiming checked-to-Program preservation for erased identities would be false.
...
| `COMPSRC-INAPPLICABLE-01` | Association direction, boundary cancellation, Compensation Start interruption, and catch-side completion flags do not change checked semantic fields. | Omitted, true/false or zero, and every Association direction preserve semantic fields modulo exact source identity. |
...
| `COMPSRC-CHECKED-01` | The optional checked declaration completely owns the trigger join, dormant handler identities, bodies, dependencies, and fixed limits. | Remove the declaration, trigger join, one scope, body id, binding endpoint, dependency endpoint, or limit; strict checked admission fails. |
...
The checkpoint Program contains `triggerCompensation`, so existing host admission must refuse it with `compensationSchedulerUnavailable` before Workflow start. A focused negative witness supplies the newly lowered Program and proves no Workflow id, Run id, Event History, or adapter state is created. This is the smallest executable refinement witness for this stage.
...
|---|---|
| Normative and machine-readable source account | BPMN 2.0.2 Clauses 10.7.2 and 13.5.5, Tables 10.87 and 10.91–10.92, official XSD/CMOF anchors, and the official Travel Booking example |
...
| Exact source profile | Schema-valid positive fixture plus one-field source mutations for topology, cardinality, body, mapping, scope, Boolean, association, local-data, and unused-definition boundaries |
...
| Host isolation | Existing exact host-admission refusal applied to the lowered Program, with no Workflow start or capability row |
