...
Historical checkpoint scope: the empty Process-start domain and unconditional host refusal below describe the approved source/lowering checkpoint at `5480b22e`. The [start-data repair](COMPENSATION-DURABILITY-START-DATA-REPAIR-PROPOSAL.md) supersedes that start domain, and the [trigger and handler durability account](COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md) supplies private exact-source hosting. Their separate receipts govern those later stages; this proposal's approved source/lowering rules and public-registration exclusion remain the historical checkpoint contract.
...
Implementation progress: the first green semantic checkpoint is implemented and independently approved at `5480b22e`. The exact source fixture compiles to independently authored checked and Program expectations; strict TypeScript and Lean admission preserve the trigger, dormant handler, restored binding, dependency, and fixed-limit joins; old checked artifacts omit the new field; and both Product 1 admission paths return `compensationSchedulerUnavailable` before any Workflow start. Two hard-ceiling failures exposed proof-reduction ownership in the Lean checkpoint; commit `b4544cf6` preserves every obligation while splitting kernel decisions by invariant family under the unchanged 3 GiB bound, with the full chronology and immutable measurements retained in the [cost ledger](../CAPSULE-COST-LEDGER.md#compensation-source-checkpoint-proof-owner-repair). Product registration, live scheduling, scenarios, capability, corpus, and Product 2 remain untouched; the approved checkpoint now authorizes a separately bounded cross-Workflow durability stage.
...
Evidence: [the Compensation source-lowering conformance module](../../BpmnSemantics/CompensationSourceLoweringConformance.lean) checks exact checked-to-Program equality and representative refusal mutations for this bounded source/lowering checkpoint; it does not claim a quantified theorem over arbitrary checked graphs or Programs. Lean adds the exact checked contract and decoder, validates the checkpoint shape and dormant scope, and lowers it independently, while existing Compensation runtime relations, evaluators, soundness bridges, and laws remain imported authorities.
...
| Semantic checkpoint | `5480b22e8981e155ac71b9f4d06b51743b16fc89` | `fork-turns-none` | `approve` | `not-required` |
