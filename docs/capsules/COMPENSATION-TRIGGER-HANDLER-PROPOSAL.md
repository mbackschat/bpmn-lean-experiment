...
The [RC dependency assessment](../INTERNAL-COMMUTATION-PROPOSAL.md#compensation-checkpoint-dependency-assessment) records the exact checked-source topology argument, two executable widening counterexamples and complete fixed-model frontier exploration. Its proposed exclusions await independent review; registration and Transaction admission remain blocked by their own unfinished obligations.
...
The private interpreter inputs carry normative references and explicitly no CIB provenance. They are not registered scenario artifacts: the current [scenario schema](../../contracts/schemas/scenario.schema.json) still requires a CIB revision and nonempty CIB references. Public registration must resolve that standards-only provenance boundary explicitly; supplying invented CIB evidence would not satisfy it.
