...
| [TS lifecycle](../../packages/semantic-core/src/flow-node-occurrence-lifecycle.ts) | 55 | delegate compensation projection |
| [TS lifecycle completeness](../../packages/semantic-core/src/flow-node-occurrence-publication-external-completeness.ts) | 63 | delegate compensation oracle |
...
The generic Lean prepared-transition lane excludes snapshot declarations more broadly than TypeScript's ordinary arming path. The private [snapshot-aware transition account](../../BpmnSemantics/SemanticProcess/CompensationEventSubProcessSnapshotTransitionTrace.lean) executes ordinary arming and other selected operations at singleton frontiers. Its multi-operation independence check still uses the declaration-free evaluator, so snapshot-declaring frontiers containing multiple ordinary arming operations remain unsupported in this Lean lane even where TypeScript accepts them. Public pipeline evidence must route admitted snapshot Programs through the selected snapshot-aware Lean account and compare complete execution and publication with TypeScript. The ordinary [scenario JSON runner](../../BpmnSemantics/SemanticProcessJsonMain.lean) and [publication runner](../../BpmnSemantics/SemanticProcessJson/Publication.lean) do not currently establish that correspondence. These are prerequisites for the queued public registration, not an expansion of this checkpoint's approved scope.
