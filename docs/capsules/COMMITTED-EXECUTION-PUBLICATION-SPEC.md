...
7. Retain the complete publication across the bounded Workflow chain as private per-Run segments with contiguous global revisions. Product 1 selects one immutable segment through a private descriptor and exact retained Run, while the public E1 page stays unchanged and never crosses a Run. Loss of the selected retained Run is typed unavailable, never repaired from Event History.
...
`committedTransitions` is nonempty exactly when the result is committed, bounded closure succeeded, and a stable state is publishable. Its first member is the exact admitted external stimulus. Every remaining member is the internal operation actually selected by closure, in canonical execution order. A rejected command, unsupported future outcome, closure-bound failure, or ambiguous-choice failure publishes no transition batch. Existing `applyStimulus` remains the result-only projection of the traced evaluator. The approved [closure atomicity repair](INTERNAL-CLOSURE-ATOMICITY-REPAIR-PROPOSAL.md#command-contract) requires both result surfaces to return `rolledBack`, the exact input before admission, and both actual closure flags on closure failure; successful stable and admission-rejected results remain exact.
...
Replay completeness selects exact historical semantic inputs with no redaction. The retained payload-bearing fields are `StartProcessStimulus.initialVariables`, `CompleteUserTaskInstanceStimulus.submittedValues`, `DeliverPayloadMessageStimulus.payload`, `CompleteEffectStimulus.result.localPatch`, and the `code` and nullable `message` of a BPMN Error result. This retains variable names and every currently admitted tagged value arm, namely Null, Boolean, String, Integer, and StringList, even after later commands supersede current state. Profile-aware command admission still decides which arm each surface may use. The other current stimuli retain their complete identity, channel, generation, and logical-time fields but introduce no additional value payload. The existing Operations-group authorization is selected as sufficient for reading and exporting this historical data. Any future value kind, additional payload-bearing field or stimulus variant, redaction requirement, or narrower authorization policy reopens this classification; a protected replay/display split would be a material redesign and requires a new cold review.
...
The production Query accepts `{ afterRevision, limit? }`, where `afterRevision` is a nonnegative safe integer and `limit` is a positive safe integer with default 50 and maximum 100 complete batches. Before the start stimulus commits, the unconditional Query returns retryable `notReady`; this transient arm neither creates nor preserves a Product 2 gap and carries no page or current state. Product 2 retries it with a bounded policy. An unknown instance is `notFound`. A history outside Temporal retention or otherwise not authoritatively queryable is `unavailable`. Failure to query the exact retained Run selected by the private immutable snapshot is also `unavailable`. The legacy direct-Query path is selected only when the private selection Query raises `QueryNotRegisteredError`; other selection or selected-Run failures do not fall back. A cursor ahead of the head, inside a batch, or behind a publication that cannot return the next contiguous revision is `gap`. None of these arms returns a partial current snapshot.
...
Given the exact admitted Semantic Process Program, the command's input RuntimeState, and its unnumbered transition list, replaying the external record through existing command admission and each complete internal record through the existing operation evaluator reconstructs the exact result RuntimeState. At each successive state, replay requires the operation ID to resolve uniquely, the record's kind and origin to equal that Program operation, and its owner to equal the actual dynamically selected scope occurrence. Erasing the trace from the traced evaluator returns the existing `applyStimulus` result.

Semantic reconstruction does not certify canonical publication-byte identity. Production emits its canonical selected sequence, while swapping independently commuting records may reconstruct the same exact state from different record bytes. The [public replay witness](../../packages/semantic-core/test/semantic-transition-publication.test.ts) swaps only the emitted parallel-start arming records for `operation:UserTask_A` and `operation:UserTask_B` and requires that positive result. Its separate initiation/fork swap violates a dependency and must fail. Dropped, duplicated, or substituted operations, state-affecting stimulus changes, and independently changed operation ID, kind, origin, or owner retain their separating corruption witnesses. Successful replay does not authorize reordering the producer's publication; canonical sequence and byte equality have their own exact oracles.
...
Under the [closure atomicity contract](INTERNAL-CLOSURE-ATOMICITY-REPAIR-PROPOSAL.md#command-contract), either closure flag after committed admission rolls the whole command back to the exact input before admission and erases every speculative selected step, batch, transition record, and lifecycle delta while retaining both actual flags. The harness classifies the failure before observation. Restoring only the post-admission or pre-batch state does not satisfy this rule.
...
- dropping, duplicating, or substituting the separating witness's operation record, or swapping its dependent initiation/fork records, is not a replay of the committed result;
- swapping the independently commuting emitted task-arming pair reconstructs the same exact state while changing the record sequence; canonical publication-byte equality remains a separate obligation.
...
The production Workflow owns one immutable paired E1/E2 publication segment for its current Run beside semantic state. Start, Update, Signal, Timer, and Activity-result stimuli continue to enter through the existing single semantic input loop. That loop invokes the traced evaluator once. A successful stable result appends one complete numbered batch and replaces the head snapshot before resolving the existing command path. At Continue-As-New, Product 1 validates and closes the paired segment under a strict descriptor, appends it to a strict contiguous directory, SHA-binds the directory in continuation input, and begins the successor Run at the prior `throughRevision`. No new semantic ingress, transition, wait, effect, cancellation mechanism, or command outcome is introduced.

Private selection obtains one immutable latest-Run directory/current-snapshot result and selects the descriptor containing the requested cursor. The client then queries exactly that descriptor's Run. A page is limited by that Run's `throughRevision`; only a later cursor request advances to another Run. Workflow ID, Run ID, descriptor, directory, and digest never enter the public result.
...
The original single-Run contract retained the complete publication in Workflow state. Under the later approved Workflow-chain contract, each Run retains only its immutable local E1 batches while the successor carries the global head and current snapshot plus a bounded private directory of digest-bound closed segments. Product 1 selects against the latest Run and queries the exact retained segment without exposing Run identity or changing the public page. It does not compact, truncate, archive, or reconstruct from Event History. If the selected retained Run is no longer queryable, Product 1 returns `unavailable` and Product 2 preserves the gap. Post-retention recovery remains a separate archive design.
...
The continuation witness uses three Runs with private segment ranges `0..8`, `8..12`, and `12..16`. It reads Run-local pages from cursors `0`, `4`, `8`, `12`, and `16`, preserves two User Task occurrence identities across adjacent Run boundaries, exposes no private host or segment field, returns the unchanged v1 terminal receipt, and replays all three Runs.
...
| `EPUB-REPLAY-01` | replay completeness theorem and independent-pair swap witness | fold and every redundant operation field equal the selected step and result RuntimeState | history replay recreates canonical publication | no semantic replay authority | independent task-arming swap positive; dependent initiation/fork swap and independently substituted ID, kind, origin, or owner negatives |
...
| `EPUB-CURSOR-01` | not applicable | strict range helpers | strict Query/client page validation, immediate-post-start `notReady`, aligned E1/E2 traversal, immutable selection, exact selected-Run query, Run-local pages, selected-Run unavailability, and `QueryNotRegisteredError`-only fallback | strict gateway/HTTP validation | zero-head page; skipped, split, ahead, regressed, changed, missing selected Run, invalid fallback, or malformed-equation record |
...
- private retained-Run traversal that preserves contiguous public revisions, current positions, and complete-batch page boundaries without exposing Workflow or Run identity;
...
- public segment descriptors, public Workflow or Run identity, compaction, archival storage, post-retention reconstruction, multi-tenant policy, or a general export framework;
...
Reopen this account when archive migration, post-retention recovery, publication compaction, another value domain or payload-bearing field beyond the exact retained set, a redaction-policy change, cross-instance ordering, generalized mining/export, public IL distribution, or another semantic record family is selected. A change that requires a new BPMN rule, profile, CIB relationship, command, repair action, global order, wall-clock semantic timestamp, or public host identity is outside this specification. Complete the remaining M5 frequency and duration views, operator-history surface, and audit export in their own bounded Product 2 increment without claiming that this E1 specification already owns them.
