...
Evidence: planned quantified initialization and committed-start laws in the existing [initialization owner](../../BpmnSemantics/SemanticProcess/RuntimeStateWellFormedInitialization.lean), using the existing [snapshot-aware admission guarantee](../../BpmnSemantics/SemanticProcess/CommandAdmission.lean) and compact kernel-decided raw-versus-reserved witnesses. The successful-admission equation and explicit position hypothesis are the complete public premises; no full post-state well-formedness premise is added.
