...
`awaitDataInputUserTask` remains classified as `compositeWaitAndActivityArming` in the exhaustive TypeScript and Lean declarer censuses. The original commutation checkpoint refused its footprint, and the legacy footprint facade retains that refusal. Current complete preparation and production batching belong to the [standalone Activity-data arming outcome](../INTERNAL-COMMUTATION-PROPOSAL.md#standalone-activity-data-arming-outcome), which owns the composite preparation, independent-frame, raw-state commutation, and publication laws. Its footprint reads but does not write the source Process binding: shared reads are permitted, while a conflicting write prevents batching. The [integration regression](../../packages/semantic-core/test/internal-commutation-activity-data-input.test.ts) distinguishes legacy-facade refusal from committed independent batching.
...
Query and Product 1 publication derive the optional task-input collection only from committed semantic state. Product 2 may copy that published field through its strict nonvisual contract but may not reconstruct it from start payloads or definition XML. Visibility and Temporal Event History remain non-authoritative. Any future UI or browser presentation is a separate Product 2 increment that first completes the source-grounded design preflight. The copied contract and its strict decoder now [admit the collection](../../platform/contracts/test/activity-data-input-execution-publication.test.ts): exactly one binding on an open User Task, optional so every other family decodes unchanged, and refused in every other shape. Its fixture publishes a binding that no observed Process variable carries, so a platform that reconstructed the collection from `variables` cannot reproduce it; that reconstruction and an unvalidated pass-through were each seeded and each failed the test.

The smallest live-history refinement witness starts the invoice-review model with a non-null summary, reaches one task with the exact one-element input collection, replaces the Worker, completes the task, obtains the terminal receipt, and replays every Run. It is [implemented](../../packages/temporal-adapter/testkit/test/activity-data-input-temporal.test.ts) and runs all three registered scenarios against one live service and one compiled program, which makes the absence-versus-explicit-null discrimination real-service evidence rather than the direct-VM witness this preflight would have accepted. The same witness terminates the unavailable-source Run and requires its committed trace to be unchanged, which is this family's failure-and-cancellation obligation. Three seeded mutations were each confirmed to fail it: publishing the present value for explicit null, opening a task for an absent source, and expecting a different committed trace after termination. The nearest adapter counterexample schedules a task from the source XML before the semantic core copies the binding, exposing an open task with no input collection; exact Query and replay evidence must reject it.
...
| TypeScript realization | Separately written readiness, copy, ownership, completion, and one-element input-collection observation logic plus focused state-preservation and negative tests; the legacy checkpoint footprint remains unavailable, while the later [complete data-arming account](../INTERNAL-COMMUTATION-PROPOSAL.md#standalone-activity-data-arming-outcome) owns prepared batching |
...
Established by implementation are the normative direct-copy and unavailable-source rules, the exact machine-readable source cardinalities and their five named exclusions, the exact source and profile boundary, the explicit-null-is-present interpretation, the occurrence-owned local scope with its issuing-discipline freshness bound, the proved Lean transition and refusal account with evaluator soundness, the independently written TypeScript realization, three answer-free cross-language scenarios, real-service durable refinement, one retained whole model, and strict Product 2 decoding.

The nearest unsupported claim is one DataOutput and DataOutputAssociation copying a completed Activity value back to Process scope. The principal common-mode risk is that source, Lean, TypeScript, and Temporal all share the proposal's chosen absence/null representation. The normative unavailable-source text, explicit null discriminator, strict wire mutations, and independent source graph constrain but do not eliminate that risk. One narrower correlation is recorded rather than left implicit: the Temporal witness takes its expected trace from the semantic core's own `runScenario`, so for trace agreement the durable-refinement and TypeScript-realization lanes cannot fail apart and do not count as two. What makes that lane independently informative is the part not derived from the core — the hand-written input-collection literals, the empty open-task assertion for the absent source, and the requirement that host termination publish no transition.
