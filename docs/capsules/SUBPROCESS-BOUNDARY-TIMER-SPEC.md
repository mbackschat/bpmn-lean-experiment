...
- The bounded-Activity deadline scheduler became [one family-parameterized owner](../../packages/temporal-adapter/src/bounded-deadline-scheduler.ts) serving both boundary-deadline host kinds, rather than a third near-duplicate beside it. The refusal identity is a descriptor field, so the two families keep distinct identities without duplicating the mechanism that raises them.
- The host assertions both direct-VM witnesses use moved to [their own owner](../../packages/temporal-adapter/test/boundary-deadline-assertions.ts). They read only emitted commands, so they are generic over which wait the deadline bounds, and the refusal identity stays a parameter precisely because the families must not share one.

`packages/temporal-adapter/src/workflow-implementation.ts` was left effectively full, and that is a standing constraint rather than a completed extraction: the next change touching it needs a behavior-preserving extraction as its own commit first.
