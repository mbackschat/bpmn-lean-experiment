# Interrupting Sub-Process boundary Timer specification

## Status

**Owner-approved on 2026-08-04; implemented, evidence-closed, and graduated.** All four decisions below are approved as recommended. The owner delegated the decision in this session rather than ruling on each one individually, so the approval rests on the recommendations and rationales recorded here and on the closed proposal review; it is not an independent owner analysis of the four alternatives. Approval authorizes exactly the scope recorded in this document and nothing beyond it.

No sentence in this document is a coverage, conformance, or CIB compatibility claim. All three governed review stages are closed with their corrections audited, so this document graduated from `-PROPOSAL` to `-SPEC` and now records the implemented contract rather than an intent. **The conditional semantic-checkpoint stage is closed:** the review of `ef4edd4` returned `approve-with-required-edits` and the same reviewer's second correction audit at `3652549` returned `AUDIT: closed`.
...
| Closure | `cf74cd8` | `fork-turns-none` | `approve-with-required-edits` | `9d2b603` |
...
**The closure row names its target while the verdict stays `pending`, and that is the receipt's own encoding rather than an omission.** The cold closure review of `cf74cd8` returned `approve-with-required-edits` across three required findings, and [the receipt guard](../../scripts/independent-review-policy.test.ts) refuses to record that verdict until it can name a resolved correction-audit commit. The row carried the target and `pending` until the same reviewer's warm audit of `9d2b603` returned `AUDIT: closed`, and the isolation, verdict, and audit cells are now filled exactly as the two rows above are. The receipt form keeps isolation at `not-recorded` while a verdict is `pending`, because isolation is attested with the completed review rather than promised in advance. The verdict cell keeps `approve-with-required-edits` because it records what the immutable target returned rather than the state after correction. The closure review also observed that the handed-off target left this row entirely `not-applicable`, which said nothing about the stage being reviewed; recording the target while the verdict was still open is the fix.

The audit classified the corrections as bounded rather than a material redesign, so they stayed in the same reviewer thread. It verified the deadline-arm bridge by execution rather than by reading: it discharged `parentOwned` from the new satisfiability witness to obtain an actual `BoundedScopeVictoryStep`, and separately kernel-decided that the guarded state is reached, which rules out the subtler failure where a guarded implication is satisfied vacuously by a lookup that fails. It left one non-blocking advisory, that three prose sites still called the joint logical-time bound "the separation law"; those are corrected in the commit carrying this paragraph, together with a fourth site the advisory had not listed, and [the ledger](../PROCESS-ASSESSMENT-LEDGER.md#findings) records the instance against the correction commit where it happened.
...
- `cancelActivity` omitted or lexically `true`; every lexeme naming *false*, which is `false` and `0`, is **rejected** as the retained hostile control. `1` is refused too, before parsing and with a different diagnostic, because it is the one valid lexeme `bpmn-moddle` inverts; the compiler refuses it rather than let this profile see it as *false*;
...
**Superseded in part.** [The Activity occurrence ownership capsule](../ACTIVITY-OCCURRENCE-OWNERSHIP-SPEC.md) added the explicit occurrence record this section argued was unnecessary, and it did so because a *different* consequence forced it: a boundary handler owned by the parent scope survived the removal of the child region it guarded. The reopen condition below was therefore discharged by a capsule rather than refuted by an admitted state, and those are different events. Every target now reads the record: the TypeScript core, the Temporal adapter, the open-set publication binding, and the Lean reference interpreter. The Lean families were migrated in a second pass, after a checkpoint review found the join asserted over both languages and enforced over one. So the falsifiable claim above is false across the project and is corrected here rather than deleted, because the claim's *shape* is what a later reader needs: it was refuted by a consequence nobody had stated, not by the repeated Activity it named as its own falsifier.

This mirrored the sibling's decision to omit a hidden record, and it carried the same adapter consequence, which this capsule owned rather than inherited: with no record, the adapter had to join the committed bounded-scope operation to the live scope occurrence and Timer wait to know that a Timer wait is a scope deadline rather than an ordinary `awaitTimer`. That join was new derivation work and was never detector reuse. The adapter no longer performs it: it reads the ownership record added by the capsule named below.
...
This capsule crossed two module-size boundaries and one design boundary, each landing as its own behavior-preserving change rather than as work done under a size squeeze inside a semantic change.

- `RuntimeState.lean` reached 602 nonblank lines against the 600 target, which forced quiescence and normal completion out into [their own owner](../../BpmnSemantics/SemanticProcess/ScopeCompletion.lean) with the rewrite selection extracted. That extraction was not merely a size fix: the component-preservation lemma both soundness bridges rest on became provable in two deterministic branch cases, where every tactic against the former nested matches had been fragile.
- The bounded-Activity deadline scheduler became [one family-parameterized owner](../../packages/temporal-adapter/workflow/src/bounded-deadline-scheduler.ts) serving both boundary-deadline host kinds, rather than a third near-duplicate beside it. The refusal identity is a descriptor field, so the two families keep distinct identities without duplicating the mechanism that raises them.
- The host assertions both direct-VM witnesses use moved to [their own owner](../../packages/temporal-adapter/testkit/test/boundary-deadline-assertions.ts). They read only emitted commands, so they are generic over which wait the deadline bounds, and the refusal identity stays a parameter precisely because the families must not share one.

`packages/temporal-adapter/workflow/src/workflow-implementation.ts` was left effectively full, and that is a standing constraint rather than a completed extraction: the next change touching it needs a behavior-preserving extraction as its own commit first.

**This section deliberately records no headroom figures.** The planning inventory that carried them belonged to this document while it was a proposal, where [the reviewability guard](../../scripts/document-reviewability.test.ts) recomputes every figure against measurement. That guard reads proposals only, so retaining the table past graduation would preserve numbers nothing checks.
...
| [product example configs](../../packages/temporal-adapter/testkit/test/product-example-configs.test.ts) | Every registered profile has a live example and every example names a registered profile; two examples over one definition are admissible. |
| [host admission](../../packages/temporal-adapter/testkit/test/host-admission.test.ts) | **New assertion work, not an existing constraint.** The classifier already rejects two concurrent managed waits in source, but no test feeds a bounded-wait-bearing program in and expects its refusal; the existing two-managed cases are both same-class races. This capsule adds the bounded-scope case rather than inheriting it. |
...
- ~~an admitted state makes the deadline-to-scope ownership ambiguous, which refutes the no-hidden-record claim and forces an explicit occurrence record~~ **discharged**, not by an admitted state but by the stranded-handler consequence recorded above;
