...
**Proposed and awaiting independent cold review.** This proposal selects one additive Boolean Process-variable value only for exact User Task completion under a new CIB Seven compatibility profile. Implementation, profile registration, retained evidence, and product use remain paused until the proposal review and owner-approval receipt are complete.
...
| [CIB observation fidelity](../../scripts/cib-observation-fidelity.test.ts) | Bind raw Java Boolean evidence to the canonical tagged value and reject text substitution. |
