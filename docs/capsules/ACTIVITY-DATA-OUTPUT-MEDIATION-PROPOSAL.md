...
Required mutations write by submitted name instead of by association, **equate the `DataOutput` id with the target `Property` id so the routed and named accounts agree by coincidence**, reverse the association direction, resolve `sourceRef` or `targetRef` by name instead of object identity, accept a second `sourceRef` (which `DataAssociation-sourceRef` admits at `upper="*"`), accept a second `DataOutput` or `OutputSet`, accept a non-empty `InputSet`, accept an `optionalOutputRefs` or `whileExecutingOutputRefs` entry, accept `isCollection="true"`, accept a `transformation`, admit a completion with zero or two bindings, admit an out-of-domain submitted value, commit the write while leaving the local scope in place, dispose the scope before executing the association, make entry depend on the target Property, and drop the write across Worker replacement or Continue-As-New.
