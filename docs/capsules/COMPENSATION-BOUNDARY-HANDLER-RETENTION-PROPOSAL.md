...
Evidence: [the Compensation Activity-retention conformance module](../../BpmnSemantics/CompensationActivityRetentionConformance.lean) proves the bounded representation, eligibility, insertion, capacity, start, and root-disposal propositions, while kernel-decided producer witnesses remain finite and do not claim source or host behavior.
...
| [TypeScript well-formedness](../../packages/semantic-core/src/runtime-state-well-formedness.ts) | 55 | add one delegated validator hook; extract before crossing 800 |
...
| [Lean RuntimeState](../../BpmnSemantics/SemanticProcess/RuntimeState.lean) | 71 | add only the collection reference and its root-owned invariant; new structures live elsewhere |
| [Lean well-formedness](../../BpmnSemantics/SemanticProcess/RuntimeStateWellFormed.lean) | 55 | add one delegated predicate; extract before crossing 800 |
...
| [Lean message-key correlation preservation](../../BpmnSemantics/SemanticProcess/MessageKeyCorrelationPreservation.lean) | 351 | reconstruct the strengthened aggregate invariant while framing the compensation register exactly |
| [Lean message-payload preservation](../../BpmnSemantics/SemanticProcess/MessagePayloadPreservation.lean) | 454 | reconstruct the strengthened aggregate invariant while framing the compensation register exactly |
...
| [Lean internal-commutation publication](../../BpmnSemantics/SemanticProcess/InternalCommutationPublication.lean) | 297 | name both final aggregate facts while projecting the existing publication prerequisites |
| [Lean parallel Multi-Instance empty-state preservation](../../BpmnSemantics/SemanticProcess/ParallelMultiInstanceRuntimeStateEmptyPreservation.lean) | 700 | carry both final aggregate facts through the unchanged empty-state result |
...
| [Lean parallel Multi-Instance closing terminal preservation](../../BpmnSemantics/SemanticProcess/ParallelMultiInstanceRuntimeStateClosingTerminalPreservation.lean) | 15 | frame the compensation register through terminal child closing; any further growth should extract the frame proof |
