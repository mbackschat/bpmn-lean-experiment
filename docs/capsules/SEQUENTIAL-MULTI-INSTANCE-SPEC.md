...
*Active is at most one* is a runtime invariant rather than a standalone law, and the two are recorded separately because they were not always both established. It is `controllersOwnLiveActivity`, which requires every controller to name exactly one Activity occurrence record, and `generatedInstanceCount_eq_active_add_completed` consumes it as a hypothesis rather than deriving it. [The preservation owner](../../BpmnSemantics/SemanticProcess/SequentialMultiInstancePreservation.lean) proves that all five rewrites of this capsule carry it from pre-state to post-state, so the hypothesis is maintained by every transition in the family rather than assumed. Evaluator soundness could not have supplied it: each relation arm concludes with the post-state its evaluator produced, so no arm can fail apart from the evaluator it certifies. Entry alone needs `runtimeStateIdentityBound`, because the minted activation is fresh only while no present record is numbered above its element's recorded count. The monotone-counter half of the same bullet remains an open lane and is not claimed here.
