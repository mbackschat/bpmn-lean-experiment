...
- every foreign attribute rejects unless its profile's projector consumes it, at every element and on every dispatch path; every resolved reference must point at its property's declared type
- generic-compiler classification refusals name their element through [one locating owner](../packages/bpmn-source/src/admission-diagnostics.ts): nullable `id`, `$type`, containment path, named property or attribute, and the capability the profile lacks; collected across loci, deduplicated, ordered by path numerically. Parser warnings keep that record in parse order
