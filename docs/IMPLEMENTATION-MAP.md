...
The nearest proposed engine claim is direct interrupting Error propagation from the implemented child-scope foundation: one exact-code Error End Event, one directly attached boundary catcher, regional child cancellation, one structurally present but unreachable normal continuation, and one outer recovery route. The [rebased proposal](capsules/SUBPROCESS-ERROR-PROPAGATION-PROPOSAL.md) reuses the current definition/runtime scope account, adds no runtime-state or public-observation field, recommends a bounded CIB Seven `2.2.0` public-agreement lane, and awaits owner approval before semantic work. Arbitrary serial composition, arbitrary graph progress, repeated or nested scope activation, Event Sub-Processes, Message payload/key/global correlation, inclusive/event-based gateways, loops, multi-instance, compensation, and general Event semantics remain unsupported. Conditional routing remains limited to the exact Simple Boolean profile, and JUEL remains demand-driven, deferred, and separately classified.
