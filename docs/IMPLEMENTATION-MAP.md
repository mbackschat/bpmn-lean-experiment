...
A separate preservation oracle keeps the payload-free Service Task profile and BPMN bytes exact, permits only `sourceOverlay: null` in its scenario, and permits only the corresponding evidence-digest rebinding. After the owner explicitly authorized a third same-reviewer audit, docs-only correction `398719b` was approved as the semantic-checkpoint correction target.
