...
**Implemented and green: source and profile.** The `cancelActivity` attribute is resolved once, in [the Timer Boundary Event source reader](../packages/bpmn-source/src/timer-boundary-event-source.ts), into a closed `BoundaryInterruption` value the checked node carries. An omitted attribute and a lexical `true` resolve to `interrupting` from the XSD and CMOF default, a lexical `false` to `nonInterrupting`, and any other parsed value is refused rather than defaulted.
...
**Implemented and green: host capability.** The Temporal adapter gains a third managed host class and a third `BoundedDeadlineFamily` under its own typed `BpmnMonitoredActivitySchedulerUnavailable` identity, because the outcome an operator loses here is a handler branch that never starts rather than a task or a region that never ends.

**Absent.** Every evidence lane after the checkpoint: the two registered answer-free schedules and their differential cases with seeded mutations, the product example configurations, the Temporal spawn and withdrawal histories with replay, and the Worker-absence run across the due instant. No conformance, CIB, or Temporal refinement claim is available for this family, and the requirement row `BPMN-NON-INTERRUPTING-BOUNDARY-TIMER-01` stays `unsupported`. The scenario directory holds the exact source and no schedule.
