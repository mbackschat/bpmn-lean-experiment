...
Product 1 starts use its closure-reviewed versioned contract, return no SDK handle, recover content-bound commands through the latest Run, expose the closed v1 receipt, traverse paired E1/E2 segments, enforce recovery, semantic-candidate, stimulus, Update, accepted-input-queue, effect-Activity, retained trace/publication, pending-Timer, Query-response, terminal-result, Event History, aggregate continuation, and 128-Run bounds, and admit Worker replacement only through the stop-the-world deployment gate. Forced Intermediate Catch Message, Intermediate Catch Timer, and Service Task effect rollover are evidenced. Routed maps own surfaces; specifications own current contracts and evidence.
...
| `ENGINE-SEMANTIC-INVARIANT` | `implemented` | [Engine semantic invariant](ENGINE-SEMANTIC-INVARIANT-IMPLEMENTATION-MAP.md) | `BpmnSemantics/`, `packages/semantic-core/`, routed `docs/capsules/` and `docs/` documentation |
