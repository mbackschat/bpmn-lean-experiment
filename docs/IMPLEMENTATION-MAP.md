...
- production recovery-ledger exhaustion at 512 entries, atomic 64 KiB state/paired-batch preflight, 64 KiB stimuli, 1,500 accepted and eight in-flight Updates per Run, and a 64-entry/256 KiB accepted queue; Product 1 refuses oversized commands before transport, Update validators reject before acceptance, accepted Signal/derived overages fail before semantic evaluation, exact queue/Update fills request rollover, typed details expose no Run ID or candidate, and failed histories replay
...
- Search Attributes
- post-retention publication reconstruction or archive, complete timer/effect/Message rollover evidence, deployment admission, and the approved capacity rows other than recovery, semantic-candidate, stimulus, Update, and accepted-input ingress enforcement
- task inbox
