...
| Differential pipeline | Three answer-free cases; one CIB batch; one Lean result emitter; one core batch; six Temporal executions; exact four-target agreement; retained-CIB comparison; exact Query/Update evidence; seeded activation mutation; live replay; isolation, cleanup, provenance, timings, and budgets | Universal equivalence, majority voting, general conformance suite |
