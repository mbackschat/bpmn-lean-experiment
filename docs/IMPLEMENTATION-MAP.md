...
**Current Terminate End boundary.** Terminate End is absent. The active [proposal](capsules/TERMINATE-END-EVENT-PROPOSAL.md) selects one nested parallel witness and a reusable no-output containing-scope termination operation that clears the selected live scope subtree before unchanged ordinary scope completion. No source profile, checked node, IL operation, Lean/core transition, registered scenario, or Temporal evidence exists yet.
