...
The owner-approved [Event-Based Gateway Message/Timer proposal](capsules/EVENT-BASED-GATEWAY-PROPOSAL.md) has reached semantic checkpoint target `e595b1c`. TypeScript and Lean implement the exact checked node, `awaitEventRace` operation, disjoint configuration-flow/control-place classification, strict source and standalone-program admission, exact checked-definition binding, hidden occurrence-owned race and activation counters, atomic arming, both winner directions, loser withdrawal, wrong/stale state-preserving refusal, canonical observation through the existing Message/Timer surfaces, quiescence blocking, interruption cleanup, and exhaustive race-aware pre-start host admission. Lean independently lowers the checked graph, checks exact equality, relates the evaluator to separate arming/winner relations, and locks bounded closure plus the nearest non-laws; the TypeScript core is separately authored. The mandatory checkpoint review remains pending, so no profile artifact, target scenario, differential case, managed Temporal scheduler, dual-ready fail-closed runtime guard, CIB relationship, or closure evidence lane has advanced. Existing standalone Message and Timer semantics and their current Temporal hosts remain unchanged.
