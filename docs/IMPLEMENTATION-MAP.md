...
A separate preservation oracle keeps the payload-free Service Task profile and BPMN bytes exact, permits only `sourceOverlay: null` in its scenario, and permits only the corresponding evidence-digest rebinding. Second warm audit `8b92040` closed every code and evidence finding. Its final docs-only correction awaits owner resolution under the two-audit bound.
