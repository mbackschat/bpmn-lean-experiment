...
**What it does not do.** One profile admits modeler notation and the rest do not. `cibseven-2.2.0-user-task-preserved-notation-draft` retains Diagram Interchange, pools, lanes, artifacts, and documentation without executing them, under [the preserve-only admission proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md); every other profile still rejects them through an exact key allowlist. That profile is **not yet a registered artifact** and has no scenario, retained evidence, or pipeline case, so its evidence is focused-gate only. Data Objects, Data Stores, and the whole BPMN data family reject under every profile. Admission is acyclic. Variables are string or null. There is no cancellation beyond the direct-parent regional case, no incidents, and no published transition record or token position.
