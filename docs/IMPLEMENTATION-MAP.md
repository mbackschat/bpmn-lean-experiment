...
The owner-approved [Inclusive Gateway proposal](capsules/INCLUSIVE-GATEWAY-PROPOSAL.md) passed independent proposal review after correction audit `0b52d15`. Semantic checkpoint target `5e0dc3b` implements the exact structured checked split/join, branch-local lowering to `selectMany` and `synchronizeSelected`, the hidden owner-scoped selected-branch record, standalone and checked-definition admission, premise-bearing Lean relations plus reusable laws, independent TypeScript behavior, selection-aware quiescence/interruption, and exhaustive Temporal host-capability classification. Its isolated checkpoint review passed required edits after exact XSD gateway-direction correction audit `ea8b019`. The existing `choose` remains first-true/single-output and `synchronize` remains all-input. Registered profile/scenario artifacts, differential cases, and Temporal refinement/replay remain absent until the active closure lanes add them; no retained evidence or CIB relationship is selected.
...
- twenty-three fetched live histories replayed before server shutdown;
