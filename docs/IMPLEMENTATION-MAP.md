...
Retained notation reaches no public observation and has no query surface. Refusals about the document or the checked graph name no element. The whole BPMN data family rejects everywhere. Existing registered profiles remain acyclic. The M2 checkpoint capability alone admits one exact root-scope cycle after proving that cutting the User Task continuation makes the graph acyclic; it is deliberately outside the registered product catalog, and no profile artifact or execution evidence has advanced yet. Variables are string or null. There is no cancellation beyond the direct-parent regional case, no incidents, and no published transition record or token position.
...
- current JSON Schemas and boundary validators for typed `initiate`, `enterScope`, `invokeProcess`, `returnProcess`, `awaitUserTask`, `awaitTimer`, `awaitMessage`, `awaitEventRace`, `awaitEffect`, `duplicate`, `synchronize`, `mergeExclusive`, `choose`, `selectMany`, `synchronizeSelected`, `throwError`, `reachNoneEnd`, and `completeScope` operations
...
- `mergeExclusive` carries a canonical nonempty input collection and one output, with reusable per-offered-token declarative pass-through and a unique-offer executable subset; only the checkpoint cycle capability fixes its input count at three
...
- checkpoint-only cyclic-control-flow proofs for exact checked/program graph policy and lowering, full-cycle interception by the selected cut, general per-offered-token merge relation, unique-offer evaluator soundness, actual execution of every finite reviewed repeat/rework schedule followed by exit, actual-reachability active-unit bounds, automatic cut-DAG closure at no more than six operations, stale/wrong/future identity preservation, and excluded internal-cycle, fan-in, scope, and wait shapes
...
- checkpoint-only cyclic-control-flow admission and execution with a separately cataloged capability, one shared frozen graph policy, a reusable nonempty Exclusive Merge contract, capability-local exact-three restriction, owner-preserving unique-offer execution, and zero/multiple-offer evaluator incompleteness kept distinct from the declarative relation
...
- passive pre-start host admission for the pending cycle program's `mergeExclusive`; no new Workflow scheduler, wait, command, timer, Activity, history, or replay mechanism is added at the checkpoint
...
- **Unsupported across families:** arbitrary serial composition, arbitrary graph progress, repeated or nested scope activation, cycles outside the exact pending root-scope User Task profile, concurrent Multi-Merge execution, Standard Loop Characteristics, multi-instance, Message payload, key, and global correlation, compensation, and general Event semantics.
