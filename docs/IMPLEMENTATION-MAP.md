...
**Owner confirmation outstanding.** Refusing only the disagreement, rather than every non-canonical lexeme, is what keeps the Event-Based Gateway specification's admitted `instantiate="0"` valid. Its consequence for this capsule is that `cancelActivity="0"` is now admitted as non-interrupting, which widens the admitted set the profile bullet records under decision 4. Decision 4's own two rejections — an omitted attribute and lexical `true` — are unchanged, and the two profiles' admitted sets stay disjoint, so what it secured is preserved. The wording change is recorded in the capsule and is not self-approved.
