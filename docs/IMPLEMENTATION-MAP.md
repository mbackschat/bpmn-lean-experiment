...
Both evaluator-soundness bridges are now implemented, together with the counter-and-history preservation law and the logical-time separation law. All four rest on a component-preservation lemma for the shared scope completion, which required extracting quiescence and normal completion out of `RuntimeState` into [their own owner](../BpmnSemantics/SemanticProcess/ScopeCompletion.lean): the outer transition decides whether a scope may complete, and a separate rewrite selector decides what completing it does. The quiescence bridge *derives* the deadline's liveness in the pre-state through that lemma rather than assuming it, since the evaluator finds the deadline in the completed state and completion leaves `timerWaits` untouched.

Two hypotheses are stated rather than assumed, and each names a fact its own transition does not establish. The quiescence bridge takes `running` because the shared completion decides it without exporting it. The deadline bridge takes `parentOwned`, and that gap is real: the evaluator erases the deadline after regional cancellation without re-checking that cancellation left it there. Atomic arming guarantees it, because the owner is the parent and therefore outside the cancelled subtree, so the case is unreachable under this profile; it is a property of arming rather than of this transition.

**Absent in Lean:** nothing in the capsule's required Lean lane. The remaining absences for this family are the two answer-free scenarios, their differential cases with seeded mutations, and the Temporal refinement and replay witness.

No answer-free scenario is registered, so no differential case, no seeded semantic mutation, and no Temporal refinement or replay witness exist for this family, and **no cross-target agreement is claimed**. The requirement row `BPMN-SUBPROCESS-BOUNDARY-TIMER-01` remains `unsupported` and the capsule remains a `-PROPOSAL`. The conditional semantic-checkpoint review required by [the cold-review rule](../CLAUDE.md#independent-cold-review) is complete: the review of `ef4edd4` returned `approve-with-required-edits` and the same reviewer's audit of `3652549` returned `AUDIT: closed`.
