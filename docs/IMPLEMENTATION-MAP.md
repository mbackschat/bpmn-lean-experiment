...
**What it does not do.** One profile admits modeler notation and the rest do not. `bpmn-2.0.2-user-task-preserved-notation-draft` retains Diagram Interchange, pools, lanes, artifacts, and documentation without executing them, under [the preserve-only admission specification](PRESERVE-ONLY-ADMISSION-SPEC.md); every other profile rejects them through an exact key allowlist. Retained notation reaches no public observation and has no query surface. Refusals about the document or the checked graph name no element. The whole BPMN data family rejects everywhere. Admission is acyclic. Variables are string or null. There is no cancellation beyond the direct-parent regional case, no incidents, and no published transition record or token position.
...
- generic-compiler classification refusals name their element through [one locating owner](../packages/bpmn-source/src/admission-diagnostics.ts): nullable `id`, `$type`, containment path, named property or attribute, and the missing capability; collected across loci, deduplicated, ordered by path numerically. Parser warnings keep that record in parse order
...
- An element on refusals over the document or checked graph, on an unadmitted own property of an executed flow node, or from the three hand-selected-shape readers, which report one `unsupportedModel` record with `element: null`. Preserved material is retained only in the exact source bytes, with no query surface or public projection
...
**Implemented and green.** `cancelActivity` resolves once, in [the source reader](../packages/bpmn-source/src/timer-boundary-event-source.ts), into a closed `BoundaryInterruption` value the checked node carries; the two boundary-Timer profiles pin disjoint dispositions so neither admits the other's source. One `awaitMonitoredUserTask` operation carries the family. Lean and the TypeScript core independently implement arming, the host-preserving spawn, and the one-sided completion. Two registered answer-free schedules with seeded mutations, the Temporal refinement, replay, Worker-absence, and shared-activation lanes are closed.

**Absent.** CIB observation of the mechanism, which this standards-only capsule does not select; therefore no conformance, CIB, or Temporal refinement claim for the family beyond the recorded slice. The capsule owns the rest, including why a repeated firing would refute its one-sided join.

## Interrupting Sub-Process boundary Timer

[The interrupting Sub-Process boundary Timer specification](capsules/SUBPROCESS-BOUNDARY-TIMER-SPEC.md) is **implemented, evidence-closed, and graduated**, for exactly one embedded Sub-Process with one child task and one interrupting `PT1S` boundary Timer.

**Implemented.** One `enterBoundedScope` operation in the Semantic Process contract with its wire-schema branch, reusing the existing scope-occurrence model rather than adding a second. Three semantic-core transitions: atomic arming, quiescence withdrawal, and deadline interruption over the child scope. In Lean, the declarative arming relation and a two-constructor `BoundedScopeVictoryStep`, each with an evaluator-soundness bridge, plus the counter-and-history preservation law, two per-evaluator logical-time laws, a quantified off-deadline refusal, and the checked non-law that interruption does **not** preserve child-scope-owned state. [Its conformance module](../BpmnSemantics/SubProcessBoundaryTimerConformance.lean) locks both victories at the public boundary and the follow-on identity that separates them.

**Evidence.** The `bpmn-2.0.2-subprocess-boundary-timer-draft` profile, two registered answer-free scenarios, and their two differential cases each carry a seeded mutation reaching a public observation. The durable host arms the deadline through the parameterized [deadline scheduler](../packages/temporal-adapter/src/bounded-deadline-scheduler.ts), survives Worker absence across the due instant, replays, and refuses a shared activation under its own `BpmnBoundedScopeSchedulerUnavailable` identity.

**Absent.** Nesting beyond one level, more than one child task, non-interrupting boundaries, other triggers, other durations, multiple boundaries on one Sub-Process, Transaction and Cancel semantics, Event Sub-Processes, child-local data, incidents, CIB compatibility, and any conformance claim. Two scenarios are excluded from the registry for structural reasons the [scenario README](../scenarios/README.md) records, and are covered in Lean and the focused core test instead.
