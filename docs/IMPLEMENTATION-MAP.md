...
Whether that inversion is safe depends on which way the consumer compares. A reader requiring `true` refuses a coerced value, which is the safe direction and covers `isExecutable` and the interrupting `cancelActivity` readers. [Scoped flow-element admission](../packages/bpmn-source/src/scoped-flow-elements.ts) admits an embedded Sub-Process when `triggeredByEvent` is absent or `false`, so it admits on the coerced value: `triggeredByEvent="1"` was an **Event Sub-Process admitted and lowered as an ordinary embedded Sub-Process**, in three requirement rows recorded `supported`.

**Implemented and green.** [The compiler](../packages/bpmn-source/src/compile.ts) refuses, on the exact decoded source, any occurrence of a `Boolean`-typed attribute whose lexeme is not one the coercion agrees with. The attribute set is derived from the metamodel manifest's `Boolean` properties rather than listed, so it spans the class the coercion spans and a boolean added to the manifest is covered when it is added; enumerating names would repeat the value-not-position mistake that left `cancelActivity='1'` admitted. Both XML attribute-value delimiters are matched. `0` is admitted and `1` is not, which is the disagreement rather than a canonicality rule.
