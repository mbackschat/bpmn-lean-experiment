...
The owner-approved [Inclusive Gateway proposal](capsules/INCLUSIVE-GATEWAY-PROPOSAL.md) passed independent proposal review after correction audit `0b52d15`. Semantic checkpoint target `5e0dc3b` implements the exact structured checked split/join, branch-local lowering to `selectMany` and `synchronizeSelected`, the hidden owner-scoped selected-branch record, standalone and checked-definition admission, premise-bearing Lean relations plus reusable laws, independent TypeScript behavior, selection-aware quiescence/interruption, and exhaustive Temporal host-capability classification. The existing `choose` remains first-true/single-output and `synchronize` remains all-input. Mandatory checkpoint review remains pending, so no registered profile/scenario artifact, differential case, Temporal refinement/replay case, retained evidence, or CIB relationship has been added.
