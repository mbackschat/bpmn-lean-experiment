...
- `@bpmn-lean/platform-operate` and `@bpmn-lean/platform-work`, with cross-backend Unicode-safe repositories, monotone lifecycles, concurrent CAS, suffix-only projections, all remaining recovery candidate sources, and bounded Audit delivery; the two Audit sinks enforce independent exact source-to-sink completeness
