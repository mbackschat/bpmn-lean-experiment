...
The profile, byte-identical answer-free scenario, product example, and one `cib: null` exact-semantic differential case are registered. The live Temporal witness proves the effect-only wait, replaces the Worker during the active Activity attempt, reconciles the same transport key, publishes and completes the trailing User Task, reaches the exact terminal core state, rejects wrong occurrences with state preservation, inspects Activity history, excludes new host mechanisms, and replays. A test-owned effect bypass exposes the trailing User Task early and reaches the public discriminator. Pinned CIB passes through the extension to that User Task with zero jobs and incidents; this trace is retained only as an exclusion oracle and creates no CIB target or relationship. Implementation and evidence are complete pending closure review.
