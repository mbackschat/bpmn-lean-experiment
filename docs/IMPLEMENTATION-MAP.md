...
**Absent:** a negative assertion for the new `BpmnBoundedScopeSchedulerUnavailable` identity. The bounded-Activity sibling has one through a direct-VM shared-activation witness; this family's identity is distinct in source but exercised by no test, so a copied identity would currently be invisible here. The capsule records that as a named common-mode risk, and it is the one required lane still open.
...
The requirement row `BPMN-SUBPROCESS-BOUNDARY-TIMER-01` and the capsule's `-PROPOSAL` role are unchanged until that identity witness lands and closure review runs. The conditional semantic-checkpoint review required by [the cold-review rule](../CLAUDE.md#independent-cold-review) is complete: the review of `ef4edd4` returned `approve-with-required-edits` and the same reviewer's audit of `3652549` returned `AUDIT: closed`.
