...
**Current M5 boundary.** The closure-reviewed [E1 specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) implements replay-complete transitions, exact current positions, a strict Workflow cursor Query, real retention/replay evidence, and fail-closed Product 2 History, Diagram, and canonical export. The independently reviewed [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md) is owner-approved and implementation is beginning; no universal flow-node lifecycle, replay-stable commit instant, frequency aggregate, or completed-duration aggregate is public yet. Later M5 increments remain open.
