...
**Current Workflow-chain hosting boundary.** Product 1 privately traverses paired E1 and occurrence-publication segments across Continue-As-New through SHA-bound descriptors and immutable latest-Run selection. The 512th recovery entry closes a nonterminal Run only after its result is retained and returned. Retry and conflict precede capacity, terminal completion wins, and no public contract or host identity changes.
...
- production recovery-ledger exhaustion at 512 entries: the filling result remains Query-recoverable, changed content remains a conflict, unseen Updates preflight both bounds before queuing, accepted Signals fail before semantic evaluation, terminal state wins, no Workflow retry occurs, and replay succeeds
...
- Search Attributes
- post-retention publication reconstruction or archive, complete timer/effect/Message rollover evidence, deployment admission, and the approved capacity rows beyond recovery-ledger exhaustion
- task inbox
