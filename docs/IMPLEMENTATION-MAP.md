...
**What it does not do.** No modeler-produced BPMN file is admissible: [the compiler](../packages/bpmn-source/src/checked-process-compiler.ts) admits an exact key allowlist, so Diagram Interchange, pools, lanes, and artifacts are rejected rather than preserved, and [root selection](../packages/bpmn-source/src/root-definition-selection.ts) requires exactly one root element. Admission is acyclic. Variables are string or null. There is no cancellation beyond the direct-parent regional case, no incidents, and no published transition record or token position.
