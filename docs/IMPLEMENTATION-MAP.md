...
**Current Terminate End boundary.** The independently approved semantic checkpoint implements one exact nested parallel source under an unregistered checkpoint profile, identity-only checked `terminateEndEvent`, no-output `terminateScope`, strict checked/program wire shapes, independent Lean and TypeScript containing-scope cancellation, and passive Temporal host admission. The selected occurrence is retained quiescent after its represented live subtree is cleared, the existing aggregate End counter increments once, and unchanged `completeScope` alone continues the parent or completes the root. The exact root-capable checked/IL representation, global-versus-incomplete cancellation discriminators, closure bounds, state-preserving refusals, CMOF/XSD/source negatives, and frozen-experiment exclusions are executable. Product profile registration, answer-free scenarios, product example, differential evidence, live Temporal Worker/replay evidence, CIB evidence, and Product 2 cancellation remain absent pending the registration, evidence, and closure lanes.
