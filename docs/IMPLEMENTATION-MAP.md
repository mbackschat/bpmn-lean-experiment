...
The [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md) has an independently approved Product 1 semantic checkpoint. Core and Lean emit exact transition-aligned starts and completed or cancelled terminals. A strict additive wire carries public identity and one replay-stable instant per E1 command batch; Workflow owns its accumulator and cursor Query. Product 1 client/API and live evidence plus every Product 2 metrics contract, projection, aggregate, route, and UI remain absent.
