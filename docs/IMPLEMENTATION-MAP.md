...
The owner-approved [Message-addressed Receive Task proposal](capsules/RECEIVE-TASK-MESSAGE-PROPOSAL.md) has reached its independently approved pre-Temporal semantic checkpoint. `MessageChannel` is now one closed `operationMessage | directMessage` union across TypeScript, Lean, all three current wire schemas, Java, existing artifacts, and Temporal command identity. Exact Receive Task source admission requires one direct root Message and a source-only nonempty Message name, lowers to the reused `awaitMessage`, and rejects the operation-addressed and unsupported Activity shapes. Lean and the TypeScript core independently establish the exact direct wait, delivery, refusal, projection, and two-step post-delivery closure. The CIB `2.2.0` phase-zero probe establishes `CIB-AGR-0009`. The independent review of checkpoint `7226733` required checked-source and observation-evidence corrections and passed correction audit at `5a74bad`. The profile/scenario/result artifact, retained CIB projection, differential, and Temporal lanes are unblocked but remain absent.
