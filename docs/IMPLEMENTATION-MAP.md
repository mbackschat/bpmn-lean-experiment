...
The owner-approved [Event-Based Gateway Message/Timer proposal](capsules/EVENT-BASED-GATEWAY-PROPOSAL.md) has reached semantic checkpoint target `e595b1c`, whose isolated review returned APPROVE WITH REQUIRED EDITS. TypeScript and Lean implement the exact checked node, `awaitEventRace` operation, disjoint configuration-flow/control-place classification, strict source and standalone-program admission, exact checked-definition binding, hidden occurrence-owned race and activation counters, atomic arming, both winner directions, loser withdrawal, wrong/stale state-preserving refusal, canonical observation through the existing Message/Timer surfaces, quiescence blocking, interruption cleanup, and exhaustive host-operation classification. Winner admission is bound to one unique immutable race operation and both complete live members. Pre-start Temporal host admission deliberately rejects every `awaitEventRace` until the managed scheduler, cancellation, dual-ready fail-closed guard, and live/replay evidence land atomically. Lean independently lowers the checked graph, checks exact equality, relates the evaluator to separate arming/winner relations, and locks bounded closure, singleton race membership, plus the nearest non-laws; the TypeScript core is separately authored. The checkpoint correction awaits the same reviewer's audit, so no profile artifact, target scenario, differential case, managed Temporal scheduler, CIB relationship, or closure evidence lane has advanced. Existing standalone Message and Timer semantics and their current Temporal hosts remain unchanged.
...
- implemented bounded mechanisms cover sequential User Task lifecycle/refusal, per-incoming-flow parallel synchronization, one exact Simple Boolean divergent Exclusive Gateway, one structured two-condition-plus-default Inclusive Gateway split and paired selected-subset join, one exact Message-versus-`PT1S` Event-Based Gateway semantic checkpoint, one exact Intermediate Catch Timer, one operation-addressed payload-free Intermediate Catch Message subscription, one direct-Message payload-free Receive Task, bounded successful Service Task execution, bounded string/null input/output mapping, one exact-code attached interrupting Service Task Error route, ordinary one-level embedded Sub-Process completion, and one direct-parent exact-code Error End propagation with regional cancellation
