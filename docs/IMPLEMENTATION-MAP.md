...
**A live admission defect, found by the non-interrupting boundary Timer capsule's guard and not created by it.** `bpmn-moddle` reduces every `xsd:boolean` attribute to `value === "true"` and reports no warning, so any lexeme that is not exactly `true` reaches the checked graph as `false`.

Every other boolean the pipeline reads compares *against* `true` and therefore refuses a coerced `false`, which is the safe direction. [Scoped flow-element admission](../packages/bpmn-source/src/scoped-flow-elements.ts) is the exception: it admits an embedded Sub-Process when `triggeredByEvent` is absent or `false`, so it admits on the coerced value. `triggeredByEvent="1"`, `'1'`, and `" true "` are schema-valid spellings of *true* and are all accepted, so an **Event Sub-Process is admitted and lowered as an ordinary embedded Sub-Process**.

**Absent:** any refusal of those spellings. The existing negative pins only the `"true"` spelling. This reaches three requirement rows already recorded as `supported` — `BPMN-SUBPROCESS-BOUNDARY-TIMER-01`, embedded Sub-Process completion, and Sub-Process Error propagation — so their evidence covers less than their disposition claims for sources using a non-canonical boolean lexeme. The `cancelActivity` guard in [the compiler](../packages/bpmn-source/src/compile.ts) covers exactly one attribute of this class and states that bound rather than implying the class. Sequencing is [ordered-work item 18](PLAN.md#ordered-work).
