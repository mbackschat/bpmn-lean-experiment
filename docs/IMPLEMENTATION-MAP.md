...
- A strict engine-neutral execution-publication mirror and representation-free gateway, epoch-3 SQLite projection with transactional contiguity, exact overlap, unavailable/gap, restart, and revision-zero rebuild semantics, authorization-first reconciliation and HTTP routes, and an Operations Process-instance detail with fresh-only Overview, exact semantic History, multi-position Diagram overlay, canonical export, and recursive host-private-fact exclusion; the closure-pending operator-history increment adds authorized, independently snapshotted Work and incident actions, strict canonical download, restart convergence, and two-width evidence
