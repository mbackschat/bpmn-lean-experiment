...
Product breadth is admission and configuration evidence, not live execution evidence. What is checked without a Temporal service is that all fifteen profiles load, compile, satisfy host capability, and satisfy semantic start admission, and that every awaited effect descriptor has a declared handler. Live durable execution of the fourteen profiles beyond the User Task example is **absent**: only the User Task example has an external-runtime acceptance test, and that test requires a running Temporal service.
