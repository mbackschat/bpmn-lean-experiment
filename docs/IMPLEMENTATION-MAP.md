...
- every foreign attribute rejects unless a projector consumes it, at every element in the parsed tree; `xsi:type`, `xsi:schemaLocation`, and `xsi:noNamespaceSchemaLocation` are admitted as content-free XML machinery under a prefix resolved from the document's own binding, while `xsi:nil` rejects
