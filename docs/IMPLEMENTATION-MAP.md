...
**Current semantic checkpoint.** The independently approved unregistered standards-only Timer Start capability admits an exact top-level `PT1S` Timer Start Event, preserves Start Event and duration identity through checked source and `initiateTimer`, executes an exact-target start stimulus in Lean and the TypeScript core, closes in two unique internal steps to the existing User Task wait, and is admitted as passive by the Temporal host boundary. It is not yet a registered profile, scenario, product example, differential case, or live Schedule witness. Product 2 scheduling remains excluded from the engine capsule.
