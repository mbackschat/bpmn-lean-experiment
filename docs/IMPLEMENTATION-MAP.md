...
The [non-interrupting boundary Timer proposal](capsules/NON-INTERRUPTING-BOUNDARY-TIMER-PROPOSAL.md) is **evidence-closed and awaiting its closure review**. This is the first capsule in which a boundary Event fires without ending its host, so the enclosing scope holds two live branches and its completion must account for both.
...
That reader cannot see the lexeme, and the parser does not preserve it, so the exact-lexeme refusal happens before parsing and is owned by [the boolean coercion section](#boolean-attribute-coercion-at-source-admission) rather than restated here. What matters for this profile is that a source naming *true* in any spelling cannot reach it.
...
**Implemented and green: Lean.** [The family module](../BpmnSemantics/SemanticProcess/MonitoredTask.lean) carries declarative arming, spawn, and two-constructor completion relations distinct from the evaluator, a soundness bridge for each, the quantified host-preservation law that separates this family from its sibling, a quantified single-token law per transition and a quantified withdrawal law, and quantified off-deadline and wrong-identity refusal. Its checked non-law is that a completion here succeeds under the exact premise on which the sibling's `completeBoundedUserTask_none_of_no_deadline_wait` refuses.
...
**Implemented and green: host capability.** The Temporal adapter gains a fourth managed host class and a third `BoundedDeadlineFamily`, under its own typed `BpmnMonitoredActivitySchedulerUnavailable` identity, because the outcome an operator loses here is a handler branch that never starts rather than a task or a region that never ends. The two counts differ because the Event-Based Gateway race is a managed class with no bounded deadline family.

**Implemented and green: the evidence lanes.** Two registered answer-free schedules run through every target with one differential case each, carrying seeded mutations for an implementation that cancels its host on firing and for one that retains a withdrawn deadline. The Temporal lane adds the Worker-absence run across the due instant, which requires the replacement Worker to *add* a branch and leave the host untouched from committed state alone.

**The scheduler needed a witness the registered schedules cannot supply.** Making the family's `ownsDeadline` never match left the complete Temporal gate green at 147 of 147, because the generic fallback path and the family scheduler agree on every separated order, and separated orders are all a schedule can express. [The shared-activation witness](../packages/temporal-adapter/test/monitored-deadline-temporal.test.ts) composes the activation carrying both callbacks through the direct-VM harness and fails three assertions under that same mutation, so the third `BoundedDeadlineFamily` is evidenced rather than asserted. Its refusal identity is its own, distinct from both interrupting families.

One product example configuration exists and is **not** deferred. Registering a profile makes [the product-example oracle](../packages/temporal-adapter/test/product-example-configs.test.ts) binding in the same change, so the obligation lands with registration rather than with the evidence lanes. Its declared plan answers the handler task first, which is reachable only after the deadline spawns it, then the still-open monitored task, then the normal follow-on. That oracle also admits the example through both pre-start gates without connecting, which is the first evidence that the new managed host class accepts this program. No CIB compatibility claim is selected for this family. The requirement row `BPMN-NON-INTERRUPTING-BOUNDARY-TIMER-01` moves to `supported` for exactly the bounded slice this capsule records, and no wider non-interrupting boundary claim follows from it.
...
`bpmn-moddle` reduces every `xsd:boolean` attribute to `value === "true"` and reports no warning. `xs:boolean` admits `true`, `false`, `1`, and `0`; the coercion agrees on three of them and maps `1` to false where the type means *true*.

Whether that inversion is safe depends on which way the consumer compares. A reader requiring `true` refuses a coerced value, which is the safe direction and covers `isExecutable` and the interrupting `cancelActivity` readers. Three readers instead admit on the coerced value, and all three were therefore fooled by a lexeme meaning *true*.

[Scoped flow-element admission](../packages/bpmn-source/src/scoped-flow-elements.ts) admits an embedded Sub-Process when `triggeredByEvent` is absent or `false`, so `triggeredByEvent="1"` was an **Event Sub-Process admitted and lowered as an ordinary embedded Sub-Process**. [Event-Based Gateway](../packages/bpmn-source/src/event-based-gateway-source.ts) and [Receive Task](../packages/bpmn-source/src/receive-task-source.ts) admission both key on `instantiate` the same way, so `instantiate="1"` was admitted as non-instantiating — and both requirement rows state that qualifier in their own text.

Five requirement rows recorded `supported` were affected, not three: `BPMN-SUBPROCESS-BOUNDARY-TIMER-01`, embedded Sub-Process completion, Sub-Process Error propagation, `BPMN-EVENT-BASED-GATEWAY-01`, and `BPMN-RECEIVE-TASK-01`. The first count was taken from the report that found the defect rather than measured against the readers.

**Implemented and green.** [The compiler](../packages/bpmn-source/src/compile.ts) refuses, on the exact decoded source, any occurrence of a `Boolean`-typed attribute whose lexeme the coercion does not preserve. Both XML attribute-value delimiters are matched. `0` is admitted and `1` is not, which is the disagreement rather than a canonicality rule. Because the comparison is by lexeme, an entity-encoded spelling of a valid boolean such as `&#116;rue` is refused as well; that over-rejection is the safe direction, and a contributor meeting it should read it as the guard's boundary rather than as a parser defect.

The attribute set is derived from the metamodel manifest's `Boolean` properties rather than listed, so it spans exactly the boolean attributes this compiler admits — the class its readers can be fooled by — and a boolean added to the manifest is covered when it is added. Enumerating names would repeat the value-not-position mistake that left `cancelActivity='1'` admitted. The coercion applies to every `xsd:boolean` in the full parser descriptor, which is wider than this manifest's declared partial coverage; no live instance exists in this repository or in the pinned external corpora.

**Owner-confirmed on 2026-08-07.** Refusing only the disagreement, rather than every non-canonical lexeme, is what keeps the Event-Based Gateway specification's admitted `instantiate="0"` valid. Its consequence for the non-interrupting boundary Timer profile is that `cancelActivity="0"` is admitted as non-interrupting, widening the admitted set that capsule's decision 4 records. Decision 4's own two rejections, an omitted attribute and lexical `true`, are unchanged, and the two profiles' admitted sets stay disjoint, so what it secured is preserved.
