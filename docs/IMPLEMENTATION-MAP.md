...
**Current configured Task checkpoint boundary.** One independently approved, unregistered checkpoint profile admits an exact BPMN Task extension in the project namespace, retains a distinct identity-plus-descriptor `configuredTask` checked node, and lowers it to the existing `awaitEffect` operation with the Activity/Probe descriptor, empty mappings, and no BPMN Error route. Source admission is profile-aware, preserves every other profile's exact plain-Task refusal, rejects duplicate `extensionElements` before parser projection can erase one, and locks the exact Start-to-configured-to-User-to-End topology. Strict schemas, independent Lean/core execution, endpoint lowering, closure bounds, effect occurrence refusal, frozen-experiment fail-closed arms, maintained XSD validation, and exact metamodel-source overrides are green. The capability remains absent from product profile, scenario, example, differential, CIB, and live Temporal registries until the active atomic evidence lane closes.
