...
**Two products, two end-to-end floors implemented.** The engine executes the bounded BPMN slice owned by twenty closed semantic capsules. The [BPM platform](BPM-PLATFORM-PROPOSAL.md) has a closed M1 definition showcase and a closed M2 covering exact-version scheduling, Message Start ingress, and [Process-instance search](BPM-PLATFORM-PROCESS-INSTANCE-SEARCH-SPEC.md). M3 human work is at its first implementation checkpoint, pending mandatory independent review. Later absences are recorded under [BPM platform](#bpm-platform).
