...
- classification refusals name their element through [one locating owner](../packages/bpmn-source/src/admission-diagnostics.ts): nullable `id`, `$type`, containment path, the named property or attribute, and the capability the profile lacks. They collect across loci, deduplicate, order by path with numeric indices, including normalized parser warnings
...
- An element on refusals stated over the document or the checked graph; those report one `unsupportedModel` record with `element: null`. Preserved material is retained only in the exact source bytes and has no query surface or public projection
