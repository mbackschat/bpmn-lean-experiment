...
The unapproved [Event-Based Gateway Message/Timer proposal](capsules/EVENT-BASED-GATEWAY-PROPOSAL.md) selects a prospective standards-only profile with one operation-addressed Message catch and one exact `PT1S` Timer catch, but nothing from that proposal is implemented. There is no Event-Based Gateway checked node, `awaitEventRace` operation, configuration-flow classification, hidden race occurrence, atomic winner/loser transition, race-aware quiescence or interruption, managed Temporal scheduler, dual-ready fail-closed guard, profile, scenario, differential case, CIB relationship, or evidence lane. Existing standalone Message and Timer semantics and their current Temporal hosts remain unchanged.
