...
**Two products, three platform showcase floors closed; M4 closure pending.** The engine executes the bounded BPMN slice owned by twenty-one closed semantic capsules. The [BPM platform](BPM-PLATFORM-PROPOSAL.md) has closed M1, M2, and M3 showcases. M4 incident operations are implemented but not graduated. Later absences are recorded under [BPM platform](#bpm-platform).
