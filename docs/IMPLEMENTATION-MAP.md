...
**Absent in Lean:** the quiescence-arm evaluator-soundness bridge, and the activation-counter and `endOccurrences` preservation laws. All three need one component-preservation lemma for `completeScopeState?` — it rewrites only control, occurrences, and tokens, so `timerWaits`, both activation lists, and `logicalTimeMs` carry through unchanged — which is not implemented. The deadline-arm bridge states `parentOwned` as an explicit hypothesis rather than deriving it, and the gap it names is real: the evaluator erases the deadline after regional cancellation without re-checking that cancellation left it there. Atomic arming guarantees it, because the owner is the parent and therefore outside the cancelled subtree, so the case is unreachable under this profile; it is a property of arming rather than of this transition.
