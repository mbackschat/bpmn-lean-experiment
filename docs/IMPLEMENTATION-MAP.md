...
**Current closure boundary.** The standards-only Message Start capability is registered, closure-reviewed, and evidence-closed with its exact top-level operation-addressed source, complete checked and `initiateMessage` channel identity, answer-free scenario, product example, isolated differential case, and direct Temporal Worker-absence/history/replay witness. Product 2's proposal-approved first-green checkpoint publishes the complete capability, owns a strict one-target public resource contract, persists the no-redispatch lifecycle through a shared SQLite schema epoch, and uses a handle-free direct-start host with Worker-independent retained comparison. The checkpoint is pending independent review; public HTTP routes, server composition, UI, and live/browser evidence remain absent. No broker route or definition-version fanout exists.
...
**Current Terminate End boundary.** The independently approved semantic checkpoint implements one exact nested parallel source, identity-only checked `terminateEndEvent`, no-output `terminateScope`, strict checked/program wire shapes, independent Lean and TypeScript containing-scope cancellation, and passive Temporal host admission. The selected occurrence is retained quiescent after its represented live subtree is cleared, the existing aggregate End counter increments once, and unchanged `completeScope` alone continues the parent or completes the root. The corrected source profile admits omitted and parser-safe XSD-false `false` and `0` with distinct exact source identities and equal structural projections, rejects true-valued or parser-ambiguous spellings, and remains bounded to the exact topology.

The standards-only Terminate profile, three answer-free scenarios, runnable example, and differential cases are registered. The live Temporal witness survives Worker replacement, recovers the committed Trigger result, publishes only Outer, rejects a stale Sibling command with state preservation, completes and replays a 20-event history; a test-owned Workflow that cancels the root provides the durable discriminator. The capability is closure-reviewed and evidence-closed. CIB evidence and Product 2 cancellation remain absent.

**Current configured Task boundary.** One registered profile admits an exact BPMN Task extension in the project namespace, retains a distinct identity-plus-descriptor `configuredTask` checked node, and lowers it to the existing `awaitEffect` operation with the Activity/Probe descriptor, empty mappings, and no BPMN Error route. Source admission is profile-aware, preserves every other profile's exact plain-Task refusal, rejects duplicate `extensionElements` before parser projection can erase one, and locks the exact Start-to-configured-to-User-to-End topology. Strict schemas, independent Lean/core execution, endpoint lowering, closure bounds, effect occurrence refusal, frozen-experiment fail-closed arms, maintained XSD validation, and exact metamodel-source overrides are green.

The profile, byte-identical answer-free scenario, product example, and one `cib: null` exact-semantic differential case are registered. The pure Lean/core witnesses reject wrong effect occurrences with state preservation; the adapter has no public effect-completion ingress.
