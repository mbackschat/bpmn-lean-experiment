...
- `@bpmn-lean/platform-postgresql-runtime`, with bounded pools and locked checksum migrations; the explicit migration app applies ordinals 0001 through 0006 at epoch 6, and the recovery runtime owns bounded token leases without owning domain work
...
- `@bpmn-lean/platform-operate` and `@bpmn-lean/platform-work`, whose cross-backend adapters share repository contracts, Unicode identity, terminal closure, race serialization, suffix-only projections, coherent snapshots, and contiguous audit-source ordinals; the two Audit sinks independently enforce exact source-to-sink completeness
...
- Product 2 deployment: persistence and leases exist, but recovery composition, bounded-freshness reads, and replica evidence remain incomplete; the [scale-out roadmap](TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md#horizon-1-remove-product-2s-single-node-scale-boundary) has no shared-mode evidence
