...
**M1 through M6 and Horizon 1 shared Product 2 persistence are closed.** Local mode remains single-node. Shared mode uses PostgreSQL 18 with replicated API and recovery-worker composition. The evaluation-only Compose distribution packages that topology with the Temporal development service and one persistent Product 1 Worker. Context-cold closure target `36c3c75` passed after correction audit `07c68d5`; no production-capacity claim is made.
...
**Current Workflow-chain hosting boundary.** Product 1 privately traverses paired E1 and occurrence-publication segments across Continue-As-New through SHA-bound descriptors and immutable latest-Run selection. The 512th recovery entry closes a nonterminal Run only after its result is retained and returned. Every chain-enrolled candidate preflights its RuntimeState and aligned E1/E2 batch before retention. Retry and conflict precede capacity, terminal completion wins over a filled recovery bound, and no public contract or host identity changes.
...
- `@bpmn-lean/platform-postgresql-runtime` and `@bpmn-lean/platform-recovery-runtime`, with bounded pools, checksum migrations through 0009 and epoch 9, database-clock token leases, and fenced applies without business ownership
...
- `@bpmn-lean/platform-server` with local filesystem/SQLite or shared PostgreSQL-only composition, epoch-9 readiness, projection-backed freshness headers, background-only recovery, and no shared startup or request-time population scan
...
- production recovery-ledger exhaustion at 512 entries: the filling result remains Query-recoverable, changed content remains a conflict, unseen Updates preflight both bounds before queuing, accepted Signals fail before semantic evaluation, terminal state wins, no Workflow retry occurs, and replay succeeds; semantic candidates atomically preflight 64 KiB state and paired batches, reject one-byte overages without revision advance, and replay
...
- Search Attributes
- post-retention publication reconstruction or archive, complete timer/effect/Message rollover evidence, deployment admission, and the approved capacity rows other than recovery-ledger and atomic state/paired-publication enforcement
- task inbox
