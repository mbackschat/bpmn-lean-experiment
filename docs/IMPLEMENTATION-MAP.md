...
- classification refusals name their element through [one locating owner](../packages/bpmn-source/src/admission-diagnostics.ts): nullable `id`, `$type`, containment path, the named property or attribute, and the capability the profile lacks. They collect across loci, deduplicate, and order by path with numeric indices. Parser warnings keep that record in parse order
...
- An element on refusals stated over the document or the checked graph; those report one `unsupportedModel` record with `element: null`. Preserved material is retained only in the exact source bytes, with no query surface or public projection
- The whole BPMN data family, foreign content at any locus a profile declares inert, and the foreign-attribute rule inside the two A12 readers, whose fixtures carry vendor attributes those readers inspect at selected loci, not uniformly
