...
- bounded Call Activity execution under the caller/root Workflow address with distinct called semantic task identity, called-completion address separation, Worker replacement, accepted-result recovery, caller-only continuation, terminal receipt, exactly two Update pairs, zero unrelated host mechanisms, replay, and separately bundled early-return and identity-erasure mutations
