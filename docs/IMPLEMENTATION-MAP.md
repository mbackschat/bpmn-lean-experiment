...
**Evidence.** The `bpmn-2.0.2-subprocess-boundary-timer-draft` profile, two registered answer-free scenarios, and their two differential cases each carry a seeded mutation reaching a public observation. Three host lanes are distinct and run in different gates: a shared-activation refusal under the family's own `BpmnBoundedScopeSchedulerUnavailable` identity, asserted serverless in `test:temporal`; a Worker-absence run across the due instant with the Worker genuinely stopped, needing the ephemeral server; and replay, which has no adapter test of its own for this family and is covered by the aggregate history assertion in `test:pipeline`. Two witnesses stay deliberately outside the registry for structural reasons the [scenario README](../scenarios/subprocess-boundary-timer/README.md) records, and are covered in Lean and the focused core test instead.
