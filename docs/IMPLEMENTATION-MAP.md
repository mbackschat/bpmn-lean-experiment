...
**Two products, four platform showcase floors closed.** The engine executes the bounded BPMN slice owned by twenty-one closed semantic capsules. The [BPM platform](BPM-PLATFORM-PROPOSAL.md) has closed M1 through M4. Later absences are recorded under [BPM platform](#bpm-platform).
...
**Current M5 boundary.** M4 is graduated. The closure-reviewed [M5 E1 committed execution publication specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) implements replay-complete committed transitions and an independent current control-position head through strict decoders/canonical bytes, a Workflow cursor Query, a representation-free client/gateway, an opaque-locator engine API, and Lean/TypeScript parity. Real Temporal evidence covers positive cursors, Worker replacement, terminal retention, repeated activation, and replay. Product 2 transactionally validates and projects contiguous pages, records unavailable/gap without inference, rebuilds from revision zero, and exposes authorized History, current Diagram positions, and canonical export. M5's later platform increments remain open.
...
**A12 product boundary.** The [implemented boundary specification](A12-ADD-ON-BOUNDARY-SPEC.md) contains no A12-specific production decision. The neutral mapped-boundary output passes the shared schemas and exact checked-to-IL Error-route verification. One inventory scans the relevant MIT product closure and validates the frozen exact-byte manifest against baseline `02330ad`. The preservation oracle keeps the payload-free Service Task source/profile exact while allowing only the schema-required empty canonical incident collection and its evidence-digest rebinding. Closure correction `8d6ea1a` and the [future adoption handoff](../adoption/a12/current/README.md#resume-point-for-a-future-a12-add-on) own the stable boundary.
...
- Strict Work HTTP routes and an HTTP-only React Aria, TanStack, and CSS-Modules string-or-Boolean form; live Temporal and Chromium evidence for Definitions/Operate/Work agreement, restart, Worker replacement, actor hiding, completion, audit, history, replay, private-fact exclusion, and task removal
- A full-content Work, Definitions, and Process instances shell with collection-to-detail tabs; source-first or digest-bound generated diagrams and modeller downloads; exact task highlighting; one responsive native task table that becomes visibly labeled no-scroll cards; distinct task and hosting identity; CSS Modules plus one token/reset owner; exact focus, state, and reduced-motion behavior; accessible real M1/M2/M3 browser floors; and a fixed-fixture production-bundle Playwright lane over four widths with direct overflow geometry and Linux-only baselines in an architecture-specific digest-pinned container
- Confirmed-locator bootstrap, all-or-error current incident aggregation, exact operator authorization, durable content-bound Retry/Cancel with recovery and conflict detection, separate cursor-paged audit, strict HTTP, and an Operations workspace covering Process search, current incidents, diagram highlighting, audit, action state, four-width baselines, and a real Temporal/browser/replay witness
- A strict engine-neutral execution-publication mirror and representation-free gateway, epoch-3 SQLite projection with transactional contiguity, exact overlap, unavailable/gap, restart, and revision-zero rebuild semantics, authorization-first reconciliation and HTTP routes, and an Operations Process-instance detail with fresh-only Overview, exact semantic History, multi-position Diagram overlay, canonical export, four-width browser evidence, and recursive host-private-fact exclusion

#### Explicitly absent

- M5 frequency and duration views, completed operator-history presentation, audit export, post-retention execution archive, cross-instance semantic ordering, and other cross-instance read models
