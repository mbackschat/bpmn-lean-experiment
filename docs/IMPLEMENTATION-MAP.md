...
The active [Inclusive Gateway proposal](capsules/INCLUSIVE-GATEWAY-PROPOSAL.md) is not owner-approved and changes no implemented surface. No checked Inclusive Gateway node, `selectMany` or `synchronizeSelected` operation, runtime selected-branch record, source profile, Lean relation or law, TypeScript behavior, Temporal refinement, differential case, or CIB relationship exists. The current `choose` remains first-true/single-output and `synchronize` remains all-input; neither is an Inclusive Gateway implementation.
