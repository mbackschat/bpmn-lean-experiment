...
**What it does not do.** One profile admits modeler notation and the rest do not. `bpmn-2.0.2-user-task-preserved-notation-draft` retains Diagram Interchange, pools, lanes, artifacts, and documentation without executing them, under [the preserve-only admission proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md); every other profile still rejects them through an exact key allowlist. Retained notation reaches no public observation and has no query surface. A rejected source still yields one diagnostic for the whole compilation rather than one per element, so a third party cannot act on it. Data Objects, Data Stores, and the whole BPMN data family reject under every profile. Admission is acyclic. Variables are string or null. There is no cancellation beyond the direct-parent regional case, no incidents, and no published transition record or token position.
...
- one preserve-enabled profile classifies parsed material three ways through [a closed recursive classifier](../packages/bpmn-source/src/preserved-element-classification.ts): a container is preserved only when every descendant it contains is preserved, references are excluded from the walk so a preserved shape may point at an executed element, and the preserved source reaches its executed-only twin's checked graph and program once exact-source identity is normalized away
- every foreign attribute rejects unless a projector consumes it, at every element in the parsed tree, and every resolved reference must point at an element of the type its property declares
- `xsi:type` is admitted as **parser-consumed**, because it selects the resolved element type that every projector then judges; `xsi:schemaLocation` and `xsi:noNamespaceSchemaLocation` are admitted as content-free schema hints. Both require a prefix resolved from the document's own binding, and `xsi:nil` rejects because it empties content
...
- Per-element admission diagnostics: a rejected source still yields one message about the whole compilation, so a third party cannot act on it. Preserved material is retained only in the exact source bytes and has no query surface or public projection
- The whole BPMN data family, foreign content at any locus a profile declares inert, and the foreign-attribute rule inside the two A12 readers, whose fixtures carry vendor attributes their own readers inspect at selected loci rather than uniformly
