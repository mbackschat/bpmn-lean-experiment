...
- A full-content Work, Definitions, and Process instances shell with collection-to-detail tabs; source-first or digest-bound generated diagrams and modeller downloads; exact task highlighting; one responsive native task table that becomes visibly labeled no-scroll cards; distinct task and hosting identity; CSS Modules plus one token/reset owner; exact focus, state, and reduced-motion behavior; accessible real M1/M2/M3 browser floors; and a fixed-fixture production-bundle Playwright lane at 1280 and 1600 pixels with direct overflow geometry
