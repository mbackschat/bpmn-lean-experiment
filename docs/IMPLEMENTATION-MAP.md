...
**Two products, four platform showcase floors closed.** The engine executes the bounded slice owned by twenty-one closed capsules. The [BPM platform](BPM-PLATFORM-PROPOSAL.md) has closed M1 through M4.
...
**Current executable-corpus boundary.** The maintained [executable model corpus](../model-corpus/README.md) binds five retained project models and seven external files across six clone families from CIB Seven, OMG, and Betsy to exact provenance, construct and closed reusable-mechanism inventories, admission, protected pipeline routes, and Product 2 readiness. External Git revisions and the official archive plus entry digests fail closed without retaining external bytes. The retained tranche reuses the existing Lean, semantic-core, selected CIB, Temporal, replay, and mutation pipeline. Product 2 readiness is one: the exact assignment/form model has a production-preview Chromium journey through start, claim, completion, finished History, and exact Work audit. The other four retained models remain ineligible.
...
- GitHub Actions verification on Ubuntu only, with a repository-wide prohibition and executable guard against GitHub-hosted macOS, unchanged warm feedback reporting, a wider hosted pathology ceiling, and derived test-process deadlines
...
- Adjustable metric periods, running pseudo-duration, charts, heatmaps, metric export, completed operator-history presentation, audit export, post-retention archive, and cross-instance semantic ordering
