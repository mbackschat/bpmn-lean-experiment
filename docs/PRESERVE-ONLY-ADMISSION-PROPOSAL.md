...
**Owner-approved on 2026-08-08 after independent cold review, and not yet implemented.** It approves the admission capability that opens M1 of [the showcase milestone ladder](PLAN.md#showcase-milestone-ladder): a third party can deploy their own BPMN file. It changes what source is admitted and what the compiler retains; it changes no runtime transition family, no semantic meaning of any executed construct, and no public observation. This document stays a `-PROPOSAL` under [DOC-DISCIPLINE.md](DOC-DISCIPLINE.md) until the capability is implemented and its closure review passes.
...
The semantic-checkpoint stage is **required** and not yet reached: this proposal changes an admission and profile capability, which is one of the governed claims in [the cold-review rule](../CLAUDE.md#independent-cold-review). The owner approved the proposal on 2026-08-08, so implementation may begin; the checkpoint review is owed at the first green implementation checkpoint, before closure.
