...
- One evaluation-only Docker Compose distribution with pinned PostgreSQL 18.4, Temporal CLI 1.8.1, five non-root project runtime images built from cache-first injected production package closures, a health-gated migration/Worker/API startup order, one public web/API port, separate database roles, and named PostgreSQL/Temporal volumes; online source preparation requires clean committed source and labels every project image with its full revision and tracked-source-tree digest, while offline restart verifies those labels before using no-build/no-pull Compose
- One manual or release-tagged publisher that uses one shared Buildx Bake graph for missing source-labelled `linux/amd64` and `linux/arm64` OCI indexes, retains the digest-pinned zero-build candidate before acceptance, and runs anonymous exact-image acceptance twice with a reset between journeys, zero runtime restarts, always-retained safe diagnostics, and 30-day resumability; Lean, CIB Seven, Java, research sources, showcases, and test harnesses remain outside every runtime image
- One text-first maintained browser walkthrough with an exact ten-image 1440 by 900 catalog, a public-UI-only Playwright capture, isolated dynamic-port Compose lifecycle, fresh temporary volumes, and explicit manual material-UI or release-candidate refresh; ordinary CI, image publication, and version tags perform no screenshot regeneration or pixel comparison
...
- An optional exact-query audience mode with a four-step seven-minute guide, evaluation-only public-interface seeding of one realistic expense task, two terminal Sequential Multi-Instance outcomes, and two independently operable incidents, plus two-width headless Chromium acceptance over diagrams, structured completion, retry-safe recovery, cancellation, correctness disclosure, and an unchanged ordinary shell
