...
- One evaluation-only Docker Compose distribution with pinned PostgreSQL 18.4, Temporal CLI 1.8.1, four non-root project runtime images built from cache-first injected production package closures, a health-gated migration/Worker/API startup order, one public web/API port, separate database roles, and named PostgreSQL/Temporal volumes; online source preparation requires clean committed source and labels every project image with its full revision and tracked-source-tree digest, while offline restart verifies those labels before using no-build/no-pull Compose
- One manual or release-tagged publisher that emits source-labelled `linux/amd64` and `linux/arm64` OCI indexes with BuildKit provenance and SBOMs plus one digest-pinned artifact containing the build-free Compose override, Docker-only lifecycle launcher, exact demo models, guide, and fallbacks, then proves anonymous zero-build startup from those published digests; Lean, CIB Seven, Java, research sources, showcases, and test harnesses remain outside every runtime image
...
- A read-only About destination with package version, the BPMN 2.0.2 target, every executable restriction owned by the capability catalog including the payload-free interrupting User Task boundary Message and definition-scoped Message key-correlation rows, separate CIB evidence, a non-conformance warning, and two-width browser acceptance
- A separate immutable MUE Preview Beta About catalog with exactly seven reviewed checkpoint boundaries in PLAN order, closed production-journey, registered-capability, generated-evidence, and checkpoint-only classifications, explicit Product 2 surfaces and remaining limits, no change to the executable-capability denominator, and exact component plus two-width browser assertions
