...
- A read-only About destination with package version, the BPMN 2.0.2 target, every executable restriction owned by the capability catalog including the payload-free interrupting User Task boundary Message row, separate CIB evidence, a non-conformance warning, and two-width browser acceptance
