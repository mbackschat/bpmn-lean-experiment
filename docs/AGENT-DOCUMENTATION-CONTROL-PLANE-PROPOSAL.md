...
3. complete a baseline-to-target matrix in which every generated unit has exactly one disposition: either a nonempty ordered list of exact target units with one nonempty reviewer-visible source-claim allocation per target, or a justified `duplicate` or `history` deletion; reject a repeated target identity or repeated allocation within one source row, and keep every non-byte-identical target separately visible;
...
The matrix is review-only evidence, stored in temporary review input rather than retained as another status source. `semantic-review-packet.ts` accepts an optional singleton `--migration-matrix <path>` input. For this closure it requires the matrix baseline and target to equal the packet commits, validates the complete structured rows, resolves every target identity, and embeds the source text, every target text, every explicit source-claim allocation, and the exact-byte SHA-256 in the packet JSON. All of that content contributes to `packetSha256`. The checker does not infer semantic claims from prose; it makes each allocation explicit for substantive review while mechanically closing source coverage, target identity, and duplicate allocation. The neutral reviewer prompt pastes that packet verbatim, so no unavailable temporary path or digest-only assertion substitutes for the matrix. The migration checker and its adversarial self-tests remain reusable. The migration may improve headings and remove exact duplicates, but it must not broaden or narrow a product, semantic, proof, evidence, or absence claim. Any such change is split into its own governed work.
...
- extend `semantic-review-packet.ts` with the optional validated `--migration-matrix` input and add adversarial tests for an unknown flag, missing or malformed matrix, baseline or target substitution, duplicate or missing source unit, empty or duplicated target allocation, unresolved target, byte mutation, normalized-content visibility, and packet-digest sensitivity;
...
- every generated baseline paragraph, list item, and table row has one disposition whose nonempty exact target list allocates every source claim once, or an explicit reviewed duplicate/history deletion;
