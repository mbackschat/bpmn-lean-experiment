...
The independently approved boundary Compensation retention representation checkpoint adds one optional strict Program declaration for a flat root scope, canonically ordered explicit handler targets, and count/canonical-byte limits. The declaration is validated against exact ordinary, sequential Multi-Instance, or parallel Multi-Instance User Task operations, but no checked-source shape, source compiler, or registered profile emits it and no existing completion operation consumes it.
...
- checkpoint-only optional `compensationActivityRetention` Program declaration with strict shared-schema and Lean decoding, exact flat-root/target/operation-family validation, and no checked-source producer or registered profile
