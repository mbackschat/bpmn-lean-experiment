...
A private non-admitting reader now resolves the root global synchronous throw, boundary attachment and Association targets, Event Sub-Process containment, and direct subject dependencies from parser-object identity while treating Association direction and Compensation Start interruption as semantically inapplicable. It emits no checked graph, Program declaration, profile, or capability. This prerequisite is non-material under the independent-review gate because it changes none of those governed surfaces; the [source-package gate](TESTING-SPEC.md#focused-gate-matrix), [source-hygiene guard](../scripts/source-hygiene.test.ts), and official-XSD validation cover it.
...
- checkpoint-only optional `compensationActivityRetention` Program declaration with strict shared-schema and Lean decoding, exact flat-root/target/operation-family validation, private runtime consumers, and no checked-source producer or registered profile
...
- one private Compensation provenance reader that canonically projects only a direct root global synchronous throw, resolved boundary-handler triples, contained Compensation Event Sub-Process parent/handler pairs, and direct Sequence Flow dependencies among those selected subjects; inline and resolved-reference Compensation definitions normalize to the same result, `associationDirection` and Compensation Start `isInterrupting` do not classify meaning, and targeted or asynchronous global throws, unresolved or ambiguous edges, unmarked handlers, and handler participation in normal Sequence Flow refuse before any checked-source admission
...
- checked Compensation representation, handler-body/effect and restored-binding source selection, profile admission, source-to-retention/snapshot/execution declaration lowering, and every registered Compensation capability; the private structural reader alone establishes none of these
