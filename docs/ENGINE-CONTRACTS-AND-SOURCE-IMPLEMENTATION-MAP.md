...
A separate answer-free engine-population contract registers exact unique, zero, ambiguous, and cross-definition schedules for Lean, the independently written TypeScript core, and Temporal; CIB is explicitly absent. The unique population input now binds one retained correlated-settlement whole model through a first-class population corpus route, the generated coverage map, one canonical capability row, and the Product 2 About disclosure. The Product 2 operational journey, cost/reflection, and closure remain ahead.
