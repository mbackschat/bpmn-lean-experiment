...
The independently approved boundary Compensation retention checkpoint adds one optional strict Program declaration for a flat root scope, canonically ordered explicit handler targets, and count/canonical-byte limits. The declaration is validated against exact ordinary, sequential Multi-Instance, or parallel Multi-Instance User Task operations, and those existing runtime producers now consume it through private completion facts. No checked-source shape, source compiler, or registered profile emits it.
