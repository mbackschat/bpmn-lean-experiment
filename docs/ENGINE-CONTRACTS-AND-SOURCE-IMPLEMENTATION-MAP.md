...
The registered composed Activity-data profile admits the claim-assessment source with two Process Properties and one User Task carrying required scalar input and output in unpaired sets. Object-identity associations lower to one checked node and operation. Four answer-free scenarios separate present, null, absent-input, and omitted-output paths; independent closure review remains pending.
