...
The Message payload catch semantic checkpoint admits one exact operation-addressed Intermediate Catch Message Event whose Message and Event DataOutput resolve to the same scalar ItemDefinition object, whose required OutputSet and direct association resolve by object identity, and whose target is one distinct Process Property. It lowers to a distinct `payloadMessageCatchEvent` checked node and `awaitPayloadMessage` operation; runtime execution, wire schemas, Lean, scenarios, differential evidence, Temporal hosting, corpus registration, and conformance-ledger support remain downstream of the required checkpoint review.
