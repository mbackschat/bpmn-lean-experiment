...
The composed Activity-data checkpoint admits the claim-assessment source with two Process Properties and one User Task carrying required scalar input and output in unpaired sets. Object-identity associations lower to one checked node and operation. Shared and Lean codecs plus TypeScript and Lean source/lowering are green; scenario/differential, Temporal, corpus, and Product 2 closure remain open.
...
- one checkpoint-only composed Activity-data identity selects one scalar input/output, distinct direct associations and Properties, unpaired sets, and String/null start/completion domains without CIB data behavior; its profile artifact and product registration remain closure work
