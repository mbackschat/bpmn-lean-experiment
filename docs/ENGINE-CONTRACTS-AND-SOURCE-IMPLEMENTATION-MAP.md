...
The registered composed Activity-data profile admits the claim-assessment source with two Process Properties and one User Task carrying required scalar input and output in unpaired sets. Object-identity associations lower to one checked node and operation. Four answer-free scenarios separate present, null, absent-input, and omitted-output paths; independent closure review is approved.
...
The registered Message key-correlation profile owns one exact source reader that resolves the definitional Collaboration, Participants, Conversation, Message Flows, one CorrelationKey/CorrelationProperty, one Process CorrelationSubscription and binding, one scalar Process Property, and exact payload/Process-property selectors by parser-graph identity. It lowers to distinct `correlatedPayloadMessageCatchEvent` and `awaitCorrelatedPayloadMessage` arms, and strict checked/program/stimulus/interaction/publication contracts carry the complete definition-global address and target revalidation facts. Independent checkpoint review approved correction target `1ce28ed5`, and independent public-boundary review approved correction `f627d06e` for the Product 1 operation's closed definition-scoped result without a Process locator or unsettled target.

A separate answer-free engine-population contract registers exact unique, zero, ambiguous, and cross-definition schedules for Lean, the independently written TypeScript core, and Temporal; CIB is explicitly absent. The unique population input binds one retained correlated-settlement whole model through a first-class population corpus route, the generated coverage map, one canonical capability row, and the Product 2 About disclosure. A production-browser/real-Temporal Product 2 journey proves target-free unique, zero-match, ambiguous, response-loss-retry, privacy, and only-target advancement behavior. Cost/reflection, clean complete gates, and independent closure approval are recorded.
...
The registered Compensation source profile consumes that parser-graph provenance for the retained travel-cancellation Process. It admits a distinct global synchronous throw node and closed checked Compensation declaration, including two boundary subjects, one dormant Event Sub-Process subject, one direct restored Process binding, one direct subject dependency, and jointly satisfiable fixed limits. TypeScript lowers the checked value to the existing retention, snapshot, execution, and `triggerCompensation` Program contracts; Lean independently decodes, admits, lowers, and binds the complete checked artifact.

Both semantic accounts derive the required String start binding from that Program and reject malformed or prospectively over-capacity starts before state creation. Product 1 admits the bound Program to the dedicated Compensation scheduler after these checks. The unchanged identity is registered with a normative profile artifact and six neutral scenarios; complete qualification and independent registration review remain pending.
...
- strict registered `correlatedPayloadMessageCatchEvent`, `awaitCorrelatedPayloadMessage`, `deliverCorrelatedPayloadMessage`, complete `CorrelatedMessageAddress`, and global correlated-payload interaction/publication arms, plus a separate answer-free engine-population scenario contract
- strict bounded `globalSynchronousCompensationThrowEvent` and optional checked `compensation` arms, preserving the selected subjects, dormant handler, bodies, restored endpoint identities, dependency, and fixed limits while remaining physically absent from older artifacts
...
- one registered composed Activity-data profile selects one scalar input/output, distinct direct associations and Properties, unpaired sets, and String/null start/completion domains without CIB data behavior
...
- one registered normative Compensation profile admits its exact checked and Program shapes plus one Program-derived String Process-start binding in both semantic accounts; prospective production snapshot and first-frontier capacity checks precede state creation, and six registered scenarios select no CIB Compensation target

### Semantic Process IL

- Implemented draft spec for a checked source-facing graph
- required closed `InternalSchedulingMode` with `rejectObservableChoice` and privately executable `requireChoiceSchedule`; every existing source profile lowers explicitly to reject mode, while no profile yet admits scheduled choice
...
- registered checked/program support for `correlatedPayloadMessageCatchEvent` and `awaitCorrelatedPayloadMessage`, preserving the exact channel, key/property identities, and context-specific scalar selectors under the Message key-correlation profile
- bounded checked Compensation declaration and global synchronous throw node, with strict shared-schema and Lean decoding, exact dormant-scope/profile admission, and deterministic lowering to the existing optional `compensationActivityRetention`, `compensationEventSubProcessSnapshots`, `compensationExecution`, and `triggerCompensation` contracts
...
- `mergeExclusive` carries canonical nonempty inputs and one output, with per-offered-token declarative pass-through, a unique-offer raw helper, and complete private owner/input frontier execution; only the registered cycle profile fixes three inputs
...
- one exact composed Activity-data compiler: two Process Properties, one scalar DataInput/DataOutput, unpaired one-member sets, parser-object-identity associations, and one composed operation rather than two lifetimes
...
- Compensation source/profile admission beyond the exact registered topology, targeted or asynchronous throws, wider handler bodies or mappings, other dependency graphs, and Product 2 failed-Process support
- addressless, operation-addressed, instantiating, or data-bearing Receive Task; Message payload beyond the exact one-output catch profile and one registered correlated source; Message key/global correlation beyond that single-property profile; modeled throw; Message Flow execution; and other Message Event loci
