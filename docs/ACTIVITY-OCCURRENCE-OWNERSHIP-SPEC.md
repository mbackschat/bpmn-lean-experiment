...
| `AOO-JOIN-02` | A static guard over the five enumerated TypeScript producers and the three Lean family modules that none compares an activation ordinal across two counter families, with the one exempt oracle asserted still to hold the pattern so the exemption cannot go vacuous, and each language's pattern shown to separate a cross-family join from a same-family identity check. Every enumerated owner must also contain a record lookup, because a module that pairs nothing satisfies the absence assertion and two Lean modules held no record reference at all before migration |
...
Nearest unsupported claim, and it is sharper than preservation: that the record's value is testable at all inside this slice. Both new-conjunct reds are hand-built state negatives, body turnover is withdrawn as unreachable, and `AOO-MIGRATE-01` is a conservation oracle that says nothing changed rather than that anything improved. What remains as positive schedule-level evidence for `AOO-JOIN-02` is the static producer guard alone, and the checkpoint review sharpened why that is thin: five producers read the record and the sixth site is the oracle, so under every registered profile the two agree and the oracle cannot fail apart from the coincidence. That is a thin lane and is recorded as thin; closure must state it that way rather than counting the migration oracle as evidence for the join. The checkpoint review found that the formal semantic authority still ran the mechanism this capsule exists to remove, which had made the join a one-language rule asserted over two. That is closed: all three Lean families now recover their pair through the record, the two declarative relations that carried ordinal hypotheses take `RecordJoins` instead, and the guard enumerates both languages. What the migration did not buy is a new separating witness. It removes a premise that was true under every registered profile, so no admitted state distinguishes before from after, and the evidence remains the conservation oracle plus the static guard. Behind it sits the older unsupported claim, that a state reached by execution satisfies the new conjuncts: this proposal reduces three unowned per-family premises to one owned conjunct with a decided negative, and does not prove preservation, which stays open.
