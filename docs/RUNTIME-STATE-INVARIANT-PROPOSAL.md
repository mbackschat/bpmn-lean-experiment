...
| `RSI-OBL-05` | refusal preservation: a rejected or semantically failed command returns the received state, so well-formedness is preserved trivially and visibly rather than by omission | `open`, see the Lean lane shape decision; no shared lemma exists, and the core carries the fact executably instead |
...
| Lean predicate and theorems | the account is stated executably and holds at every admitted start, and a withdrawal is final under an assumed uniqueness hypothesis | preservation across the registered arms, which is the open lane, so no law consuming a conjunct over a live wait is discharged; also correctness of the core, adapter, parser, or CIB |
| Lean negative fixtures | each added conjunct rejects its own malformed state, decided in the kernel, beside an admitted unperturbed state | that the conjunct list is complete, or that any branch of a conjunct other than the witnessed one is exercised; the parity emitter applies the same predicate to these same fixture terms, so the two Lean producers count once |
| Core executable preservation lane | every state five schedules reach, including the microsteps inside each stimulus closure, is admitted, and every stimulus commits so a wrongly refusing conjunct fails rather than emptying the assertion | preservation on any schedule those five do not reach: none arms a boundary Timer, holds an effect wait or incident, starts from a Message or Timer, or cancels, so the effect, incident, and event-race owner branches and the declaration instance scoping are unexercised by execution |
...
The exact claim this establishes is that the Lean account's admitted state set is stated executably, holds at every admitted start, and is refused for each named malformed class in both languages, with preservation established executably over five schedules in the semantic core. Preservation by every registered transition is **not** established: that obligation is open, so every Lean law consuming a conjunct over a live wait assumes it. The nearest claim that remains unsupported is that the conjunct list is complete: a missing conjunct is invisible to preservation, because a predicate that omits a fact preserves it vacuously. Only review and a later capsule that needs a fact the list lacks can find that, which is why the reopen trigger below is written against need rather than against a schedule.

The common-mode risk is that the same author writes the predicate and the preservation proofs, so a conjunct weakened to make an arm provable would leave both green. The counter is that each conjunct must be justified by a negative witness that fails without it, and that weakening a conjunct is visible as a diff in the reviewed rule table rather than only in a proof. The granularity is the conjunct, not the branch: ownership covers eight collections and uniqueness, declaration, and order several each, and only the witnessed branch of each is exercised, so deleting an unwitnessed branch would leave every lane green.
...
Reopen when a consumer needs a conjunct the list already states to be discharged rather than assumed, when a capsule needs a state fact the list lacks, when a new operation kind or stimulus is registered, or when the optional gate's cost measurement changes. The first trigger has already fired twice and is not hypothetical: the [interrupting Activity boundary Timer](capsules/ACTIVITY-BOUNDARY-TIMER-SPEC.md) and [Sub-Process boundary Timer](capsules/SUBPROCESS-BOUNDARY-TIMER-SPEC.md) capsules both carry deferrals waiting on preservation of `waitIdentitiesUnique`, so they are standing demand rather than future demand.
