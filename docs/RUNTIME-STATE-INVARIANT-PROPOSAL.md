...
| `RSI-ORDER-01` | the four collections that declare a canonical order hold it: `waits`, `activations`, `selectedBranchSets`, and `eventRaces` | maintained by their insertion functions, unproved |

`RSI-ORDER-01` is deliberately narrow, and the narrowness is a fact about the evaluator rather than a simplification. Exactly four canonical insertion functions exist: `insertUserTaskWait` and `insertTaskActivation` in [`RuntimeState.lean`](../BpmnSemantics/SemanticProcess/RuntimeState.lean), `insertSelectedBranchSetCanonical` in [`InclusiveGateway.lean`](../BpmnSemantics/SemanticProcess/InclusiveGateway.lean), and `insertEventRaceCanonical` in [`EventBasedGateway.lean`](../BpmnSemantics/SemanticProcess/EventBasedGateway.lean). Every other collection prepends, starting with `addToken`. Since `RuntimeState` derives `DecidableEq`, list order is therefore retained state for those collections, and an invariant asserting canonical order everywhere would be refuted by ordinary reachable states and could not be proved before enforcement because the evaluator does not maintain it. Widening this conjunct is a representation change with its own separating witnesses, not part of this slice.
...
`RSI-MONO-03` is the one monotonicity fact the state conjuncts cannot supply, and the reason is worth recording rather than hiding in a hypothesis. Every time-advancing arm assigns `logicalTimeMs := wait.deadlineMs` after checking only `logicalTimeMs = wait.deadlineMs`, in `fireTimer` and in the three boundary victory transitions. A state holding one correctly owned, uniquely keyed, live Timer wait whose deadline is below current logical time satisfies every Layer 1 and Layer 2 conjunct, and firing it lowers logical time. `RSI-MONO-03` therefore carries the explicit hypothesis `wait.deadlineMs >= state.logicalTimeMs` at each of those arms.

Promoting that hypothesis to a state conjunct is deliberately **not** proposed, because "no live deadline below logical time" is itself refutable as soon as two Timer waits with different deadlines are concurrently live and the later one fires first. Which of the two shapes is right depends on the multi-timer account that `INTERNAL-COMMUTATION` and parallel Multi-Instance must settle, so this slice names the hypothesis and records the question rather than choosing a conjunct that the next capsule would have to withdraw.

### Derived rather than assumed

| Rule | Consequence | Derivation |
|---|---|---|
| `RSI-FOREST-01` | the live scope-occurrence graph is an acyclic parent-linked forest | under a `programWellFormed` hypothesis, whose `scopeForestWellFormed` and `acyclicClosed` conjuncts in [`GraphValidation.lean`](../BpmnSemantics/SemanticProcess/GraphValidation.lean) already require an acyclic definition-scope forest, `RSI-BIND-02` projects each runtime parent onto the program's declared parent scope, so no runtime cycle can project onto it |
...
`W1`, a Timer wait or event-race record whose owner occurrence has been removed, violates `RSI-OWN-01`. This is the genuinely unchecked half of ownership: the existing predicate reaches tokens and occurrences but no wait family other than the incident-retained effect wait.
...
Two gates that this work must leave green are red at this target for a reason outside it, and the obligation is therefore stated as "must not be worsened, and must be green before implementation lands" rather than "must stay green". `check:harness-types`, which is `tsc -p tsconfig.harness.json` and not an executable guard file, fails with four `TS2345` errors, and `test:infrastructure:runtime` fails one of 371 assertions with `schema omits awaitSequentialMultiInstanceUserTask`. Both follow from the reserved Multi-Instance operation kind entering the union without the exhaustive host switches and the publication schema being extended in the same change, both reproduce at the clean baseline, and both are unrelated to any documentation change. The compile-time contract gate `check:semantic-types` also applies to the core validator.
