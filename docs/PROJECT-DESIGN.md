...
The [BPMN requirement ledger](BPMN-REQUIREMENT-LEDGER.md) owns standards dispositions. The [CIB–BPMN register](CIB-BPMN-RELATION-REGISTER.md) owns relation classifications. [PLAN.md](PLAN.md#executable-model-corpus-decision-gate) owns the corpus increment's executable acceptance gate and the concrete ordered queue. Corpus evidence selects which requirement to review next; it never approves the semantic account for that requirement.
