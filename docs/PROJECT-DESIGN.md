...
Every capsule proposal and specification carries exactly one `## Lean assurance lane` section whose first content line is exactly one of `Lane shape: proved`, `Lane shape: checked`, or `Lane shape: deliberately open`. A deliberately open field links the runtime/proof implementation map entry that owns its reason and reopen trigger. The documentation-reviewability guard enforces the closed field and owner link; prose may explain scope and residual limits without becoming a second field.
