...
[`repository-statistics.ts`](../scripts/repository-statistics.ts) owns the generated top-level README statistics block. It masks Lean comments and literals before counting public theorem declarations, supporting lemma declarations, and the declared command set; it obtains Java, Kotlin, TypeScript, and Lean file/code/comment/blank counts from `tokei`, retaining an explicit zero row when a selected language is absent. The infrastructure gate tests the classifier, untrusted Tokei-output decoder, deterministic renderer, exact marker replacement, and tracked hook wiring without making `tokei` a CI dependency. Contributors opt into the tracked pre-push hook with `./scripts/install-git-hooks.sh`; a stale block is regenerated and the push stops until `README.md` is committed. `stats:update` writes directly, while `stats:check` is read-only.
