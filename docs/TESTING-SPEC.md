...
Each material proposal-to-specification lifecycle has one mandatory cold proposal review, one conditional cold semantic-checkpoint review, and one governed closure review. Closure is cold by default; it may use the exact checkpoint reviewer warm only under the continuity rule below.
...
| Semantic checkpoint | The first committed green checkpoint after a wire/schema, checked graph/IL, runtime/public-observation, admission/profile, transition/proof, scope, cancellation, or concurrency change | `fork-turns-none` | Before the next implementation lane |
| Closure | One committed implementation and documentation closure after focused/full gates, evidence matrices, epistemic closure, cost record, and exact implemented/absent status are complete | `fork-turns-none`, or `checkpoint-reviewer-warm` only after an approved checkpoint and a green continuity manifest | Before `-PROPOSAL` → `-SPEC` graduation or beginning the next material capsule |
...
| Semantic checkpoint | `<commit>` or `not-applicable` | `fork-turns-none`, historical `external-fresh-session`, `not-recorded`, or `not-applicable` | `approve`, `approve-with-required-edits`, `reject`, `pending`, `not-required`, or `not-reached` | matching audit state |
| Closure | `<commit>` or `not-applicable` | `fork-turns-none`, `checkpoint-reviewer-warm`, historical `external-fresh-session`, `not-recorded`, or `not-applicable` | `approve`, `approve-with-required-edits`, `reject`, `pending`, or `not-reached` | matching audit state |
```
