...
The reviewed full observational checked-source-to-program-run preservation proposition is not claimed. A provisional direct checked-source relation exists in the separately gated experiment lane, and its Stage 1 checkpoint proves constant operation-prefix order preservation plus exact enabled-operation/successor agreement at four states of a two-segment serial chain. General enabled-transition, closure, admission, observation, and run correspondence remain unresolved; structural lowering preservation and exact artifact equality remain the production proved boundary.
