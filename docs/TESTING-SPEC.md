...
13. mutates the observed activation ordinal in sequential cases, omits one initial parallel open task, drops the live sibling after stale A, changes the timer deadline, changes the Service Task handler, changes the CreateDocument final Process variable, changes the boundary-error mapped null to a string, and requires exact disagreement paths; each of these pipeline mutations is applied to a clone of the semantic core's canonical result before comparison, so it proves comparator field sensitivity only — the raw-to-canonical evidence projection is exercised separately by the verifier-side mutations in the contract gate;
