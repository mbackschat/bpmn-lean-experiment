...
The reviewed full observational checked-source-to-program-run preservation proposition is not claimed. A provisional direct checked-source relation exists in the separately gated experiment lane. Its Stage 1 checkpoint proves constant operation-prefix order preservation plus exact enabled-operation/successor agreement at four states of a two-segment serial chain. Stage 2 adds reusable finite-fuel reachability/co-reachability/acyclicity checks, exact producer/consumer program validation, an executable structured serial/balanced-pair parser with exact wait-surface checks, and kernel-checked positive and negative witnesses. Stage 2b adds a declarative checked-graph tail-decomposition relation, executable-tail-parser soundness, graph-derived uniqueness up to parallel-branch exchange, and a path-soundness theorem for positive executable reachability. Its approved stop records that whole-process-wrapper soundness, vertex-count fuel adequacy, completeness of executable path detection, and declarative acyclicity remain unresolved; no negative bounded-search result is treated as proof that no path exists. Standalone checked-source validation also retains three kernel-checked regressions for empty, flowless, and dangling-reference graphs so a maximally extending `fun` body cannot make later checks vacuous. Direct Timer/effect source clauses, closure-selector soundness, the four-step closure theorem, generalized correspondence, and run-level induction remain unresolved; structural lowering preservation and exact artifact equality remain the production proved boundary.
