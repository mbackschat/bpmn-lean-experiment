...
- validates the four draft profiles, eight answer-free scenarios, canonical results, CIB evidence, checked BPMN graph, and Semantic Process program shapes with Ajv Draft 2020-12;
...
The package script supplies the exact `--replace` opt-in to the underlying replacement program. The program refuses direct execution without that flag, executes all eight answer-free scenarios through the pinned runner, verifies cleanup and producer identity, and rewrites only their retained CIB evidence. Every new evidence projection must first gain a meaningful verifier mutation.
...
2. loads eight answer-free scenarios and content-bound CIB evidence;
...
6. runs one eight-case CIB plain-success batch, one raw-only CIB Service Task fail-once execution, one eight-result Lean emitter over that definition batch, the pure core, and sixteen isolated Temporal Workflows; the two same-key Service Task executions are serialized because their probe stores are intentionally per execution, while the other fourteen Workflows remain batched;
...
10. compares CIB, Lean, and the core exactly by scenario identity; requires the explicit target relation for each case, including live-sibling stale rejection and Service Task success; and for sequential stale requires exact Temporal prefix agreement plus a separate `processClosed` adapter assertion;
...
13. mutates the observed activation ordinal in sequential cases, omits one initial parallel open task, drops the live sibling after stale A, changes the timer deadline, changes the Service Task handler, and requires exact disagreement paths;
