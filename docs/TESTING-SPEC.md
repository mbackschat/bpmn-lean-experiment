...
[`publication-statistics.ts`](../scripts/publication-statistics.ts) is a maintainer-only publication aid for the four marked top-level README blocks: Lean declarations plus Java, TypeScript, and Lean Tokei rows. `publication-stats:update` writes those blocks and `publication-stats:check` verifies them without writing. This pre-release repository has no release assembler; any future release assembler must invoke only the read-only command. No pre-push hook, contributor setup step, normal build, test, or CI gate depends on Tokei. Tokei is available only on the maintainer's machine, where the small updater reads its JSON output directly.
...
The warm-pipeline measures are two tiers, neither a semantic invariant. The 15000ms soft target is the developer feedback loop the pipeline is designed for: exceeding it prints the exact measurement so a real regression stays visible, and it never fails a gate. The 40000ms default ceiling is a pathology detector for a hang or runaway lane, deliberately far above the soft target so unrelated CPU load on a shared host cannot fail a correctness gate; a slower environment still declares its own ceiling through `BPMN_PIPELINE_WARM_BUDGET_MS`.
...
10. compares every target by scenario identity under its explicit relation: exact semantic relations for the existing bounded cases; the sequential-stale Temporal prefix plus separate `processClosed`; exact Lean/core/Temporal semantic agreement plus separate synchronous CIB final-state host relations for CreateDocument and boundary error; exact four-target agreement for the direct-Message Receive Task under `CIB-AGR-0009`/`CIB-OP-0005` and for the four ordinary embedded Sub-Process schedules under `CIB-AGR-0007`; Error-propagation recovery lifecycle under `CIB-AGR-0008` plus stale host-task refusal mapping under `CIB-OP-0001`; Lean/core/Temporal agreement with CIB explicitly absent for Simple Boolean, Intermediate Catch Message, structured Inclusive Gateway, and both Event-Based Gateway winner schedules; and no false CIB claim for semantic effect intent, typed result, operation-addressed Message subscription, deferred-choice hosting, or project-language truth;
...
|---|---|---|
| Proposal | Normative interpretation, selected account, scope, exclusions, Temporal preflight, separating counterexamples, and whether the guards and test oracles listed under [required capsule content 15](capsules/README.md#required-capsule-structure) are complete and each obligation achievable against those contracts today | Do not run implementation gates for lanes that do not exist; read those existing contracts statically rather than running their gates |
