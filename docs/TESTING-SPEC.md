...
|---|---|---|
| Proposal | Normative interpretation, selected account, scope, exclusions, Temporal preflight, separating counterexamples, and whether every obligation the proposal places on an already-implemented component satisfies that component's current guard | Do not run implementation gates for lanes that do not exist; read those existing contracts statically rather than running their gates |
