...
The shell entry point validates the BPMN fixtures and then replaces itself with the directly executed TypeScript Maven orchestrator. Each release-specific Maven invocation owns a 60-second workstation deadline and a detached process group. A cold or contended environment may declare a larger positive integer of milliseconds through `BPMN_CIB_MAVEN_TIMEOUT_MS` without changing that default; the runner reports exhaustion as `CIB_MAVEN_BUDGET_EXCEEDED` with the release and applied budget. Timeout or parent interruption terminates the complete Maven/Surefire descendant group, with a forced-kill grace period. The infrastructure test reproduces both timeout and parent-interruption cases with an escaping grandchild, so a stopped gate cannot leave hidden CIB work to contend with the next measurement.
