...
Each material proposal-to-specification lifecycle has two mandatory cold sub-agent reviews and one conditional intermediate review:
...
| Closure | One committed implementation and documentation closure after focused/full gates, evidence matrices, epistemic closure, cost record, and exact implemented/absent status are complete | `fork-turns-none` | Before `-PROPOSAL` → `-SPEC` graduation or beginning the next material capsule |
...
| Closure | `<commit>` or `not-applicable` | `fork-turns-none`, historical `external-fresh-session`, `not-recorded`, or `not-applicable` | `approve`, `approve-with-required-edits`, `reject`, `pending`, or `not-reached` | matching audit state |
```
