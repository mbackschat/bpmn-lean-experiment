...
The checked-source experiment has one explicit default-gate gap. `verify.sh` runs `lake build checkCheckedSourceRelationExperiment` but does not run `lake exe checkCheckedSourceRelationExperiment`. The build kernel-checks the theorem declarations, including the Stage 3a witnesses, but it does not evaluate executable Boolean aggregations such as `stageTwoAdmissionChecks`; therefore the retained Stage 2 negative graph/admission witnesses and fuel-one three-cycle discriminator are not exercised by default verification. Until an owner decision either adds the executable to the default gate or replaces those executable-only locks with kernel-checked declarations, completion evidence for this lane must report the focused `lake exe` command separately. The existing infrastructure guard proves only that the default gate builds the target and reaches the Stage 3a modules; it does not close this execution gap.
...
- validates the six draft profiles, ten answer-free scenarios, canonical results, CIB evidence, checked BPMN graph, and Semantic Process program shapes with Ajv Draft 2020-12;
...
- token projection is independent of storage order, a synthetic mixed-kind state fixes `activeWaits` order as User Task then Timer then effect with element-ID order inside each kind, and duplicate-left/no-right is a checked non-law;
...
2. loads ten answer-free scenarios and content-bound CIB evidence;
...
6. groups the ordinary CIB executions by exact release and runs one eight-case `2.2.0` batch plus one two-case `2.0.0` batch, runs the raw-only CIB Service Task fail-once execution only against `2.2.0`, emits ten Lean results over one definition batch, executes the pure core once per scenario, and runs twenty isolated Temporal Workflows; the two executions for each effect schedule-substitution case and the two plain boundary-error executions are serialized because their probe stores are intentionally per execution, while ordinary Workflows remain batched;
...
10. compares every target by scenario identity under its explicit relation: exact semantic relations for the existing bounded cases; the sequential-stale Temporal prefix plus separate `processClosed`; exact Lean/core/Temporal semantic agreement plus separate synchronous CIB final-state host relations for CreateDocument and boundary error; and no false CIB claim for semantic effect intent or typed result;
...
13. mutates the observed activation ordinal in sequential cases, omits one initial parallel open task, drops the live sibling after stale A, changes the timer deadline, changes the Service Task handler, changes the CreateDocument final Process variable, changes the boundary-error mapped null to a string, and requires exact disagreement paths;
...
15. replays all ten primary live histories plus the Service Task and CreateDocument failure-schedule histories;
