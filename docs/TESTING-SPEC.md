...
The exact-byte-locked small package tests and calibration or documentation utilities may remain `.mjs` only while unchanged and while they remain straightforward JavaScript callers of already compiled packages. Any new or modified script-level module must move into the direct TypeScript gate; substantial orchestration, decoded external boundaries, or reusable typed contracts independently require migration.
