...
External-input readiness is a gate, not an optimization. [`external-sources.lock`](../scripts/external-sources.lock) pins every registered Git evidence checkout by canonical remote, exact commit, and any exact tag association, while the OMG manifest pins the official download set per file. [`setup-external-sources.sh`](../scripts/setup-external-sources.sh) provisions the selected `verify` or `all` scope without rewriting existing trees; [`check-external-sources.sh`](../scripts/check-external-sources.sh) requires exact remotes/revisions/tags and pristine tracked/untracked source; and [`doctor.sh`](../scripts/doctor.sh) checks toolchain, installed workspace dependencies, corpus hashes, and the selected source scope. The doctor additionally lists every declared external source, hashes every dependency owner, and reports all known dependency/build/download caches from [`workspace-cache.lock`](../scripts/workspace-cache.lock), including those not required by the selected scope. The complete verification entry point runs the `verify` doctor first, and hosted verification provisions the same scope before running it. Missing inputs, wrong revisions, and local source changes are infrastructure failures. The A12 source check, CMOF calibration, MIWG observation, and CIB breadth inventory may not skip or weaken their claims when registered external material is absent. While the pending Receive Task semantic-review barrier freezes the Java runner, the CIB entry point also requires that runner's hard-coded A12 witness at default sibling `../oss`; it fails explicitly instead of allowing the JUnit assumption to skip and is deleted when the first permitted runner edit adopts `BPMN_EXTERNAL_ROOT`. [The contributor setup guide](CONTRIBUTOR-SETUP-GUIDE.md) owns the clean-machine and coding-agent procedure.
