...
| A live status fact was restated away from its owner and outlived the change that falsified it: an ordered-work item kept a blocked label for eleven commits after the same document recorded it unblocked and approved, and a capsule Status section copied an absence inventory that three landed lanes made wrong | [Interrupting Activity boundary Timer](capsules/ACTIVITY-BOUNDARY-TIMER-PROPOSAL.md) proposal | 2 | `executable guard` | [ordered-work status agreement](../scripts/plan-status-consistency.test.ts) |
