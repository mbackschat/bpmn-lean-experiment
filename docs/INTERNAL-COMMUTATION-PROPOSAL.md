...
The checkpoint's Lean lane is **proved**. `BpmnSemantics/SemanticProcess/InternalCommutation.lean` owns one private `InternalArmingPatch` representation, the sole `prepareInternalArm?` constructor that derives it from the program and exact pre-state, the atom and footprint projection from that patch, the non-interference predicate, one generic patch-frame theorem, one generic raw-state commutation theorem, and one single-arm runtime-invariant preservation theorem with four constructor cases. `TransitionTrace.lean` proves actual accepted unnumbered transition metadata and lifecycle pairing. A downstream `BpmnSemantics/SemanticProcess/InternalCommutationPublication.lean` imports `ControlPosition.lean` and proves equality of the complete logical-time, transition-metadata, and public-position-delta record plus canonical pair numbering; `SemanticProcessJson/Publication.lean` consumes that theorem instead of re-deciding it. `BpmnSemantics.InternalCommutationConformance` imports both focused modules and is the exact conformance target. This split avoids an import cycle, avoids sixteen constructor-pair proofs, and prevents the footprint and state-update decision tables from drifting.
