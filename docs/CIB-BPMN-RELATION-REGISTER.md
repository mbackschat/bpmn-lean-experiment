...
This is an additional CIB public-service extension over the BPMN User Task lifecycle, not general BPMN data-association, form, or variable-type meaning. The proposed project profile maps one exact tagged Boolean completion value to the existing atomic merge-before-continuation behavior while retaining string/null Process Start and refusing Boolean under every old profile and non-selected surface.

**Evidence:** The Java-21 [Boolean phase-zero probe](../runners/cibseven/src/test/java/org/bpmnlean/cibseven/CibSevenBooleanProcessDataPhaseZeroProbeTest.java) uses pinned CIB Seven `2.2.0`, the public Model API, Task Service, Runtime Service, and History Service under `CIB-CFG-0001`. The [Boolean Process-data proposal](capsules/BOOLEAN-PROCESS-DATA-PROPOSAL.md) owns the proposed project rules, profile gate, Lean question, and Temporal preflight.
...
| User Task completion with a primitive Boolean | Proposed `CIB-EXT-0010` under `CIB-CFG-0001` | The phase-zero probe retains an actual Java Boolean through next-task, runtime, and final-history public observations and separates it from string `"true"`; project profile selection remains blocked on the proposal review |
