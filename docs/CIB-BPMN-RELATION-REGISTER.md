...
The [BPMN conformance target](BPMN-CONFORMANCE-TARGET.md) owns the normative goal, [semantic capsules](capsules/README.md) own bounded project meaning, [profiles](../profiles/README.md) own selected compatibility contracts, the [runtime and proof implementation map](ENGINE-RUNTIME-AND-PROOF-IMPLEMENTATION-MAP.md) owns current semantic coverage, the [assurance and adoption implementation map](ASSURANCE-AND-ADOPTION-IMPLEMENTATION-MAP.md) owns current CIB evidence coverage, and [the testing guide](TESTING-SPEC.md) owns evidence procedure.
