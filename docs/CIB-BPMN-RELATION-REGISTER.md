...
**Status:** Selected bounded extension; semantic checkpoint implemented, profile registration pending
...
This is an additional CIB public-service extension over the BPMN User Task lifecycle, not general BPMN data-association, form, or variable-type meaning. The owner-approved project profile maps one exact tagged Boolean completion value to the existing atomic merge-before-continuation behavior while retaining string/null Process Start and refusing Boolean under every old profile and non-selected surface. The first green semantic checkpoint implements that bounded value policy; artifact registration and retained evidence remain pending the checkpoint review.
...
| User Task completion with a primitive Boolean | Selected `CIB-EXT-0010` under `CIB-CFG-0001`; registration pending | The phase-zero probe retains an actual Java Boolean through next-task, runtime, and final-history public observations and separates it from string `"true"`; the first green checkpoint implements the exact new-profile completion policy while retained profile evidence remains paused for review |
