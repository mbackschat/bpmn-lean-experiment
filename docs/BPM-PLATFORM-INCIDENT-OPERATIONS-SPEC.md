# BPM platform incident operations specification

## Status

**Implemented, closure-reviewed, evidence-closed, and graduated.** Product 1 checkpoint `6e2827b0ade639d5fc58d9be1b69b036d4bde941` and correction `c9415167631653c58f15cc6b620354bb989f1a79` are independently approved. Product 2 retains confirmed locators, fresh-aggregates current incidents, authorizes and durably submits exact Retry and root-Process Cancel interactions, records separate incident audit, exposes strict HTTP routes and an Operations workspace, and carries fixed-fixture plus real-Temporal browser evidence. It adds no semantic transition, profile capability, Lean rule, CIB mapping, checked graph, or IL operation.

The implemented [shared-persistence addendum](BPM-PLATFORM-SHARED-PERSISTENCE-AND-PROJECTION-PROPOSAL.md) preserves this public contract through immutable PostgreSQL incident generations, one-statement database-clock-bounded reads, an exact registration lookup for mutations, one lease-fenced action recovery family, and a separately ordered incident-audit family. Shared requests never run the local startup bootstrap, fleet-wide incident Query, or audit delivery. The SQLite bootstrap and per-request aggregation passages below describe local mode only.
...
The Workflow Query is registered for every admitted profile so ordinary confirmed instances return an empty observed set. Product 2 accepts incidents only in the two graduated M4 profiles and requires their exact payload-free effect shape. A later incident profile with another generation, effect argument, or action set reopens this specification rather than widening silently.
...
The shared DataTable collection/card pattern must remain one semantic DOM with no horizontal page or row scrolling at 1280 and 1600 CSS pixels. No right inspector is introduced. Product 2 Playwright evidence remains path-scoped and outside `verify.sh` and every Product 1 semantic feedback loop.
...
This specification adds one Product 1 public observation through a production Workflow Query and exact incident projection while leaving semantic state, interactions, Updates, receipts, Lean, CIB relationships, checked graph, and IL unchanged.

The Workflow Query is deterministic and read-only over already replayed state. It adds no command, marker, timer, side effect, or history event and selects no Workflow patch or Build ID policy. Stage 1, Stage 2, and nonincident histories replay under the resulting bundle, with exact pre-change histories retained as compatibility evidence. A history-producing branch or version marker reopens this specification.
...
No new dependency was selected. Existing React Aria, TanStack Query/Table, SQLite, Temporal client, and bpmn-js boundaries are sufficient.
...
| Semantic checkpoint | `6e2827b0ade639d5fc58d9be1b69b036d4bde941` | `fork-turns-none` | `approve-with-required-edits` | `c9415167631653c58f15cc6b620354bb989f1a79` |
| Closure | `f55c427d628b0052936969762c93084679832540` | `fork-turns-none` | `approve-with-required-edits` | `914950eb59470b21f44e098145133b41092cdae2` |

Context-cold review of `c3bcd44` required three bounded correction groups. Warm audits of `f66c174` and final correction target `fe0e639` closed them without changing the selected account. The owner approved all nine selected decisions on 2026-08-14. Context-cold checkpoint review of `6e2827b` required one correction to keep the private Workflow-address mint and decoder outside the public engine API; warm audit approved `c941516`. Context-cold closure review of `f55c427` required fail-closed binary review-packet inventory, one shared duplicate-key-safe Product 2 JSON boundary, exact-detail currentness before incident actions mount, and corrected Operations information architecture. Warm audit approved `914950e` with no change to the selected contract, exclusions, or evidence strategy.
