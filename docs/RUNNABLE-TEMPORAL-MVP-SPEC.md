...
The `process` configuration is a strict union of exactly `{ instanceId, initialVariables }` for manual start and exactly `{ instanceId, startEventId, channel }` for Message start. The key sets are mutually exclusive and the shared compiled-program-to-start constructor handles them exhaustively. The union deliberately has no added tag because existing manual example bytes are preserved; an optional-field mode bag is rejected. A Message-start channel mismatch rejects before Temporal connection or Workflow start.
