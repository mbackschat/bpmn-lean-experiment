...
Declared response order — never observation order — decides between two simultaneously enabled interactions, so host iteration order can never present itself as BPMN behavior. Each response is consumed at most once. Distinct Message responses use distinct command identities bound to their declared plan positions and the published subscription, allowing repeated delivery while that subscription remains live. Repeating the same response uses the same identity and existing content-bound recovery. This applies to plain and payload-bearing Message command construction without adding payload admission to a profile.
