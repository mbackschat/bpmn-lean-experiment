...
| [Embedded Sub-Process Error propagation](capsules/SUBPROCESS-ERROR-PROPAGATION-SPEC.md) | `be7845d..afc0436` | `+3370/-398` | `+216/-131` | Unknown | Materially smaller than the ordinary completion foundation while adding a new checked-node and transition family across every evidence lane; regional cancellation and direct-parent handler resolution reuse the existing scope-occurrence model as intended. The closure boundary includes the review correction that extends the Temporal bypass through its stale-command discriminator. |
