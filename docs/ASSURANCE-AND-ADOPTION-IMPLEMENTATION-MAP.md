...
- Temporal VM-bearing test lanes retain their selected file concurrency while disabling V8 parallel scavenging only for those test processes, excluding the upstream [`ClearStaleLeftTrimmedPointerVisitor` race](https://chromium.googlesource.com/v8/v8/+/40fdd17031f9afbbbf788e85010f41d3d99887c8) observed as a native Node 24 crash during direct Workflow VM-context collection without changing production Node behavior
...
The Temporal test-lane V8 correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes no runtime, BPMN meaning, semantic relation, observation, admission, transition, proof, or production-hosting claim. The native crash occurred inside Node 24's parallel scavenger while V8 collected direct Workflow VM contexts; every VM-bearing test lane therefore disables that optimization while retaining its selected file concurrency. [The feedback-efficiency guard](../scripts/test-feedback-efficiency.platform-test.ts) owns the exact commands, and the complete Temporal package gate exercises every affected test convention.
