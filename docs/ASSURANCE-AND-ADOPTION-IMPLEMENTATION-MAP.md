...
- one explicit staged semantic catalog for registered answer-free cases whose Lean/core boundary is green before Temporal refinement exists; it rejects missing mutations, duplicate registration, and any CIB target, while the complete pipeline still requires every non-staged case to carry its declared full target set
