...
- a non-routine Linux evaluation publisher that refuses to overwrite commit-addressed GHCR tags, emits missing images through one shared five-target Buildx Bake graph, derives the five registry-reported multi-platform OCI index digests without traversing image or attestation blobs, retains the no-build candidate before acceptance, then logs out of the registry and completes two reset-separated seeded audience journeys against the anonymous exact-digest stack while requiring zero service restarts and always retaining bounded safe diagnostics for a resumable 30-day acceptance window
