...
- one independently approved boundary Compensation retention representation checkpoint with independent Lean and TypeScript declaration/state validators, exact operation-family completion facts, zero/one/many and interruption discriminators, append-only ordinal/capacity refusal, root-register lifetime, strict shared/Lean readers, old-byte omission, and an exhaustive internal-commutation census entry; producer, source/profile, Temporal, differential, public, and Product 2 evidence remain outside while producer integration is active
