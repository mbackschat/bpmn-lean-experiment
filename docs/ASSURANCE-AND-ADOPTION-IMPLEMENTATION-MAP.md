...
Registered answer-free scenarios run their declared Lean, TypeScript, Temporal, and pinned CIB lanes with seeded mutations and content-bound evidence. CIB is selected compatibility evidence rather than semantic authority, A12 remains an optional external adoption input, and no coverage denominator or license boundary is combined.
...
- dependency-free source-hygiene enforcement over tracked and pending Lean/TypeScript/JavaScript/Java source with an 800-nonblank-line review target, 1200-line hard ceiling, exact import-only Lean umbrellas, allowlist-free rejection of every hand-written `.js`/`.cjs`/`.mjs` module, required Lean module-document placement, descriptive public names for maintained non-experimental conformance facts, one composed Lean assessment shared by the live scan and explicit sparse-valid anti-boilerplate fixture, literal-aware diagnostics, and no current reviewed exceptions
- a source-derived runtime-collection removal guard that treats required and optional `RuntimeState` collections as live owners, recognizes optional conditional-spread preservation, and proves each regional and called-Process route independently reports a seeded missing filter
- source-derived default-root reachability for maintained non-experimental Lean conformance files and named theorem owners; literal-aware discovery rejects fake declarations in comments, strings, and quoted identifiers. Reachability establishes build coverage. [The initialization repair](capsules/RUNTIME-INITIALIZATION-ASSURANCE-REPAIR-SPEC.md) owns this assurance correction
...
- [Internal Commutation evidence](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) retains independent TypeScript permutation oracles, kernel witnesses, atomic fuel refusal, and cross-language operation/state census mutations. [`implementation-status-owner:ENGINE-RUNTIME-PROOF`](ENGINE-RUNTIME-AND-PROOF-IMPLEMENTATION-MAP.md#current-boundary) owns batch implementation and proof status; the [finite arming](INTERNAL-COMMUTATION-PROPOSAL.md#finite-prepared-arming-checkpoint) and [local-control accounts](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite) own the complete preparation, frontier, raw-state, prefix and accepted-publication guarantees. Internal batching establishes no additional source/profile/Temporal admission.
- one independently approved boundary Compensation retention checkpoint with independent Lean and TypeScript declaration/state validators, exact ordinary and current sequential/parallel Multi-Instance completion producers, zero-item issuance, zero/one/many and interruption discriminators, append-only ordinal/capacity refusal before existing mutation, root-register lifetime, strict shared/Lean readers, old-byte omission, and exact owner-keyed zero-entry commutation footprints; source/profile, Temporal refinement, differential, public, and Product 2 evidence remain outside
- one independently approved Compensation Event Sub-Process parent-snapshot checkpoint with independent Lean and TypeScript declaration/state validation, provisional reservation, success-only completion-time context promotion, unsuccessful and root-lifetime purge, selected-root terminal ownership, three-arm refusal with whole-stimulus rollback and no trace/publication, exact declaration-free delegation, replay/erasure laws, Error/Timer/root discriminators, snapshot-specific commutation footprints, and hard-capped proof-cost evidence; source/profile, trigger/restoration/handler, differential, Temporal, public, and Product 2 evidence remain outside
...
- [Public Compensation registration](capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md#public-registration-implementation) implements normative scenarios and corpus disclosure pending qualification/review. Private evidence, CIB and browser exclusions remain separate.
...
- routine GitHub Actions verification on Ubuntu only, with concurrent same-checkout Lean-library and runtime stages, a restored-output interpreted Lean-check stage, and one restored-output integration pipeline that owns every Lean-dependent package parity test and serializes its Lean evidence executions inside one resource-exclusive sub-lane, one stable union verdict, executable guards that derive the complete maintained-conformance import closure and package-test lane partition, reject programmatic bare-Lake and native-executable invocations, keep Lean self-builds out of ordinary package tests, and permit standard GitHub-hosted macOS solely for an explicitly manual clean-machine or Apple Silicon smoke job, an unchanged 30-minute Lean cold-build ceiling, unchanged warm feedback reporting, a wider hosted pathology ceiling, and batch-size-derived Lean-interpreter plus aggregate test-process deadlines
- Temporal VM tests retain file concurrency; concurrent tests also disable Sparkplug. The [diagnostic record](CAPSULE-COST-LEDGER.md#public-compensation-registration-2026-09-24) retains native failures and mitigation evidence.
...
- one exact MUE Preview Beta integration guard that binds PLAN's seven content IDs and order, the separate immutable Product 2 checkpoint catalog, the approved evidence-kind and surface matrix, every acceptance/documentation owner, and the no-new-package boundary; `test:release:mue-preview-beta` builds the Product 2 graph once and reuses the existing Alpha, definition-scoped correlated Message, and UI-quality gates
...
- three exact Message payload catch schedules across Lean, the core, and Temporal with CIB absent, separating a supplied String, supplied explicit null, and absent payload refusal, plus operation-address, routed-Property identity, retained-subscription, Signal durability, continuation, recovery, replay, retained-corpus, and Product 2 capability-disclosure discriminators
- four exact Message key-correlation population schedules across Lean, the core, and Temporal with CIB absent, separating unique, zero, ambiguous, and equal-local-id cross-definition populations; the retained unique whole model enters the corpus through a first-class population route whose complete gate reuses the existing Lean/core and public Temporal refinement owners, and its canonical capability row feeds the Product 2 About disclosure
- two exact interrupting Activity boundary Message schedules across Lean, the core, and Temporal with CIB absent, ordering task completion and payload-free Message delivery first in turn, preserving both stale-loser refusals, and discriminating losing-wait retention, route selection, tagged Activity ownership, occurrence publication, durable Signal/Update scheduling, co-readiness refusal, continuation, Worker replacement, replay, retained-corpus binding, and Product 2 disclosure
...
- exact all-complete, first-complete, and Timer-interrupted parallel Multi-Instance schedules across Lean, the core, and Temporal with CIB semantic execution absent, plus source-role, expression, limit, indexed-order, binding, identity, sibling-withdrawal, no-partial-output, public-progress, E1/E2, continuation, coalesced-readiness, production-capacity, replacement, recovery, and replay discriminators
- three exact Activity data-input schedules across Lean/core/Temporal without CIB, separating a present string, an explicit null, and an absent source, plus source-ownership, association-direction, copy-target, local-owner, disposal, completion-domain, and public-input-collection mutation discriminators
- three exact Activity data-output schedules across Lean/core/Temporal without CIB, separating a supplied string, a supplied explicit null, and an omitted required output, plus routed-versus-named-write, association-direction, cardinality, refusal-preservation, disposal, and empty-input-collection mutation discriminators
- four [composed Activity-data schedules](../scenarios/activity-data-input-output-user-task/README.md), retained claim-assessment evidence, and Product 2 capability disclosure; form/browser-catalog support remains excluded
...
- one registered interchange composition case binds the notation-bearing answer-free scenario to exact CIB evidence, the existing User Task Process-data semantic relation in Lean and the core, Temporal exact-semantic scheduling, Worker replacement, terminal result, exact history, replay, and a final-value mutation
...
A second infrastructure correction is classified the same way: the doctor invocation in [the contributor-setup guard](../scripts/contributor-setup.test.ts) bounded a command that measures 5.9 to 6.9 seconds idle at ten seconds, so it reported a timeout as a doctor failure under the suite's concurrent runner. The bound is a hang guard rather than a performance budget and is widened; the covering guard is that test itself, which exercises the doctor on every infrastructure run.

The Temporal test-lane V8 correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes no semantic or production-hosting claim. VM-bearing lanes disable parallel scavenging. The concurrent lane additionally disables Sparkplug after native crashes persisted in two files through its baseline-entry frame; the [cost ledger](CAPSULE-COST-LEDGER.md#public-compensation-registration-2026-09-24) retains failures and qualification. File concurrency and tests remain unchanged. [The feedback-efficiency guard](../scripts/test-feedback-efficiency.platform-test.ts) binds the commands; the complete Temporal package gate exercises them. This is a test-runtime mitigation, not an upstream fix.

The guards that read enumerated worktree paths read them through one tolerant reader, so a source deleted between enumeration and read is dropped rather than failing the scan. That correction is classified non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes no BPMN meaning, profile, representation, observation, admission, transition family, or proof boundary, and it opens no review cycle. It is covered by [the Lean source-contract guard](../scripts/lean-source-contracts.test.ts), which owns the vanishing-source case, together with [the source-hygiene](../scripts/source-hygiene.test.ts) and [import-boundary](../scripts/lean-import-boundaries.test.ts) guards that consume the same reader.

The source-probe isolation correction is likewise non-material. Node runs selected test files concurrently, so repository-root probe files from the source-hygiene and Lean-source guards could transiently enter the document router's pending-path census and fail an otherwise clean CI run. Syntax and vanishing-source probes now use disposable external directories, while pending-source discovery uses isolated temporary Git repositories; the concurrent document/source guard command locks the class rather than relying on file-level scheduling.

The strict package-dependency correction is non-material. A developed root install could let package tests resolve `ajv`, `typescript`, or `@temporalio/client` even though their owning workspace package did not declare the direct import; a clean pnpm layout then failed before the test body ran. Each affected package now declares its test-only dependency, and the [pnpm project guard](../scripts/pnpm-project-config.test.ts) derives every workspace package's direct TypeScript imports and rejects any package import absent from that package's own dependency sets.

The command-verdict and pipeline-registry correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes neither a registered case nor any semantic claim, relation, observation, transition, or proof boundary. [The long-running command policy guard](../scripts/long-running-command-policy.test.ts) proves durable success, failure, pending, and malformed receipt handling, the build-free harness typecheck rejects unregistered case identifiers, and [the fast pipeline catalog gate](../packages/differential/test/pipeline-catalog.test.ts) owns exact ordered registry equality.

The cross-product publication-vocabulary correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes no operation meaning, public schema, observation, transition, or product dependency direction. [The execution-publication coverage guard](../scripts/execution-publication-contract-coverage.test.ts) compares the producer schema with Product 2's intentionally copied closed decoder vocabulary during shared build-free verification, rejects missing and extra values, and plants the omission that escaped the narrower downstream-only gate.

The specification review-state correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes no BPMN meaning, profile, representation, runtime behavior, observation, transition, proof, or Temporal refinement claim. [The independent-review guard](../scripts/independent-review-policy.test.ts) now scans every active specification and rejects transient pending-review wording whose lifecycle belongs to the governing proposal receipt.

The restored-output lane correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): it changes no BPMN meaning, profile, runtime behavior, observation, transition, proof, or Temporal refinement claim. CI run `33447135471` exposed both mechanisms on 2026-08-31: two newly registered package parity tests entered the Lean-empty runtime job through a direct helper import, while the dedicated Lean job remained demonstrably productive until its 20-minute ceiling cancelled it. [The Lean-test lane guard](../scripts/lean-test-lane-policy.test.ts) now derives Lean-dependent package test gates from their direct command helpers, requires them only in the restored-output integration lane, rejects bare programmatic Lake invocations, and preserves the 30-minute Lean job ceiling.

CI run `33748926595` exposed a different hard-ceiling mechanism on 2026-09-03, and the correction preserves the complete evidence instead of raising that ceiling. A fallback source cache restored successfully, the library build elaborated 270 jobs in 25m47s, and the test driver then regenerated and native-compiled the 194-module `checkConformance` executable closure even though its `main` only printed success. The checked-source executable began the same duplicate path, and the unchanged 30-minute job ceiling cancelled it before the JSON emitter.

The repair makes the semantic library Lake's test driver, moves executable-root elaboration and interpreted checks into a restored-output 10-minute stage, and rejects programmatic native-executable runs. The source-derived reachability guard found fourteen maintained conformance modules outside the default root, including four outside the previous complete command union; eight top-level imports now cover them transitively. The same audit found that Message-correlation population parity existed but entered no complete gate, so it now runs in the restored-output integration lane. These assurance-topology and proof-coverage corrections change no selected BPMN account, runtime transition, public observation, or Temporal refinement claim, but proof coverage is material.

Independent cold review approved target `d51cfa70` after two bounded correction audits closed exact owner-section routing, workflow-derived local-gate discovery across both GitHub filename extensions, and root-import cache identity.

The prepared four-target pipeline executes its primary Lean comparison and three Lean mutation witnesses sequentially inside one resource-exclusive sub-lane. This prevents its wrapper-correct invocations from contending for the fail-closed repository lock while its CIB and Temporal siblings remain parallel.

CI run `33769630495` exposed a nested-deadline defect after the scenario catalog grew from eight cases when the fixed bound was introduced to 68 cases: the contended hosted Lean interpreter emitted ten complete results before its 10000ms child process was terminated. The correction is non-material under [the negative case](TESTING-SPEC.md#independent-cold-review-gate): one shared interpreter entry point now derives its deadline from the selected batch size at the portable pipeline rate while retaining the 10000ms focused-batch floor, and the executable Lean-lane guard rejects a future differential interpreter invocation that bypasses it. No scenario, semantic result, relation, observation, proof, or hosting claim changes.
