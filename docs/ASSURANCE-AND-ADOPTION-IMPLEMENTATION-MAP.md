...
- one independently approved internal-commutation assurance checkpoint with independently implemented closed ordinary-arming footprints, exact-pair Lean invariant, enabledness, raw-state, and actual-publication proofs, complete-frontier classifiers in both languages, a kernel-decided six-permutation three-task discriminator, TypeScript arbitrary-frontier production closure with permutation evidence outside its classifier, atomic fuel refusal, explicit refusal outside the checkpoint boundary, and a proposal-derived cross-language census guard whose mutations add operations and RuntimeState fields independently or drift their mappings
