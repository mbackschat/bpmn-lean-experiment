...
Owner-approved on 2026-08-09 after independent review of the material preservation correction. The owner directed that A12-specific work be preserved but moved out of the core product and that the resulting boundary prepare for a later separately owned add-on. Approval authorizes the exact revised payload-free Service Task guarantee below and no legacy or mixed-schema reader. Context-cold checkpoint `aa83b2f` required corrections; owner-authorized third audit of docs-only correction `398719b` approved it as the semantic-checkpoint correction target. Closure target `9a56f19` was rejected because live non-null-overlay Temporal replay evidence, exact closure cost, and exact status/process records remained incomplete. Those corrections are complete without changing the selected semantic account, public contract, exclusions, or evidence strategy, and the complete gate is green. Same-reviewer closure correction audit is pending.
...
| Closure | `9a56f19` | `checkpoint-reviewer-warm` | `reject` | `not-required` |

The original proposal review used two correction rounds. The context-cold review of `8e46a2e` required a profile-owned descriptor allowlist, preservation of the existing non-A12 Service Task profile and BPMN source, separate frozen and current evidence generations, fresh neutral CIB relationships, and complete guard and preservation obligations. Correction `6bbcf42` closed those findings on substance but left the preservation baseline unnamed. Correction `4619b74` bound every legacy oracle and manifest to immutable pre-extraction target `02330ad`, and the same reviewer approved that proposal version. Semantic-checkpoint target `59bcefc` was rejected because the shared Semantic Process schema retained A12 constants, the frozen dependency closure was incomplete, and the proposal's four-artifact byte-preservation guarantee conflicted with the mandatory wire replacement. Context-cold review of material correction `a44689e` required the owner-decision section to name the revised guarantee, and warm audit `8ae9b66` approved that correction. Replacement checkpoint `aa83b2f` then found an incomplete scalar/catalog inventory, manifest-seeded rather than fixed-root dependency closure, accepted checked-to-IL BPMN Error route drift, and contradictory status text. Its first correction `2af7d2f` closed the frozen closure but still missed a typed BPMN Error message, derived route output from the program under test, and left the exact-resume section stale. Its second correction `8b92040` closed those three mechanisms; the audit left only a stale instruction to commit that already committed target. Docs-only correction `398719b` resolved that sentence, and the owner explicitly authorized a third same-reviewer audit; that audit approved `398719b` as the semantic-checkpoint correction target. Closure review of `9a56f19` then required live non-null-overlay Temporal replay evidence and exact cost and status records. The revised owner decision below remains unchanged.
...
|---|---|---|
| Product source contains no A12-specific decision or optional-root dependency | [`a12-boundary.test.ts`](../scripts/a12-boundary.test.ts) derives the forbidden inventory from immutable baseline `02330ad`, scans complete product roots and script catalog closure, and seeds product-root literals, an add-on reader, and optional-root imports | Research, provenance, guard fixtures, and the optional adoption root may retain A12 names and values |
| The overlay is data-only, content-bound, profile-owned, and bounded | [`source-overlay-admission.test.ts`](../packages/bpmn-source/test/source-overlay-admission.test.ts) covers exact identity/bytes, profile and descriptor isolation, closed shape, ordering, duplicate keys/bindings/loci, Unicode, scalar, entry-count, and byte limits | Only the two reviewed mapped Service Task source shapes and their closed mapping forms are supported |
| Overlay identity changes provenance and transport, not semantic behavior | [`MappedSuccessConformance.lean`](../BpmnSemantics/MappedSuccessConformance.lean), [`service-task-effect.test.ts`](../packages/semantic-core/test/service-task-effect.test.ts), and [`effect-transport.test.ts`](../packages/temporal-adapter/test/effect-transport.test.ts) retain and compare non-null identity while preserving operations and canonical results; the mapped boundary-Error Lean and TypeScript fixtures retain the required null identity only | This is finite checked non-interference, not a universal proof over arbitrary overlays or programs |
| Neutral profiles retain the mapped success and caught Error mechanisms | Registered neutral scenarios run through Lean, the independent TypeScript core, Temporal Activities/replay, and their selected CIB Seven `2.0.0` host relations with seeded result, mapping, route, and transport mutations; [`mapped-success-temporal-tests.ts`](../packages/temporal-adapter/test/mapped-success-temporal-tests.ts) additionally compiles, executes, receipts, and replays one exact non-null overlay | CIB observes synchronous host relations, not the project's semantic effect-in-flight state or Temporal retry model |
| Checked and Semantic Process artifacts preserve the exact boundary Error route | [`contract-definition-artifacts.test.ts`](../scripts/contract-definition-artifacts.test.ts) rejects independent and coherently renamed route fields, and the compiler lowering tests bind output to the checked Sequence Flow | Catch-all, multiple, ancestor, unmatched, or non-direct handlers remain outside the profile |
| No A12 evidence or payload-free Service Task artifact was silently discarded | [`a12-preservation.test.ts`](../scripts/a12-preservation.test.ts) verifies the fixed-root frozen dependency closure and the payload-free byte/wire-only guarantee; the optional adoption gate compares legacy and current projections outside the approved identity translation | The retained current A12 artifacts are calibration evidence, not a production add-on or distribution |
| The complete MIT product remains independent of the A12 checkout | The `verify` doctor and default gate select no adoption input, while the explicit `adoption` scope fails closed on a missing or changed pinned checkout | Exact A12 source evidence is established only by the separate adoption gate |

## Epistemic closure

The exact established claim is that products 1 and 2 contain no A12-specific production decision, while the already implemented generic mapping and boundary-Error mechanisms and all project-authored calibration evidence remain available behind a closed data-only overlay boundary. The closest unsupported claim is a deployable A12 add-on: product 2 has no published package/API boundary, no product-3 overlay registry, handlers, façade adapter, migration, or Workflows-enabled blueprint integration, and the closed exact-model product count remains zero.

The strongest common-mode risk is unchanged: one TypeScript compiler produces the checked graph and Semantic Process program consumed by Lean, the core, and Temporal. Lean independently checks graph-to-program lowering but does not parse BPMN XML. Selected CIB host relations and the immutable baseline projector separate bounded source/host facts, while the preservation oracle deliberately reuses original baseline tooling and therefore establishes preservation rather than another independent semantic account.

Canonical runtime observations depend only on the admitted neutral program, committed runtime state, and explicit semantic inputs. Overlay identity is retained in definition and transport provenance and is never inspected by an evaluator branch. The realistic wrong accounts are an add-on reader, a cross-profile descriptor, a private product literal/catalog entry, a manifest-seeded frozen closure, and a coherently renamed Error-route place; each now has an executable rejection.

The pre-release atomic replacement policy remains in force. No Event History compatibility baseline, legacy reader, profile alias, or migration path is claimed. [The implementation map](IMPLEMENTATION-MAP.md#current-claim) owns exact implemented and absent status, [the cost ledger](CAPSULE-COST-LEDGER.md) owns the commit-bounded measurement, and [the current adoption handoff](../adoption/a12/current/README.md#resume-point-for-a-future-a12-add-on) owns how product 3 resumes without crossing this boundary.
