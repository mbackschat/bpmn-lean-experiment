...
Owner-approved on 2026-08-09 after independent review of the material preservation correction. The owner directed that A12-specific work be preserved but moved out of the core product and that the resulting boundary prepare for a later separately owned add-on. Approval authorizes the exact revised payload-free Service Task guarantee below and no legacy or mixed-schema reader. Context-cold checkpoint `aa83b2f` required four corrections; second warm audit `8b92040` closed every code and evidence finding, and the owner accepted docs-only correction `398719b` as resolution of its remaining stale next-action sentence under the two-audit bound. The semantic checkpoint is approved and the contract and closure record are complete, but closure review has not started because two attempts incorrectly applied the 60-second per-test ceiling to the aggregate complete-gate wrapper. The owner-authorized replacement reached the final infrastructure suite with zero failed assertions before the outer timer cancelled the remaining tests. A correctly scoped complete green gate remains required.
...
| Overlay identity changes provenance and transport, not semantic behavior | The Lean mapped conformance modules, [`mapped-success-service-task.test.ts`](../packages/semantic-core/test/mapped-success-service-task.test.ts), [`mapped-boundary-error-service-task.test.ts`](../packages/semantic-core/test/mapped-boundary-error-service-task.test.ts), and [`effect-transport.test.ts`](../packages/temporal-adapter/test/effect-transport.test.ts) retain and compare identity while preserving operations and canonical results | This is finite checked non-interference, not a universal proof over arbitrary overlays or programs |
| Neutral profiles retain the mapped success and caught Error mechanisms | Registered neutral scenarios run through Lean, the independent TypeScript core, Temporal Activities/replay, and their selected CIB Seven `2.0.0` host relations with seeded result, mapping, route, and transport mutations | CIB observes synchronous host relations, not the project's semantic effect-in-flight state or Temporal retry model |
