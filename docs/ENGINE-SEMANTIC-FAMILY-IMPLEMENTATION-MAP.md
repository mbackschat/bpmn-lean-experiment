...
The families with a section here are the three boundary-Timer loci, the two Multi-Instance User Tasks, and the two direct Activity data User Tasks. Each section names its own rules, evidence lanes, and absences, and is the authority for that family rather than a summary of one.
...
The [Activity data-output mediation proposal](capsules/ACTIVITY-DATA-OUTPUT-MEDIATION-PROPOSAL.md) is **implemented and awaiting closure review** for one exact required direct `DataOutput`-to-Process-`Property` association on one ordinary User Task, with an empty `InputSet` and no transformation.
