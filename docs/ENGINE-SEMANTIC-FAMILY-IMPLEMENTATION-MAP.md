...
- the sole-claimant premise remains open: nothing refuses two records naming one body. Wait-key freshness is no longer a preservation-law premise, because the User Task branch of `RSI-BOUND-01` and turnover's next-count definition derive it. The Activity issuing discipline is separately discharged, but no corresponding result exists for User Task, Timer, Message, Effect, Event race, Call, or Scope identities
