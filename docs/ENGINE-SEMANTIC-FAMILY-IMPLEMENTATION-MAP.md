...
The families with a section here are the runtime-state well-formedness predicate, Activity occurrence ownership, Activity body turnover, the three boundary-Timer loci, the two Multi-Instance User Tasks, and the Activity data-input User Task. Each section names its own rules, evidence lanes, and absences, and is the authority for that family rather than a summary of one.
