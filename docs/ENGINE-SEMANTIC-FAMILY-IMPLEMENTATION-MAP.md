...
- nothing refuses two records naming one body, so the account admits a state on which every body-keyed lookup degrades to `none`. The body-side determinism theorem therefore carries its at-most-one bound as an explicit hypothesis rather than drawing it from a conjunct. The active Parallel Multi-Instance closing proof has now supplied the trigger this map previously lacked: an unrelated record may claim one live parallel child, after which progress or closure strands that alias and falsifies whole-state preservation. A child-scope claimant supplies the same mechanism independently of Multi-Instance. The draft [Activity body-claim uniqueness proposal](ACTIVITY-BODY-CLAIM-UNIQUENESS-PROPOSAL.md) selects a pairwise cross-family refusal; the absence remains until that account is approved and implemented
...
- the continuation decoder's populated parallel-body branch is exercised by the Parallel Multi-Instance continuation fixture. Duplicate task-claim and child-scope-claim refusal mutations remain absent, so no continuation oracle proves that a second record cannot survive Continue-As-New while claiming the same live body. The refusal itself is wired and gated; what is absent is the alias mutation evidence
...
- the sole-claimant premise remains open: nothing refuses two records naming one body. The draft [Activity body-claim uniqueness proposal](ACTIVITY-BODY-CLAIM-UNIQUENESS-PROPOSAL.md) would derive it from production well-formedness for both User Task and child-scope bodies, but no implementation claim is made yet. Wait-key freshness is no longer a preservation-law premise, because the User Task branch of `RSI-BOUND-01` and turnover's next-count definition derive it. The Activity issuing discipline is separately discharged, but no corresponding result exists for User Task, Timer, Message, Effect, Event race, Call, or Scope identities
