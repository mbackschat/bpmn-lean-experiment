...
The families with a section here are the three boundary-Timer loci, the two Multi-Instance User Tasks, the two direct Activity data User Tasks, and the implemented Message payload Catch Event. Each section names its own rules, evidence lanes, and absences, and is the authority for that family rather than a summary of one.
...
The [Message payload catch mediation proposal](capsules/MESSAGE-PAYLOAD-CATCH-MEDIATION-PROPOSAL.md) is **implemented and awaiting closure review** for one exact operation-addressed Intermediate Catch Message Event with one required scalar Event-owned `DataOutput`, one direct association, and one distinct Process `Property`.
...
**Absent.** Closure review and graduation remain open. Structure-bearing or collection `ItemDefinition`s, optional or while-executing outputs, a second output, association, Property, or EventDefinition, expressions, transformation, correlation, modeled throw, Message Flow, and payload on every other Event or Task host remain outside the bounded profile.
