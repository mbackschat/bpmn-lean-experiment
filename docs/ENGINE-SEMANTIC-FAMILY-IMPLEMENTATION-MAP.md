...
The families with a section here are the three boundary-Timer loci, the two Multi-Instance User Tasks, the two direct Activity data User Tasks, and the in-progress Message payload Catch Event. Each section names its own rules, evidence lanes, and absences, and is the authority for that family rather than a summary of one.
...
The [Message payload catch mediation proposal](capsules/MESSAGE-PAYLOAD-CATCH-MEDIATION-PROPOSAL.md) is **implementation-in-progress** for one exact operation-addressed Intermediate Catch Message Event with one required scalar Event-owned `DataOutput`, one direct association, and one distinct Process `Property`.

**Implemented.** Source admission resolves the Message, `DataOutput`, `OutputSet`, association, and target by parser-graph identity and lowers the distinct checked and IL arms. Lean and the independently written TypeScript core retain the ordinary Message-wait shape while selecting the payload-bearing interaction, accept String, Boolean, Integer, and explicit Null payloads, atomically route only the associated Property binding while withdrawing the subscription and producing the outgoing token, and preserve exact state for the payload-free arm, collections, wrong channel or occurrence, Event-race ownership, and stale delivery. Lean proves the declarative delivery bridge and quantified complete runtime-state preservation; strict Lean JSON codecs lock the additive checked, IL, stimulus, interaction, and publication-kind shapes. The constrained Lean module-cost record is bound to exact source commit `9ea2a203` and discloses the payload conformance module and changed Timer Start consumer as near-cap.

**Absent.** Shared JSON Schema, answer-free cross-language scenarios, differential evidence, retained whole-model and Product 2 disclosure, Temporal command-content binding and durable refinement, the commit-bounded closure-cost record, requirement-ledger support, closure review, and graduation remain open. Structure-bearing or collection `ItemDefinition`s, optional or while-executing outputs, a second output, association, Property, or EventDefinition, expressions, transformation, correlation, modeled throw, Message Flow, and payload on every other Event or Task host remain outside the bounded profile.
