...
The interaction Lean lane began red by importing absent exact-task and interaction types. The green account adds semantic activation ordinal, exact completion admission, state-derived open tasks and enabled interactions, and independent expected traces for successful, wrong-activation, and stale-completion scenarios. It proves exact completion for the bounded model; proves rejection, exact state preservation, an empty semantic microtrace, and no closure-bound involvement for any submitted task occurrence whose Process instance, BPMN element, or activation differs from the active occurrence; retains wrong activation as a named corollary; and checks that matching only `elementId` is insufficient.
