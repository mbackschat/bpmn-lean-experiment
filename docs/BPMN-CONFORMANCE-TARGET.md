...
BPMN expression support and CIB JUEL compatibility are separate claims. BPMN supplies `Expression`/`FormalExpression`, definition-level and expression-level language selection, and BPMN rules that consume expression results; JUEL is a CIB-selected language and does not become part of BPMN 2.0.2 by use in the target corpus. Delegating JUEL parsing and evaluation to the pinned runtime can close a bounded CIB profile while leaving the vendor-neutral expression family and standard XPath-language surface unsupported. Conversely, the BPMN account need not implement a language parser itself when an approved profile supplies an exact runtime, but it must still own the visible context, evaluation point, required result, failure consequence, and consuming transition.
