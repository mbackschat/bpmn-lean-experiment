...
The progress view shows planned, completed, active, and pending counts plus the exact current input item. Natural completion shows the ordered terminal output variable from the validated terminal state. The interrupting journey shows the committed `fireTimer` record and the absence of the output variable. It does not synthesize a durable interruption status or claim that absence alone proves a route. Browser-session samples are discarded when the exact instance selection is left and are unavailable after reload; they have no status authority and are labelled `Observed in this browser session`.
...
For the natural journey, the actor submits `accepted`, `flagged`, and `archived` in the retained order. It advances only after the browser has rendered the preceding committed progress sample, which makes the visible progression an oracle rather than a timing hope. For the interrupted journey, it submits only `accepted`, waits for the browser to render one completed item, and then submits no further command; the production deadline scheduler fires the exact `PT1S` Boundary Timer. The browser never receives a Workflow ID, Run ID, Task Queue, Event History, Activity attempt, or private locator.
...
4. show the natural run progress through 0, 1, and 2 completed items, then terminal completion and the ordered `accepted`, `flagged`, `archived` output;
5. start a second exact instance, show one completed item, then show the committed Boundary Timer command, terminal completion, and no output collection;
...
The preview actor uses the existing User Task Update and existing deadline scheduler. Commands remain ordered by the Workflow; each content-bound completion is submitted once by the actor and may be transport-retried under the existing recovery contract. The actor queries the current public interaction before submission and stops on refusal, closure, or identity change. The interrupted branch issues no Timer command from the browser or actor. Worker replay and replacement must preserve the same public samples and terminal result.

The smallest live refinement witness is the two-branch browser run above plus exact Workflow replay. The nearest host counterexamples are a reserved start recovered with different initial data, a browser sample fabricated between two observed revisions, a completion using an actor-constructed activation, and a partial output shown after the Timer route. Each must fail at its owning contract boundary.
...
The first green committed checkpoint includes strict start-data transport and recovery, recursive publication decoding, and their adversarial tests. It changes a public command and public observation consumer, so it receives a cold semantic-checkpoint review before UI and showcase work continue. Closure follows only after the complete browser journeys, replay, local and PostgreSQL gates, implementation maps, exact status, and cost record are green. Warm closure continuity may be used only if the checkpoint reviewer and executable manifest satisfy the existing rule.
