...
Generation is invoked by the Definitions presentation service after semantic admission and durable definition identity are known. The Product 2 definition-projection foundation owns the adapter and the one private parser graph also used by the independently closed Human Task catalog projection; Definitions owns the durable diagram record and public route. The durable key is `{schemaEpoch, sourceSha256, effectiveGeneratorSha256}`. `effectiveGeneratorSha256` binds adapter epoch 1 plus the complete output-affecting locked graph: `bpmn-auto-layout@1.3.0`, `bpmn-moddle@10.0.0`, `moddle@8.2.0`, `moddle-xml@12.1.0`, `min-dash@5.1.0`, and `saxen@11.1.0`. An SQLite `BEGIN IMMEDIATE` insert-or-compare transaction makes generation idempotent across independent connections: an equivalent candidate reuses the winner, while any field or byte conflict is an integrity failure.
...
Use [`bpmn-auto-layout@1.3.0`](https://github.com/bpmn-io/bpmn-auto-layout/tree/v1.3.0) behind the diagram entry point of the Product 2 definition-projection foundation. It is MIT-licensed, produces BPMN DI from BPMN XML, and belongs to the same `bpmn-io` representation ecosystem as the selected viewer. The definition-projection package is the sole Product 2 exception to the semantic parser boundary: raw moddle objects and generator diagnostics remain private, each entry point emits its own closed result, no parser type or generated non-DI model escapes, and the package has no semantic authority. `@bpmn-lean/bpmn-source`, Lean, semantic-core, engine APIs, and Temporal remain byte-unchanged by presentation generation.
...
Source DI is usable when resolution finds exactly one candidate presentation for the selected executable Process. A candidate is either one BPMNPlane bound directly to the exact Process ID, or one BPMNPlane bound to a Collaboration containing exactly one Participant whose `processRef` is that Process. The Collaboration arm additionally requires exactly one finite positive-bounds BPMNShape for the selected Participant. In either arm, every displayed Process flow node has exactly one finite positive-bounds BPMNShape, every displayed Sequence Flow has exactly one BPMNEdge with at least two finite waypoints, and all DI references resolve to exact source IDs. Duplicate selected-Process participants, multiple direct or Collaboration candidates, duplicate diagram IDs or coverage, a plane for a different root, non-finite geometry, or missing required coverage makes source DI unusable. Source-owned diagrams may contain other valid presentation elements, but the task highlight still requires its exact element in the imported registry before the prior marker is changed.
