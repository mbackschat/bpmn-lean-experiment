...
- one exemption, recorded with its reopen trigger: the publication completeness relation is an independent reconstruction whose contract forbids reading state differences, so handing it the producer's records would collapse the oracle into the producer;
...
| Activity occurrence identity | Process instance, Activity element, and the new per-element counter | Not in this slice. The first projection is the sequential Multi-Instance controller's `id`, whose approved public contract reuses this identity, and that is this capsule's own named reopen trigger reached by an amendment rather than by an admitted state | Strictly increasing per element; never reissued after removal, on the same issuing discipline `RSI-MONO-04` still leaves unstated |
...
| `AOO-JOIN-02` | Every *TypeScript* producer of a body-to-handler pair reads it from the record: the four transition owners and the open-set publication binding. One independent oracle is exempt by construction and named in the guard, so this is a producer rule rather than a whole-tree one. The rule is scoped to one language on purpose, because the Lean lane is not migrated: two of its three family modules hold no reference to the record at all. Stating the rule over both languages while enforcing it over one is what the closure review must not be allowed to inherit. | Project representation |
...
Two risks are named rather than resolved. Continue-As-New must not be able to carry a record whose body did not survive with it, which is why the continuation validator gets the conjunct rather than only the command path. And the host's durable deadline join still assumes an identity is never reissued after withdrawal, which is `RSI-MONO-04` and still unstated; this proposal narrows what that assumption is *used* for but does not discharge it. The amendment below widens that use rather than narrowing it, and the consequence is sharper than the wording: once the sequential Multi-Instance controller projects this identity, a reissued identity after removal is visible to an *external consumer* as the same open controller, so an unstated invariant acquires a public consumer. That is a claim about `RSI-MONO-04`'s exposure, not about the amendment's shape, and it belongs to whichever capsule lands that projection.
...
| `AOO-JOIN-02` | A static guard over the five enumerated TypeScript producers that none compares an activation ordinal across two counter families, with the one exempt oracle asserted still to hold the pattern so the exemption cannot go vacuous, seeded by reintroducing a comparison in the migrated publication owner. The guard's enumeration is TypeScript-only and Lean is outside it, so no evidence here covers the reference interpreter |
...
One passage in that set is deliberately **not** corrected here. [The non-interrupting capsule's recorded common-mode exposure](capsules/NON-INTERRUPTING-BOUNDARY-TIMER-SPEC.md#common-mode-risks) says all four targets recover the family by joining element identity to activation ordinal. That narrows to three of four, because Lean still does, and it is an evidence claim rather than a mechanism description. It is routed to this capsule's closure review so the change is governed once, instead of being edited unreviewed or opening a second cycle.

One approved capsule's declared public contract is also affected and is named rather than left to be discovered. [The sequential Multi-Instance proposal](capsules/SEQUENTIAL-MULTI-INSTANCE-PROPOSAL.md#public-contract) is `implementation-in-progress` with an approved review, and its controller record declared the owning `ScopeOccurrenceId`, the immutable operation ID, and the one Timer occurrence identity, alongside its own `MultiInstanceActivityInstanceId` over exactly this record's three identity fields. All four are subsumed by `ActivityOccurrence`. The amendment has landed: that capsule's controller now retains its loop payload, the snapshot, counters, output slots, and the shared identity, and references this record for the rest; its duplicate identity type is retired rather than aliased, which changes no projected byte because the shape was identical; and its adapter paragraph reads the record instead of wait cardinality. That is required by that capsule's own rule against a second disagreeing fact, which is why it landed with this implementation instead of being deferred to it.
...
| [Lean bounded task](../BpmnSemantics/SemanticProcess/BoundedTask.lean) | 185 |
| [Lean monitored task](../BpmnSemantics/SemanticProcess/MonitoredTask.lean) | 125 |
| [Lean bounded scope](../BpmnSemantics/SemanticProcess/BoundedScope.lean) | 120 |
...
Nearest unsupported claim, and it is sharper than preservation: that the record's value is testable at all inside this slice. Both new-conjunct reds are hand-built state negatives, body turnover is withdrawn as unreachable, and `AOO-MIGRATE-01` is a conservation oracle that says nothing changed rather than that anything improved. What remains as positive schedule-level evidence for `AOO-JOIN-02` is the static producer guard alone, and the checkpoint review sharpened why that is thin: five producers read the record and the sixth site is the oracle, so under every registered profile the two agree and the oracle cannot fail apart from the coincidence. That is a thin lane and is recorded as thin; closure must state it that way rather than counting the migration oracle as evidence for the join. Thinner still, and the sharpest thing this capsule now knows about itself: **the formal semantic authority still runs the mechanism this capsule exists to remove.** `BoundedTask.lean` and `MonitoredTask.lean` hold no reference to the record and pair entirely by activation ordinal, including in their declarative relation hypotheses; `BoundedScope.lean` reads the record for arming and withdrawal while `boundedScopeChildFor?` and `BoundedScopePairing` still pair by ordinal. Closure cannot claim the join is record-based across the project while that is true, and migrating the Lean families is real semantic work with its own bridge implications rather than a transcription pass. Behind it sits the older unsupported claim, that a state reached by execution satisfies the new conjuncts: this proposal reduces three unowned per-family premises to one owned conjunct with a decided negative, and does not prove preservation, which stays open.
