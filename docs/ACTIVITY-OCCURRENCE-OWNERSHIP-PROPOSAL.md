...
| `AOO-JOIN-02` | Every *producer* of a body-to-handler pair reads it from the record: the four transition owners and the open-set publication binding. One independent oracle is exempt by construction and named in the guard, so this is a producer rule rather than a whole-tree one. | Project representation |
...
| `AOO-JOIN-02` | A static guard over the five enumerated producers that none compares an activation ordinal across two counter families, with the one exempt oracle asserted still to hold the pattern so the exemption cannot go vacuous, seeded by reintroducing a comparison in the migrated publication owner |
...
| [bounded scope runtime](../packages/semantic-core/src/semantic-process-bounded-scope-runtime.ts) | 322 |
