...
| Progress | Distinct body and child iteration plus exact current counts | Read only the project-owned committed `openMultiInstances` projection and label browser-session retention | Task-count, transition-count, state-difference, or timer-derived progress | The actor advances only after the browser renders the preceding exact revision |
