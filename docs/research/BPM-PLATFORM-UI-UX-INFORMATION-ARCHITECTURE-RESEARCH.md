...
### Definition-scoped Message correlation preflight

The Message key-correlation journey is an exact-version trigger rather than an instance operation. CIB Seven `2.2.0` exposes a global `POST /message` operation backed by `RuntimeService.createMessageCorrelation(messageName)`. Its DTO and builder accept business key, Process variables, local variables, tenant, and even Process-instance id as caller-selected correlation criteria, while `resultEnabled` and `all` choose whether the operation returns the selected result or every result. The matching community JavaScript SDK wraps that global operation. Inspection of the pristine checkout at `5a45b47ea22688d774de97277c3ff7013f54fdd2` covered `CorrelationMessageDto.java`, `MessageRestServiceImpl.java`, `MessageCorrelationBuilder.java`, and `webapps/frontend/camunda-bpm-sdk-js/lib/api-client/resources/message.js`; the community web application source supplied locale strings for Message correlation but no complete user interaction to adopt.

Camunda 8 independently distinguishes buffered publication from synchronous correlation. Its current correlation endpoint takes a message name and correlation key, returns immediate feedback, and does not buffer an unmatched Message. When multiple subscriptions share the same name and key, Camunda 8 deliberately selects one nondeterministically so duplicate keys surface as a design defect. This project's approved account instead makes ambiguity an explicit semantic refusal and preserves every candidate, because choosing any winner outside the pure matcher would violate the reviewed correlation contract.

The Product 2 surface therefore belongs in **Definitions → Triggers** for the selected exact definition version. It accepts an engine-published correlation capability, a caller-owned command identity, and the bounded string Message payload whose selected scalar is the correlation value. It does not expose a Process locator, Process-instance id, business key, tenant, arbitrary variable predicates, fan-out control, or buffering. The platform passes the target-free definition-scoped command through its public engine gateway; it neither derives candidates nor chooses a winner.

The immediate result keeps semantic and host conditions distinct: committed correlation identifies the engine-published Process instance; no match and ambiguous match are separate semantic refusals; admission capacity and infrastructure-indeterminate outcomes remain separate from both. Zero and ambiguity keep every Process unchanged. Transport failure or infrastructure indeterminacy retains the same command identity, correlation value, payload, and retry action rather than manufacturing another publication. A definition without the admitted capability renders the trigger unavailable rather than presenting controls that cannot succeed.

Production acceptance requires one real-Temporal browser journey over the retained correlated-settlement model. It must show unique, zero, and ambiguous outcomes through the public platform boundary; prove that only the unique match advances; keep the command context available after uncertain delivery; and expose neither a Process locator nor Temporal identity. Prerequisite candidate setup may use an explicitly labeled showcase actor over public Product 1 operations because Product 2 does not publish the direct payload Message needed to reach the correlated wait; that actor is test composition, not a hidden Product 2 command surface.
...
### Sequential Multi-Instance preview preflight

CIB Seven's current Process-instance view and pinned source keep a Multi-Instance body distinct from its child Activity instances. `client/scripts/pages/processInstance.js` joins `#multiInstanceBody` with the inner Activity for selection while retaining the separate activity-instance tree. `diagramPlugins/instanceCount.js` shows the exact current running-instance count as an element badge, and incident state uses a separate badge. The tree template names running, completed, and canceled instances explicitly. The transferable pattern is a visible body with exact child progress, not a count inferred from task turnover.

Camunda 8 supplies an independent semantic and product check where CIB's UI does not own aggregation output. Its [Multi-instance documentation](https://docs.camunda.io/docs/components/modeler/bpmn/multi-instance/) distinguishes the body from inner Activity instances, states that sequential execution has one active inner instance at a time, retains output collection order, and documents termination of active instances after an interrupting Boundary Event. This project does not import Camunda's engine representation, variable conventions, or product terminology.

The Alpha decision is bounded by the implemented [Sequential Multi-Instance public contract](../capsules/SEQUENTIAL-MULTI-INSTANCE-SPEC.md#public-contract). Operations Overview adopts exact planned, completed, active, and pending counts plus the exact current input item from committed `openMultiInstances`. It deliberately adds an `Observed in this browser session` progression because the current export retains no prior state snapshots. Every displayed sample must therefore be a validated current state the browser actually received at one exact revision.

Interruption stays separate from progress. The surface may show the exact committed `fireTimer` record already present in semantic History and may show that the terminal output variable is absent. It must not turn task disappearance, controller absence, a platform timestamp, CIB history, or Temporal Event History into an interruption fact. Natural completion shows the ordered terminal output exactly as published.

| Decision boundary | Adopt | Deliberately change | Exclude | Acceptance oracle |
|---|---|---|---|---|
| Placement | Exact Process-instance detail and existing diagram/history context | Add one conditional preview card inside Operations Overview, not a destination | Dashboard, new primary navigation, or Work visibility for metadata-free tasks | The same selected instance retains Overview, History, Diagram, and Operator history |
| Progress | Distinct body and child iteration plus exact current counts | Read only the project-owned committed `openMultiInstances` projection and label browser-session retention | Task-count, transition-count, state-difference, or timer-derived progress | The actor queries the current public interaction before every Update and may advance without waiting on intermediate browser polling; the browser retains each exact state it actually receives |
| Completion | Ordered output collection after natural completion | Show the existing typed terminal Process variable | Recomputing output from submitted task values | `accepted`, `flagged`, `archived` appears in that order from the terminal state |
| Interruption | Separate lifecycle/incident treatment | Show the exact committed Timer stimulus and absent output without synthesizing a status | Partial output, inferred route, or host-history fact | One completed item is visible before `fireTimer`; terminal output remains absent |
| Host actor | Product operations stay distinct from human task work | Use an explicitly labelled showcase actor over Product 1 public interactions | Claim bypass, fabricated occurrence identity, production bot, or private HTTP API | Browser JSON contains no Workflow, Run, Task Queue, Event History, attempt, or private locator |
...
- [Camunda 8 Messages](https://docs.camunda.io/docs/components/concepts/messages/) documents subscription matching by Message name and correlation key, immediate versus buffered delivery, duplicate-subscription selection, idempotency identity, and synchronous result behavior. [Correlate message](https://docs.camunda.io/docs/apis-tools/orchestration-cluster-api-rest/specifications/correlate-message/) and [Publish message](https://docs.camunda.io/docs/apis-tools/orchestration-cluster-api-rest/specifications/publish-message/) define the corresponding current REST operations used by the Message-correlation preflight.
