...
The flow-node occurrence and commit-time publication has now closed together with the bounded Product 2 Flow-node metrics surface. The next proposal should select the Product 2-only operator-history and canonical audit-export contract described by Pattern 12. No Product 1 publication is required because both selected audit streams already exist, but their independent ordering, completeness, authorization, and failure boundaries must be explicit. No separate proposal is recommended for a router, report builder, dashboard, chart library, generalized mining store, or saved views.
