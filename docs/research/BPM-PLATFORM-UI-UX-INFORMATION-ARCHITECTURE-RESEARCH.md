...
### Guided audience-demo preflight

The time-bounded guided-demo surface was researched against the pristine CIB Seven checkout at `5a45b47ea22688d774de97277c3ff7013f54fdd2`. Its Tasklist keeps Process start as an explicit navbar action, model-selection modal, and start form, while the selected-task surface keeps task and Process context together. The inspected owners were `webapps/frontend/ui/tasklist/client/scripts/process/plugins/action/cam-tasklist-navbar-action-start-process-plugin.html`, `webapps/frontend/ui/tasklist/client/scripts/process/modals/cam-tasklist-process-start-modal.html`, `webapps/frontend/ui/tasklist/client/test/specs/process-start-spec.js`, and `webapps/frontend/ui/tasklist/client/scripts/task/directives/cam-tasklist-task.html`. No presenter or guided-audience layer was found. Camunda 8 supplies the independent comparison already recorded above: Process start remains explicit, and Tasklist and Operate remain distinct objectives rather than becoming one demo dashboard.

| Demo boundary | Adopt | Deliberately change | Exclude | Acceptance oracle |
|---|---|---|---|---|
| Product navigation | Keep Work, Definitions, Operations, and About as the stable destinations | `?audience=demo` adds an optional presentation guide that selects those existing destinations | A dashboard, fifth navigation destination, or alternate product shell | The ordinary URL renders the unchanged shell; the audience URL can traverse the complete guide and exit it |
| Preparation | Start from explicit deployed definitions and public Product 1 interactions | One evaluation-only actor deterministically prepares the reviewed expense, sequential Multi-Instance, and incident examples before the presenter opens the browser | Manual BPMN upload, semantic-profile typing, private database writes, fake states, or a production seed endpoint | A fresh Docker reset creates one realistic expense task, two exact Multi-Instance outcomes, and two operable incidents through public contracts |
| Presenter path | Keep form-first work and adjacent Process context | Lead with the business outcome and disclose exact technical evidence on demand | Host-runtime identity, raw Temporal Event History, or implementation vocabulary as the primary story | The seven-minute path reaches the form, diagram, natural and deadline outcomes, incident actions, and correctness explanation without setup fields |
| Correctness disclosure | Keep BPMN support, CIB evidence, Product 2 behavior, and assurance as separate claims | Summarize the Lean reference, independently written TypeScript core, Temporal durability host, and PostgreSQL public projections as a stack | Claiming that one browser run proves formal correspondence, general conformance, or unsupported BPMN breadth | Audience copy links each claim to its existing About or evidence owner and states the exact bounded capability |

The guide is presentation composition over existing public facts, not a new semantic, authorization, or operational surface. Its evaluation actor may use the same published interaction contract as any Product 1 client, but the browser continues to read only PostgreSQL-backed Product 2 projections. Different business labels for the two sequential Multi-Instance schedules are carried by exact source/version identity and must never be inferred from host history or a state difference.
