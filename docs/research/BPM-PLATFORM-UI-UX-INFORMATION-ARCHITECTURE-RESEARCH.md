...
The Product 2 surface therefore belongs in **Definitions → Triggers** for the selected exact definition version. It accepts an engine-published correlation capability, a caller-owned command identity, the correlation value, and the bounded string payload. It does not expose a Process locator, Process-instance id, business key, tenant, arbitrary variable predicates, fan-out control, or buffering. The platform passes the target-free definition-scoped command through its public engine gateway; it neither derives candidates nor chooses a winner.
