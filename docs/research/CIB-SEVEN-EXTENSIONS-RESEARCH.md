...
Together the probes establish source ordering, resolver feasibility, the Boolean/error boundary, and CIB command rollback relevant to the implemented [Exclusive Gateway standards capsule](../capsules/EXCLUSIVE-GATEWAY-CONDITION-SPEC.md) and deferred JUEL overlay. They do not establish a Temporal Java Worker, an adopted dependency, cross-SDK payload compatibility, speculative-command hosting, retained CIB evidence, or Simple Boolean truth.
