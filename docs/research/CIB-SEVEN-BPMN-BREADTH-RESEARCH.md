...
The owner-approved, independently reviewed [Message-addressed Receive Task proposal](../capsules/RECEIVE-TASK-MESSAGE-PROPOSAL.md) selects the smallest closed Message-address representation. Its project-authored phase-zero probe is green against packaged CIB Seven `2.2.0`, selecting `cibseven-2.2.0-message-addressed-receive-task-draft` and `CIB-AGR-0009` before any wire replacement. Implementation atomically replaces the current Interface/Operation/Message-only `MessageChannel` with explicit `operationMessage` and `directMessage` arms, preserving the existing operation-addressed case without optional Interface fields, an invented Operation, or a compatibility reader under the pre-release policy.
