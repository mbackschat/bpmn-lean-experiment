...
The draft [Message-addressed Receive Task proposal](../capsules/RECEIVE-TASK-MESSAGE-PROPOSAL.md) selects the smallest closed Message-address representation for review. It atomically replaces the current Interface/Operation/Message-only `MessageChannel` with explicit `operationMessage` and `directMessage` arms, preserving the existing operation-addressed case without optional Interface fields, an invented Operation, or a compatibility reader under the pre-release policy.
