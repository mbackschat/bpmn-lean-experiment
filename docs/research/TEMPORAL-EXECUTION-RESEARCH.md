...
- [Native deployment specification](../TEMPORAL-WORKER-DEPLOYMENT-REPAIR-SPEC.md): exact bundle-derived Build IDs, native `PINNED` inheritance, explicit Current-based enrollment, and retained-Run Query obligations. The closure-reviewed evidence separates replay from Query preservation with a valid Query-only mutation and covers cross-version correlation, Schedule dispatch, and evaluation composition. The retained callback sequencing helper does not enforce production routing or authorize legacy conversion.
