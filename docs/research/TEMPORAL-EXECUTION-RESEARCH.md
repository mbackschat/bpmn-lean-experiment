...
**Current implementation note:** the pre-release adapter implements the [production Process lifecycle](../TEMPORAL-PROCESS-LIFECYCLE-SPEC.md) with exact Query plus acknowledged Update, semantic-state-derived completion, content-bound Workflow, Update, and timer-command identities, retained-result-first closure recovery, and typed adapter lifecycle results. The [Intermediate Catch Timer spec](../capsules/INTERMEDIATE-CATCH-TIMER-SPEC.md) adds one race-free durable timer composition derived exclusively from committed semantic state. The [Intermediate Catch Message spec](../capsules/INTERMEDIATE-CATCH-MESSAGE-SPEC.md) adds one direct payload-free subscription through durable Signal ingress, a read-only result Query, and completed-receipt recovery without making Signal acceptance the semantic result. Focused gates start fresh in-memory servers, reconcile Query-derived evidence with durable Update and Message-ledger results, timer/Signal history, and terminal receipts, replay histories created during those gates, and then discard all server state. The [production-lifecycle experiment](../experiments/TEMPORAL-PRODUCTION-LIFECYCLE-EXPERIMENT.md) records the Worker-restart, accepted-result, closed-command, payload-alias, handler-order, and race separators. The adapter has no generic Signal compatibility path, committed history fixture, patch branch, or legacy IR reader. General production history/versioning research below remains valid future guidance; [PROJECT-DESIGN.md](../PROJECT-DESIGN.md#pre-release-evolution-policy) owns the current evolution policy.
...
| Signal | Adopted only for direct payload-free Intermediate Catch Message ingress | Durable asynchronous transport; a separate result Query/receipt ledger returns semantic refusal or commitment, while User Task completion continues to use Update |
...
The adapter implements this shape for the sequential and balanced two-task parallel User Task capsules, the exact Intermediate Catch Timer capsule, and the direct payload-free Intermediate Catch Message capsule. The Workflow receives one start stimulus, including canonical string/null initial Process variables, and an admitted Semantic Process program; it performs semantic deployment admission through the core, queues start before registering external handlers, and exposes the diagnostic `bpmn-trace` Query plus the exact `bpmn-open-user-tasks` Query. The runnable MVP adds `bpmn-user-task-detail`, which requires one complete active occurrence and returns only caller-selected committed Process bindings; it is a read-only known-Workflow detail surface, not discovery or canonical authority. The `bpmn-complete-user-task` Update validates transport shape, queues the exact task-instance stimulus, and waits for a command-result ledger entry. The `bpmn-deliver-message` Signal accepts one well-formed exact subscription/channel stimulus, records or coalesces it in a durable Message ledger, and the read-only result Query exposes pending, semantic, or identity-conflict resolution. One main loop alone calls the core’s incremental `advanceScenario` operation and mutates semantic state and trace.

The adapter derives each Update ID from a canonical typed encoding of the complete semantic stimulus and a SHA-256 digest. The Workflow also records the accepted stimulus and first semantic outcome, so exact delivery of the same semantic command returns the original result without a second transition, while reuse of one command ID for a different payload reaches a non-retryable `BpmnCommandIdentityConflict`. The pinned server’s same-Update-ID deduplication is payload-blind at this boundary, and the retained command-ID-only mutation demonstrates why every payload field must affect the key. The Workflow ledger remains Run-local, and cross-Run deduplication remains absent until Continue-As-New is designed.

The runner starts a full local Temporal development server through CLI `v1.8.1`, starts one Worker using SDK `1.21.0`, receives deployment-time compiled project IR, observes stable waits and exact open tasks through Queries, delivers User Task completion through Update, and compares each scenario under its explicit target relation. The timer scenario's explicit firing remains target input for Lean, TypeScript, and CIB but is never delivered to the Workflow; the Workflow derives the identical content-bound firing from committed `openTimers` state after its durable timer resolves. The Message scenario delivers well-formed Signals, resolves their exact semantic or request result through Query/receipt state, and retains wrong-channel refusal, exact duplicate coalescing, Worker absence, stale refusal, command-identity conflict, exact Signal payload history, and reverse-order replay. The gate also covers exact, wrong-activation, sequential post-terminal stale, both parallel completion orders, live-sibling stale, duplicate, concurrent race, Worker-down-at-timer-due, timer-bypass, bounded Service Task Activity, and typed business-error behavior. It fetches Event History, reconciles Query command outcomes with durable Update/Message ledger results, timer observations with one timer-started/timer-fired pair, Message observations with exact Signal Events, Activity observations with the approved effect schedules, and the terminal Query state with the completed receipt, replays every live history, and then discards the in-memory server. JUEL evaluator Activities, timer races and forms beyond the exact capsules, Message payloads/key-based routing/modeled throw, Search Attributes, Continue-As-New, and retained history baselines remain absent.
...
This bounded mapping is feasible and has focused live evidence: both ordered completion histories produce the exact intermediate Query state, duplicate delivery is stable, live-sibling stale rejection reaches the core, a concurrent same-occurrence race produces one commit and one rejection without pinning the winner, every accepted completion Update finishes before Workflow completion, and all produced histories replay. It does not authorize Activities, timer behavior beyond the exact separate capsule, cancellation, Continue-As-New, a production canonical-observation API, or a task-inbox design.
