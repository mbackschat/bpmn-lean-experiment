...
**Current implementation note:** the pre-release adapter implements the [production Process lifecycle](../TEMPORAL-PROCESS-LIFECYCLE-SPEC.md) with exact Query plus acknowledged Update, semantic-state-derived completion, content-bound Workflow, Update, Message, and timer-command identities, chain-relative recovery, and typed adapter lifecycle results. Product 1 starts use the exact `bpmn-workflow-chain-v1` patch with production budgets, expose no SDK handle, and keep Run identity private. A forced cyclic User Task witness crosses three Runs and two Continue-As-New boundaries while preserving committed state, occurrence identity, exact retry/conflict behavior, terminal receipt, and replay. The public v1 terminal receipt contains no Message ledger; the private Workflow result carries bounded recovery entries, and one decode-only seam normalizes exact pre-v1 receipts. The [Intermediate Catch Timer spec](../capsules/INTERMEDIATE-CATCH-TIMER-SPEC.md) adds one race-free durable timer composition derived exclusively from committed semantic state. The [Intermediate Catch Message spec](../capsules/INTERMEDIATE-CATCH-MESSAGE-SPEC.md) adds one direct payload-free subscription through durable Signal ingress and a read-only result Query without making Signal acceptance the semantic result. Focused gates start fresh in-memory servers, reconcile Query-derived evidence with durable Update results, private recovery entries, timer/Signal history, and terminal receipts, replay histories created during those gates, and then discard all server state. The [production-lifecycle experiment](../experiments/TEMPORAL-PRODUCTION-LIFECYCLE-EXPERIMENT.md) records the Worker-restart, accepted-result, closed-command, payload-alias, handler-order, and race separators. The adapter now privately traverses paired E1/E2 publication segments across retained Runs. Strict descriptors and a contiguous directory are SHA-bound through continuation; each observation attempt takes one immutable latest-selection snapshot before querying the exact selected Run, returns `unavailable` when that retained Run is missing, and uses the legacy direct Query only when the selection Query raises `QueryNotRegisteredError`. Workflow/Run identity and all segment metadata remain private, and public APIs, shapes, and the terminal receipt remain unchanged. The adapter still has no generic Signal compatibility path, committed history fixture, general legacy IR reader, post-retention reconstruction/archive, or approved Worker deployment strategy. General production history/versioning research below remains valid future guidance; [PROJECT-DESIGN.md](../PROJECT-DESIGN.md#pre-release-evolution-policy) owns the current evolution policy.
...
| Signal | Adopted only for direct payload-free Intermediate Catch Message ingress | Durable asynchronous transport; a separate result Query/private recovery ledger returns semantic refusal or commitment, while User Task completion continues to use Update |
...
The production recovery count bound is active rather than documentary. When the 512th entry commits, the Workflow retains and returns that outcome, fences unseen commands, lets terminal state complete normally, and otherwise fails only after accepted handlers drain. The retained failed Run's recovery Query resolves the filling identity and conflicts before its capacity fallback. Separately, the Workflow constructs an immutable semantic successor and canonically measures its complete RuntimeState plus its newly appended aligned E1/E2 batch before assigning publication, trace, Message resolution, recovery, or state. The 64 KiB bounds are inclusive; one-byte overages retain revision zero or the preceding committed revision and fail without a synthetic semantic transition. The typed details contain only the approved bound, observed value, semantic Process identity, public revision, and Run ordinal; no Temporal Run ID or speculative candidate crosses the boundary.

The runner starts a full local Temporal development server through CLI `v1.8.1`, starts one Worker using SDK `1.21.0`, receives deployment-time compiled project IR, observes stable waits and exact open tasks through Queries, delivers User Task completion and incident retry through Updates, and compares each scenario under its explicit target relation. The timer scenario's explicit firing remains target input for Lean, TypeScript, and CIB but is never delivered to the Workflow; the Workflow derives the identical content-bound firing from committed `openTimers` state after its durable timer resolves. The Message scenario delivers well-formed Signals, resolves their exact semantic or request result through Query/private recovery state, and retains wrong-channel refusal, exact duplicate coalescing, Worker absence, stale refusal, command-identity conflict, exact Signal payload history, production patch enrollment, and reverse-order replay. The incident scenario receives one typed technical result, observes literal generation 1, replaces the Worker, retries the same effect occurrence, then succeeds and replays. Separate live witnesses prove a second technical result is host failure with no second incident and two distinct retry Updates yield exactly one commit and one rejection. The gate also covers exact, wrong-activation, sequential post-terminal stale, both parallel completion orders, live-sibling stale, duplicate, concurrent race, Worker-down-at-timer-due, timer-bypass, bounded Service Task Activity, and typed business-error behavior. It fetches Event History, reconciles Query command outcomes with durable Update results and private recovery entries, timer observations with one timer-started/timer-fired pair, Message observations with exact Signal Events, Activity observations with the approved effect schedules, and the terminal Query state with the closed public receipt, replays every live history, and then discards the in-memory server. A separate forced cyclic witness crosses three Runs and two Continue-As-New boundaries, and the paired publication witness traverses Run-local E1/E2 pages across the same chain. JUEL evaluator Activities, timer races and forms beyond the exact capsules, Message payloads/key-based routing/modeled throw, Search Attributes, retained history baselines, post-retention publication reconstruction/archive, and complete cross-mechanism rollover evidence remain absent.
