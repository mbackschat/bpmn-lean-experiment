...
### Profile-selected expression evaluation Activities
...
Deployment-time syntax validation cannot use an Activity outside a Workflow. The selected transport is therefore a short-lived TypeScript `validateJuelConditions` Workflow started by the deployment orchestrator after structural compilation and before any Process Workflow. It schedules one batched parse-only Activity on the same Java evaluator Worker, verifies a content-bound receipt, and completes. Syntax failure rejects the definition; Worker absence, timeout, malformed transport, or validation-Workflow failure is deployment infrastructure failure. This uses the approved Java Temporal/JUEL graph without adding a direct evaluator endpoint or fourth runtime root, and its Event History is deployment evidence rather than Process history.
...
| Exclusive Gateway | `if`/`else` decision | Implementation technique only | The semantic core owns candidate order, evaluation point, default-flow rules, and the selected outgoing Sequence Flow; under the selected CIB profile, the pinned JUEL runtime evaluates condition truth from the complete approved context and returns a bound receipt |
...
The adapter implements this shape for the sequential and balanced two-task parallel User Task capsules plus the exact Intermediate Catch Timer capsule. The Workflow receives one start stimulus and admitted Semantic Process program, performs semantic deployment admission through the core, queues start before registering external handlers, and exposes the diagnostic `bpmn-trace` Query plus the exact `bpmn-open-user-tasks` Query. The `bpmn-complete-user-task` Update validates transport shape, queues the exact task-instance stimulus, and waits for a command-result ledger entry. One main loop alone calls the core’s incremental `advanceScenario` operation and mutates semantic state and trace.
...
The runner starts a full local Temporal development server through CLI `v1.8.1`, starts one Worker using SDK `1.21.0`, receives deployment-time compiled project IR, observes stable waits and exact open tasks through Queries, delivers User Task completion through Update, and compares each scenario under its explicit target relation. The timer scenario's explicit firing remains target input for Lean, TypeScript, and CIB but is never delivered to the Workflow; the Workflow derives the identical content-bound firing from committed `openTimers` state after its durable timer resolves. The gate covers exact, wrong-activation, sequential post-terminal stale, both parallel completion orders, live-sibling stale, duplicate, concurrent race, Worker-down-at-timer-due, timer-bypass, bounded Service Task Activity, and typed business-error behavior. It fetches Event History, reconciles Query command outcomes with durable Update results, timer observations with one timer-started/timer-fired pair, Activity observations with the approved effect schedules, and the terminal Query state with the completed receipt, replays every live history, and then discards the in-memory server. JUEL evaluator Activities, timer races and forms beyond the exact capsules, Search Attributes, Continue-As-New, and retained history baselines remain absent.
...
11. The executable Java/TypeScript shared schema, payload-converter proof, task-queue registration, cross-SDK compatibility evidence, and Worker deployment unit for the policy selected by the [Exclusive Gateway condition proposal](../capsules/EXCLUSIVE-GATEWAY-CONDITION-PROPOSAL.md). The validation-Workflow transport, runtime attempt-exhaustion behavior, and task-queue role are selected; their executable forms remain implementation work.
