...
- [Production Process lifecycle](../TEMPORAL-PROCESS-LIFECYCLE-SPEC.md): exact Query/acknowledged Update, semantic completion, content-bound identities, chain-relative recovery, private Run identity, and closed terminal receipts.
- [Workflow-chain production contract](../TEMPORAL-PROCESS-LIFECYCLE-SPEC.md#workflow-chain-production-contract): stable-checkpoint rollover, SDK-suggestion precedence, lower-only project triggers, activation reserves, per-payload/aggregate limits, and the 128-Run ceiling. These are hosting facts, never BPMN or CIB observations.
- [Native deployment repair](../TEMPORAL-WORKER-DEPLOYMENT-REPAIR-PROPOSAL.md): exact bundle-derived Build IDs, native `PINNED` inheritance, explicit Current-based enrollment, and retained-Run Query obligations. The implemented checkpoint separates replay from Query preservation with a valid Query-only mutation. The retained callback sequencing helper does not enforce production routing or authorize legacy conversion; cross-version correlation, Schedule dispatch, and evaluation composition remain closure work.
- [Intermediate Catch Timer](../capsules/INTERMEDIATE-CATCH-TIMER-SPEC.md), [Intermediate Catch Message](../capsules/INTERMEDIATE-CATCH-MESSAGE-SPEC.md), and [Message payload catch](../capsules/MESSAGE-PAYLOAD-CATCH-MEDIATION-SPEC.md): exact committed-state-derived host mechanisms, forced continuation, duplicate/conflict recovery, replacement, and unchanged semantic-core traces.
- [Temporal test evidence](../TEMPORAL-TEST-EVIDENCE-MAP.md) and the [production-lifecycle experiment](../experiments/TEMPORAL-PRODUCTION-LIFECYCLE-EXPERIMENT.md): fresh-server witnesses, durable-result reconciliation, retained publication selection, and replay. General Signal compatibility, committed history fixtures, a general legacy IR reader, and post-retention publication reconstruction remain absent. [Pre-release evolution policy](../PROJECT-DESIGN.md#pre-release-evolution-policy) owns the current baseline boundary.
