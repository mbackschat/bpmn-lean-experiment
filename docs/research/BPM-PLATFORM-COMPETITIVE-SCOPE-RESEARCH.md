...
There is currently no `platform/` implementation tree. The [implementation map](../IMPLEMENTATION-MAP.md) now records product 2 explicitly as entirely absent, including the package trees, deployment store, projections, read models, UI, HTTP API, guards, narrowed engine entry points, dependency controls, and engine prerequisites E1 and E2. This means the proposal is a reviewed product contract, not evidence of an implemented surface.
