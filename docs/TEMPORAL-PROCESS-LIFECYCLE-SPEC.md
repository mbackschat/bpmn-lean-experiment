...
The exact unregistered Compensation source checkpoint may cross Continue-As-New only while its committed maximal frontier is wholly unscheduled. The next Run preflights and starts the complete frontier before observing an Activity result; no scheduled or unreconciled Compensation Activity crosses a Run boundary. Definition, trigger, handler, effect, descriptor, and frozen arguments determine each stable Activity request and retry identity.
...
- the exact unregistered Compensation checkpoint crosses Continue-As-New only before B/C scheduling, replaces the Worker, retries B with one mutation and the same frozen request, executes B/C concurrently, starts A only after B, returns the exact completed receipt, and replays all six Runs;
