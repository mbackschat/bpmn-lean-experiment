...
The first green flow-node occurrence checkpoint adds a second immutable accumulator and unconditional `bpmn-flow-node-occurrences` Query beside E1. A complete committed semantic step must carry one lifecycle delta for every E1 transition. One cohesive Workflow transaction first derives the E1 successor, samples `Date.now()` exactly once, derives and validates the occurrence successor against the exact Program and E1 batch, and only then records the existing command outcome, with no `await` between those boundaries. Rejected, bounded, ambiguous, duplicate-recovery, or incompletely publishable commands sample no time and advance neither accumulator. The public occurrence identity is `(processInstanceId, startRevision, startIndex)`; private wait, scope, Call Activity, and transition anchors remain only in Workflow state. The Query uses E1's exact complete-batch cursor and limit result, exposes `currentOpen` only at the head, and is pure. At a positive cursor, representation-free validation checks only the visible suffix, while Workflow validation uses its retained private open-anchor map and prior committed time. Product 1 client/API exposure and live replay evidence remain outside this checkpoint.
