...
Run-local command admission derives queue pressure from the current reserved entries and bytes. Exact queue fills request rollover only while that pressure remains; draining releases the queue restriction even when an armed managed Timer defers Continue-As-New. The accepted-Update count remains cumulative for the Run and continues to request rollover after its queue entries drain, including when one Update fills both bounds. Event History, retained recovery, and publication bounds remain independently enforced. The [SDK-VM witness](../packages/temporal-adapter/testkit/test/managed-wait-host-wakeup.test.ts) drains the exact production entry and byte limits beside a Timer, then requires the current task's completion to commit and withdraw that original Timer.
...
Outgoing Run-count and continuation-byte failures retain the first exact capacity fact before throwing. Recovery therefore resolves an existing command or identity conflict first, then a terminal receipt if present, then the retained capacity failure for unseen work. A failed nonterminal Run has no semantic terminal receipt: its committed state remains nonterminal, and retained Queries depend on Temporal history retention. State export, rehosting, and reconstruction after retention expiry are not implemented.
...
Maintained Workers use native Worker Deployment Versioning: the SHA-256 of the exact loaded Workflow bundle is the Build ID in the stable `bpmn-lean` deployment, and both Process and correlation-ingress Workflows use `PINNED`. Ordinary startup registers an inactive candidate without changing Current or a ramp. New Process, definition, Message, ingress, and Schedule creation require a native readiness snapshot with the same versioned Current and no ramp on both Workflow and Activity queues, plus exact dual-queue registration. That snapshot does not reserve a version at dispatch or prove poller liveness. Existing ingress recovery may validate its exact identity/configuration Query without creating another Workflow or requiring Current.

Explicit fresh-Namespace initialization first requires successful native Namespace creation, then registers the exact Worker and selects the initial Current through a native conflict token. An existing Namespace is refused before Worker creation or routing mutation; ordinary connection never initializes it. Existing unversioned chains, pending Schedules, histories, addresses, and required Workers remain intact. Their conversion is excluded by the [native deployment repair contract](TEMPORAL-WORKER-DEPLOYMENT-REPAIR-SPEC.md#existing-unversioned-environments).

Pinned chains and their Continue-As-New successors retain their executable version while another version becomes Current. The original Worker and its runtime/configuration remain required for every supported running chain and retained completed, failed, or continued Run Query. Drained status alone does not authorize retirement. An absent original Worker yields an infrastructure availability failure; candidate replay cannot authorize a substitute Query answer. The retained callback-based deployment helper is diagnostic sequencing evidence only. The [deployment specification](TEMPORAL-WORKER-DEPLOYMENT-REPAIR-SPEC.md#invariant-and-evidence-matrix) owns the established cross-version correlation and future Schedule dispatch evidence.
...
Update and Message resolution capture the complete stimulus before the first asynchronous boundary. Submission, retries, recovery Queries, and retained results share one absolute native connection deadline; expiry terminates outstanding RPC work and preserves the SDK infrastructure error. A local polling-budget exhaustion is a `TimeoutError`. Neither error decides whether the semantic command committed: a caller retries the original command identity to recover the retained result. The [Worker-absence witness](../packages/temporal-adapter/testkit/test/workflow-command-deadline.test.ts) binds native timeout, replacement, one retained commit, terminal equality, and replay.
...
## Compensation durability checkpoint

The exact registered Compensation source profile may cross Continue-As-New only while its committed maximal frontier is wholly unscheduled. The next Run preflights and starts the complete frontier before observing an Activity result; no scheduled or unreconciled Compensation Activity crosses a Run boundary. Definition, trigger, handler, effect, descriptor, and frozen arguments determine each stable Activity request and retry identity.

Concurrent Compensation Activities use `WAIT_CANCELLATION_COMPLETED` and an exact one-second heartbeat timeout. An Activity implementation that may remain live must heartbeat so a sibling's typed BPMN Error can deliver cancellation. The Workflow may publish the committed failed semantic state before cancellation acknowledgement, but it must not return the failed receipt until every removed sibling Activity has acknowledged cancellation. This heartbeat is a delivery mechanism and creates no BPMN fact.

The retained durability witness replaces the Worker before B/C scheduling, permits B response-loss retry without another mutation, preserves B's frozen input, permits C before B, starts A only after B, and returns the exact completed receipt. Its failure twin commits C's typed semantic failure before B cancellation drain, never starts A, returns the exact failed receipt only after B acknowledgement, orders those Activity and Workflow events in retained history, and replays every Run. These durability witnesses predate the [public registration](capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md#public-registration-implementation), which reuses the same exact topology and scheduler; they establish no wider Compensation or Transaction capability.
...
- the exact registered Compensation profile crosses Continue-As-New only before B/C scheduling, replaces the Worker, retries B with one mutation and the same frozen request, executes B/C concurrently, starts A only after B, returns the exact completed receipt, and replays all six Runs;
- the Compensation failure witness publishes C's typed failed state while B is live, delivers cancellation through the exact one-second heartbeat policy, withholds terminal completion until B acknowledgement, never starts A, returns the exact failed receipt, and replays all five Runs;
- retained Activity and Workflow history orders C completion before B cancel request before B cancellation before Workflow completion, and the independent exact-checkpoint occurrence oracle rejects a missing initial handler, premature A, or incomplete fail-fast terminal set;
