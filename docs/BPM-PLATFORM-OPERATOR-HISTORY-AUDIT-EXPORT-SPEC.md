...
The implemented [shared-persistence addendum](BPM-PLATFORM-SHARED-PERSISTENCE-AND-PROJECTION-PROPOSAL.md) retains the two independently ordered streams in PostgreSQL with transactional source heads, sink heads, and bounded leased delivery. Shared reads capture each source head and fail unavailable until the corresponding sink covers it; requests perform no delivery. Canonical bytes, authorization, filters, ceilings, and the no-merged-chronology rule remain unchanged. The SQLite reconciliation passages below describe local mode only.
