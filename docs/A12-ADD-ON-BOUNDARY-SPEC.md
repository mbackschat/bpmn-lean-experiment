...
| Overlay identity changes provenance and transport, not semantics | Lean and TypeScript mapped-success checks plus [`mapped-success-temporal-tests.ts`](../packages/temporal-adapter/testkit/test/mapped-success-temporal-tests.ts) retain non-null identity, discriminate transport keys, execute a live Activity, compare the canonical result, and replay | Finite checked non-interference, not a universal proof |
