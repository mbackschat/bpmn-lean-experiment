...
| Lean kernel-decided negatives | each added conjunct rejects its own malformed state beside an admitted control, with siblings asserted intact, and each monotonicity relation is refused by a successor that satisfies every conjunct, the counter rewind breaking `RuntimeStateMonotone` and the clock rewind breaking `RuntimeStateTimeMonotone` under a holding firing hypothesis, with each relation admitting the successor the other refuses | that the conjunct list is complete, or that any unwitnessed branch of a conjunct is exercised |
