...
**Implemented and independently closure-reviewed for one executable program-indexed predicate over `RuntimeState`, with quantified Lean preservation declared a deliberately open lane by owner decision.** The predicate, its initialization theorems, its per-conjunct negatives, the independently written TypeScript validator on the fail-closed command path, and the Workflow-continuation witness are implemented. Preservation across the registered transition arms is not, and every Lean law consuming a conjunct over a live wait assumes it rather than discharging it.
...
Reason, measured: preservation of the uniqueness conjunct alone reaches ninety-one wait-collection assignment sites across fifteen semantic modules, and proving a newly armed wait's key fresh additionally needs `RSI-MONO-04`, which no relation states.
...
The gate decides a narrower set than the predicate. Program-agreement classes are excluded from it, because deciding them needs the complete definition and the program handed to a command is only the hosting one.
...
| Lean kernel-decided negatives | each added conjunct rejects its own malformed state beside an admitted control, with siblings asserted intact, and a successor that rewinds a counter satisfies every conjunct while breaking `RuntimeStateMonotone`, which is what makes that relation irreducible to a conjunct | that the conjunct list is complete, that any unwitnessed branch of a conjunct is exercised, or that `RuntimeStateTimeMonotone` is falsifiable, for which no negative exists |
| Core validator and preservation lane | five schedules reach only admitted states at every microstep, and each stimulus commits | schedules those five do not reach; measured, the unexercised conjunct branches are the effect wait and incident ones and the declaration instance scoping, since the schedules do reach Message waits, Timer waits, event races, selected-branch sets, and called records |
...
| Temporal continuation witness | a recovered state whose logical time passed a live deadline is refused | the structural classes: the guard does call the gate, but the pre-existing checkpoint conditions refuse every structural class measurable on that fixture first, so the call is redundant there and no witness attributes anything to it |
...
Established: the admitted state set is stated executably, holds at every admitted start, is refused for six named malformed classes in both languages, and is preserved across five executed schedules in the core.
...
Common-mode risk: one author wrote the predicate, its negatives, and this document. The counter is that each conjunct has a negative failing without it, with siblings asserted intact so a refusal is attributable to the named conjunct.
