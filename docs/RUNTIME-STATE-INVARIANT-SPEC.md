...
**The existing predicate and `RSI-BOUND-01` amendment retain their independent closure reviews; Activity-family `RSI-ISSUE-01` retains its checkpoint approval.** The external-review corrections strengthen lifecycle emptiness and task-body owner agreement and compose the existing complete Compensation execution predicate into Lean's aggregate. Their focused Lean and TypeScript gates are green; [PLAN](PLAN.md#external-review-correction-checklist) owns correction-stage completion. General quantified preservation remains a deliberately open lane. The two-state Activity rule, guarded writer classification, issuer freshness, and predecessor-identity preservation/removal proofs do not establish preservation across every registered transition.

The approved [initialization assurance repair](capsules/RUNTIME-INITIALIZATION-ASSURANCE-REPAIR-PROPOSAL.md) now proves the complete invariant at actual committed start admission under the existing output-position hypothesis. Its quantified laws and retained kernel witnesses pass their focused gates. The former unrestricted raw-constructor guarantee is withdrawn: root snapshot reservation occurs after that intermediate constructor. [The proposal's receipt](capsules/RUNTIME-INITIALIZATION-ASSURANCE-REPAIR-PROPOSAL.md#independent-cold-review-receipt) owns its review stage.
...
Exact implemented and absent status, including which conjunct branches a witness reaches, is owned by [`implementation-status-owner:ENGINE-SEMANTIC-INVARIANT`](ENGINE-SEMANTIC-INVARIANT-IMPLEMENTATION-MAP.md#runtime-state-well-formedness).
...
| Rule | Proposition |
|---|---|
| `RSI-LIFE-01` | `notStarted` admits no occurrence, token, wait, hidden record, incident, Activity-local scope, Compensation retention/snapshot/trigger/handler work, or pending initiation |
...
`RSI-OBL-01` retains the exact empty-state theorem and its `runtimePositionValid` hypothesis. `RSI-OBL-02` now covers actual committed ordinary, Message, and Timer start admission from `initialState`, through both snapshot-aware and legacy admission. Every result still assumes `runtimePositionValid` of the admitted output; the initialization proof does not establish that premise, and `programWellFormed` rides inside it. Successful admission supplies the started-state frame and any root snapshot reservation, without assuming the full invariant or its missing snapshot conjunct. The raw builder and its raw Message/Timer corollaries separately require absence of a snapshot declaration. [The initialization owner](../BpmnSemantics/SemanticProcess/RuntimeStateWellFormedInitialization.lean) and [retained witnesses](../BpmnSemantics/RuntimeStateInitializationConformance.lean) distinguish those boundaries, capacity refusal, and child-only declarations. Internal closure after admission remains outside the initialization theorem.
...
The falsifiable executable replacement is runtime-state well-formedness preservation. The TypeScript lane compiles every registered single-instance differential program, replays every committed transition prefix, and checks both admitted-state well-formedness and monotone measures; a rejected command must preserve the exact received state and emit no committed transition. The quantified Lean form remains the open lane above. Neither substitutes for the other, and a citation must say which it means.

## Evidence lanes

| Lane | What passage establishes | What it cannot establish |
|---|---|---|
| Lean predicate and initialization | the account is executable; the complete invariant holds for the empty state and actual committed ordinary/Message/Timer starts under the explicit output-position hypothesis | unconditional position validity, the declaration-bearing raw constructor, internal closure after start, or general preservation |
...
| Core validator and preservation lane | every registered single-instance differential program reaches only admitted states after every replayed committed transition; rejected commands preserve the exact state and emit no committed transition; the identity-bound test independently refuses a live User Task, Timer, and Activity identity after only its matching counter is removed; the independent Activity pair oracle refuses exact reissue, admits body preservation and removal, admits a real later activation-two issue, and receives evaluator-produced pairs from every current TypeScript issuer | unregistered programs and schedules; quantified preservation; declaration-instance mutations no registered schedule reaches; the narrowed identity bound leaves Message, Effect, Event race, Call, and ordinary Scope outside the predicate; pair coverage for another identity family |
...
Established by the current executable evidence: the admitted state set is stated executably, holds at the initialization boundary under the position hypothesis above, refuses each implemented malformed class in both languages, and is preserved after every committed transition in every registered single-instance differential schedule. The three implemented identity-bound families have separate negatives, and the User Task branch is observed again at Workflow continuation. Subject to the guarded current-writer census, the Activity family also has a pairwise issuing rule whose conjunction with its bound and monotone high-water mark establishes Activity identity non-reissue.

Nearest unsupported claim: general quantified preservation. Family-specific Lean preservation theorems cover selected transition families, but no theorem composes every registered arm. The core lane constrains the actual registered schedules, including effect-wait and incident states, without establishing unregistered schedules or arbitrary states.
