...
The implemented commutation checkpoint applies when the complete enabled frontier contains at least two distinct ordinary `awaitUserTask`, `awaitMessage`, `awaitPayloadMessage`, `awaitCorrelatedPayloadMessage`, `awaitTimer`, `awaitEffect`, or composed `awaitDataInputOutputUserTask` arming operations; the Program declares no Compensation Event Sub-Process snapshots; every member is independently enabled from the same admitted exact pre-state; each operation has one unique complete-family declarer; every affected untagged open occurrence anchor is absent; and every unordered pair of footprints derived from the prepared pre-state transitions has no directional write/read, write/write, or publication collision. The correlated arm additionally reads its exact Process Property and publishes its exact candidate atom, so a Process-variable writer or equal candidate publication conflicts while an equal-key candidate with a distinct subscription occurrence may commute. The composed arm reads its Process binding and issues separate task/Activity counters, an Activity record, and an Activity-owned local scope. Footprints are not derived from successors, and the classifier does not execute operation orders to decide independence. The internal batch mechanism retains every complete preparation, refuses insufficient fuel before mutation, and checks complete re-derived preparation equality before applying each retained local patch. The public command boundary applies whole-command rollback on failure, including external admission and any earlier successful internal prefix.
...
The [finite prepared arming theorem](INTERNAL-COMMUTATION-PROPOSAL.md#finite-prepared-arming-checkpoint) derives each independent prefix's runtime-state well-formedness, open-occurrence projectability, canonical storage, and complete remaining preparations from the admitted predecessor. It proves that every permutation of one finite independent prepared batch produces the same raw `RuntimeState` and defined canonical accepted transition/lifecycle publication at any starting index, and binds those results to the production Lean batch fold. TypeScript executes the complete prepared frontier in canonical operation-ID order and sorts complete publication keys before assigning transition indices. The multi-operation classifier excludes zero and singleton frontiers, which receive normal evaluator handling. An unavailable footprint, an unsupported operation, or any pairwise collision remains `ambiguousInternalChoice`. Explicit schedules, production region-family integration, and preparation laws for remaining operation families stay open. This theorem covers one finite independent arming batch; it is neither a necessary commutation criterion nor general BPMN scheduler meaning.

TypeScript additionally preserves its existing ordinary-only arming batches in snapshot-declaring Programs. The snapshot-aware attempt has no reservation, promotion, or root-completion effect for those six ordinary arming families, and their retained patches match that attempt. Every closure iteration and batch re-evaluation still gives snapshot refusal precedence. Composed data preparation and any batch containing it refuse a snapshot declaration. This ordinary-only TypeScript compatibility path is outside the snapshot-free Lean prepared-batch theorem; it supplies no new cross-language equivalence claim.

Before token-producing or region families join production batches, the [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) requires a place-wide `tokenOwners(place)` census read for `onlyTokenOwner`, with writes for every insertion/removal that can change the census, including regional removal, alongside owner-specific multiplicity atoms. The [census checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#place-wide-token-owner-census-checkpoint) supplies those atoms in existing TypeScript preparation helpers and Lean ordinary/composed arming footprints without enabling another production family. Remaining families' relation-soundness, premise-preservation, complete-preparation frame, and patch/publication commutation laws must quantify over every admitted Program and every runtime-well-formed, canonical, open-set-projectable intermediate state. Execution must apply the classified prepared patch or check complete re-derived preparation equality, including the publication template; operation-ID equality alone is insufficient. These remain future integration obligations, and an out-of-domain producer does not refute the ordinary-arm pair theorem. The [Lean region definitions](../BpmnSemantics/SemanticProcess/InternalCommutationRegion.lean) are included by [the maintained library root](../BpmnSemantics.lean), without establishing production region-family integration or a general region-commutation proof.
...
The internal-commutation proof owners establish complete ordinary/composed-data preparation, relation soundness, independent preparation preservation, runtime/open-projection preservation, raw patch commutation, and accepted paired publication. [The finite prepared arming checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#finite-prepared-arming-checkpoint) lifts those family laws through arbitrary independent prefixes and list permutations without assuming intermediate invariants or equal successors. Its erasure bridge reaches the production batch evaluator; [whole-command rollback](#internal-scheduling) handles insufficient fuel and failed complete-preparation revalidation. The [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) retains the wider census, regional and token-owner obligations, and missing family laws. Explicit choice semantics, schedules, and production region-family integration remain open; census guards establish coverage rather than those laws.
