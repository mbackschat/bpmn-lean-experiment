...
- Compensation source/profile provenance, trigger execution or restoration, RuntimeState trigger/handler collections, handler activation/execution/reverse-order realization/failure/cancellation, Multi-Instance Sub-Process snapshots, dedicated Temporal refinement, CIB behavior, public projection, corpus registration, Product 2 use, implicit compensation, Transactions, and Cancel Events beyond the hidden retention, independently approved parent-snapshot checkpoint, and strict Program-only trigger/handler declaration;
