...
TypeScript additionally preserves its existing ordinary-only arming batches in snapshot-declaring Programs. The snapshot-aware attempt has no reservation, promotion, or root-completion effect for those six ordinary arming families, and their retained patches match that attempt. Every closure iteration and batch re-evaluation still gives snapshot refusal precedence. Composed-data, local-control, scope-creation, ordinary End, exact Merge, and regional preparation refuse a snapshot declaration. The raw unique-offer Merge fallback remains available. The ordinary-only TypeScript compatibility path is outside the snapshot-free Lean prepared-batch theorem; it supplies no new cross-language equivalence claim.
