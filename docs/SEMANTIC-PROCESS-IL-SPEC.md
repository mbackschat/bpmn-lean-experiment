...
Canonical runtime storage orders User Task, Message, Timer, and effect waits by complete occurrence identity, their activation-counter families by element identity, and Activity-variable scopes by complete effect-occurrence owner. Every producer inserts directly into that order. Canonical observation does not repair a noncanonical stored state, and a transition-order argument may not depend on sorting only at projection time.
