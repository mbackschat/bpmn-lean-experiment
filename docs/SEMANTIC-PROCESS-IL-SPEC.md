...
Production closure supports complete independent frontiers containing at least two distinct operations from ordinary `awaitUserTask`, `awaitMessage`, `awaitPayloadMessage`, `awaitCorrelatedPayloadMessage`, `awaitTimer`, and `awaitEffect` arming; composed `awaitDataInputOutputUserTask` arming; and local `duplicate`, `synchronize`, `choose`, `selectMany`, and `synchronizeSelected` control. The shared Lean/TypeScript mixed account requires a Program without Compensation Event Sub-Process snapshots, complete preparations from the same admitted exact predecessor, unique operation alternatives, and every unordered pair's sufficient independence. Arming pairs retain their state and publication collision checks; pairs involving local control require state-footprint separation, with distinct instantaneous anchors supplied by unique alternatives and canonical publication indices. Each family retains its complete preparation premises, including arming declarer and open-anchor freshness checks and local token, branch, ownership, and provenance checks, as owned by the [mixed account](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite).
...
TypeScript additionally preserves its existing ordinary-only arming batches in snapshot-declaring Programs. The snapshot-aware attempt has no reservation, promotion, or root-completion effect for those six ordinary arming families, and their retained patches match that attempt. Every closure iteration and batch re-evaluation still gives snapshot refusal precedence. Composed-data and local-control preparation refuse a snapshot declaration. This ordinary-only TypeScript compatibility path is outside the snapshot-free Lean prepared-batch theorem; it supplies no new cross-language equivalence claim.
...
The internal-commutation proof owners establish complete ordinary/composed-data and five-family local-control preparation, relation soundness, independent preparation preservation, runtime/open-projection preservation, raw patch commutation, and actual accepted publication. The [finite mixed execution proof](../BpmnSemantics/SemanticProcess/InternalTransitionBatch.lean) and [canonical accepted-publication theorem](../BpmnSemantics/SemanticProcess/InternalTransitionCanonicalBatchPublication.lean) lift those laws through arbitrary independent prefixes and list permutations without assuming intermediate invariants or equal successors. Production Lean calls the proved `acceptedPreparedTransitionBatch?` function; the earlier finite arming erasure bridge remains a narrower helper result. [Whole-command rollback](#internal-scheduling) handles insufficient fuel and failed complete-preparation revalidation, with the [production kernel witness](../BpmnSemantics/MixedLocalControlClosureConformance.lean) covering later fuel exhaustion. The [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) retains the missing family and regional laws. Explicit choice semantics, schedules, and production region-family integration remain open; census guards establish coverage rather than those laws.
