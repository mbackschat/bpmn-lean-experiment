...
- the pending User Task cycle profile has exactly one initiation, one three-input Exclusive Merge, one User Task wait, one declaration-ordered conditional choice, one end-reaching operation, one root completion, six control places, one root scope, and no cycle remaining after the User Task continuation cut;
...
- one root-scope resumption-bounded cycle containing one identity-only converging Exclusive Gateway, one User Task, one declaration-ordered two-condition-plus-default divergent Exclusive Gateway, and one None End Event at the pending semantic-checkpoint boundary;
