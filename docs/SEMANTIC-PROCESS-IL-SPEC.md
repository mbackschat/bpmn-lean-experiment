...
- cycles outside the exact root-scope User Task profile, concurrent Exclusive Multi-Merge execution, Standard Loop Characteristics, sequential or parallel Multi-Instance beyond the exact registered User Task/data/completion/Timer slices, condition consumers beyond the admitted Exclusive and Inclusive Gateway profiles, general expressions, Boolean data outside the registered exact User Task completion and Message payload profiles, value kinds beyond the exact registered scalar and String-list surfaces, general variables or scopes, and mappings beyond one literal-string input plus one simple Activity-local-variable output per mapped Service Task;
