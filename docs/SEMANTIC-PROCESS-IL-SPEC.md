...
The `dataInputOutputUserTask` and `awaitDataInputOutputUserTask` arms implement the first semantic checkpoint of the [composed Activity data proposal](capsules/ACTIVITY-DATA-INPUT-OUTPUT-MEDIATION-PROPOSAL.md). Exact source admission and lowering preserve both direct association contracts on one operation; independent TypeScript and Lean transitions keep one Activity occurrence, wait, record, and input-bearing local scope from activation through accepted output completion. The internal-transition footprint deliberately remains unavailable until the following `INTERNAL-COMMUTATION` RC unit, and no scenario/differential, live Temporal, corpus, or Product 2 closure follows from this checkpoint alone.
...
- execution-byte limits are safe integers from seven through 65,536, so the canonical empty pair `[[],[]]` fits before any trigger exists; the [empty-state capacity repair](capsules/COMPENSATION-EMPTY-STATE-CAPACITY-REPAIR-PROPOSAL.md) preserves the separate two-byte retention and snapshot minima;
