...
Production closure supports independent batches from ordinary `awaitUserTask`, `awaitMessage`, `awaitPayloadMessage`, `awaitCorrelatedPayloadMessage`, `awaitTimer`, and `awaitEffect` arming; composed `awaitDataInputOutputUserTask` arming; local `duplicate`, `synchronize`, `choose`, `selectMany`, and `synchronizeSelected` control; ordinary child `enterScope` or Call `invokeProcess` creation; ordinary `reachNoneEnd`; exact `mergeExclusive` owner/input alternatives; and regional `returnProcess`, `completeScope`, `throwError`, or `terminateScope`. The shared Lean/TypeScript prepared account requires a structurally valid Program without Compensation Event Sub-Process snapshots and complete preparations from the same runtime-valid and open-set-projectable predecessor. Normalization selects only operations with a single offered alternative that is independent of every other offer in the complete frontier. Members of a multiple-alternative operation never normalize independently.
