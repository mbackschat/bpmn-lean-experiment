...
The internal-commutation proof owners establish complete ordinary/composed-data, five-family local-control, and child-entry/Call-invocation preparation, relation soundness, independent preparation preservation, runtime/open-projection preservation, raw patch commutation, and actual accepted publication. The [finite mixed execution proof](../BpmnSemantics/SemanticProcess/InternalTransitionBatch.lean) and [canonical accepted-publication theorem](../BpmnSemantics/SemanticProcess/InternalTransitionCanonicalBatchPublication.lean) lift those laws through arbitrary independent prefixes and list permutations without assuming intermediate invariants or equal successors. Production Lean calls the proved `acceptedPreparedTransitionBatch?` function; the earlier finite arming erasure bridge remains a narrower helper result.

[Whole-command rollback](#internal-scheduling) handles insufficient fuel and failed complete-preparation revalidation. The [local-control kernel witness](../BpmnSemantics/MixedLocalControlClosureConformance.lean) covers later fuel exhaustion; the [scope-creation kernel witness](../BpmnSemantics/ScopeCreationMixedBatchConformance.lean) covers reached child/Call mixed frontiers, exact accepted publication, and rollback of the entire Start command. Its structurally valid Programs explicitly fail profile capability admission. These proofs establish neither registered-source reachability nor cross-target equivalence. The [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) retains the missing family and regional laws. Explicit choice semantics, schedules, and production region-family integration remain open; census guards establish coverage rather than those laws.
