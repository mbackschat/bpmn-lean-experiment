...
The `payloadMessageCatchEvent` and `awaitPayloadMessage` arms are implemented through source admission, Lean and TypeScript runtime semantics, strict wire publication, full differential execution, and production Temporal refinement. The bounded account remains subject to the [Message payload catch capsule](capsules/MESSAGE-PAYLOAD-CATCH-MEDIATION-PROPOSAL.md)'s closure review and exclusions.
...
The implemented commutation checkpoint applies when the complete enabled frontier contains at least two distinct ordinary `awaitUserTask`, `awaitMessage`, `awaitPayloadMessage`, `awaitTimer`, or `awaitEffect` arming operations; every member is independently enabled from the same admitted exact pre-state; each operation has one unique complete-family declarer; every affected untagged open occurrence anchor is absent; and every unordered pair of footprints derived from the prepared pre-state transitions has no directional write/read, write/write, or publication collision. Footprints are not derived from successors, and the classifier does not execute operation orders to decide independence. The evaluator refuses the complete batch before mutation when it exceeds remaining fuel and restores the exact pre-batch state without selected steps or publication when any expected member disappears during execution.
