...
The reusable Lean proof remains exact-two: firing either member of a classified pair preserves the other member's enabledness, runtime-state well-formedness, open-occurrence projectability, and canonical storage, and both orders produce the same raw `RuntimeState` and actual transition/lifecycle publication. A finite Lean discriminator additionally checks that all six permutations of three ordinary User Tasks reach one raw state and that the complete-frontier classifier accepts them. TypeScript executes any finite complete frontier accepted by the pairwise classifier in canonical operation-ID order and sorts complete publication keys before assigning transition indices. The multi-operation classifier excludes zero and singleton frontiers, which receive normal evaluator handling. For larger frontiers, an unavailable footprint, an unsupported operation, or any pairwise collision remains `ambiguousInternalChoice`; arbitrary-cardinality confluence and publication theorems, explicit schedules, production region-family integration, and preparation laws for the remaining operation families remain open. This checkpoint is neither a necessary commutation criterion nor general BPMN scheduler meaning.

Before token-producing or region families join production batches, the [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) requires a place-wide `tokenOwners(place)` census read for `onlyTokenOwner`, with writes for every insertion/removal that can change the census, including regional removal, alongside owner-specific multiplicity atoms. Its relation-soundness, premise-preservation, complete-preparation frame, and patch/publication commutation laws quantify over every admitted Program and every runtime-well-formed, canonical, open-set-projectable intermediate state. Execution must apply the classified prepared patch or check complete re-derived preparation equality, including the publication template; operation-ID equality alone is insufficient. These remain future integration obligations, and an out-of-domain producer does not refute the ordinary-arm pair theorem. The [Lean region definitions](../BpmnSemantics/SemanticProcess/InternalCommutationRegion.lean) are included by [the maintained library root](../BpmnSemantics.lean), without establishing production region-family integration or a general region-commutation proof.
...
The internal-commutation Beta checkpoint adds a complete-frontier ordinary-arming classifier and production batch while retaining an exact-two reusable proof boundary. Lean derives one private prepared patch and complete semantic footprint for each eligible arm from the admitted pre-state. `InternalArmingOrder.lean` owns canonical insertion and replacement laws. Responsibility-split flow-node validity owners retain private family validators and expose exact insertion-preservation facts, while focused lifecycle-order, Process-identity, and projection-shape proof leaves establish the reusable cross-surface contracts those validators imply. The projection, state-frame, runtime-preservation, and open-projection modules compose those facts into four-constructor preservation of `runtimeStateWellFormed`, open-occurrence projectability, and accepted lifecycle publication. The public commutation facade proves exact-pair opposite-arm enabledness and equal raw state with canonical affected collections; focused transition-record and control-position-delta leaves establish the remaining public facts; and the downstream publication module proves equality of the complete actual accepted pair publication plus canonical numbering. The complete-frontier classifier checks every unordered pair; insufficient fuel or a disappearing batch member is subject to [whole-command rollback](#internal-scheduling), and finite three-task Lean and TypeScript discriminators cover all six independent permutations plus a later-pair collision after an independent canonical prefix. Arbitrary-cardinality confluence and publication theorems, explicit choice semantics, schedules, production region-family integration, and operation families outside the selected arming set remain open. The [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) owns the full census, intermediate-premise preservation, and complete prepared-value obligations; the census guards establish coverage, not those semantic laws.
