...
The `correlatedPayloadMessageCatchEvent` and `awaitCorrelatedPayloadMessage` arms are implemented through the mandatory semantic checkpoint only: exact source/checked/IL preservation, strict shared and Lean codecs, a pure finite matcher, the per-instance transition, and the internal-commutation classification are green. Independent checkpoint review approved correction target `1ce28ed5`; no registered profile, global Temporal discovery or ingress, public engine client, scenario, corpus, or closure claim exists yet, and the durable ingress is the next lane. The complete account belongs to the [Message key-correlation proposal](capsules/MESSAGE-KEY-CORRELATION-PROPOSAL.md).
...
- one checkpoint-only context-backed correlated-payload Intermediate Catch Message Event with one key/property binding and exact scalar selectors; it has source/checked/IL, Lean/core matcher and transition, and commutation evidence but no registered profile or host ingress;
