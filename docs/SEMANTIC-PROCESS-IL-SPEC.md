...
The private [repeatable-subscription amendment](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#definition-and-runtime-representation) extends the contracts below with `awaitMessageMonitoredUserTask`, `enterMonitoredScope`, optional non-interrupting Timer recurrence, and a disjoint checked duration/cycle representation. Its selected admission and lifetime rules own these additions. The [proof-domain amendment](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#scheduling-proof-domain-amendment) limits the new integrated prepared constructors by an immutable full admission certificate while retaining legacy domains and finite-batch guarantees; implementation and review status belong to the [`implementation-status-owner:ENGINE-RUNTIME-PROOF`](ENGINE-RUNTIME-AND-PROOF-IMPLEMENTATION-MAP.md#current-boundary).
...
The optional `compensationActivityRetention` declaration is an independently approved representation-checkpoint fact for one flat root scope. Its canonically ordered nonempty targets bind exact ordinary, sequential Multi-Instance, or parallel Multi-Instance User Task operation identities to distinct boundary-Event and Compensation-Activity identities, while positive record and 2–65,536-byte limits bound the hidden register. One checkpoint-only source compiler emits the exact two-target declaration selected by the source/lowering capsule; no registered product profile emits it, and the declaration alone does not select a throw, handler, snapshot, order, cancellation, or public contract. The complete runtime boundary belongs to the [boundary-handler retention proposal](capsules/COMPENSATION-BOUNDARY-HANDLER-RETENTION-PROPOSAL.md).

The optional `compensationEventSubProcessSnapshots` declaration is the independently approved semantic-checkpoint representation for exact parent/handler pairs. Targets are a canonical nonempty collection ordered by `parentScopeId` then `handlerScopeId`, with unique parent and handler identities and positive record and 2–65,536 canonical-byte limits. A declaring Program has exactly one parentless root. Each selected parent is that root or one direct embedded child of it; each selected handler is a distinct immediate child definition scope of its parent, owns no operation or control place, and has no entry or completion operation. A selected non-root parent has exactly one current `enterScope` or `enterBoundedScope` operation. Declaration-derived dormant handlers are the only exception to the ordinary one-entry/one-completion graph rule. The checkpoint-only source compiler emits one exact parent/handler target after resolving `triggeredByEvent`, Compensation Start, containment, and restored-data provenance into the checked graph; no registered product profile emits it. The complete runtime boundary belongs to the [Compensation Event Sub-Process snapshot proposal](capsules/COMPENSATION-EVENT-SUB-PROCESS-SNAPSHOT-PROPOSAL.md).

The optional `compensationExecution` declaration is the owner-approved pre-profile checkpoint for one root-owned `triggerCompensation` operation. It admits a canonical collection of zero or more boundary-Activity or Event-Sub-Process subjects whose coverage equals the existing retention and snapshot declarations, closed `singleEffect` handler bodies using only the registered Compensation descriptor and exact empty/restored-binding input arms, canonical acyclic Sequence Flow dependencies, and positive safe trigger/handler limits with the execution-byte range defined by [well-formedness](#well-formedness). Subject, handler, effect, trigger, and ordinary-operation identities remain disjoint. The zero-subject case preserves the standard global-throw outcome when no eligible work exists. The independent TypeScript and Lean evaluators implement its bounded private transition account, and the checkpoint-only source profile constructs the exact three-subject declaration and trigger operation through independently checked lowerings. Product 1 privately admits only that exact source checkpoint into the durability-evidenced scheduler; every registered product profile still rejects the operation, and no public Compensation capability exists. The complete intended runtime semantics and exclusions belong to the [Compensation trigger and handler proposal](capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md), while source correspondence belongs to the [Compensation source and lowering proposal](capsules/COMPENSATION-SOURCE-LOWERING-PROPOSAL.md).
...
- one checkpoint-only optional hidden boundary Compensation retention declaration and root-owned RuntimeState register with exact ordinary and outer Multi-Instance completion facts, append-only chronology, exact count/byte refusal, and one exact source-checkpoint producer but no registered product profile, live host, or public surface;
- one independently approved checkpoint-only optional Compensation Event Sub-Process parent/handler declaration and hidden provisional/promoted context collection, with one-parentless-root admission, exact root/direct-child reservation, success-only pre-completion capture and promotion, Error/Timer/cancellation and unselected-root purge, selected-root terminal ownership, atomic count/canonical-byte refusal, whole-stimulus rollback, declaration-free delegation, and one exact source-checkpoint producer but no registered product profile, live host, public, corpus, CIB, or Product 2 surface;
- one checkpoint-only exact Compensation source graph with two boundary subjects, one dormant Event Sub-Process subject, one direct restored Process binding, one direct dependency, a global synchronous throw, and fixed limits; strict TypeScript and Lean checked admission independently lower it to the existing optional execution/retention/snapshot declarations and root-owned trigger while Product 1 refuses it before Workflow start;
- one pre-profile optional Compensation execution declaration and root-owned trigger, with exact retention/snapshot coverage, closed single-effect bodies, canonical acyclic dependencies, bounded limits, strict Lean/TypeScript/schema admission, hidden trigger/effect-wait runtime state, and independent TypeScript/proved Lean transition accounts; no registered profile, live host, or public surface exists;
...
- Compensation source/profile provenance beyond the exact checkpoint, targeted or asynchronous throws, wider subjects, handler graphs, restored mappings, or dependencies, concurrent same-root triggers and sibling-trigger terminal lifecycle, Multi-Instance Sub-Process snapshots, dedicated live Temporal refinement, CIB behavior, enabled-interaction projection, corpus registration, Product 2 use, implicit compensation, Transactions, and Cancel Events beyond the hidden retention, independently approved parent-snapshot checkpoint, and bounded private trigger/handler semantic account;
...
- the checkpoint-only Compensation compiler resolves attachment, containment, restored binding, and dependency from parser-object identity, emits the exact optional checked declaration and global throw node, and both TypeScript and Lean admission/lowering reject cross-artifact identity, scope, body, dependency, order, and limit mutations while older checked bytes continue to omit the field;
- strict Lean and TypeScript Program readers require exact `compensationExecution`/`triggerCompensation` agreement, reject malformed coverage, identity, dependency, body, or limit shapes, preserve old Program bytes by omission, execute the bounded private account in both evaluators, and keep the trigger unavailable to every current profile and host;
