...
- one checkpoint-only optional hidden boundary Compensation retention declaration and root-owned RuntimeState register with exact ordinary and outer Multi-Instance completion facts, append-only chronology, exact count/byte refusal, and no source/profile producer, trigger, handler, host, or public surface;
- one independently approved checkpoint-only optional Compensation Event Sub-Process parent/handler declaration and hidden provisional/promoted context collection, with one-parentless-root admission, exact root/direct-child reservation, success-only pre-completion capture and promotion, Error/Timer/cancellation and unselected-root purge, selected-root terminal ownership, atomic count/canonical-byte refusal, whole-stimulus rollback, declaration-free delegation, and no source/profile producer or handler, host, public, corpus, CIB, or Product 2 surface;
- one pre-profile optional Compensation execution declaration and root-owned trigger, with exact retention/snapshot coverage, closed single-effect bodies, canonical acyclic dependencies, bounded limits, strict Lean/TypeScript/schema admission, hidden trigger/effect-wait runtime state, and independent TypeScript/proved Lean transition accounts; no profile, source, live host, or public surface exists;
