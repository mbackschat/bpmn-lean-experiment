...
Production closure supports complete independent frontiers containing at least two distinct operations from ordinary `awaitUserTask`, `awaitMessage`, `awaitPayloadMessage`, `awaitCorrelatedPayloadMessage`, `awaitTimer`, and `awaitEffect` arming; composed `awaitDataInputOutputUserTask` arming; local `duplicate`, `synchronize`, `choose`, `selectMany`, and `synchronizeSelected` control; ordinary child `enterScope` or Call `invokeProcess` creation; ordinary `reachNoneEnd`; and regional `returnProcess`, `completeScope`, `throwError`, or `terminateScope`. The shared Lean/TypeScript mixed account requires a structurally valid Program without Compensation Event Sub-Process snapshots, complete preparations from the same runtime-valid and open-set-projectable predecessor, unique operation alternatives, and every unordered pair's sufficient independence.
...
The approved [closure atomicity repair](capsules/INTERNAL-CLOSURE-ATOMICITY-REPAIR-SPEC.md#command-contract) requires external admission before closure. After committed admission, either `internalStepBoundExceeded` or `ambiguousInternalChoice` returns `rolledBack` with the exact input RuntimeState before admission, preserves both actual flags, and erases selected steps, batches, transition records, and lifecycle deltas. Result-only and traced APIs agree; the harness classifies either flag before observation. Existing successful stable and admission-rejected results remain exact. The future scheduled evaluator inherits that rollback: fuel precedes the next batch or choice directive and is never reclassified as a schedule defect or unused directive. This rule adds no admission outcome, Program or RuntimeState field, wire/profile change, or batchable family.

The [finite mixed-batch theorem](../BpmnSemantics/SemanticProcess/InternalTransitionCanonicalBatchPublication.lean) derives each independent prefix's runtime-state well-formedness, open-occurrence projectability, canonical storage, and complete remaining preparations from the admitted predecessor. Every permutation produces the same raw `RuntimeState` and defined canonical accepted transition/lifecycle publication at any starting index. Production Lean classifies through `prepareInternalTransitionBatch?` and calls the theorem's actual checked execution/publication function, `acceptedPreparedTransitionBatch?`. TypeScript executes the complete prepared frontier in canonical operation-ID order; both targets sort complete templates before assigning indices and check actual accepted publication at each execution prefix. Zero and singleton frontiers receive normal evaluator handling. An unavailable preparation, unsupported member, or pairwise collision remains `ambiguousInternalChoice`. Explicit schedules and preparation laws for remaining families stay open. The theorem covers one finite independent mixed batch, with no general fuel-adequacy, registered XML reachability, cross-language correspondence, or general BPMN scheduler claim.

TypeScript additionally preserves its existing ordinary-only arming batches in snapshot-declaring Programs. The snapshot-aware attempt has no reservation, promotion, or root-completion effect for those six ordinary arming families, and their retained patches match that attempt. Every closure iteration and batch re-evaluation still gives snapshot refusal precedence. Composed-data, local-control, scope-creation, ordinary End, and regional preparation refuse a snapshot declaration. This ordinary-only TypeScript compatibility path is outside the snapshot-free Lean prepared-batch theorem; it supplies no new cross-language equivalence claim.
...
The internal-commutation proof owners establish complete ordinary/composed-data, five-family local-control, child-entry/Call-invocation, and four-family regional preparation, relation soundness, independent preparation preservation, runtime/open-projection preservation, raw patch commutation, and actual accepted publication. The [finite mixed execution proof](../BpmnSemantics/SemanticProcess/InternalTransitionBatch.lean) and [canonical accepted-publication theorem](../BpmnSemantics/SemanticProcess/InternalTransitionCanonicalBatchPublication.lean) lift those laws through arbitrary independent prefixes and list permutations without assuming intermediate invariants or equal successors. Production Lean calls the proved `acceptedPreparedTransitionBatch?` function; the earlier finite arming erasure bridge remains a narrower helper result.

[Whole-command rollback](#internal-scheduling) handles insufficient fuel and failed complete-preparation revalidation. The [local-control kernel witness](../BpmnSemantics/MixedLocalControlClosureConformance.lean) covers later fuel exhaustion; the [scope-creation kernel witness](../BpmnSemantics/ScopeCreationMixedBatchConformance.lean) covers reached child/Call mixed frontiers, exact accepted publication, and rollback of the entire Start command. Its structurally valid Programs explicitly fail profile capability admission. The [regional matrix](INTERNAL-COMMUTATION-PROPOSAL.md#finite-regional-batch-outcome) adds complete-frontier, permutation, actual-publication, conflict, and fuel discriminators. These witnesses establish neither registered-source reachability nor cross-target equivalence. The [final preparation account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints) retains the remaining family laws. Explicit choice semantics and schedules remain open; census guards establish coverage rather than those laws.
