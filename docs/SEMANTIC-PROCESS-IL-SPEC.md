...
The first proved commutation profile is sufficient and deliberately exact. It applies only when the complete enabled frontier contains exactly two distinct ordinary `awaitUserTask`, `awaitMessage`, `awaitTimer`, or `awaitEffect` arming operations; both are independently enabled from the same admitted exact pre-state; each operation has one unique complete-family declarer; every affected untagged open occurrence anchor is absent; and footprints derived from the prepared pre-state transitions have no directional write/read, write/write, or publication collision. Footprints are not derived from successors, and the classifier does not execute both orders to decide independence.

For an admitted pair, firing either operation preserves the other operation's enabledness, runtime-state well-formedness, open-occurrence projectability, and canonical storage. Both orders produce the same raw `RuntimeState` and the same actual transition/lifecycle publication after sorting complete paired publication keys before assigning transition indices. Canonical operation-ID order selects the production evaluation order only after this proof makes that choice observationally irrelevant. An unavailable footprint, an unsupported operation, a frontier of any other size, or any collision remains `ambiguousInternalChoice`; this profile is neither a necessary commutation criterion nor general BPMN scheduler meaning.
...
The internal-commutation Beta checkpoint adds a proved exact-two arming lane without changing the open general closure obligation. Lean derives one private prepared patch and complete semantic footprint for each eligible arm from the admitted pre-state. `InternalArmingOrder.lean` owns canonical insertion and replacement laws. Responsibility-split flow-node validity owners retain private family validators and expose exact insertion-preservation facts, while focused lifecycle-order, Process-identity, and projection-shape proof leaves establish the reusable cross-surface contracts those validators imply. The projection, state-frame, runtime-preservation, and open-projection modules compose those facts into four-constructor preservation of `runtimeStateWellFormed`, open-occurrence projectability, and accepted lifecycle publication. The public commutation facade proves opposite-arm enabledness and equal raw state with canonical affected collections; focused transition-record and control-position-delta leaves establish the remaining public facts; and the downstream publication module proves equality of the complete actual accepted transition/lifecycle pair plus canonical numbering. Kernel-decided positive and refusal fixtures cover mixed wait families, called-owner Message identity, composite declarer collisions, incident-anchor collisions, same-effect writes, abstract write/read interference, unsupported constructors, larger frontiers, and publication equality. Arbitrary-cardinality confluence, explicit choice semantics, and operation families outside the selected arming set remain open.
