...
The `correlatedPayloadMessageCatchEvent` and `awaitCorrelatedPayloadMessage` arms are implemented through the mandatory semantic checkpoint only: exact source/checked/IL preservation, strict shared and Lean codecs, a pure finite matcher, the per-instance transition, and the internal-commutation classification are green. No registered profile, global Temporal discovery or ingress, public engine client, scenario, corpus, or closure claim follows until the independent checkpoint review approves that target. The complete account belongs to the [Message key-correlation proposal](capsules/MESSAGE-KEY-CORRELATION-PROPOSAL.md).
