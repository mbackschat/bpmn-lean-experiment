...
The first product acceptance model is the existing private executable `None Start Event → User Task → None End Event` shape plus the owner-approved completion-data extension in the [User Task completion-data proposal](capsules/USER-TASK-COMPLETION-DATA-PROPOSAL.md). The runtime must use the same source compiler, Semantic Process program, semantic core, production Process Workflow, Update command boundary, and Temporal replay-safe code as the maintained evidence path. A separate model-specific Workflow or generated TypeScript file is not an MVP shortcut.
...
The dummy actor does not mutate semantic state directly. Completion data is a separately reviewed CIB-profile extension of the existing semantic command, owned by the [User Task completion-data proposal](capsules/USER-TASK-COMPLETION-DATA-PROPOSAL.md). That capsule must complete its pinned CIB Seven `2.2.0` probe and atomic wire/Lean/core/Temporal replacement before the MVP may claim that simulated form values become Process variables.
