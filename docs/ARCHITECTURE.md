...
`platform/foundation/bpmn-presentation/` is Product 2 presentation infrastructure. It privately owns the exact `bpmn-auto-layout` parser graph, returns only closed DI bytes and provenance, and has no semantic authority. This is the sole presentation-only exception to the semantic `bpmn-moddle` boundary: raw moddle values and generated non-DI XML never escape it. Definitions owns digest-bound persistence, resolution, and the public presentation route; the browser owns rendering and task highlighting. No Product 1 semantic, Lean, engine API, or Temporal gate imports this foundation package or invokes its Playwright acceptance.
