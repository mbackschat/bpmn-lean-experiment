...
The production bundle keeps the Work landing workspace eager and loads Definitions, Operations, About, and their workspace-only HTTP clients at the existing navigation boundary. The bpmn-js viewer runtime and global viewer styles load only when a Diagram surface mounts; its marker and port contract remains lightweight and eager. A production-artifact guard enforces a sub-500 kB entry chunk and rejects the viewer runtime in that entry without weakening the existing browser journeys, focus behavior, watermark, or license evidence.
