...
The product-boundary guard must discover current and future source files rather than compare a hand-maintained package list. It rejects platform source outside an approved owner; internal platform imports that violate the dependency graph, including a web-to-service import; product-1 imports into `platform/`; platform deep imports into engine internals; public engine imports outside the engine gateway; platform Event History imports; production imports of showcase evidence; and production JUEL placement under `runners/`. Each prohibited class carries a planted violation in the guard's own tests.
