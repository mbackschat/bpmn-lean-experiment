...
Flow-node occurrence publication is an additive sibling of committed execution publication rather than a field extension to it. The semantic core and Lean derive exact starts and completed or cancelled terminals at the same evaluator boundaries as E1 while retaining private pairing anchors outside the public wire. The Workflow assigns public occurrence identity from E1 revision plus start index, samples its deterministic wall clock once per complete committed command batch, advances both immutable accumulators before recording the command result, and serves a separate pure Query with the same complete-batch cursors. Product 2 may later consume only the representation-free transport surface; it cannot reconstruct occurrence lifecycle or duration from E1 state differences, Temporal Event History, or platform ingestion time.
...
Product 2 browser quality is a separate path-filtered CI lane. It never runs from `verify.sh`, a Lean/semantic-core/BPMN-source/CIB/differential gate, or an ordinary Product 1 change. The lane uses fixed public-API fixtures and a production Vite bundle for responsive geometry, state, focus, and reduced-motion evidence at 1280 and 1600 pixels. The exact same functional command runs locally before push and in GitHub Actions. M5 E1 adds path-scoped History and Diagram behavior to this lane. One wide Process Diagram screenshot remains an optional manually invoked human-review aid in the digest-pinned Linux Playwright environment, not a blocking regression gate. The M3 and M4 showcases separately retain real Temporal browser acceptance, and `test:release:m4` composes the M4 real-host witness with the deterministic functional UI-quality lane without entering Product 1 verification.
