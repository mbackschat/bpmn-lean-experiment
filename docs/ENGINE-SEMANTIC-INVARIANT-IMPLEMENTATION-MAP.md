...
The [initialization repair](capsules/RUNTIME-INITIALIZATION-ASSURANCE-REPAIR-PROPOSAL.md) preserves `RSI-OBL-01` and corrects `RSI-OBL-02` to actual committed ordinary/Message/Timer starts from `initialState`, with the result's `runtimePositionValid` hypothesis explicit. The raw guarantee now requires no snapshot declaration. Quantified laws and raw/admitted, child-only, capacity, and profile-refusal witnesses pass their focused kernel gates; the proposal receipt owns review status. TypeScript separately checks every registered single-instance differential schedule's committed prefixes. Workflow continuation witnesses cover recovered time and a User Task above its absent counter; the monotonicity relations remain separate.
