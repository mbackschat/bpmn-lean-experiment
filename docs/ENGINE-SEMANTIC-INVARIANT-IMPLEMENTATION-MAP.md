...
Canonical collection order now includes Activity counters in both implementations. Reversed or duplicate keys fail the aggregate; the four previously prepending Lean issuers use ordered insertion while preserving prior high-water marks and the separate task counter. The [kernel writer witness](../BpmnSemantics/ActivityActivationOrderConformance.lean) and independent TypeScript mutations cover this correction. Its proof consumer and approved checkpoint review are owned by [the composed arming footprint contract](INTERNAL-COMMUTATION-PROPOSAL.md#final-closure-evidence-and-adversarial-oracles).

The [token-storage checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-token-storage-checkpoint) adds canonical Lean token insertion and both aggregate order checks. Quantified multiplicity, permutation, order, and mixed-update laws preserve the existing proof-consumer contracts. Complete Lean/core gates and immutable resource calibration pass; repository-wide verification and the [independent checkpoint receipt](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) govern further integration. Producer/regional batching stays open.
...
The Lean aggregate consumes lifecycle agreement, occurrence uniqueness, scope/token binding, hosting-root count, event-race and incident associations, and the complete Compensation execution validator. Lifecycle emptiness includes Activity-local data and every Compensation collection. Singular and parallel task bodies require both exact wait cardinality and owner agreement; filtering by owner cannot conceal a duplicate task identity. The [retained invariant witnesses](../BpmnSemantics/RuntimeStateWellFormedInvariantConformance.lean) and [Activity witnesses](../BpmnSemantics/RuntimeStateActivityOccurrenceConformance.lean) have focused kernel checks; independently structured TypeScript mutations cover lifecycle and task ownership. The [collection-to-conjunct inventory](RUNTIME-STATE-INVARIANT-SPEC.md#collection-to-conjunct-inventory) identifies the narrower TypeScript aggregate and the open running-state Activity-local attachment predicate. It also records required representation reviews before overlapping Inclusive iterations or parent-owned Event Sub-Process start subscriptions are admitted.

The [initialization repair](capsules/RUNTIME-INITIALIZATION-ASSURANCE-REPAIR-SPEC.md) preserves `RSI-OBL-01` and corrects `RSI-OBL-02` to actual committed ordinary/Message/Timer starts from `initialState`, with the result's `runtimePositionValid` hypothesis explicit. The raw guarantee now requires no snapshot declaration. Quantified laws and raw/admitted, child-only, capacity, and profile-refusal witnesses pass their focused kernel gates; the specification receipt records closure approval. TypeScript separately checks every registered single-instance differential schedule's committed prefixes. Workflow continuation witnesses cover recovered time and a User Task above its absent counter; the monotonicity relations remain separate.
...
- the well-formedness predicate carries body liveness, family-tagged attached-wait liveness and unambiguity, identity uniqueness, owner agreement, canonical order, and lifecycle emptiness in both languages; singular and parallel task-body mutations preserve live owners while isolating owner disagreement, and cross-owner duplicate identities still fail cardinality
- [regional lifetime witnesses](../BpmnSemantics/ScopeCancellationConformance.lean) retain exact Activity-local disposal for both region dispositions and parent-owned Message withdrawal while preserving another activation and a same-coordinate Timer. These are constructed ownership graphs, not new profile admission or public scenarios
...
- one defect label still carries two rules: `ActivityOccurrenceBodyAbsent` reports body liveness and owner disagreement. The focused singular/parallel cross-owner tests now pin that label while excluding dangling-owner and duplicate-identity failures; they do not introduce separate public defect classes
- the older stranding negatives use different state shapes. The regional correction additionally checks Lean's parent-owned Message attached to a child-scope body, exact tagged-family separation, and both region dispositions; it does not turn a constructed graph into a registered execution schedule
