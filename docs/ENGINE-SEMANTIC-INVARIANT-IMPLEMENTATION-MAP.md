...
The independently approved [token-storage checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-token-storage-checkpoint) adds canonical Lean token insertion and both aggregate order checks. Quantified multiplicity, permutation, order, and mixed-update laws preserve the existing proof-consumer contracts. Immutable resource calibration and the clean complete repository gate pass; the [review receipt](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) records their exact targets and limits. Producer/regional batching stays open.
