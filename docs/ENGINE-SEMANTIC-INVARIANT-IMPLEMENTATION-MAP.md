...
The [token-storage checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-token-storage-checkpoint) adds canonical Lean token insertion and both aggregate order checks. Quantified multiplicity, permutation, order, and mixed-update laws preserve the existing proof-consumer contracts. Full validation and independent review remain in progress; producer/regional batching stays open.
