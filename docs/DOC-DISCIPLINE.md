...
The later 5,000-word runtime/proof decision does not reverse that split or restore per-family status. The accurate Activity boundary Message graduation moved the remaining cross-cutting owner from 3,993 to 4,011 words, and active `EVENT-SUBSCRIPTIONS` is routed there for reusable subscription identity, cancellation, deterministic-race, and proof mechanisms. A 1,000-word increment is a 25% margin for that named programme, not an open-ended exemption; the next pressure at 5,000 requires a new stated routing axis rather than another raise.
