...
Every routed detail map carries a reviewed word budget enforced by [the documentation control plane](../scripts/document-control-plane.ts). The default remains 4,000 words. On 2026-08-31 the repository owner explicitly approved a 5,000-word budget for [`implementation-status-owner:ENGINE-RUNTIME-PROOF`](ENGINE-RUNTIME-AND-PROOF-IMPLEMENTATION-MAP.md) and later raised [`implementation-status-owner:TEMPORAL-HOSTING`](TEMPORAL-HOSTING-IMPLEMENTATION-MAP.md) from 5,000 to 6,000 words for the active high-risk Event Subscriptions hosting programme. On 2026-09-02 the owner authorized proportionate increases where demonstrated content required them: the contracts/source map received a 4,500-word budget for the active Compensation source checkpoint, and the runtime/proof map later received a 5,200-word budget when a cold-review correction brought its accurate cross-cutting account to 5,042 words. On 2026-09-03 the owner authorized a further proportionate increase of the Temporal-hosting map to 7,000 words after its exact Event Subscriptions continuation and accepted-before-quarantine account exhausted the 6,000-word allowance, then authorized the runtime/proof map to rise to 5,400 words when the registered Message-correlation population and refinement account brought its accurate total to 5,215 words. The same authorization permits a thorough but realistic 4,300-word assurance/adoption map after the exact CI hard-ceiling record brought its accurate total to 4,019 words. These scoped decisions leave the default unchanged. The budget exists because a map that outgrows review stops being read, and because the cheap way to fit under one is to compress or delete the absence content a review required, which changes what the status owner asserts. An agent may not raise a value or add another exception unilaterally; an explicit owner-authorized raise must update this owner and its executable guard together and remain proportionate to demonstrated reviewable content.

The Temporal map reached 6,024 words while recording the exact cross-Run continuation relation and a correctness repair for publications accepted before an earlier target quarantines their address. The 7,000-word ceiling is scoped completion margin for the remaining public route, refinement, profile, and closure evidence already owned by the approved Message key-correlation account; it does not license unrelated hosting history or change a source-code limit. Pressure at 7,000 requires a stated routing split rather than compression or another casual raise.

The assurance/adoption map reached 4,019 words only after recording the exact failed CI run, completed-library time, redundant native-closure mechanism, hidden conformance reachability gap, and corrective gates. Its 4,300-word ceiling is 281 words of completion margin for this reviewed repair, not permission to absorb unrelated history; pressure there requires a stated split.

The runtime-and-proof map was the exception twice, raised to 5,000 and then to 6,500, and both raises taught the same thing: its size scaled with semantic families closed rather than with the engine boundary it describes, so each raise bought roughly one capsule of headroom. The second reached its limit during the sequential Multi-Instance checkpoint corrections, where landing a correction required tightening wording rather than adding content, which is the failure the budget exists to prevent rather than an application of it.

The standing commitment was to split instead of raising a third time, and that split happened. Per-family status moved to [`implementation-status-owner:ENGINE-SEMANTIC-FAMILY`](ENGINE-SEMANTIC-FAMILY-IMPLEMENTATION-MAP.md) under its own area ID, which returned both maps below the default and retired the old exception. The axis remains load-bearing: a newly closed family grows the family map, not the shared runtime/proof owner.

The later runtime/proof decisions do not reverse that split or restore per-family status. The accurate Activity boundary Message graduation moved the remaining cross-cutting owner from 3,993 to 4,011 words, and active `EVENT-SUBSCRIPTIONS` is routed there for reusable subscription identity, cancellation, deterministic-race, and proof mechanisms. The Compensation start-data cold-review correction then required explicit command-admission, inherited-capacity, and maximum-frontier evidence and brought the map to 5,042 words. The registered Message-correlation population and refinement account brought the same cross-cutting owner to 5,215 words; the 5,400-word ceiling retains 185 words of measured margin for closure of that active programme without restoring per-family detail. Pressure at 5,400 requires a new stated routing axis rather than compression or another casual raise.

The contracts/source map had reached 3,984 words before its Compensation provenance prerequisite. Recording that reader and the checked-source capabilities it deliberately does not establish moved the accurate owner to 4,134 words. Its 4,500-word budget is a 12.5% increment for the named source checkpoint, not permission to compress exclusions or absorb another programme; pressure at 4,500 requires a stated split between source ingestion and cross-language contract status.

That commitment was tested and kept for the semantic-family maps. The family map reached 4,000 words while a closure-review correction was landing, and the owner chose the split over a raise to 5,000 on 2026-08-29 after the arithmetic showed 5,000 buying roughly three families. Cross-cutting mechanism status moved to [`implementation-status-owner:ENGINE-SEMANTIC-INVARIANT`](ENGINE-SEMANTIC-INVARIANT-IMPLEMENTATION-MAP.md). The axis is what a section is about: a section belongs to the invariant map when its subject adds no BPMN capability, operation kind, profile, scenario, or public observation field, and to the family map when it closes one BPMN element family. Both semantic maps retain the 4,000-word default.
