...
Every routed detail map carries one reviewed word budget of 4,000 words, enforced by [the documentation control plane](../scripts/document-control-plane.ts). The budget exists because a map that outgrows review stops being read, and because the cheap way to fit under one is to compress or delete the absence content a review required, which changes what the status owner asserts. There is no exception, and an agent may not add one.
