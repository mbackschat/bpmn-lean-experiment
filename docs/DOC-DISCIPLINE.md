...
The later runtime/proof decisions do not reverse that split or restore per-family status. The accurate Activity boundary Message graduation moved the remaining cross-cutting owner from 3,993 to 4,011 words, and `EVENT-SUBSCRIPTIONS` routed reusable subscription identity, cancellation, deterministic-race, and proof mechanisms there. The Compensation start-data cold-review correction then required explicit command-admission, inherited-capacity, and maximum-frontier evidence and brought the map to 5,042 words. The registered and closure-reviewed Message-correlation population and refinement account brought the same cross-cutting owner to 5,215 words; the 5,400-word ceiling retains 185 words of measured margin without restoring per-family detail. Pressure at 5,400 requires a new stated routing axis rather than compression or another casual raise.
