...
Durable ingress is unchanged; replacement is an internal consequence of a completion the host already delivers. **Durable Workflow state is not unchanged**, and that is this preflight's one amendment rather than a detail: the publication-anchor lane adds a handler list to each retained open occurrence in the flow-node occurrence accumulator, which lives in Workflow state and crosses Continue-As-New. Three facts bound it. It reaches no published artifact, so no schema, scenario, retained evidence, or canonical byte moves. It is written only where a body occurrence opens, from the record in the committed post-state, because that is the one point where the record and the new entry coexist. And the continuation decoder recomputes it from the restored `RuntimeState` and refuses a payload that disagrees, so a forged pairing cannot survive a restore; that recomputation is the reason the field can be trusted downstream by a relation that holds no state.

The shape change is unversioned, which [the pre-release policy](../CLAUDE.md#pre-release-evolution) permits because no immutable history baseline is approved and local Temporal gates start clean state and discard it. It stops being permitted at the first durable baseline, and what that baseline owes is named here so it is not rediscovered: a retained accumulator written before this field existed decodes as absent, not as empty, and the decoder's exact-key check refuses it. That is the correct pre-release behaviour and the wrong post-baseline behaviour, so the baseline must either carry a migration for the retained set or declare that no pre-field history is supported. The mechanism this preflight must settle is the durable deadline across the boundary: the managed scheduler pairs a deadline to its Activity through the record, so `AOO-TURNOVER-03` is what stops task turnover from detaching a still-live Timer or rearming it. The refinement risk is that a host cancels and recreates the native Timer at an iteration boundary, which would preserve the semantic identity while changing the durable one, and no public fact would show it.
