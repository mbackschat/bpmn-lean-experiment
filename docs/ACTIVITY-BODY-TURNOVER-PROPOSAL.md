...
The first threads the Activity occurrence records of the semantic **pre-state** into the checker, changing the public signature of a relation that is deliberately state-free. Its cost is that the checker stops being reconstructible from the publication alone, which is the property that made it usable as an independent oracle in the first place; the demotion this section already accepts becomes a larger one.
