...
`prepare` pulls only the exact recorded digests, verifies every project image's source labels, removes only the bundle's demo volumes, starts Compose with `--no-build`, and seeds five exact scenarios through public Product 1 and Product 2 interfaces. Before offering the artifact, the acceptance job logs out of GHCR, executes this same command, completes the full audience journey twice in headless Chromium with an exact reset between journeys, requires the five long-running services to remain running with zero restarts after each journey, and resets the presenter state again. That proves anonymous pull, exact published-image startup, the real browser path, repeatable preparation, and process stability rather than merely proving a separately built local image. Every acceptance attempt retains sanitized endpoint failures, Playwright traces and reports, timestamped Compose logs, service state, and restart counts for 30 days.
...
Screenshot refresh is not part of ordinary commit CI and performs no pixel comparison. An explicit manual screenshot-refresh input to the [evaluation distribution workflow](../.github/workflows/evaluation-distribution.yml) regenerates and uploads the catalog as a review artifact; image publication and version tags do not regenerate it. Direct execution against an already-running origin is an advanced package-level path documented in the [screenshot project README](../showcase/platform-browser-walkthrough/README.md).
