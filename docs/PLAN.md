...
**Next action: implement the first green lifecycle, wire, and Query checkpoint from the owner-approved [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md), commit it, and pause for the required context-cold semantic-checkpoint review before Product 2 work begins.** The remaining operator-history and audit-export increments stay ordered after this capsule.

Item 1 is **implementation in progress after owner approval**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and M5 E1 is closed through the graduated [committed execution publication specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md). The first governed pause is the semantic checkpoint after the green lifecycle, wire, and Query boundary. Event History, state differencing, Product 2 ingestion time, and semantic logical time remain forbidden substitutes for an exact public wall-clock duration fact.
