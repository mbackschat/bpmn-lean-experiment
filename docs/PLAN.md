...
Next action: review the [RC completion scope amendment](INTERNAL-COMMUTATION-PROPOSAL.md#rc-completion-scope-amendment), then classify the remaining obligations against current and selected future RC capabilities before resuming proof implementation. The owner closed the [architecture review and experiments](experiments/INTERNAL-COMMUTATION-PROOF-ARCHITECTURE-EXPERIMENT.md); neither replacement is adopted. Preserve the unfinished [bounded Sub-Process checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#bounded-sub-process-arming-outcome). Its regional, publication, and finite-integration obligations remain unresolved. The regional trial remains pending; the current RC boundary remains binding until amendment approval.
