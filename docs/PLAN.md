...
**One generic standards profile now admits the complete selected notation set.** `bpmn-2.0.2-user-task-preserved-notation-draft` retains Diagram Interchange, pools, lanes, artifacts, and documentation without executing them. Every other standards profile rejects that set through an exact key allowlist. The two product-neutral mapped Service Task profiles may consume only a content-bound data-only overlay that maps alternate exact source bindings to an existing profile-owned descriptor and names exact inert expanded-name loci; it adds no reader or semantic operation. The preserve-enabled profile is registered with its own answer-free scenario, pipeline case, and live example, and a rejected file names each element the classification refused. Five structural requirement rows record what it admits without letting retention read as executable support. The M1 engine admission slice is closed, and the complete M1 showcase remains in progress because its platform increments are absent.

**The A12 product-boundary semantic checkpoint is approved.** Second warm audit `8b92040` closed every code and evidence finding, and the owner accepted docs-only correction `398719b` as resolution of its remaining stale next-action sentence under the two-audit bound. The product guard derives typed string values, typed BPMN Error messages, and the script catalog import closure from immutable baseline `02330ad`; the frozen manifest derives the original A12-aware validator, projector, and effect-projection closure from fixed baseline roots; and the shared artifact verifier derives the exact route output ID from the checked Sequence Flow rather than from the program under test. Built-in profiles, scenarios, source readers, Lean, the semantic core, Temporal, CIB evidence, product examples, and shared schemas remain product-neutral.
...
| The `CreateDocument` and boundary-Error slices, their legacy `2.0.0` profiles, and their exact source readers | The approved checkpoint moves production decisions to neutral profiles and the data-only overlay contract while preserving the complete baseline A12-specific dependency closure under the optional adoption root. The [A12 boundary proposal](A12-ADD-ON-BOUNDARY-PROPOSAL.md) is owner-approved; second warm audit `8b92040` closed every code and evidence finding, and the owner accepted docs-only correction `398719b` as final checkpoint resolution. Do not repeat the shape for further A12 models. |
...
1. **Blocked: close the corrected A12 product boundary without discarding the implemented slices.** The schema-aware scalar/catalog inventory, fixed-root frozen dependency closure, canonical checked-to-IL BPMN Error route output binding, shared schema, payload-free preservation oracle, closure evidence matrix, product-3 resume handoff, and commit-bounded cost record close every pre-review obligation against checkpoint `aa83b2f`. Owner resolution of docs-only correction `398719b` approves the checkpoint after second warm audit `8b92040`. Two aggregate-wrapper attempts were incorrectly constrained by the 60-second per-test ceiling: the first reached the semantic-core gate, and the owner-authorized replacement reached the final infrastructure suite with zero failed assertions before the outer timer cancelled the remaining tests. Closure review, specification graduation, and proposal archival remain blocked on one correctly bounded complete green gate.
...
**Next action: obtain owner authorization for one correctly scoped complete gate that does not apply the 60-second per-test ceiling to the aggregate `verify.sh` wrapper.** The authorized replacement was consumed and must not be repeated. If the correctly scoped gate is authorized and green, commit the exact tested A12 closure target and submit it to the exact checkpoint reviewer under guarded warm continuity. After closure approval, graduate the boundary specification and archive the proposal. The M1 engine admission slice is closed, while the complete M1 showcase remains in progress until ordered-work item 2 supplies and verifies its platform increments.

**Blockers: the aggregate wrapper was given the timeout intended for each JavaScript or TypeScript test command.** The first attempt exited `124` as the semantic-core gate started. The owner-authorized replacement also exited `124`, but reached the final infrastructure suite after 281 green BPMN-source tests and 14 green differential tests; the timer then cancelled 35 of 44 infrastructure tests after 9 passed, with zero failed assertions. These results do not support the earlier CPU-contention attribution. No semantic, code, evidence, or external-input blocker is known, but closure requires a complete green target-bound gate and the authorized replacement budget is exhausted.

**Last verified command:** `/opt/homebrew/bin/gtimeout 60s env CI=true ./scripts/verify.sh`, replacement exit `124` at the outer 60-second deadline on the prepared A12 closure worktree. The exact combined output is retained outside the repository with SHA-256 `ea3d96fa3a20941a14ec5514138f235b095e447e470ed7fba51e58f4d3e393e4`. The run reached the final infrastructure suite after 281 green BPMN-source tests and 14 green differential tests. The timer cancelled 35 of the 44 infrastructure files after 9 passed, with zero failed assertions; Node reported cancellation because the parent timer terminated the runner. This is not a semantic failure and is not a complete gate. The latest complete gate remains exit 0 at `fb4b7dc`, with 35 agreements, 35 seeded mutations detected, 18 retained-CIB comparisons, and 37 live histories; its timing is contended and is not a baseline.
