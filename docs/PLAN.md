...
1. **Next: specify M4 Product 2 incident operations.** Inventory the existing confirmed-instance Operate projection, exact private locator retention, engine-published generation-1 incident and Retry/Cancel interactions, authorization boundary, durable action lifecycle, and platform audit. Produce the smallest Product 2 proposal for incident list/detail plus exact retry and cancellation actions, obtain its required independent review and owner approval, and do not derive semantic facts from Temporal Event History, state differences, or platform storage.
