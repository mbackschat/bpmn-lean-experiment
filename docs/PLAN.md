...
The `~/Projects/oss` root has been inspected and all eleven project-created external research checkouts are now explicitly navigable from [SOURCES.md](SOURCES.md). The owner excluded `claudian`, `defender`, and `monkey2`. No remaining checkout is the actual CIB Seven application to migrate: `cibseven/cibseven` is the pinned engine oracle, while the other registered trees are standards, SDK, corpus, or semantic-workbench references.

The active implementation boundary is [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md).

## Last verified baseline

The complete directly executed TypeScript harness gate passed on 2026-07-27. `./scripts/verify.sh` included the strict no-emit harness check against workspace source, twenty contract tests, the Lean gate plus thirty-seven semantic-core tests, seventeen BPMN-source tests, eighteen packaged-CIB tests, nine differential unit tests, thirty-three infrastructure guards, and thirty-three focused Temporal tests in 14.152 seconds. The eight-case prepared pipeline executed through the direct `.ts` orchestrators in 8.938 seconds warm, inside the unchanged 15-second budget, with sixteen isolated Temporal executions and nine replayed histories.
...
10. **Owner target identity pending, non-blocking — migration-target inventory:** the supplied `~/Projects/oss` root has been inspected, its project-created research checkouts are registered, and none is the actual target application after the owner exclusions. Once the target solution is named or cloned, record its read-only checkout in [SOURCES.md](SOURCES.md) and produce a defined-denominator ledger of BPMN and extension usage, binding styles, exact delegate APIs, variables, expressions, errors, listeners/forms/retries/incidents, external tasks, and engine API consumers. Use it to re-prioritize later capsules.
...
The first incomplete ordered item is the migration-target inventory. The supplied `~/Projects/oss` root contains only registered research references after the owner exclusions, so the actual target CIB Seven application must still be named or cloned. Once available, record its read-only checkout in [SOURCES.md](SOURCES.md), run the defined-denominator inventory, and let its results select the variable/expression and delegate-compatibility work. Until then, D1 may be reassessed as an owner-facing proposal but remains unapproved for implementation. Keep C2 frozen under the non-extension and reopening rules in [the experiment record](experiments/CHECKED-SOURCE-RELATION-EXPERIMENT.md); do not extend it with new capsules or silently treat the unresolved theorem as permanently waived.
