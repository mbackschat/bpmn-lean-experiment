...
Next action: correct the [review advisory](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) by deduplicating selected-join readiness dependency atoms while retaining runtime-record multiplicity, then remove the classifier's trace-proof import dependency as a separate refactor before production integration. The [local-control proof checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite), kernel discriminators, [immutable bounded calibration](CAPSULE-COST-LEDGER.md#finite-mixed-local-control-calibration), and clean complete gate are independently approved. The [Inclusive pairing repair](capsules/INCLUSIVE-GATEWAY-SPEC.md#lean-assurance-lane), token-patch validity, and selected-record algebra retain checkpoint approval.

Pending production batch source and the separate Compensation headroom edit are preserved in Git stash `f35bdd6b0424b2e3d5d06f101eb822bd285ab29e` for the clean proof checkpoint; restore its exact paths after review. Earlier stash `76a2b017a630d23203ded482227382003f296f0b` remains retained. Keep token-unit multiplicity distinct from dependency-set membership. Regions, scheduled choice, and final closure remain open. Preserve the [3 GiB ceiling](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction).
