...
Next action: implement the [subscription semantic checkpoint](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#review-questions-and-implementation-obligations): bounded source/Program admission through independent Lean/core lifetime execution and publication, starting with the retained-body discriminator. Its [receipt](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#independent-cold-review-receipt) owns approval. Resolve `AOO-RETAINED-BODY-01` before subscribed child/Terminate admission; prove only source-reachable frontiers within each group's effort bound. Helpers stay in the complete outcome. Production hosting and registration follow checkpoint review; SDK preflight proves no capability. Snapshot arming and pipeline correspondence are accepted; subscription/Transaction closure remains open. Preserve the bounded checkpoint and regional trial.
