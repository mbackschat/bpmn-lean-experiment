...
- no canonical parallel CIB evidence or cross-lane parallel evidence is implemented.

The active implementation boundary is [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md).

## Last verified baseline

`env CI=true /opt/homebrew/bin/timeout 60 ./scripts/verify.sh` passed on 2026-07-26 after the focused parallel Temporal refinement witnesses were implemented. The full command completed in 24.20 seconds, passed eleven focused contract tests, twenty-seven semantic-core tests, twelve BPMN-source/lowering tests, five CIB tests, nine differential unit tests, nineteen infrastructure guards, and four Temporal integration tests; it checked seven synchronized documentation fragments and the fourteen-class/nine-property bounded CMOF manifest. The mandatory focused Temporal gate completed in 2.58 seconds and replayed the three sequential histories plus two ordered parallel histories and one concurrent-submission parallel history. The current three-case sequential four-target pipeline completed in 5.58 seconds with three replayed histories, exact checked/program provenance, exact Lean lowering equality, and rejection of the seeded operation-origin mutation. The separately gated representation experiment previously passed with `lake build checkSemanticRepresentationSpike && lake exe checkSemanticRepresentationSpike`.
...
The semantic decisions, bounded Semantic Process IL proposal, current wire contracts, contract mutations, R5 semantic-policy ownership correction, canonical checked graph, deterministic TypeScript lowerer, Lean definition lane, independent TypeScript parallel evaluator, mandatory Temporal hosting/refinement preflight, and focused parallel Temporal evidence are recorded. Lean independently checks exact lowering before evaluating the received program, and Lean plus TypeScript establish the bounded parallel account. Ordered, duplicate, and concurrent Query/Update histories establish the bounded conformance-host mapping and replay, while the current Workflow remains a finite scenario host rather than the production lifecycle.

Resume at ordered work item 8. Read [the parallel capsule](capsules/PARALLEL-FORK-JOIN-PROPOSAL.md), [the CIB–BPMN relationship register](CIB-BPMN-RELATION-REGISTER.md), [the test specification](TESTING-SPEC.md), the CIB runner guide and adapter source, and the current scenario/evidence contracts. Add failing CIB projection tests for two active tasks, semantic identity sorting independent of query order, and per-element wait multiplicity before adding answer-free A-then-B and B-then-A scenarios and retaining exact content-bound evidence.
