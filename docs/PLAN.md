...
**The exact next implementation boundary is the remaining single-item recovery handlers.** Complete incident-action recovery with its acknowledged reserved-audit prerequisite, then bind the exact-instance execution, occurrence, Work-snapshot, and incident-snapshot roots. Keep discovery and Audit delivery bounded, re-read each durable key before host work, and lease one key per audit stream rather than per event. Do not yet compose the worker or shared API. Review target `c9cf16e` and correction audit `e03240c` bind this boundary.

Item 1 is **in progress** and unblocked. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a` through `df3d7a2` implement the approved dependencies, runtime, Promise repositories, PostgreSQL artifacts and business repositories, exact Audit sinks, six-owner migration application at epoch 6, generic leases, all eleven candidate families, bounded Audit batches, and nine lease-fenced recovery families: four Definitions lifecycles, incident action, both ordered Audit streams, committed execution, and flow-node occurrence. Cross-backend evidence covers exact Unicode, gap-free allocation, canonical values, suffix-only projections, terminal monotonicity, concurrent retries, Audit completeness, lease reclaim, stale-token refusal, same-lease start dispatch, atomic subscriber delivery, and terminal publication without host redispatch. Ordinary checkpoints remain database-free; PostgreSQL 18 uses named witnesses. The Work and incident snapshot families, both process compositions, freshness-backed reads, and multi-replica closure are incomplete. No capacity claim exists.
