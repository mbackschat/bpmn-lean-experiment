...
The engine and BPM platform remain separate MIT products under [PROJECT-DESIGN.md](PROJECT-DESIGN.md#product-division) and the package boundaries in [ARCHITECTURE.md](ARCHITECTURE.md). M0 through M6 are closed, so the functional MVP is complete on its explicitly single-node Product 2 deployment. [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) owns the exact implemented and absent surfaces; the graduated [specifications](README.md) own closed contracts and evidence. The first post-MVP work is Horizon 1 of the owner-approved [Temporal BPMN execution scalability proposal](TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md), because Product 2's node-local persistence, artifact storage, recovery, and request-time Query fan-out are the larger horizontal-scaling blocker.

The final M5 [operator-history and audit-export specification](BPM-PLATFORM-OPERATOR-HISTORY-AUDIT-EXPORT-SPEC.md) is implemented, closure-reviewed, evidence-closed, and graduated. Its Product 2 contract keeps Work and incident actions separately ordered and independently snapshotted, fails closed, excludes private host facts, and retains exact canonical download bytes at 1280 and 1600 pixels. The curated executable BPMN model corpus contains 24 retained business-purpose-shaped project models that cover all 25 executable BPMN element or semantic variants registered by the production pipeline, plus seven external candidates across six deduplicated CIB Seven, OMG, and Betsy families. Three models are catalog-ready through production-preview Chromium journeys: sequential assignment/form review, parallel content-and-risk review, and structured expense-exception review. Product 2 also exposes the exact package version, restrictions, and separately classified CIB evidence through the About capability table.
...
1. **In progress: remove Product 2's single-node scale boundary.** The PostgreSQL runtime, checksum-bound migration seam, atomic Promise-only repository conversion, exact admitted-source artifact store, Definitions persistence, Operate projections and incident actions, and Work state with its audit-source outbox are Green without adding a database to the ordinary package loop. Complete the last domain-owned persistence slice with Audit stores and exact source-to-sink completeness heads. That slice owns its migration, PostgreSQL adapters, reusable SQLite/PostgreSQL behavioral contracts, focused database-free package gate, and separate real-PostgreSQL gate. Only then add the bounded lease mechanism one recovery family at a time, compose the shared-mode API and recovery worker, replace request-time fleet fan-out with freshness-bounded projection reads, and close with concurrent multi-replica correctness evidence. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.

2. **Queued after item 1: provide a containerized evaluation distribution.** After the shared runtime topology is implemented and stable, review and implement one evaluation-only Docker Compose path containing PostgreSQL, a Temporal development service, the Product 1 BPMN Worker, the Product 2 API serving the built web application, and the Product 2 recovery Worker. The distribution must start with one documented command, use health checks and named volumes, and exclude Lean, CIB Seven, Java, research sources, and test harnesses from runtime images. It is not a production Temporal deployment or a capacity claim.

3. **Queued for the first release candidate: make the browser walkthrough screenshot-backed.** Drive the containerized evaluation path through one dedicated Playwright documentation project, reuse public accessible UI landmarks from the acceptance suites, and write stable named images for the maintained browser walkthrough. Regeneration is an explicit release or material-UI-change operation, not an ordinary commit gate or pixel-comparison suite. Container image publication and screenshot refresh run only for tagged releases or manual dispatch, on Linux rather than GitHub-hosted macOS.
...
- **PostgreSQL dependency set:** use exact direct intent `pg@8.22.0` for the low-level production driver, `@types/pg@8.20.0` for TypeScript declarations, and `node-pg-migrate@9.0.0` for programmatic migration execution. All three are MIT-licensed and owner-approved. A narrow project-owned SHA-256 applied-prefix verifier supplements the runner because immutable checksums are part of this proposal's contract; it must not become an ORM or generic repository framework.
- **Horizon 1 implementation and feedback order:** make the Promise-only repository seam the sole broad atomic conversion. Thereafter land Definitions/artifacts, Operate, Work, and Audit as separate domain-owned vertical slices, each with one reusable adapter contract executed against local SQLite and real PostgreSQL. Keep the ordinary package, platform-foundation, platform-checkpoint, and `verify.sh` loops free of a database service; real PostgreSQL adapter, recovery, and multi-replica evidence belongs to explicitly named local/pre-push commands and a separate path-selected CI workflow. Add one recovery family only after the shared lease counterexamples are Green, and compose shared-mode processes only after their dependencies pass independently.
- **Reader onboarding and evaluation packaging order:** keep the root README free of milestone planning, maintain one text-first full browser tutorial now, and defer costly distribution artifacts until their referent is stable. The containerized test drive follows shared persistence so it presents the intended runtime topology rather than a short-lived SQLite showcase. Automated walkthrough screenshots follow that Compose path at the first release candidate and refresh only after material UI changes or for a tagged release. Ordinary CI retains functional browser assertions and does not regenerate documentation images or use GitHub-hosted macOS.
...
**The exact next implementation boundary is the Audit PostgreSQL vertical slice.** Add ordinal-0005 migration ownership, Work and incident-action sink adapters, strict source-ordinal suffix application, one-statement source-to-sink completeness reads, reusable cross-backend contracts, the database-free package gate, and the separate real-PostgreSQL gate. Keep the two streams independently ordered and make lag, a gap, changed overlap, or corruption fail closed without request-time delivery. Do not begin leases, recovery, migration composition, or shared API composition. Review target `c9cf16e` and correction audit `e03240c` bind the repositories, artifact store, repair families, request-time fan-out, projection freshness, audit completeness, and same-database publication invariant.

Item 1 is **in progress** and blocked on nothing. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a`, `5deb00d`, `58cacb7`, `78c59a4`, `8c1e4eb`, `c115d21`, and `d8c610e` implement the dependency set, bounded runtime, checksum migrations, Promise-only repositories, PostgreSQL artifacts, five Definitions adapters, four Operate repository families, Work persistence, and rollback-safe domain-owned audit-source heads. Cross-backend evidence covers artifact ordering and corruption, rollback-safe gap-free allocation, exact full-Unicode identity including U+0000, canonical catalogs, generated-DI integrity, lifecycle CAS races, suffix-only execution projections, closed-state monotonicity, Work claim/completion serialization, and exact concurrent retries. Ordinary package and platform checkpoints remain database-free; PostgreSQL 18 runs only through named witnesses. Audit sinks, leases, recovery, migration composition, shared API composition, and multi-replica closure remain incomplete. No capacity claim exists.
