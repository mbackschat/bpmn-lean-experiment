...
- no parallel four-target comparison is implemented.

The active implementation boundary is [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md).

## Last verified baseline

`env CI=true /opt/homebrew/bin/timeout 60 ./scripts/verify.sh` passed on 2026-07-26 after the multiple-task CIB projector, normative parallel draft profile, content-bound balanced CIB evidence, raw-to-canonical verifier, and explicit evidence-replacement guard were implemented. The full command completed in 27.86 seconds, passed twelve focused contract tests, twenty-seven semantic-core tests, twelve BPMN-source/lowering tests, nine CIB tests, nine differential unit tests, twenty-one infrastructure guards, and four Temporal integration tests; it checked seven synchronized documentation fragments and the fourteen-class/nine-property bounded CMOF manifest. The mandatory focused Temporal gate completed in 3.27 seconds and replayed the three sequential histories plus two ordered parallel histories and one concurrent-submission parallel history. The current three-case sequential four-target pipeline completed in 6.63 seconds with three replayed histories, exact checked/program provenance, exact Lean lowering equality, and rejection of the seeded operation-origin mutation. The separately gated representation experiment previously passed with `lake build checkSemanticRepresentationSpike && lake exe checkSemanticRepresentationSpike`.
...
9. **Next — complete differential comparison:** add the parallel cases to the four-target pipeline, preserve exact checked/program definition binding, and add provenance-erasure and projection mutations.
...
The semantic decisions, bounded Semantic Process IL proposal, current wire contracts, contract mutations, R5 semantic-policy ownership correction, canonical checked graph, deterministic TypeScript lowerer, Lean definition lane, independent TypeScript parallel evaluator, mandatory Temporal hosting/refinement preflight, focused parallel Temporal evidence, R6 multiple-task CIB projection, normative parallel draft profile, and content-bound balanced CIB evidence are recorded. Lean independently checks exact lowering before evaluating the received program, and Lean plus TypeScript establish the bounded parallel account. Ordered, duplicate, and concurrent Query/Update histories establish the bounded conformance-host mapping and replay, while the current Workflow remains a finite scenario host rather than the production lifecycle.

Resume at ordered work item 9. Read [the parallel capsule](capsules/PARALLEL-FORK-JOIN-PROPOSAL.md), [the test specification](TESTING-SPEC.md), the differential pipeline source and tests, and the five current scenario/evidence contracts. Add failing pipeline expectations for both parallel completion orders, exact intermediate task projections, and the required parallel provenance/projection mutations before extending the prepared four-target batch.
