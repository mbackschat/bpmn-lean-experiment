...
**Next action: implement the first green semantic checkpoint of the owner-approved [parallel User Task metadata composition proposal](capsules/PARALLEL-USER-TASK-METADATA-COMPOSITION-PROPOSAL.md).** Add the exact source and successor profile, strict checked-source and Semantic Process admission, the proved Lean composition owner, independent semantic-core evidence, and the exact passive-pair host-capability witness. Commit that checkpoint and obtain its required context-cold semantic review before adding combined CIB evidence, full differential and Temporal closure, or the Product 2 catalog journey. In the engine lane, classify Script Task execution against BPMN, CIB, deterministic sandboxing, data, security, and Temporal requirements before deciding whether its two-family prevalence justifies a proposal. The remaining operator-history and audit-export increments stay ordered after this corpus decision gate.
