...
1. **Next: Specify Horizon 2 Workflow-chain bounds and Continue-As-New.** Define project-owned Event History, payload, pending-operation, and publication budgets; a safe rollover checkpoint; complete carried semantic state; exact command-result and publication continuity; and forced low-threshold evidence over open User Tasks, Messages, Timers, effects, duplicate and conflicting inputs, Worker replacement, terminal completion, history, and replay. This is the highest current durable-hosting risk because every long-lived Process and every later repeating BPMN mechanism inherits it; Temporal Run identity remains private.
