...
1. **Next: close the M2 Message Start Event proposal review.** Submit the committed correction descendant of proposal target `8bd3fe9` to the same context-cold reviewer, record the completed audit, and stop for explicit owner approval before implementation. The corrected [Message Start Event proposal](capsules/MESSAGE-START-EVENT-PROPOSAL.md) selects one payload-free top-level resolved trigger and direct Workflow-start refinement while bounding closure, start selection, and Temporal retry claims.
