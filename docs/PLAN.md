...
1. **Next: review and approve M4 Product 2 incident operations.** The [draft proposal](BPM-PLATFORM-INCIDENT-OPERATIONS-PROPOSAL.md) selects exact private locator retention, one dedicated current-incident Query, all-or-error Operate aggregation, configured operator authorization, durable content-bound Retry/Cancel actions without platform priority, separate incident audit, and an Operations collection/detail workspace. Commit the immutable proposal target, obtain context-cold review, close any findings, and request owner approval before implementation.
...
**Next action: independently review the M4 Stage 3 Product 2 incident-operations proposal.** The read-only owner and contract inventory is complete and the [draft proposal](BPM-PLATFORM-INCIDENT-OPERATIONS-PROPOSAL.md) reuses only the engine-published incident, Retry, Cancel, Process identity, and opaque private locator. Run proposal guards, commit the immutable review target, obtain context-cold review, close any findings, and request explicit owner approval before implementation.
