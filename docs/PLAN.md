...
The BPMN coverage assessment now exposes thirteen reusable Process Execution mechanism families and their dependency edges without claiming an exhaustive line-by-line denominator or percentage. The [compositional BPMN admission proposal](COMPOSITIONAL-BPMN-ADMISSION-PROPOSAL.md) recommends a bounded grammar that serially composes existing waits and exact balanced two-User-Task parallel regions, rejects arbitrary acyclic admission, and makes standalone program validation plus the reopened checked-source preservation theorem shipping prerequisites. No compositional implementation, new profile, CIB relationship, or next semantic capsule has begun.
...
The complete repository gate passed on 2026-07-27 after the BPMN mechanism-family and compositional-admission assessment. `./scripts/verify.sh` passed twenty-two contract tests, the Lean build plus forty-five semantic-core tests and the strict `DeepReadonly` type locks, twenty-two BPMN-source tests, twenty-seven packaged-CIB tests plus eight release-bound `2.0.0` boundary tests, nine differential unit tests, forty-two infrastructure guards, and forty-four Temporal tests. The ten-case pipeline completed in 6.826 seconds warm, ran twenty isolated Temporal executions, replayed twelve histories, and remained within the unchanged feedback budget. The assessment changed documentation only: no production semantics, semantic profile, evidence, or gate assertion changed.
...
13. **Completed assessment — BPMN coverage denominator and compositional mechanisms:** the requirement ledger now maps thirteen normative mechanism families and dependencies, and the [review-ready proposal](COMPOSITIONAL-BPMN-ADMISSION-PROPOSAL.md) selects structured composition over named whole-model topologies or arbitrary acyclic admission. No implementation or profile change is authorized.
14. **Next — independent review and owner decision on compositional admission:** review the selected grammar, composed-profile requirement, accidental-order witness, standalone `programWellFormed` strengthening, and checked-source preservation-proof prerequisite. If approved, set a separate proof effort bound before implementation begins.
15. **Planned, semantic approval still required — standard mechanism sequence:** Exclusive Gateway and conditional Sequence Flow first, then Message Events/correlation, then nested scopes/Sub-Processes/event propagation. Apply the CIB on-demand gate to each and keep A12 work at prioritization/adoption boundaries.
16. **Deferred — D1 seeded fuzz experiment:** reassess after the compositional-admission decision; do not add a dependency or weaken the structural/semantic result boundary.
17. **Completed — substantial directly executed TypeScript harnesses:** the substantial pipeline, artifact, evidence-replacement, Java-resolution, strict-JSON, and bounded-process harnesses execute directly as TypeScript under Node 24 and pass a strict no-emit gate against workspace source. The explicitly approved direct `@types/node` dependency supplies host typings; its locked type-only `undici-types` dependency is recorded in [SOURCES.md](SOURCES.md).
...
The first incomplete ordered item is work unit 14: obtain an independent review and owner decision on the [compositional BPMN admission proposal](COMPOSITIONAL-BPMN-ADMISSION-PROPOSAL.md). Review the exact structured grammar, composed-profile boundary, accidental-order discriminator, standalone program-validation obligations, and the required reopening of observational lowering preservation. Stop before implementation, proof work, or the next semantic capsule.
