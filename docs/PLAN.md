...
The engine and BPM platform remain separate MIT products under [PROJECT-DESIGN.md](PROJECT-DESIGN.md#product-division) and the package boundaries in [ARCHITECTURE.md](ARCHITECTURE.md). M0 through M6 are closed, so the functional MVP is complete. Horizon 1 of the owner-approved [Temporal BPMN execution scalability proposal](TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md) has reached an implementation- and evidence-complete closure candidate: Product 2 now has one PostgreSQL 18 shared mode with replicated API and recovery-worker composition, while local mode retains its focused single-node contract. [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) owns the exact implemented and absent surfaces; the graduated [specifications](README.md) own closed contracts and evidence. Closure review remains before Horizon 1 is removed from this active plan.
...
1. **In progress: close Product 2 Horizon 1.** The PostgreSQL persistence graph, exact migration application through ordinal 0009 and epoch 9, Audit source-to-sink completeness, generic bounded lease kernel, all eleven candidate and lease-fenced recovery families, append-only freshness reads, recovery-worker and shared-API composition, and concurrent multi-replica evidence are Green without adding a database to the ordinary package loop. Freeze the full-gate closure candidate and complete the required independent review. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.
...
**The next boundary is Horizon 1 closure review.** Freeze the implementation/docs candidate after the complete ordinary and isolated PostgreSQL gates, mint the neutral review packet, obtain the required context-cold verdict, and correct any findings with the same reviewer. Review target `c9cf16e` and correction audit `e03240c` bind the approved proposal. After closure approval, delete this completed item from the plan, create the first release-candidate container artifact, and then produce the text-first walkthrough with automated screenshots.

Horizon 1 implementation and required correctness evidence are complete and unblocked. The migration catalog now applies ordinals 0001 through 0009 at epoch 9. Local mode retains filesystem/SQLite behavior and its database-free package loops. Shared mode composes PostgreSQL-only API replicas plus exactly eleven bounded recovery families, performs no startup population scan or request-time fleet Query fan-out, and serves Work, incidents, committed execution, occurrences, and metrics from bounded-freshness projections. Real PostgreSQL 18 evidence covers exact Unicode and canonical values, gap-free allocation, suffix-only projection, terminal monotonicity, Audit source-to-sink completeness, response-loss recovery, lease reclaim and stale-token refusal, exact current Work mutations, H/H+1 population refusal, zero-observation freshness, two replicas sharing exact definition/catalog/presentation bytes, two workers claiming disjoint work, and API/worker readiness with 5,000 retained registrations. The gate records database version, schema epoch, replica counts, batch and lease settings, and wall time. No capacity claim exists.
