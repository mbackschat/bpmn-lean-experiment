...
Approve or reject the corrected [Service Task effect proposal](capsules/SERVICE-TASK-EFFECT-PROPOSAL.md). It now distinguishes the standard implementation URI as protocol identity from the handler's business-effect identity; treats CIB's pre-activation wait as adapter-decided host evidence; uses one shared effect descriptor and occurrence shape; pins domain-separated transport and completion encodings; specifies per-execution store isolation, under/over-inclusion key discriminators, the eight-scenario/sixteen-execution/nine-replay matrix, typed exhausted-Activity Workflow failure, the refinement state relation, cancellation risk, and exact gate budgets. Production `awaitEffect` work still requires this capsule decision.
...
**Authorized next action:** obtain the owner decision on the corrected Service Task effect proposal. If approved, implement it in red/green order beginning with the shared occurrence/descriptor and `awaitEffect` contract changes. Do not begin production `awaitEffect` before that decision. Begin the read-only target-solution inventory as soon as the owner supplies its checkout; it does not delay the capsule.
...
9. **Owner decision pending — Service Task Activity capsule:** approve or reject the corrected [Service Task effect proposal](capsules/SERVICE-TASK-EFFECT-PROPOSAL.md), then implement only its approved success-only semantic account and typed adapter-failure boundary.
