...
**Horizon 2 now implements production enrollment, chain recovery, closed v1 receipts with legacy normalization, private paired E1/E2 traversal, and recovery, semantic-candidate, command-ingress, effect, and retained-Run capacity.** Implement the remaining timer, Query, terminal, Event History, aggregate, Run, and deployment matrix next. Keep forced Message/Timer/effect rollover evidence and stop-the-world deployment admission as later closure lanes.
