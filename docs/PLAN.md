...
Next action: obtain independent proposal review of the [public Compensation registration amendment](capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md#public-registration-amendment), then implement its complete Product 1 registration outcome. It preserves exact admission and scopes CIB research provenance to reused ordinary User Tasks. Extend the existing test harness to drive multiple real handler Activities reached after human work, then register scenarios, corpus/disclosure and the runnable example together. Product 2 failed-Process support and bounded Transaction cancellation remain required subsequent outcomes. Preserve the unequal raw Lean/TypeScript admission domains. No further proof infrastructure is selected.
