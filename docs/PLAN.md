...
**The owner-approved Horizon 2 Workflow-chain contract has production-default enrollment, chain-relative Product 1 command recovery, versioned terminal receipts, retained legacy normalization, private paired E1/E2 traversal, recovery-ledger exhaustion, atomic committed-state/paired-publication capacity, bounded stimulus/Update/semantic-input ingress, and bounded effect Activity transport implemented.** Implement retained per-Run trace/publication rollover next, then the remaining timer, Query, terminal, history, aggregate, Run, and deployment matrix. Keep forced Message/Timer/effect rollover evidence and stop-the-world deployment admission as later closure lanes.
