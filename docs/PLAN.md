...
**One profile now admits modeler notation.** `bpmn-2.0.2-user-task-preserved-notation-draft` retains Diagram Interchange, pools, lanes, artifacts, and documentation without executing them; every other profile still rejects them through an exact key allowlist. It is registered with its own answer-free scenario, pipeline case, and live example, and a rejected file now names each element the classification refused. Five structural requirement rows record what that admits without letting retention read as executable support. The reference-target-type rule now runs once above the profile dispatch, because two of the four source readers had never installed it. M1's engineering is complete and its closure review is the remaining stage boundary.

**Evidence is differential rather than self-reported.** Thirty-four registered answer-free scenarios run through Lean, the semantic core, Temporal, and selected pinned CIB Seven lanes; every case carries a seeded semantic mutation that must produce a disagreement, and retained CIB observations are content-bound. Neutral scenario inputs stay physically separate from expected results.
...
1. **In progress — implement M1's preserve-only admission:** [the proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md) is owner-approved as of 2026-08-08. Implemented and registered: the recursive classifier, the one-executable-entry rule, the preserved set of D2, reference-target admission, the foreign-attribute rule, D5's non-interference evidence with its seeded defect, the profile artifact with its scenario, pipeline case, and live example, **D3's per-element diagnostics over the classification lane**, and the five `BPMN-STRUCT-` rows in [the requirement ledger](BPMN-REQUIREMENT-LEDGER.md#reviewed-requirements), each `supported` on its own evidence while the operational requirement it names stays `unsupported`. Every engineering item the proposal requires is now landed, together with the cost row and the process-assessment answers the closure boundary requires. The semantic-checkpoint review returned `approve-with-required-edits` and its corrections were audited and owner-accepted; the closure review is the only remaining stage boundary, after which this item and the proposal's `-PROPOSAL` suffix both retire. Its correction of the reference-target rule's placement changes admission for the two A12 profiles and is part of the closure target rather than a separate cycle.
2. **Then — process hardening carried out of the 2026-08-06 instruction-surface review:** these five items exist in no other owner, so losing this entry loses the review that produced them. Guard the capsule-to-requirement-ledger clause-citation pair, which is the first concrete application of the duplicated-fact rule in [DOC-DISCIPLINE.md](DOC-DISCIPLINE.md#writing-and-linking) and would have caught the stale sibling that produced round two of the 2026-08-06 proposal audit; key capsule dispositions to their rule identifiers, which turns the Required-versus-absent contradiction into a comparable pair rather than prose and is the half-day change that converts the highest-count `unguardable` finding into a test, retrofittable one capsule at a time with the guard enforcing only converted ones; widen the bare-`lake` scan from its enumerated set to the executable surfaces, where it finds zero offenders today and so costs nothing to adopt but closes the class permanently; merge the two `unguardable` [process-assessment](PROCESS-ASSESSMENT-LEDGER.md) rows that cite one reusable question, because a shared question understates both instance counts and delays the escalation that converts attention into guards; collapse the publication-statistics restatement in [CLAUDE.md](../CLAUDE.md) to a link, because [TESTING-SPEC.md](TESTING-SPEC.md) owns that detail and the duplication is what carried one wrong generated-block count in both owners until 2026-08-06; and bind the source-hygiene exception map to an owner approval the way [the review-policy guard](../scripts/independent-review-policy.test.ts) binds its grandfather set, which is prevention with no live instance because the map is empty and is therefore last.
3. **Then — three increments the preserve-only closure review deferred rather than dropped:** each is scoped, none changes BPMN meaning, and the first two were verified as genuinely deferred rather than overlooked. **Locate the two remaining classification loci:** an unadmitted own property on an executed flow node, and the three readers admitting one hand-selected shape, both of which refuse safely today while naming no element; this needs a shared per-type key inventory extracted from [the projection predicates](../packages/bpmn-source/src/checked-element-projection.ts) and flow elements classified before projection, which widens where classification is decided and therefore opens its own review. **Compress [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) by auditing each bullet against the capsule that owns it**, which is the mechanism [the reviewability guard](../scripts/document-reviewability.test.ts) names in its own comment; a vocabulary proxy puts 2022 words across 91 bullets inside a single capsule, and that is a candidate list requiring per-bullet judgement, not a deletion list, because two of its largest entries are bullets that capsules deliberately delegate to the map. **Give [the cost-ledger rank guard](../scripts/capsule-cost-comparison.test.ts) a table-contiguity assertion**, since it parses rows by prefix and stayed green over a row that a blank line had pushed out of the table it compares against.
...
**Next action: run the cold closure review for the preserve-only admission capsule.** Every engineering item the proposal requires is landed, so the review target is the whole capsule: the classifier and its diagnostics, the profile with its scenario, pipeline case, and live example, the D5 non-interference evidence, and the five `BPMN-STRUCT-` requirement rows. The checkpoint reviewer's continuity is not eligible here, because the account gained structural requirement dispositions after that checkpoint; spawn a context-cold closure reviewer.

**Blockers: none.** The semantic-checkpoint review closed against `2deb802` with its corrections audited at `ab29717` and `9923b02` and the owner accepting `c915628`; [the receipt](PRESERVE-ONLY-ADMISSION-PROPOSAL.md#independent-cold-review-receipt) owns those targets. No source owner is near its ceiling: `preserved-element-classification.ts` holds 173 nonblank lines of headroom, `checked-process-compiler.ts` 227, `admission-diagnostics.ts` 368, and `workflow-implementation.ts` 50. **[IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) has none:** it sits at exactly its 10000-word reviewability backstop, so the next capability that must record scope needs a compression pass in its own change first, and this capsule already paid for its last two bullets by tightening three others.
