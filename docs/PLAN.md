...
1. **In progress: specify the post-MVP scalability roadmap.** Draft and obtain context-cold review of a theoretical proposal for BPMN Process execution on Temporal and the platform above it. Preserve Product 1's per-Workflow horizontal independence, rank its history/payload/longevity risks, and make replacing Product 2's synchronous single-node SQLite and request-time sequential Query aggregation the first post-MVP scale-out priority. Record explicit horizons, budgets, unsupported claims, and the future distributed evidence needed without treating single-machine benchmarks as scalability proof.
2. **Pending: measure and reduce the initial web bundle.** Produce a dependency-level composition report for the current 809.89 kB minified main chunk, then lazy-load diagram/modeling code and split major workspaces or detail tabs only at measured natural boundaries. Keep the existing browser journeys and exact loading/focus behavior green; do not silence the warning by raising its threshold.
3. **Pending: research richer Human Tasks and forms as a candidate M6.** Inspect CIB Seven first through its current documentation and pristine pinned source, compare current state-of-the-art task and form products, and recommend the smallest useful MVP extension beyond the existing one-field string-or-Boolean form. Research does not establish M6 or authorize a general form builder until an owner-approved proposal selects the boundary.
...
**Next action: complete ordered-work item 1 by drafting the post-MVP scalability proposal and obtaining its required context-cold proposal review.** Its first roadmap horizon must resolve Product 2's single-node persistence, full-history projection, sequential reconciliation, and request-time N+1 Temporal Query bottlenecks before claiming that the platform can inherit Temporal's horizontal scale. The proposal also records Product 1 Continue-As-New, history/payload budgets, worker isolation, backpressure, and observability as dependency-ordered later horizons. No benchmark claim is authorized at this theoretical stage.

Item 1 is **blocked on nothing**. M5 is closed, the prior cold architecture assessment identified the concrete scale boundaries, and current repository evidence is sufficient to specify a theoretical roadmap without running premature single-machine performance tests.

**Latest evidence:** M5 operator-history closure target `46bde82` received a context-cold closure review; correction `cb2a026` closed the bodyless transport, recursive private-field, two-width selection, and selected-population findings, and the same reviewer approved the correction. The complete Product 2 platform and UI checkpoints are green, including 18 server tests and 77 browser cases. The existing production web build still reports one 809.89 kB minified main chunk, which ordered-work item 2 measures rather than suppresses.
