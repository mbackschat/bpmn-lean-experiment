...
1. **Next: obtain owner approval for the M2 cyclic-control-flow proposal.** Context-cold review target `31dd294` and owner-authorized fourth correction audit target `d5e7bb9` are approved with no open findings. Stop for explicit owner approval before implementation. If approved, the first implementation commit creates the immutable baseline catalog from exact baseline `7529150`, before extraction or semantic changes. The selected slice is one root-scope User Task Sequence Flow cycle, identity-only Exclusive Merge derived from checked Sequence Flows, and profile-gated resumption-cut admission. Keep the decided-fixture admission lane under the recorded build ceiling.
...
**Next action: obtain explicit owner approval for the independently approved [resumption-bounded cyclic control flow proposal](capsules/CYCLIC-CONTROL-FLOW-PROPOSAL.md).** Context-cold proposal target `31dd294` and final owner-authorized correction audit target `d5e7bb9` have no open findings. No implementation is authorized before owner approval. If approved, begin with the immutable baseline catalog generated from exact baseline `7529150`, before extraction or semantic changes. The mandatory decided-fixture review is closed by the proof-lane split and its recorded CPU and resident-memory figures. M1 remains closed through the public API and UI over real Temporal hosting. The A12 product boundary remains closed, its stable contract is active, and its proposal is archived.

**Owner blocker:** explicit approval of the independently approved cyclic-control-flow proposal is required before M2 implementation. The earlier owner approval of concrete `@temporalio/client@1.21.0` Product 2 reachability and the no-umbrella Temporal subsystem package layout remains binding.
