...
Next action: independently review the [subscription proposal](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md), including its bounded admission, retained-body amendment, exact consumer migration, and limited assurance lane. Its [diagnostic preflight](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#diagnostic-preflight-results) supports SDK ordering and Timer handoff but proves no production capability. Registration reuses the existing normative-profile/User Task provenance pattern; it requires no schema expansion. The retained-body amendment must resolve `AOO-RETAINED-BODY-01` before subscribed child/Terminate admission. No implementation approval is recorded. Snapshot arming and pipeline correspondence are accepted; subscription/Transaction closure remains open. Preserve the bounded checkpoint and regional trial.
