...
The [human-work proposal](BPM-PLATFORM-HUMAN-WORK-PROPOSAL.md) selects one atomic public contract for current cross-instance tasks, private exact observation locators, actor claims, one typed field, retry-safe completion, platform audit, and a CSS-Modules React inbox. Redesigned target `58aa25e` received context-cold `APPROVE WITH REQUIRED EDITS`; its bounded corrections select one durable confirmed-start publication outbox, retained completion-action retry precedence, route-scoped error decoding, exact audit ordering/timestamps, and the complete existing-owner plan. Product 2 implementation may not consume metadata until E2 closure and the human-work proposal correction audit are approved.
