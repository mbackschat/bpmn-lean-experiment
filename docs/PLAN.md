...
**Next action: take ordered-work item 1 and prepare the governed proposal for the two remaining classification loci.** Start from the shared per-type key inventory in [the projection predicates](../packages/bpmn-source/src/checked-element-projection.ts), classify flow elements before projection, and establish the smallest separating cases for the executed-flow-node property and all three specialized readers. This changes where admission classification is decided, so implementation waits for the required cold proposal review. The M1 engine admission slice is closed, while the complete M1 showcase remains in progress until ordered-work item 3 supplies and verifies its platform increments.
