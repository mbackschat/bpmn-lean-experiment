...
Next action: implement the Lean counterpart of the approved TypeScript [scope-creation preparation prerequisite](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite), starting with predecessor selection and retained-patch refinement before validity, frame, commutation, publication, and production scope/region batching. The [canonical scope and counter-storage prerequisite](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-scope-and-counter-storage-checkpoint) is independently approved and completely verified at `da9657e0`; [immutable resource calibration](CAPSULE-COST-LEDGER.md#canonical-scope-storage-immutable-calibration) and module-cost binding cover its committed source. Preserve exact complete-state equality, high-water records, and the distinction between collection algebra and transition preservation.
