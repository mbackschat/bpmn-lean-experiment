...
**The first A12 product-boundary checkpoint was rejected and its material proposal correction is independently reviewed and owner-approved.** Built-in profiles, scenarios, source readers, Lean, the semantic core, Temporal, CIB evidence, and product examples use product-neutral mapped-success and mapped-boundary-Error identities, but the shared Semantic Process schema still contains legacy A12 boundary IDs and the frozen export omits two retained-evidence dependencies. The approved correction preserves the atomic wire contract and narrows the payload-free Service Task promise to exact profile/BPMN bytes plus exactly bounded scenario/evidence changes. Implementation may resume at the three reviewed root corrections.
...
1. **In progress: correct the A12 product boundary without discarding the implemented slices.** Context-cold review rejected checkpoint `59bcefc` because a shared schema still encodes A12 boundary IDs, the neutrality and frozen-dependency inventories are incomplete, and the proposal's payload-free Service Task byte guarantee contradicted its mandatory wire replacement. The smallest coherent proposal correction is independently reviewed and owner-approved. Fix the schema and guards at their shared roots, complete the frozen dependency closure, add the exact preservation oracle, and submit a new cold checkpoint. Closure documentation, cost, the complete gate, and closure review remain after checkpoint approval.
...
**Next action: correct the shared Semantic Process schema and A12 inventory mechanisms, complete the frozen dependency closure, add the exact payload-free preservation oracle, and submit a new context-cold semantic checkpoint.** The M1 engine admission slice is closed, while the complete M1 showcase remains in progress until ordered-work item 2 supplies and verifies its platform increments.

**Blockers: none.** The revised guarantee is independently reviewed and owner-approved. Checkpoint `59bcefc` remains rejected until its shared-schema, inventory, frozen-dependency, and preservation-oracle findings are corrected and a new cold checkpoint passes.
