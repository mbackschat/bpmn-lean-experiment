...
1. **In progress: review the M4 Service Task incident and retry proposal.** The M4 inventory selects three separately governed stages: Service Task incident/retry semantics, incident-scoped root-Process cancellation, then Product 2 incident operations. The [stage-one proposal](capsules/SERVICE-TASK-INCIDENT-RETRY-PROPOSAL.md) defines one typed technical failure, one literal-generation incident, one exact retry of the same effect occurrence, exact CIB failed-job mapping, a proved Lean lane, and a Temporal preflight. Obtain context-cold proposal approval before any profile or runtime implementation.
...
**Next action: complete and obtain context-cold approval for the second redesign of the M4 Service Task incident and retry proposal.** M3 is closed end to end: the semantic and human-work contract is graduated through correction audit `23892a5`, diagram presentation integrity is approved through `83de531`, the reusable Product 2 information architecture and UI design are approved through `83de531` and `afc4b63` respectively, and the maintained browser walkthrough exposes the complete owner-review path.

Item 1 is **in progress and blocked at the required proposal-review boundary**. The first proposal target `8be7e5f` was rejected because its transport, artifact, CIB configuration, generation, owner, and cancellation accounts were not jointly implementable. The first redesign target `278e63b` was rejected because it wrapped existing Activity results, omitted incident ownership from normal scope quiescence, and missed exhaustive consumers. The second redesign preserves the existing bare semantic Activity payloads, adds one unwrapped technical-failure arm, moves the full effect wait into an identity-bound incident that blocks scope completion and counts as a retry resumption surface, and removes the unresumable generation-two state. Packaged CIB research at `ee6c244` proves the enabled/disabled incident discriminator, same-job reset, raw incident replacement, and later success; the selected semantic mapping stops after the first reset. Playwright remains a Product 2 harness outside every Product 1 feedback loop.

**Last verified commands:** the complete semantic `./scripts/verify.sh` passed at proposal target `278e63b`, including every registered CIB, Lean, differential, and Temporal lane. `c77b6e8` keeps Playwright in the approved separate Product 2 workflow, and its focused setup/UI-isolation/source-hygiene gate passes 28/28. The configured CIB incident phase-zero test passes 2/2 at `ee6c244`, with its focused architecture and hygiene guards 32/32. The retained Product 2 browser evidence remains unchanged and separate.
