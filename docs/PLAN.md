...
Next action: independently review and completely verify the new [scope-creation commutation laws](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite); the complete Lean gate and immutable calibration pass. They derive exact complete-state equality and all successor validity from predecessor premises; Call-list equality uses valid unique anchors and canonical order. Constructed both-order witnesses pass under 3 GiB. After acceptance, close accepted-publication correspondence and production scope/region batching. Preserve high-water records and existing predicate coverage.
