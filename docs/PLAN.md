...
- Composed Activity-data footprint checkpoint. Command: `./scripts/pnpm.sh run test:pre-push`. Status: `exit 1`. Date: `2026-09-08`. Commit: `5eae9923646633f47e7ba80b0a2f86c325245dd0`. The [independent review](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) approved with no findings. Lean, core, source, CIB, and differential lanes passed before SDK cache expiry blocked Temporal startup. The [cache correction](../packages/temporal-adapter/SOURCE-MAP.md#package-ownership) passes the complete Temporal gate; the clean committed repository-wide rerun remains required before pushing.
...
Next action: run the clean committed repository-wide gate after the Temporal cache correction, then integrate retained prepared execution and prove arbitrary finite ordinary/composed-data batches. The [composed Activity-data footprint contract](INTERNAL-COMMUTATION-PROPOSAL.md#final-closure-evidence-and-adversarial-oracles) is independently checkpoint-approved. The [cold library memory correction](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction) and [immutable calibration](CAPSULE-COST-LEDGER.md#composed-arming-footprint-checkpoint-calibration) passed under the unchanged 3 GiB ceiling with zero memory events and pressure totals. Pairwise preparation, preservation, execution, and canonical publication are proved from predecessor invariants and noninterference. Preserve the unavailable production footprint until the finite-prefix and publication obligations close; then continue regions and scheduled choice in the approved [dependency order](INTERNAL-COMMUTATION-PROPOSAL.md#selected-final-closure-account).
