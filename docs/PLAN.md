...
**Next action: complete Product 1 client/API exposure and live Temporal retention and replay evidence for the independently approved [flow-node occurrence metrics checkpoint](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md), then implement its Product 2 metrics contract, projection, aggregate, route, and UI.** Semantic-checkpoint review target `33ffa62` and same-reviewer correction audit `4aa081b` are approved. The remaining operator-history and audit-export increments stay ordered after this capsule.
