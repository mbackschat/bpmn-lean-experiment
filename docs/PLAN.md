...
The driving product goal is recorded in [PROJECT-DESIGN.md](PROJECT-DESIGN.md): replace A12 Workflows `release/2025.06`, the main A12 Process product layered on CIB Seven, rather than one representative downstream application. The [A12 Workflows compatibility ledger](research/A12-WORKFLOWS-COMPATIBILITY-LEDGER.md) defines 62 physical and 50 distinct exact-byte BPMN models, the maintained extension/delegate/API surface, and the A12 Full Stack Project Template as the canonical downstream blueprint. The exact target-profile projector now admits one of those 50 models unchanged at the static source/lowering boundary; zero models have closed product execution evidence. Typed runtime variables and mappings, the exact target expression subset, `BpmnError`, a Java-friendly delegate bridge, task/message façades, and a materialized Workflows-enabled template integration remain target-backed priorities. The rejected-core review's wire exactness findings remain implemented through JavaScript-safe integers, language-neutral identifier ordering, and strict JSON edge behavior.

The [CIB Seven 2.0 A12 target baseline](research/CIB-SEVEN-A12-BASELINE-RESEARCH.md) keeps the A12-used `2.0.0` release distinct from the current `2.2.0` oracle profiles. The first approved CreateDocument implementation slice is green: the immutable `2.0.0` profile and relationship records are distinct; a cohesive A12 projector admits the exact registered source bytes unchanged when available and a project-authored equivalent fixture in mandatory gates; hostile executable drift rejects; and checked mapping data lowers identically through TypeScript and Lean. Runtime scopes, arguments, result patches, CIB evidence, and Temporal refinement remain pending.
...
**Authorized next action after checkpoint review:** implement the approved CreateDocument runtime semantics with red/green Lean and TypeScript evidence: string Process and Activity-local scopes, literal input evaluation, immutable effect arguments, exact success patch validation, output mapping, cleanup, and canonical Process variables. Do not begin CIB or Temporal host work until that semantic checkpoint is green.
...
11. **Approved; implementation in progress — CIB target alignment and CreateDocument data capsule:** the distinct immutable `2.0.0` profile, relationship records, project-authored fixture, optional exact external-source gate, strict A12 admission, checked mappings, hostile-source controls, and TypeScript/Lean lowering are green. Next implement string scopes, committed arguments, typed patch validation, output mapping, and canonical variables before CIB and Temporal host evidence. Typed `BpmnError` follows immediately after closure.
...
The first incomplete ordered item is the semantic runtime checkpoint of [the approved CreateDocument data and mapping proposal](capsules/CREATE-DOCUMENT-DATA-PROPOSAL.md). Add the smallest separating Lean and TypeScript tests for literal argument commitment, exact successful local patch, Process output mapping, local cleanup, and malformed-patch state preservation; then implement the shared string-only scope mechanism. CIB `2.0.0` and Temporal work begin only after that checkpoint is green. The missing materialized Workflows-enabled full-stack-template variant remains an integration input. Typed `BpmnError` is next after capsule closure. D1 remains unapproved and behind this product-forced work. Keep C2 frozen under the non-extension and reopening rules in [the experiment record](experiments/CHECKED-SOURCE-RELATION-EXPERIMENT.md).
