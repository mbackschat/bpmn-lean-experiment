...
The [human-work proposal](BPM-PLATFORM-HUMAN-WORK-PROPOSAL.md) selects one atomic public contract for current cross-instance tasks, private exact observation locators, actor claims, one typed field, retry-safe completion, platform audit, and a CSS-Modules React inbox. Exact redesigned target `7444ce3` received context-cold `APPROVE WITH REQUIRED EDITS`; its bounded correction selects logical audit outcomes, audit-silent policy hiding, a same-transaction Work audit outbox with idempotent delivery, and the missing configuration/schema owners. Product 2 implementation may not consume metadata until E2 closure and the human-work proposal correction audit are approved.
