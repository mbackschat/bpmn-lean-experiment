...
The [documentation gate-routing correction](DOC-DISCIPLINE.md#verification) is non-material: it delegates to the existing testing policy without changing semantic claims. The complete infrastructure gate, including control-plane, reviewability, and link guards, plus `git diff --check` cover this correction.
...
Next action: execute the approved [RC dependency classification](INTERNAL-COMMUTATION-PROPOSAL.md#rc-completion-scope-amendment), beginning with the current admitted capability predicates and the remaining composite preparations. Identify future subscription and Compensation/Transaction admission dependencies before selecting further proof work; unresolved dependencies block their admission and RC closure. The [architecture review and experiments](experiments/INTERNAL-COMMUTATION-PROOF-ARCHITECTURE-EXPERIMENT.md) are closed with neither replacement adopted. Preserve the unfinished [bounded Sub-Process checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#bounded-sub-process-arming-outcome); its regional, publication, and finite-integration obligations remain unresolved. The regional trial remains pending.
