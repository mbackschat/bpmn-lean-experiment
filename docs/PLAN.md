...
Next action: close accepted lifecycle publication for [scope creation](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite), then production scope/region batching. Derive successor open projection and actual candidate acceptance from predecessor validity, predecessor open projection, and successful preparation; preserve existing private boundary-Timer classification through the static entry census. Complete-state commutation and control-position correspondence have independent approval, immutable calibration, and clean complete verification. Preserve high-water records, existing predicate coverage, and the unchanged 3 GiB ceiling.
