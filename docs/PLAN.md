...
Next action: derive the reverse complete local-control preparation frame under independent regional execution, using the existing `prepareInternalLocalControl_read_frame` and regional removal laws. Reuse the regional/local-control pair matrix and dependency-omission negatives; derive every required observation from predecessor validity and footprint independence. The [forward frame](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preservation-of-regional-preparation) is independently approved at `e185caca`, with clean complete integration. Reassess before any new representation or substantial prerequisite expansion; no helper-only checkpoint. Pair commutation/publication remains the following obligation. E1/E2 change no RC scope.
