...
**Next action: execute ordered item 1, the curated executable BPMN model corpus, before selecting another semantic family.** Begin with the provenance/licence and clone-family manifest, retain only licence-compatible project bytes, then add the smallest locally executable tranche and generated mechanism-blocker ranking. The closure-reviewed [flow-node occurrence metrics specification](capsules/FLOW-NODE-OCCURRENCE-METRICS-SPEC.md) is complete. The remaining operator-history and audit-export increments stay ordered after the corpus decision gate.

Item 1 is **at its corpus-definition and first-tranche boundary**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and the current M5 observation increments are closed through the graduated [committed execution publication](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) and [flow-node occurrence metrics](capsules/FLOW-NODE-OCCURRENCE-METRICS-SPEC.md) specifications. The corpus must now turn whole-model source, licence, clone-family, mechanism, execution, and Product 2 readiness facts into one executable manifest and a generated blocker report. No catalog-visible model may outrun its production-backed local user journey.
