...
**Status: in progress.** [Stage 1](capsules/SERVICE-TASK-INCIDENT-RETRY-SPEC.md), one bounded Service Task incident and exact retry, and [Stage 2](capsules/SERVICE-TASK-INCIDENT-CANCELLATION-SPEC.md), exact incident-scoped hosting-root Process cancellation, are implemented, closure-reviewed, evidence-closed, and graduated. Product 2 incident operations remain Stage 3.
...
1. **In progress: review the first-green M4 Product 1 incident-operations checkpoint.** The owner-approved [incident-operations proposal](BPM-PLATFORM-INCIDENT-OPERATIONS-PROPOSAL.md) selects exact private locator retention plus retrospective startup bootstrap, one dedicated current-incident Query, all-or-error Operate aggregation, configured operator authorization, durable content-bound Retry/Cancel actions without platform priority, separate paged incident audit, and an Operations collection/detail workspace. Immutable checkpoint `6e2827b` contains the Query/API/wire and replay boundary; downstream Product 2 implementation is paused until its conditional semantic review closes.
...
**Next action: independently review immutable M4 Stage 3 Product 1 checkpoint `6e2827b`.** The checkpoint adds the dedicated deterministic current-incident Workflow Query, strict Product 1 process-operations API, neutral byte-identical locator owner, representation-free gateway, and red/green zero-incident, terminal, malformed-interaction, retained-receipt, history-nonauthority, and replay evidence. Downstream Product 2 implementation stays paused until the conditional review approves the checkpoint.

Item 1 is **paused at the conditional semantic checkpoint review**. Stages 1 and 2 are graduated. The checkpoint reads the canonical stable-state observation rather than reconstructing Retry/Cancel, rejects nonempty effect arguments and unsupported incident-state interactions, preserves direct/Message/Schedule locator bytes, corroborates terminal status with the exact retained receipt, and proves that the Query creates no Event History event while both histories replay. Product 2 persistence, authorization, actions, audit, HTTP, UI, and Playwright remain absent at this checkpoint.

**Last verified commands:** The Stage 3 checkpoint passed the protocol, Workflow, Temporal client, engine API, engine gateway, both harness type gates, focused current Query/history/replay live Temporal gate, and the source, package, product, architecture, and ownership guards. Hosted CI run `31755256545` is green on Ubuntu, macOS, and `verify-complete` after release-specific CIB compilation closed the shared-output and cross-release API-drift failure class. Stage 2 previously passed the complete repository, four-target, Lean, semantic-core, CIB, and live Temporal gates. Playwright remains in its separate Product 2 workflow.
