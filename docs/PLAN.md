...
1. **In progress: obtain owner approval for the next bounded M5 frequency and duration increment.** The source-grounded Product 2 preflight over current CIB Seven Cockpit documentation, the pinned pristine source, and an independent Camunda Optimize comparison is recorded in the [UI/UX research](research/BPM-PLATFORM-UI-UX-INFORMATION-ARCHITECTURE-RESEARCH.md#pattern-11-flow-node-metrics-require-occurrence-facts-not-transition-counts). The independently reviewed [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md) selects an exact Product 1 occurrence lifecycle and deterministic engine commit time before Product 2 frequency or duration. Review target `2503464` and final same-reviewer correction audit `5a75af9` are approved; owner approval and implementation have not occurred. Do not infer duration from Temporal Event History, Product 2 ingestion time, or semantic logical time. The closure-reviewed [E1 specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) remains the only committed semantic-history and current-position authority while completed operator history and audit export stay later M5 increments.
...
**Next action: request owner approval of the independently reviewed [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md) and its nine selected decisions.** No implementation begins before that approval. The remaining operator-history and audit-export increments stay ordered after it.

Item 1 is **preimplementation owner approval in progress**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and M5 E1 is closed through the graduated [committed execution publication specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md). The next proposal is independently reviewed but owner-unapproved. Event History, state differencing, Product 2 ingestion time, and semantic logical time remain forbidden substitutes for an exact public wall-clock duration fact.
