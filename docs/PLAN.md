...
**The first A12 product-boundary checkpoint was rejected and the proposal is under material correction.** Built-in profiles, scenarios, source readers, Lean, the semantic core, Temporal, CIB evidence, and product examples use product-neutral mapped-success and mapped-boundary-Error identities, but the shared Semantic Process schema still contains legacy A12 boundary IDs, the frozen export omits two retained-evidence dependencies, and the original payload-free Service Task byte-preservation promise conflicts with the mandatory `sourceOverlay` wire field. The recommended correction preserves the atomic wire contract and narrows that promise to exact profile/BPMN bytes plus exactly bounded scenario/evidence changes. A new context-cold proposal review and owner approval are required before implementation resumes.
...
| The `CreateDocument` and boundary-Error slices, their legacy `2.0.0` profiles, and their exact source readers | The rejected first checkpoint moved production decisions to neutral profiles and the data-only overlay contract while preserving most of the legacy generation under the optional adoption root. The materially corrected [A12 boundary proposal](A12-ADD-ON-BOUNDARY-PROPOSAL.md) must complete review and owner approval before the missing frozen dependencies and remaining guard/schema corrections land. Do not repeat the shape for further A12 models. |
...
1. **In progress: correct the A12 product boundary without discarding the implemented slices.** Context-cold review rejected checkpoint `59bcefc` because a shared schema still encodes A12 boundary IDs, the neutrality and frozen-dependency inventories are incomplete, and the proposal's payload-free Service Task byte guarantee contradicts its mandatory wire replacement. Review and obtain owner approval for the smallest coherent proposal correction, then fix the schema and guards at their shared roots, complete the frozen dependency closure, add the exact preservation oracle, and submit a new cold checkpoint. Closure documentation, cost, the complete gate, and closure review remain after checkpoint approval.
...
**Next action: commit and submit the materially corrected A12 boundary proposal to a new context-cold proposal review, then request owner approval of its exact payload-free Service Task preservation guarantee.** After approval, correct the shared schema and inventory mechanisms, complete the frozen dependency closure, and submit a new context-cold semantic checkpoint. The M1 engine admission slice is closed, while the complete M1 showcase remains in progress until ordered-work item 2 supplies and verifies its platform increments.

**Blocker: owner approval is required after the revised proposal passes context-cold review.** Checkpoint `59bcefc` was rejected. Its schema and frozen-export findings remain inside the previously selected contract, but the corrected preservation guarantee materially narrows an explicit evidence claim and may not be implemented under the prior approval.

**Last verified command:** `/opt/homebrew/bin/gtimeout 60s env CI=true ./scripts/verify.sh`, exit 124 at checkpoint `59bcefc` during the first Lean build after 47 of 56 modules completed with continuing output. The timeout left no test process running and is not a semantic failure, but it is not a complete gate. The latest complete gate remains exit 0 at `fb4b7dc`, with 35 agreements, 35 seeded mutations detected, 18 retained-CIB comparisons, and 37 live histories; its timing is contended and is not a baseline.
