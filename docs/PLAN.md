...
Next action: finish the complete affected gates and independent semantic-checkpoint review for [local-control preservation of regional preparation](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preservation-of-regional-preparation). The quantified frame derives selection, exact footprint, and publication equality from predecessor validity and footprint independence; all regional/local-control pairings and overlapping dependency-omission negatives supply the executable separator. Reassess before any new representation or substantial prerequisite expansion; no helper-only checkpoint. [Complete/Return preservation](INTERNAL-COMMUTATION-PROPOSAL.md#complete-and-return-runtime-preservation) is approved at `f725745d`; [Error/Terminate preservation](INTERNAL-COMMUTATION-PROPOSAL.md#error-and-terminate-runtime-preservation), at `0f4e8a51`. E1/E2 change no RC scope.
