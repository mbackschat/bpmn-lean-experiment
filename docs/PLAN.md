...
Next action: close the regional/arming pair obligation, reusing the existing prepared arming account, patch algebra, and individual preservation/publication laws. Derive both complete preparation frames, exact final equality, successor validity, and accepted per-operation publication from predecessor facts. The [regional/local-control pair](INTERNAL-COMMUTATION-PROPOSAL.md#regionallocal-control-commutation-and-accepted-publication) is independently approved with clean complete integration. Reassess before substantial prerequisite expansion; no helper-only checkpoint. Other family frames and batching remain open. E1/E2 change no RC scope.
