...
- the bounded production-lifecycle experiment replaces a Worker at the semantic wait, retrieves an accepted Update result after Workflow closure, proves same-Update-ID payload aliasing, rejects conflicting command identity without a Workflow Task failure, rejects a distinct late Update and Workflow-ID reuse, checks identity separation, and replays; its first red run exposed and fixed completion overtaking start because handlers were registered before the start stimulus led the queue;
...
The separately invoked focused Temporal gate passed in 3.71 seconds on the same source state, including Worker replacement, retained accepted-Update lookup after Workflow closure, same-Update-ID payload aliasing, non-retryable conflicting-command refusal without Workflow Task failure, refusal of a distinct late Update and Workflow-ID reuse, identity separation, and replay.
...
11. **Owner decision — approve or revise the production Temporal lifecycle before expanding BPMN scope:** the bounded experiment supplies Worker-restart, accepted-result retrieval, same-Update-ID payload-aliasing, explicit conflicting-command refusal, distinct late-command refusal, Workflow-ID non-reuse, replay, cleanup, and identity-separation evidence. The [production lifecycle proposal](TEMPORAL-PROCESS-LIFECYCLE-PROPOSAL.md) recommends a semantic-lifetime Workflow with content-bound Update identity and a retention-bounded `semantic`/`processClosed`/`processUnknown` result. Production Workflow behavior and the next semantic capsule remain blocked on owner approval.
...
Resume at ordered work item 11 by obtaining owner approval or revision of the [production lifecycle proposal](TEMPORAL-PROCESS-LIFECYCLE-PROPOSAL.md). Its exact typed result union, content-bound Update identity, command-retry lookup order, completed receipt, Workflow-ID non-reuse, Temporal-retention limit, accepted-handler drain rule, evidence requirements, exclusions, and re-open conditions are ready for review. Do not change production Workflow lifetime until that decision is recorded.
