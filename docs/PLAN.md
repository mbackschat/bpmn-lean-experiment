...
**Next action: implement [ordered-work item 1(a)](#ordered-work), the behavior-preserving dispatch registry.** Establish complete-result equality across every current dispatch path and adversarial foreign-attribute case, then make policy enforcement unavoidable through the registry without moving foreign-attribute findings out of classification collection. The M1 engine admission slice is closed, while the complete M1 showcase remains in progress until ordered-work item 2 supplies and verifies its platform increments.

**Blockers: none.** Item 1(a) is non-material only while the complete public compilation result remains exactly equal. Any result drift moves it into governed admission work and stops this refactor.
