...
- a mandatory per-capsule Temporal hosting/refinement preflight now separates Lean-to-TypeScript correspondence from adapter feasibility before production implementation, and the parallel preflight records a feasible Query/Update/single-loop host composition;
...
7. **Next — exercise the Temporal preflight:** add failing parallel adapter witnesses before treating the pure-core result as durably realizable; require A-then-B and B-then-A Updates, exact intermediate Queries, duplicate-command stability, a concurrent-submission history that realizes one permitted order, Update completion before Workflow completion, and same-gate replay. Keep the finite scenario-host lifecycle explicit and do not silently select a production post-completion command policy.
8. **Complete R6 and CIB evidence:** generalize the CIB adapter to multiple active tasks with semantic sorting and per-element wait multiplicity; add answer-free A-then-B and B-then-A scenarios; retain content-bound raw/canonical evidence and a meaningful projection mutation; preserve the negative deviation probe outside the normative target result.
9. **Complete differential comparison:** add the parallel cases to the four-target pipeline, preserve exact checked/program definition binding, and add provenance-erasure and projection mutations.
10. **Close and graduate the capsule epistemically:** complete the rule-to-evidence matrix, state exact established and unsupported claims, review common-mode assumptions and the nearest counterexample, graduate the Semantic Process IL and parallel proposals into `-SPEC` contracts, archive the resolved proposals when their rationale remains useful, update the implementation map and walkthrough fragments, run focused and full gates, and reassess the next semantic discriminator.
...
The semantic decisions, bounded Semantic Process IL proposal, current wire contracts, contract mutations, R5 semantic-policy ownership correction, canonical checked graph, deterministic TypeScript lowerer, Lean definition lane, independent TypeScript parallel evaluator, and mandatory Temporal hosting/refinement preflight are recorded. Lean independently checks exact lowering before evaluating the received program, and Lean plus TypeScript establish the bounded parallel account. The parallel host composition is plausible but has no live Temporal evidence, and the current Workflow remains a finite scenario host rather than the production lifecycle.

Resume at ordered work item 7. Read [the parallel capsule](capsules/PARALLEL-FORK-JOIN-PROPOSAL.md), [the Temporal execution research](TEMPORAL-EXECUTION-RESEARCH.md), [the test specification](TESTING-SPEC.md), and the current Workflow, runner, and Temporal tests. Add failing live adapter witnesses for both ordered completion sequences, exact Query projection after the first completion, duplicate delivery, concurrent submissions realizing one permitted order, completed Update history before Workflow completion, and replay before changing adapter implementation.
