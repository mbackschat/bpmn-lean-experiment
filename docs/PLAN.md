...
1. **Closure pending: finish the curated executable BPMN model corpus decision gate.** The manifest and generated report bind 23 retained business-purpose-shaped project models covering all 25 executable element or semantic variants in the registered pipeline, plus seven exact external files across six CIB Seven, OMG, and Betsy clone families. The guard derives support from pipeline XML and fails unless the canonical capability/restriction catalog and retained-model union match it exactly; the Product 2 About page consumes that same catalog. The retained suite reuses the existing Lean, semantic-core, selected CIB, Temporal, replay, and mutation pipeline; the external tranche verifies Git revisions or archive and entry digests, reruns exact compiler admission, and ranks clone-family-deduplicated gaps without retaining external bytes. The sequential assignment/form model and the new exact parallel content-and-risk model are catalog-ready through CIB-preflighted production-preview Chromium journeys. The parallel composition's registered profile, combined CIB evidence, both differential schedules, live Temporal replacement/replay, corpus registration, and exact Product 2 journey are green; the governed closure review and graduation remain. For the next semantic-family decision, assess Script Task execution's two-family practical reach against the co-equal high-risk semantic families named below; prevalence alone does not select it. No model may enter the browser catalog before its exact advertised workflow passes locally. The report never overrides BPMN dependencies, creates a combined conformance percentage, or turns an external engine into semantic authority.
...
**Next action: run the complete gate and governed closure review for the implemented parallel User Task metadata composition, then graduate it and begin the remaining M5 operator-history and audit-export increment.** The registered profile, retained CIB evidence, both differential schedules, live Temporal replacement/replay, corpus entry, and production-preview Product 2 journey are complete. The next semantic-family decision remains after M5: classify Script Task execution against BPMN, CIB, deterministic sandboxing, data, security, and Temporal requirements before deciding whether its two-family prevalence justifies a proposal.

Item 1 is **at governed closure**. The exact manifest, 23 retained models covering all 25 registered executable variants, seven external candidates across six clone families, provenance and catalog guards, Product 2 About disclosure, and two production browser journeys are implemented. Product 2 readiness is two; 21 retained models remain excluded from its catalog.

**Latest evidence:** The combined semantic and source tests, exact CIB Java checkpoint, content-bound CIB replacement and projection guards, differential catalog mutations, runnable configuration, Query mutations, live Temporal replacement/replay witness, static corpus oracle, and production M3 Chromium journey are green. The earlier exact-topology correction remains approved at `35fc881`. The complete repository gate and closure review remain the only open evidence steps.
