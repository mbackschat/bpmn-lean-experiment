...
**Recommendation:** approve a timer capsule followed by a service-task Activity capsule. A timer forces the first executable relation between semantic time and a durable Temporal wakeup. A service task then forces the external-effect lifecycle, idempotency, and the separation of Temporal transport retries from CIB-visible retry or incident behavior. Another pure-topology capsule would exercise no new adapter mechanism, where the residual feasibility risk is concentrated. No capsule has begun.
