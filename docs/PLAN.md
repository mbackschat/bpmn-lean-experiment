...
**The owner-approved Horizon 2 Workflow-chain contract has passed its first-green semantic-checkpoint review.** Implement production-default enrollment and chain-relative Product 1 command recovery next, including the versioned terminal-receipt migration and retained legacy-receipt normalization. Keep publication segment traversal, the remaining capacity matrix, Message/Timer/effect forced evidence, and stop-the-world deployment admission as explicit later closure lanes.
