...
**The exact next implementation boundary is the Operate PostgreSQL vertical slice.** Add its migration, adapters, reusable cross-backend contracts, database-free gate, and separate real-PostgreSQL gate. Preserve terminal closure, insertion identity, incident recovery, gap-free audit source ordinals, one-statement projection snapshots, overlap/gap refusal, and suffix-only writes. Do not begin Work, Audit sinks, leases, recovery, or shared API composition. Review target `c9cf16e` and correction audit `e03240c` bind the repositories, artifact store, repair families, request-time fan-out, projection freshness, audit completeness, and same-database publication invariant.

Item 1 is **in progress** and blocked on nothing. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a`, `5deb00d`, `58cacb7`, and `78c59a4` implement the dependency set, bounded runtime, checksum migrations, Promise-only repositories, PostgreSQL artifacts, and five Definitions adapters. Cross-backend evidence covers artifact ordering and corruption, rollback-safe gap-free allocation, exact full-Unicode identity including U+0000, canonical catalogs, generated-DI integrity, and lifecycle CAS races. Ordinary package and platform checkpoints remain database-free; PostgreSQL 18 runs only through named witnesses. Operate, Work, Audit sinks, leases, recovery, shared API composition, and multi-replica closure remain incomplete. No capacity claim exists.
