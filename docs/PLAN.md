...
The engine and BPM platform remain separate MIT products under [PROJECT-DESIGN.md](PROJECT-DESIGN.md#product-division) and the package boundaries in [ARCHITECTURE.md](ARCHITECTURE.md). M0 through M5 are closed, and M6 is implemented pending its context-cold closure review. [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) owns the exact implemented and absent surfaces; the graduated [specifications](README.md) own closed contracts and evidence.
...
**Status: implemented, closure review pending.** The approved [structured Human Work proposal](BPM-PLATFORM-STRUCTURED-HUMAN-WORK-PROPOSAL.md) is implemented across the generic five-arm value wire, assignment-only User Task metadata, opaque BPMN Rendering preservation, Product 2's exact-source-bound catalog, Zod-backed validation and canonical patch computation, durable Work completion, responsive web form, retained model, and both-width production browser journey.
...
1. **In progress: close M6 structured Human Work.** The implementation and focused Product 1, Product 2, corpus, bundle, and two-width browser evidence are green. Run the complete applicable gate once, bind the exact closure target and cost record, obtain the required context-cold closure verdict, apply any reviewed corrections, graduate the proposal to a maintained specification, and close the milestone. CIB Seven forms remain research inspiration and the old bounded M3 profiles only; they do not define the new semantic contract.
...
**Next action: close the implemented M6 [structured Human Work proposal](BPM-PLATFORM-STRUCTURED-HUMAN-WORK-PROPOSAL.md).** Run the complete applicable gate once, commit the exact target, measure and record its cost, mint the neutral review packet, and obtain a `fork-turns-none` context-cold closure verdict. After approval, graduate the proposal to a maintained specification and update every inbound link atomically.

Item 1 is **blocked on nothing**. The implementation is complete, the host is available for the one remaining integrated gate, and the closure review has not yet started.

**Latest evidence:** the approved Product 1 checkpoint retains its focused Lean, semantic-core, BPMN-source, shared-contract, Java CIB-prefix, differential, engine-API, Temporal-protocol, publication, live Workflow, Worker-replacement, history, and replay evidence. Product 2 focused evidence covers strict catalog projection and canonical bytes, definition persistence, Zod-backed computation, exact retry/conflict and zero-mutation rejection, HTTP 422, server composition, web contracts and controls, and the retained expense-exception corpus row. The production web build's complete default Work static-import graph is 462,859 bytes, down from the pre-M6 464,930-byte graph; Definitions, Operations, About, task detail, structured form, definition diagram, and bpmn-js are split at natural navigation, task-selection, detail, or Diagram boundaries. The exact 1280 and 1600 Chromium journey is green for all three actions, every field kind, conditional reason input, client and server validation, canonical list retry, changed-action conflict, History, and Work audit. The approved scalability proposal remains the first post-MVP scale-out roadmap.
