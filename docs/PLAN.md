...
**Message Start semantics are implemented, closure-reviewed, evidence-closed, and graduated; Product 2 ingress is implemented and evidence-green pending closure review.** The registered standards-only profile preserves exact Process, Start Event, Message, Interface, and Interface Operation identity through source, checked graph, IL, Lean, the independent core, the answer-free differential case, the runnable example, and direct Temporal Workflow creation. Product 2 now publishes one exact target through a durable no-redispatch resource, strict HTTP and UI surfaces, response-loss recovery, Worker-absent and replacement execution, history/replay, and private-fact exclusion without changing BPMN meaning.
...
**Status: in progress.** The resumption-bounded cyclic-control-flow, Message Start Event, Timer Start Event, Terminate End Event, [configured Task extension](capsules/CONFIGURED-GENERIC-TASK-SPEC.md), and [exact-version definition scheduling](BPM-PLATFORM-DEFINITION-SCHEDULING-SPEC.md) increments are implemented, closure-reviewed, evidence-closed, and graduated. The M2 engine boundary and Timer Start scheduling increment are complete. Published Message Start ingress and instance search remain.
...
**Last verified commands:** `/usr/bin/time -p env CI=true ./scripts/verify.sh` passed at immutable Message ingress checkpoint target `8f3c7c1` in 231.72 seconds with output SHA-256 `bc5cdfe93cbd0af6e10e7cc3ec5934f6f103bad9a67ee2bc4ea097b3be687aa3`; all registered pipeline cases and 44 live histories passed. The warm feedback target was exceeded under recorded CPU contention, but no correctness or resource ceiling failed. The context-cold semantic checkpoint review approved the target with no findings after independently passing engine, Temporal client, contracts, gateway, definitions, web, harness, boundary, hygiene, and doctor gates plus a one-start concurrent-request probe.
