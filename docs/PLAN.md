...
The complete repository gate passed on 2026-07-27 after the idiomatic TypeScript contract cleanup. `./scripts/verify.sh` passed twenty-two contract tests, the Lean build plus forty-five semantic-core tests and the strict `DeepReadonly` type locks, twenty-two BPMN-source tests, twenty-seven packaged-CIB tests plus eight release-bound `2.0.0` boundary tests, nine differential unit tests, forty-two infrastructure guards, and forty-four Temporal tests. The ten-case pipeline completed in 7.261 seconds warm, ran twenty isolated Temporal executions, replayed twelve histories, and remained within the unchanged feedback budget. The new compiler-backed source-hygiene checks prove that directly executed TypeScript rejects non-erasable syntax and that every configured direct harness root stays inside Node's erasable-syntax subset.
