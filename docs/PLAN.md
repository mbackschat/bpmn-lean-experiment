...
- current schemas and boundary guards freeze the checked BPMN graph and Semantic Process program wire shapes, the bounded source path produces both sequential and parallel artifacts, and Lean freezes the reviewed proof proposition signatures without supplying a lowerer, evaluator, or proof;
...
- no Lean artifact decoder, Lean lowerer/evaluator/proof, parallel production execution, or cross-lane parallel evidence is implemented.

The active implementation boundary is [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md).

## Last verified baseline

`env CI=true /opt/homebrew/bin/timeout 60 ./scripts/verify.sh` passed on 2026-07-26 after the atomic checked-graph and Semantic Process replacement. The full command completed in 20.18 seconds, passed eleven focused contract tests, nineteen semantic-core tests, twelve BPMN-source/lowering tests, five CIB tests, nine differential unit tests, and nineteen infrastructure guards; it checked seven synchronized documentation fragments and the fourteen-class/nine-property bounded CMOF manifest. The mandatory focused Temporal gate completed in 1.71 seconds with live replay, and the three-case four-target pipeline completed in 4.48 seconds with three replayed histories and exact checked/program provenance. The separately gated representation experiment previously passed with `lake build checkSemanticRepresentationSpike && lake exe checkSemanticRepresentationSpike`.
...
5. **Next — implement the Lean definition lane:** decode the checked graph and Semantic Process program, recompute and require exact lowering equality, define the declarative program relation and executable evaluator, prove evaluator soundness, pursue the reviewed source-to-program preservation statement, and add the parallel laws and duplicate-left/no-right non-law.
...
The semantic decisions, bounded Semantic Process IL proposal, current wire contracts, contract mutations, reviewed Lean proof signatures, R5 semantic-policy ownership correction, canonical checked graph, and deterministic TypeScript lowerer are recorded. The sequential semantic core, Temporal adapter, and differential harness now consume only the Semantic Process program; parallel execution remains gated.

Resume at ordered work item 5. Read [the Semantic Process IL proposal](SEMANTIC-PROCESS-IL-PROPOSAL.md), [the semantic-representations research](research/SEMANTIC-REPRESENTATIONS-RESEARCH.md), [the shared contracts](../contracts/README.md), and the applicable Lean source. Add failing Lean artifact-decoding and lowering-equality checks for both bounded definitions, then define the declarative program transition relation and independent executable evaluator before proving soundness.
