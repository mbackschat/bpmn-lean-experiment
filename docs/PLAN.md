...
1. **In progress: remove Product 2's single-node scale boundary.** The complete PostgreSQL persistence graph, exact migration application, Audit source-to-sink completeness, generic bounded lease kernel, four Definitions recovery families, Operate and Work candidate discovery, and bounded Audit delivery are Green without adding a database to the ordinary package loop. Complete the remaining single-item handlers, then compose the recovery worker, shared-mode API, freshness-bounded projection reads, and concurrent multi-replica closure evidence. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.
...
Item 1 is **in progress** and unblocked. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a` through `83037df` implement the approved dependencies, runtime, Promise repositories, PostgreSQL artifacts and business repositories, exact Audit sinks, six-owner migration application at epoch 6, generic leases, all eleven candidate families, bounded Audit batches, and four Definitions handlers. Cross-backend evidence covers exact Unicode, gap-free allocation, canonical values, suffix-only projections, terminal monotonicity, concurrent retries, Audit completeness, lease reclaim, stale-token refusal, and terminal publication without host redispatch. Ordinary checkpoints remain database-free; PostgreSQL 18 uses named witnesses. Remaining handlers, both process compositions, freshness-backed reads, and multi-replica closure are incomplete. No capacity claim exists.
