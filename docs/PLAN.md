...
**The owner-approved Horizon 2 Workflow-chain contract has production-default enrollment, chain-relative Product 1 command recovery, versioned terminal receipts, retained legacy normalization, private paired E1/E2 traversal, recovery-ledger exhaustion, and atomic committed-state/paired-publication capacity implemented.** Implement bounded stimulus, Update, and semantic-input queue ingress next, then the remaining matrix. Keep Message/Timer/effect forced rollover evidence and stop-the-world deployment admission as later closure lanes.
...
**Latest evidence:** The production Workflow-chain gate passes 225 ordinary Temporal tests plus two production-limit capacity witnesses. Product starts carry the production Initial envelope; raw results are opaque; public receipts contain no Message ledger; and Product 1 preserves retry/conflict precedence. The reviewed checkpoint proves recursive continuation validation, terminal fencing, three Runs, two boundaries, and replay. Paired publication evidence covers Run-local ranges `0..8`, `8..12`, and `12..16`, boundary cursors, cross-Run occurrence identity, private Run identity, and replay. The recovery witness commits 512 cyclic commands, returns the filling result before typed failure, recovers it from the retained failed Run, preserves conflict precedence, maps unseen work to Product 1 capacity, proves no Workflow retry, and replays. The semantic-candidate witness accepts an exact 64 KiB paired batch, refuses its one-byte successor and a 64 KiB-plus-one RuntimeState before revision zero advances, and replays both failed Runs. Horizon 1 closure target `36c3c75` passed after correction `07c68d5`; M6 target `c1ee125` passed after correction `7c35fd1`. Product 2 evidence covers strict catalog projection, durable definitions, exact retry/conflict, HTTP, server composition, the retained expense-exception model, and the 1280/1600 Chromium journey.
