...
8. **Authorized next — Service Task CIB binding:** run the packaged-engine unknown-implementation, exact expanded-QName, bean-resolution, fail-once retry-decrement, and clean manual re-execution probes. Stop at the proposal boundary if any required fact fails.
