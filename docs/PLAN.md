...
The [human-work proposal](BPM-PLATFORM-HUMAN-WORK-PROPOSAL.md) selects one atomic public contract for current cross-instance tasks, private exact observation locators, actor claims, one typed field, retry-safe completion, platform audit, and a CSS-Modules React inbox. Its first cold proposal review returned `APPROVE WITH REQUIRED EDITS`; the correction materially redesigned completion serialization, fake-policy visibility, and shared contract ownership, so the resulting complete contract requires a new context-cold proposal review. Product 2 implementation may not consume metadata until E2 closure is approved and the human-work proposal is owner-approved.
