...
1. **Next: finish the owner-approved M2 cyclic-control-flow semantic checkpoint.** Baseline preservation commit `47296c2` and the behavior-preserving checked-source graph-admission extraction `c406575` are complete. Cold review target `5789223`, same-reviewer correction audit `58417e0`, and owner re-approval on 2026-08-10 authorize the corrected condition ownership, general per-arriving-token merge relation, profile-specific exact-one reachability, ordering, and owner inventory. Resume the paused semantic-core, BPMN-source, Lean, schema, artifact-consistency, and Temporal host-admission lanes, then stop at the first committed green semantic checkpoint for context-cold review.
