...
1. Implement the approved [User Task discovery/completion capsule](USER-TASK-DISCOVERY-COMPLETION-CAPSULE.md) in the same fast lane: versioned neutral scenario and CIB probe → Lean transition and law/non-law → executable IR extension and pure semantic core → Temporal Query/Update mechanics → differential, refinement, replay, and seeded-mutation evidence.
...
- Next implementation target: add the versioned neutral User Task interaction scenario/profile and retained CIB discovery/completion witnesses defined by the approved capsule; no Search Attribute, inbox, actor, assignment, variable, or form semantics is approved.
