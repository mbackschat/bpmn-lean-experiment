...
The engine and BPM platform remain separate MIT products under [PROJECT-DESIGN.md](PROJECT-DESIGN.md#product-division) and the package boundaries in [ARCHITECTURE.md](ARCHITECTURE.md). M0 through M6 and Horizon 1 of the owner-approved [Temporal BPMN execution scalability proposal](TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md) are closed. Product 2 now has one PostgreSQL 18 shared mode with replicated API and recovery-worker composition, while local mode retains its focused single-node contract. [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) owns the exact implemented and absent surfaces; the graduated [specifications](README.md) and closure-reviewed [shared-persistence proposal](BPM-PLATFORM-SHARED-PERSISTENCE-AND-PROJECTION-PROPOSAL.md) own closed contracts and evidence. The evaluation Compose distribution and screenshot-backed maintained browser walkthrough are complete. The owner approved the bounded Horizon 2 Workflow-chain proposal on 2026-08-17. Product 1 starts now enroll in the versioned chain contract, return no SDK handle, recover content-bound User Task, Message, Retry, and Cancel commands through the latest Run, expose only the closed v1 terminal receipt, and traverse paired E1/E2 publication segments across retained Runs without exposing Run identity. The reviewed cyclic User Task witness supplies the three-Run/two-boundary proof. The remaining capacity matrix, forced Message/Timer/effect rollover evidence, deployment admission, and closure review remain open.
...
**The owner-approved Horizon 2 Workflow-chain contract has production-default enrollment, chain-relative Product 1 command recovery, versioned terminal receipts, retained legacy-receipt normalization, and private paired E1/E2 publication traversal implemented.** Implement the remaining capacity matrix next. Keep Message/Timer/effect forced rollover evidence and stop-the-world deployment admission as later closure lanes.
...
**Latest evidence:** The production Workflow-chain integration passed the complete Temporal gate with 225/225 tests. Product starts carry the exact production Initial envelope; the raw Workflow result is opaque; the adapter decodes either the private v1 terminal envelope or the exact retained legacy receipt; the public receipt contains no Message ledger; and Product 1 recovery preserves exact retry/conflict precedence through the latest Run. Production Message witnesses assert the chain patch, exact Signal history, and ordered private recovery entries. The reviewed first-green checkpoint proves recursive continuation validation, terminal fencing, three Runs, two Continue-As-New boundaries, and replay of every Run. The paired publication witness proves Run-local E1/E2 segments `0..8`, `8..12`, and `12..16`, cursor pages at `0`, `4`, `8`, `12`, and `16`, two cross-Run occurrence identities, an unchanged public receipt, private Run identity, and replay of every Run. Horizon 1 closure target `36c3c75` passed context-cold review after correction audit `07c68d5`; the correction rejects future and missing nonterminal Work observation times. M6 closure target `c1ee125` passed context-cold review after correction audit `7c35fd1`. Its correction closes complete-form validation ordering, inert dynamic field keys, and the default Work bundle boundary; the complete default Work static-import graph is 462,859 bytes, down from 464,930 before M6. Product 2 evidence covers strict catalog projection and canonical bytes, definition persistence, Zod-backed computation, exact retry/conflict and zero-mutation rejection, HTTP 422, server composition, the retained expense-exception corpus row, and the exact 1280/1600 Chromium journey for all three actions and every field kind.
