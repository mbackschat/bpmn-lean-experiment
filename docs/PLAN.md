...
Next action: complete the repository-wide rerun and independent correction audit of the [finite prepared arming checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#finite-prepared-arming-checkpoint), then continue in the approved [dependency order](INTERNAL-COMMUTATION-PROPOSAL.md#selected-final-closure-account). The first full gate exposed an overbroad snapshot exclusion; the correction restores existing TypeScript ordinary-only batches without extending composed-data execution or Lean proofs. Complete core/source gates pass; two documentation review findings are corrected pending audit. Prepared execution, finite state/publication laws, bounded fixtures, and [immutable resource calibration](CAPSULE-COST-LEDGER.md#finite-prepared-arming-checkpoint-calibration) are integrated. Regions, scheduled choice, and final closure remain open. The [cold library memory correction](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction) retains the unchanged 3 GiB ceiling.
