...
Next action: complete bounded Sub-Process arming under the [complete-family account](INTERNAL-COMMUTATION-PROPOSAL.md#complete-operation-family-census): complete preparation frames, canonical state, successor validity, and accepted finite-batch publication. Valid task/Timer probes reproduce rollback. Reuse existing selectors and frame owners; reassess before substantial expansion, with no helper-only checkpoint. Confirm increment paths and use the complete Product 1 gate for engine-only integration; [the cost reflection](CAPSULE-COST-LEDGER.md#timer-task-arming-2026-09-22) owns the gate-selection correction. Regional trial and RC scope remain unchanged.
