...
Next action: implement the independently approved [selected-join readiness dependency amendment](INTERNAL-COMMUTATION-PROPOSAL.md#selected-join-readiness-dependency-amendment), including its population atom, complete readiness reads, and regional writer obligations. Then integrate token-producing local control batches with complete sibling preparation, canonical token multiplicity, and accepted publication. The approved [local-control prerequisite](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite), [canonical token-storage checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-token-storage-checkpoint), and [place-wide token-owner census checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#place-wide-token-owner-census-checkpoint) supply the existing helper, algebra, and token-selection boundaries. Regions, scheduled choice, and final closure remain open; the [cold library memory correction](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction) retains the unchanged 3 GiB ceiling.
