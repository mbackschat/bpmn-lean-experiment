...
The engine and BPM platform remain separate MIT products under [PROJECT-DESIGN.md](PROJECT-DESIGN.md#product-division) and the package boundaries in [ARCHITECTURE.md](ARCHITECTURE.md). M0 through M6 and Horizon 1 of the owner-approved [Temporal BPMN execution scalability proposal](TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md) are closed. Product 2 now has one PostgreSQL 18 shared mode with replicated API and recovery-worker composition, while local mode retains its focused single-node contract. [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) owns the exact implemented and absent surfaces; the graduated [specifications](README.md) and closure-reviewed [shared-persistence proposal](BPM-PLATFORM-SHARED-PERSISTENCE-AND-PROJECTION-PROPOSAL.md) own closed contracts and evidence. The evaluation Compose distribution and screenshot-backed maintained browser walkthrough are complete. This is the natural post-MVP pause; selecting another product increment requires owner direction.
...
1. **Blocked on owner direction: select the next post-MVP increment.** Horizon 1, the containerized evaluation distribution, and the screenshot-backed text-first walkthrough are complete. Do not infer the next semantic, platform, distribution, or capacity increment from the remaining roadmap.
...
**The repository is at a natural post-MVP pause.** Select the next increment from the owner-approved roadmap before changing semantic, platform, distribution, or capacity scope.

Item 1 is **blocked on owner direction**. The completed evaluation distribution builds and starts four production-closure project images with separate migration/runtime PostgreSQL credentials, health-gated Product 1 and Product 2 runtimes, one public API/web origin, and named PostgreSQL/Temporal volumes. Its maintained text-first walkthrough embeds an exact ten-image catalog generated through one isolated public-UI-only Playwright journey. Horizon 1 remains closed at context-cold review target `36c3c75` after correction audit `07c68d5`. Local mode retains filesystem/SQLite and database-free package loops. Shared mode uses the epoch-9 PostgreSQL catalog, replicated API, and exactly eleven bounded recovery families with no startup population scan or request-time fleet Query fan-out.
