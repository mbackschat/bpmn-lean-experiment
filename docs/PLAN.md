...
Next action: continue [local-control batch integration](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite). Complete preparation, mixed-pair frames, arbitrary finite prefix validity, exact checked/actual execution and canonical accepted-publication commutation, and the mixed kernel discriminators are implemented. Finish immutable bounded calibration, then checkpoint review and production integration. The [Inclusive pairing repair](capsules/INCLUSIVE-GATEWAY-SPEC.md#lean-assurance-lane), token-patch validity, and selected-record algebra retain checkpoint approval.
