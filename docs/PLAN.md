...
2. **Then — three increments the preserve-only closure review deferred rather than dropped:** each is scoped, none changes BPMN meaning, and the first two were verified as genuinely deferred rather than overlooked. **Locate the two remaining classification loci:** an unadmitted own property on an executed flow node, and the three readers admitting one hand-selected shape, both of which refuse safely today while naming no element; this needs a shared per-type key inventory extracted from [the projection predicates](../packages/bpmn-source/src/checked-element-projection.ts) and flow elements classified before projection, which widens where classification is decided and therefore opens its own review. **Compress [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) by auditing each passage against the owner that holds it**, which is the mechanism [the reviewability guard](../scripts/document-reviewability.test.ts) names in its own comment. Produce a candidate inventory naming each passage's real owner and its projected recovery *before* editing; a vocabulary proxy puts 2022 words across 91 bullets inside a single capsule, but that is a candidate list requiring per-passage judgement rather than a deletion list. **Three** capsules delegate their implemented and absent scope to the map — the interrupting Activity, interrupting Sub-Process, and non-interrupting boundary Timers — and all three keep their current scope. Recovery should exceed 1000 words without touching them: the `### BPMN conformance` subsection alone is 1551 words, most of it the Sub-Process boundary Timer block, against 347 for the comparable Activity block. Take it from chronology, restated semantic laws, catalog-derived case and profile detail, and capsule-owned exclusions, and record the resulting count and headroom. **Fix the delegation guard in the same change**, because it is what makes the audit safe and it does not work: its regex matches only the exact phrase `recorded in [IMPLEMENTATION-MAP.md]`, so it sees one of the three capsules while its own comment says two, and its assertion accepts any occurrence of the capsule filename anywhere in the map rather than a map-owned status section — both other filenames already appear under `## Nearest unsupported claim`, so their status blocks could be deleted with the gate still green. Replace the prose regex with a stable delegation marker or an explicit registry. **Give [the cost-ledger rank guard](../scripts/capsule-cost-comparison.test.ts) a table-contiguity assertion**, since it parses rows by prefix and stayed green over a row that a blank line had pushed out of the table it compares against.
