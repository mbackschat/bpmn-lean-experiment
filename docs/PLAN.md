...
**Status: in progress.** The resumption-bounded cyclic-control-flow increment is implemented and evidence-complete pending closure review. Message Start Event, Timer Start Event, Terminate End Event, the configured generic Task, and the platform increments remain.
...
1. **Next: close the M2 cyclic-control-flow capsule.** Submit the committed closure-review correction and its fresh target-bound full-gate evidence to the same reviewer. If the correction audit approves it, record the closure receipt and graduate the proposal to a specification before selecting the next M2 mechanism.
...
**Next action: close the implemented M2 cyclic-control-flow capsule.** The cold closure review of target `4251dc6` required executable candidate-output and occurrence-identity mutations plus status/evidence corrections. Those corrections are implemented with test-only candidate-output swap, element-ID-only Workflow, and reset-to-one Workflow discriminators. Submit the committed correction and fresh target-bound full-gate evidence to the same reviewer; after approval, record the closure receipt and graduate the proposal. The mandatory first-change Lean audit passed under the standing one-CPU, no-swap, 3 GiB cgroup ceiling with Lean 4.31.0; the admission target completed in 18.93 seconds with 2,606,672 KiB GNU maximum RSS and 2,400,399,360 bytes cgroup peak memory. M1 remains closed through the public API and UI over real Temporal hosting. The A12 product boundary remains closed, its stable contract is active, and its proposal is archived.

**No owner blocker.** The owner approved corrected proposal target `58417e0` on 2026-08-10 after cold review target `5789223` and its same-reviewer correction audit closed all four findings. The correction preserves the selected M2 topology while preventing its exact-one profile restriction from redefining general BPMN Multi-Merge semantics. The earlier approval of concrete `@temporalio/client@1.21.0` Product 2 reachability and the no-umbrella Temporal subsystem package layout remains binding.

**Last verified commands:** clean closure-review target `4251dc6` passed the complete engine verifier in 162.10 seconds on 2026-08-10 with 38 live Temporal histories and `dirty:false`; its cycle witness recorded 25 events and 10,340 run-specific service-reported bytes. The complete platform M1 gate passed in 14.53 seconds, the registered M1 showcase passed in 9.11 seconds, and the infrastructure gate passed 218/218 in 37.25 seconds. The bounded pre-M2 Lean measurements passed both split lanes under 3 GiB at 2,682,840 KiB and 1,921,416 KiB maximum RSS respectively. The showcase admits and renders runtime-created third-party source, advances changed source to version 2, starts selected version 1 through the concrete Temporal client while the production Worker polls the same Task Queue, and rejects an unsupported element without advancing the catalog.
