...
**Status: in progress.** The resumption-bounded cyclic-control-flow, Message Start Event, Timer Start Event, Terminate End Event, and [configured Task extension](capsules/CONFIGURED-GENERIC-TASK-SPEC.md) increments are implemented, closure-reviewed, evidence-closed, and graduated. The M2 engine boundary is complete. Platform definition scheduling for Timer Start is selected next; published message ingress and instance search follow.
...
**Last verified commands:** `/usr/bin/time -p env CI=true ./scripts/verify.sh` passed at immutable configured Task closure target `e1f81ca` in 313.91 seconds with output SHA-256 `30b7773e68114858b0d2c0248e2e58f1c7df42f369f22d857012f86c5065b04d`; all 42 registered cases, 42 seeded mutations, 18 retained-CIB comparisons, and 44 live histories passed. The 27391.305625ms warm pipeline total is correctness evidence only because the host was under substantial concurrent CPU load. The warm correction audit approved `8959a0c` after the configured Task closure guard, evidence ownership, and cost endpoint corrections passed their focused gates. The mandatory Linux/arm64 Lean admission audit remains green under Lean 4.31.0, one CPU, a 3 GiB memory-and-swap ceiling with effective swap disabled, and no OOM. The complete platform M1 gate and registered M1 showcase remain green.
