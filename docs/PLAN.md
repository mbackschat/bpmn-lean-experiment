...
Next action: finish the [subscription draft's open design obligations](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#open-design-obligations): source/operation consumer inventory, exact reachable propositions and effort bound, and shared standards-only registration contract. Its [diagnostic preflight](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#diagnostic-preflight-results) supports bounded source shapes, flagged SDK ordering, queued callbacks, and Timer handoff; it proves no production capability. The retained-body amendment must resolve `AOO-RETAINED-BODY-01` before subscribed child/Terminate admission. Complete independent proposal review before production work. Snapshot arming and pipeline correspondence are accepted; subscription/Transaction closure remains open. Preserve the bounded checkpoint and regional trial.
