...
**Next action: commit the verified exact-balanced-topology correction, then ask the same checkpoint reviewer for the required warm audit.** The cold review verified the immutable target and returned `approve-with-required-edits` with one blocker and no optional findings. The correction applies one reusable exact balanced two-branch topology invariant at checked and Semantic Process admission for both parallel profiles and retains the same-cardinality sequential-before-split/direct-to-join counterexample. After that audit, narrow the broad Lean imports that unnecessarily rebuild proof-heavy conformance modules, keeping the import-boundary refactor semantically separate and measuring it through one root-owned serialized build. Do not add combined CIB evidence, profile or scenario registration, full differential or live Temporal closure, or the Product 2 catalog journey until the warm audit approves the correction. In the engine lane, classify Script Task execution against BPMN, CIB, deterministic sandboxing, data, security, and Temporal requirements before deciding whether its two-family prevalence justifies a proposal. The remaining operator-history and audit-export increments stay ordered after this corpus decision gate.

Item 1 is **at its combined-profile and next-family research boundary**. The exact manifest, 22 retained models covering all 25 registered executable variants, seven external candidates across six clone families, provenance and catalog guards, Product 2 About disclosure, and first production browser journey are implemented. Product 2 readiness is one; 21 retained models remain excluded from its catalog.

**Latest evidence:** `doctor.sh research` and the 13-case corpus gate passed. Before review, the checkpoint's focused Lean build, 31 semantic-core tests, 42 BPMN-source tests, and two host-admission tests were green. For the correction, both TypeScript builds, checkpoint tests 9/9 and 7/7, predecessor regressions 9/9 and 12/12, and the 16-case Lean-wrapper guard are green. The root-owned `./scripts/lake.sh build BpmnSemantics.ParallelBalancedTopologyConformance` gate passed in 24.76 seconds with a 2,402,009,088-byte maximum resident set and zero swaps; the earlier unsafe direct umbrella build was terminated and is not evidence.
