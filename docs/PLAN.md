...
7. **Approved next — cross-language wire hardening:** atomically pin every JavaScript-number integer to the safe domain, specify and implement exact language-neutral identifier ordering without silent normalization, and lock unknown/missing fields, closed enums, `null` versus absence, duplicate keys, unsafe/non-integral numbers, and canonical arrays through TypeScript and Lean. Use red/green boundary tests and do not add a compatibility reader or dependency.
8. **Authorized preflight — Service Task CIB binding:** after item 7 is green, run the packaged-engine unknown-implementation, exact expanded-QName, bean-resolution, fail-once retry-decrement, and clean manual re-execution probes. Stop at the proposal boundary if any required fact fails.
...
Begin item 7 with red boundary cases for unsafe integers, non-ASCII identifier ordering, and strict JSON behavior in both TypeScript and Lean, then apply the one current wire shape atomically. After its focused and complete gates are green, run the packaged CIB Seven Service Task phase-zero probe and record its exact source/oracle facts before seeking the capsule decision. If the owner supplies the target-solution checkout meanwhile, record it in [SOURCES.md](SOURCES.md) and run the read-only inventory without delaying the probe. Keep D1 behind the Service Task adapter-feasibility work. Keep C2 frozen under the non-extension and reopening rules in [the experiment record](experiments/CHECKED-SOURCE-RELATION-EXPERIMENT.md); do not extend it with new capsules or silently treat the unresolved theorem as permanently waived. Keep the substantial `.mjs`-to-direct-`.ts` harness migration separate and deferred until after a direct Node typing dependency decision.
