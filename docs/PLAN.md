...
- Lean scope-creation aggregate validity. Command: `./scripts/pnpm.sh run test:pre-push`. Status: `exit 0`. Date: `2026-09-18`. Commit: `4f71f86132fda40182759e08b985002307526d4d`. The [review receipt](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) records independent approval with no required findings, the exact-target focused build, and the separately asserted clean complete gate. All 72 pipeline cases agree; complete preparation frames and commutation remain open.
...
Next action: prove complete preparation preservation through independent child and Call [scope-creation patches](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite). Derive each opposite preparation from the predecessor and declared footprint, including definition/instance population exclusions, exact token-owner reads, and issuance counters. Aggregate runtime-validity preservation is independently approved and completely verified; consume it without assuming successor validity. Then close commutation, accepted-publication correspondence, and production scope/region batching. Preserve exact complete-state equality, high-water records, existing predicate coverage, and the unchanged 3 GiB ceiling.
