...
Owner-approved on 2026-09-23: finish ordinary arming in the focused snapshot-aware Lean closure, a [Compensation registration prerequisite](capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md#public-registration-prerequisites). Preserve finite independent batches, complete prefix preparation, runtime/projectability preservation, canonical accepted publication, refusal precedence, and whole-command rollback. Connect the six ordinary arming families to the actual attempt, batch, publication, and replay path. The reviewed two-task Start frontier is the discriminator; stale preparation, conflicts, refusal, and insufficient fuel must still fail. The actual-runner permutation theorem now derives refusal absence through every prefix and canonical publication equality. Complete package/resource gates and independent review remain. Reassess before adding an aggregate invariant or substantially expanding prerequisites. Generic snapshot exclusion and public admission remain unchanged; future subscription/Transaction dependencies and the bounded Sub-Process checkpoint remain unresolved.
...
- Exact Merge and private scheduling. Command: `./scripts/pnpm.sh run test:pre-push:ui`. Status: `exit 0`. Date: `2026-09-22`. Commit: `28bac9007f4e2be8467923e756420de755c74593`. Pinned Chromium provisioning and all 83 browser tests pass. The [review receipt](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) combines this clean UI result with the unchanged successful engine, platform, PostgreSQL, and showcase lanes and semantic approval at `52ed7496`; the original failed umbrella receipt remains retained.
...
Next action: close the ordinary snapshot-aware arming outcome named in Ordered work under the approved [RC scope amendment](INTERNAL-COMMUTATION-PROPOSAL.md#rc-completion-scope-amendment). The source Start and fuel-rollback regressions pass kernel checks under 3 GiB. The actual-runner proof is green. Finish complete gates, resource binding, and independent review before reassessing selected RC scheduling dependencies. Future subscription/Transaction dependencies remain unresolved and block their admission and RC closure. Preserve the unfinished [bounded Sub-Process checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#bounded-sub-process-arming-outcome) and pending regional trial. Neither [proof-architecture experiment](experiments/INTERNAL-COMMUTATION-PROOF-ARCHITECTURE-EXPERIMENT.md) is adopted.
