...
Next action: prove the actual control-position delta agrees with the retained [scope-creation publication template](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite), including differently owned token units and the new complete scope. Then close accepted lifecycle publication and production scope/region batching. Complete-state commutation has independent approval and clean complete verification; it derives all successor validity from predecessor premises. Preserve high-water records, existing predicate coverage, and the unchanged 3 GiB ceiling.
