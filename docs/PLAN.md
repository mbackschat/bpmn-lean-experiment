...
**No modeler-produced BPMN file is admissible.** [The compiler](../packages/bpmn-source/src/checked-process-compiler.ts) admits an exact key allowlist, so Diagram Interchange, pools, lanes, and artifacts are rejected rather than preserved, and [root selection](../packages/bpmn-source/src/root-definition-selection.ts) requires exactly one root element. Every file a modeler saves carries DI. This is what M1 of the ladder below exists to change, and it is the gap between a working engine and a shippable one.
...
1. **In progress — implement M1's preserve-only admission:** [the proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md) is owner-approved as of 2026-08-08 and fixes the executed, preserved, and rejected partitions, the diagnostic contract, the one-executable-entry rule, and the profile successor. Its declared Lean lane is `checked`, its unresolved boundary is recorded in D5, and it answers the C2 reopen trigger without reopening. The behavior-preserving extraction of [the checked-process compiler](../packages/bpmn-source/src/checked-process-compiler.ts) lands as its own commit before any semantics, because 82 nonblank lines of headroom will not hold the classifier calls, the partition assembly, and the diagnostic list. A semantic-checkpoint review is **required** at the first green implementation checkpoint.
...
**Latest complete gate: `9e08b4a`, 2026-08-07, exit 0 on a clean tree.** All **34** registered cases reached agreement across their declared targets, all 34 seeded semantic mutations produced their required disagreement, and all 18 retained-CIB evidence comparisons agreed. Its 20406.710ms warm total reported the soft target and is **correctness evidence only**, not a baseline: the run shared a host with earlier gates in the same session.
...
**Next action: split [the checked-process compiler](../packages/bpmn-source/src/checked-process-compiler.ts) as a behavior-preserving commit, then implement ordered-work item 1.** The proposal is owner-approved as of 2026-08-08 and its three open decisions are decided in D2 and its open-decisions section. The extraction lands first because the compiler holds 82 nonblank lines and the classifier, the partition assembly, and the diagnostic list will not fit under a size squeeze.

**Blockers: none.** `checked-process-compiler.ts` at 518 nonblank is the only owner near its ceiling; `compile.ts` holds 284, `root-definition-selection.ts` 410, and `contracts.ts` 530. `semantic-process-runtime.ts` holds 249 after command admission moved to [its own owner](../packages/semantic-core/src/semantic-command-admission.ts), and `workflow-implementation.ts` holds 50.

**Last verified command:** `./scripts/verify.sh` at `9e08b4a`, exit 0, clean tree, 34 agreements and 34 seeded mutations detected. Its timing is contended and is not a baseline; see [the baselines above](#last-verified-baseline).
