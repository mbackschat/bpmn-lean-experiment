...
- Lean scope-creation preparation frames. Command: `./scripts/pnpm.sh run test:pre-push`. Status: `exit 0`. Date: `2026-09-18`. Commit: `b8213b4106b2e0b62ca9ecf352ed637d001196fb`. The [review receipt](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) records independent approval without findings, the exact-target focused build, and the separately asserted clean complete gate, including the preceding timing-margin failure and unchanged retry. All 72 pipeline cases agree; commutation and accepted publication remain open.
...
Next action: prove exact complete-state commutation for independent child/Call creations, using the accepted [preparation frame and aggregate-validity laws](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite). The frame checkpoint has independent approval and clean complete verification. Derive Call-list equality from valid unique anchors and canonical order rather than assuming its partial comparator orders arbitrary payloads. Then close accepted-publication correspondence and production scope/region batching. Preserve high-water records, existing predicate coverage, and the unchanged 3 GiB ceiling.
