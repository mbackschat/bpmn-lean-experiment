...
1. **In progress: remove Product 2's single-node scale boundary.** The complete PostgreSQL persistence graph, exact migration application through epoch 7, Audit source-to-sink completeness, generic bounded lease kernel, all eleven candidate families, and ten lease-fenced recovery families are Green without adding a database to the ordinary package loop. Complete the incident snapshot generation and remaining freshness-bounded reads, then compose the recovery worker and shared-mode API and close concurrent multi-replica evidence. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.
...
**The next implementation boundary is incident snapshots.** Add gap-free population, immutable bounded generations, fenced one-Process replacement, and one-statement reads without changing bodies or authorization order. Then add append-only freshness reads and compose the worker and shared API. Review target `c9cf16e` and correction audit `e03240c` bind this boundary.

Item 1 is **in progress** and unblocked. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a` through `f180c3b` implement the approved dependencies, runtime, Promise repositories, PostgreSQL artifacts and business repositories, exact Audit sinks, the six-owner migration application through ordinal 0007 and epoch 7, generic leases, all eleven candidate families, bounded Audit batches, and ten lease-fenced recovery families: four Definitions lifecycles, incident action, both ordered Audit streams, committed execution, flow-node occurrence, and Work snapshot. Work now allocates a committed gap-free population, builds immutable bounded generations through one-Process lease-fenced observations, and proves complete age-bounded task coverage plus live claims in one PostgreSQL statement. Cross-backend evidence covers exact Unicode, gap-free allocation, canonical values, suffix-only projections, terminal monotonicity, concurrent retries, Audit completeness, lease reclaim, stale-token refusal, same-lease start dispatch, atomic subscriber delivery, terminal publication without host redispatch, and stale-generation refusal. Ordinary checkpoints remain database-free; PostgreSQL 18 uses named witnesses. The incident snapshot family, append-only freshness reads, both process compositions, and multi-replica closure are incomplete. No capacity claim exists.
