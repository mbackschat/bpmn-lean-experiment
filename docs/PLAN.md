...
**Next action: open ordered-work item 3, the preserve-only admission proposal for M1.** Nothing about it is started. It is a material admission-capability change, so it begins with a governed proposal cycle and no implementation. The proposal must fix the executed, preserved, and rejected partitions and the diagnostic contract, declare its Lean lane shape, and decide the C2 reopen that M1 triggers by widening admission beyond the two fixture-pinned topologies.
