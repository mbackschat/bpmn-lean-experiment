...
**One profile now admits modeler notation.** `cibseven-2.2.0-user-task-preserved-notation-draft` retains Diagram Interchange, pools, lanes, artifacts, and documentation without executing them; every other profile still rejects them through an exact key allowlist. The profile is not yet a registered artifact, so its evidence is focused-gate only. Closing M1 means registering it and giving a rejected file per-element diagnostics a third party can act on.
...
1. **In progress — implement M1's preserve-only admission:** [the proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md) is owner-approved as of 2026-08-08. The recursive classifier, the one-executable-entry rule, the preserved set of D2, and D5's non-interference evidence with its seeded defect are implemented. Still owed: the atomic profile artifact, scenario, and pipeline-case registration; D3's per-element diagnostics; and the structural requirement-ledger rows. A semantic-checkpoint review is **required** and not yet obtained; the implementation has passed its first green checkpoint, so it is now due.
...
**Next action: register the preserve-enabled profile as an artifact, then add per-element admission diagnostics.** The classifier, the one-executable-entry rule, and the non-interference evidence landed at `3903d2a`, `73780a6`, and `d8ec4ec`. `cibseven-2.2.0-user-task-preserved-notation-draft` exists in code and in the admission capability table but has no `profiles/` artifact, scenario, retained evidence, or pipeline case, so its evidence is focused-gate only; that registration is one atomic change across the roundtrip and catalog guards. D3's per-element diagnostics remain unimplemented.

**Blockers: the artifact registration needs a Temporal host port,** because `PipelineCase.temporalRelation` is non-nullable and a registered scenario cannot skip that target. Request port-binding authorization before the first `./scripts/verify.sh` or `test:pipeline` attempt. No owner is near its ceiling: `checked-process-compiler.ts` holds 350 nonblank lines of headroom after its extraction, the new classifier 330, and `workflow-implementation.ts` 50.
