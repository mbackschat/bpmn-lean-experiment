...
Next action: draft and independently review the RC subscription-lifetime account from the [dependency findings](INTERNAL-COMMUTATION-PROPOSAL.md#subscription-dependency-findings): exact admission, repeated identity/lifetime, race linearization, scope cleanup, and Timer handoff/rollover. Derive scheduling obligations from its reachable frontiers before implementation. Snapshot arming and private pipeline correspondence are accepted; future subscription/Transaction combinations remain unresolved. Preserve the bounded Sub-Process checkpoint and regional trial. No additional proof infrastructure is selected.
