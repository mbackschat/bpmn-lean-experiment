...
**Next action: continue ordered-work item 1 with the narrowed definition-version start boundary.** The public raw-XML deployment API, exact-source reads, Node server composition root, React definition workspace, licensed viewer-only `bpmn-js` adapter, and required headless Chromium acceptance are implemented. Playwright and its browser binary are development/CI-only. The A12 product boundary is closed, its stable contract is active, and its proposal is archived.

**Blocker: one dependency decision.** The owner approved `bpmn-js` 18.22.1 with its bpmn.io watermark and notice obligations, exact React 19.2.8/React DOM 19.2.8/Vite 7.3.6 web packages, and development-only Playwright automation on 2026-08-09. The start lane still needs explicit owner approval for the narrow product-1 Temporal client dependency and the resulting platform reachable-package budget increase; importing the current broad Temporal adapter would pull test and Worker packages into the server graph and is not the recommended boundary.

**Last verified command:** `CI=true ./scripts/pnpm.sh run test:platform-web:e2e`, exit 0 in 8.2 seconds on the M1 browser-acceptance worktree. Its one Chromium case admitted and rendered a runtime-created third-party source, advanced a changed source to version 2, and rejected an unsupported element without advancing the catalog. The preceding complete M1 platform gate passed its 78 focused checks, and the repository infrastructure gate passed 207/207 checks. The last complete engine gate remains `/usr/bin/time -p env CI=true ./scripts/verify.sh`, exit 0 in 149.71 seconds on commit `adf757b`, with output SHA-256 `c7a3a47f18c27163a723900e403da718eb6982e84089c754401ce913c7efd6f8`.
