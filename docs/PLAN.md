...
**Status: in progress.** The engine admission slice, narrow compilation and exact-version start gateway, exact artifact byte store, strict public definition contract, durable per-process versioning workflow, public definition API, server composition, React definition workspace with viewer-only diagram rendering, and required headless Chromium acceptance are implemented. The UI start action and executable showcase are absent, so the milestone has not reached its exit gate.
...
1. **In progress: complete the open M1 showcase.** The public definition API and HTTP-only React definition workspace expose deployment, listing, diagnostics, exact source retrieval, and viewer-only diagram rendering, required headless Chromium acceptance exercises an unseen runtime-created source, and the exact-version start API is bound to the concrete Temporal client. Add the UI start action and registered end-to-end showcase. Close M1 only when that exit gate proves an externally supplied file is admitted, stored, versioned, rendered, and started while an unsupported file is rejected before Workflow start with its element identity.
...
**Next action: continue ordered-work item 1 by adding the UI start action and registered M1 end-to-end showcase.** The exact-version public start route, stored-artifact integrity checks, pre-start source/profile/Process binding, handle-free concrete Temporal client call, lazy server-owned client lifecycle, and Product 2 Temporal dependency guard are implemented. The showcase must compose production deployment, versioning, exact-source rendering, a real Worker and client start, and the existing located rejection through public HTTP; it may use test-only Temporal infrastructure but may not move reusable behavior under `showcase/`. Playwright and its browser binary remain development/CI-only. The A12 product boundary is closed, its stable contract is active, and its proposal is archived.
...
**Last verified command:** `/opt/homebrew/bin/gtimeout 60s env CI=true ./scripts/pnpm.sh run test:platform-m1` passed in 18.21 seconds with the pnpm-native production licence report, exact-version start boundary, lazy server-owned Temporal client, and all M1 packages green. The Product 2 and Temporal package-boundary guards, source hygiene, plan consistency, Markdown links, and scoped diff check also pass. The required headless Chromium acceptance passed in 8.72 seconds: it admits and renders runtime-created third-party source, advances changed source to version 2, and rejects an unsupported element without advancing the catalog, but does not yet invoke the new start route. The last complete engine gate remains `/usr/bin/time -p env CI=true ./scripts/verify.sh`, exit 0 in 149.71 seconds on commit `adf757b`, with output SHA-256 `c7a3a47f18c27163a723900e403da718eb6982e84089c754401ce913c7efd6f8`.
