...
**Next action: perform the required context-cold semantic-checkpoint review of the first green lifecycle, wire, and Query checkpoint from the owner-approved [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md).** Product 1 client/API exposure, live retention and replay evidence, and every Product 2 metrics owner remain paused until that review and any correction audit close. The remaining operator-history and audit-export increments stay ordered after this capsule.

Item 1 is **at its first green Product 1 checkpoint after owner approval**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and M5 E1 is closed through the graduated [committed execution publication specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md). The TypeScript and Lean lifecycle relation, strict additive wire, replay-stable command-batch time, immutable Workflow accumulator, and E1-aligned Query are green. The required semantic-checkpoint review is now the governing pause. Event History, state differencing, Product 2 ingestion time, and semantic logical time remain forbidden substitutes for an exact public wall-clock duration fact.
