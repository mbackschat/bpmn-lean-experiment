...
**The owner-approved Horizon 2 Workflow-chain contract has production-default enrollment, chain-relative Product 1 command recovery, versioned terminal receipts, and exact retained legacy-receipt normalization implemented.** Implement private publication-segment traversal next, preserving contiguous E1/E2 revisions and retained open-occurrence anchors across Runs without exposing Run identity. Keep the remaining capacity matrix, Message/Timer/effect forced rollover evidence, and stop-the-world deployment admission as explicit later closure lanes.
