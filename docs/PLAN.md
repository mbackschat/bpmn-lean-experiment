...
Next action: complete independent checkpoint review and the clean repository-wide gate for the [composed Activity-data footprint contract](INTERNAL-COMMUTATION-PROPOSAL.md#final-closure-evidence-and-adversarial-oracles). The [cold library memory correction](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction) and [immutable calibration](CAPSULE-COST-LEDGER.md#composed-arming-footprint-checkpoint-calibration) passed under the unchanged 3 GiB ceiling with zero memory events and pressure totals. Pairwise preparation, preservation, execution, and canonical publication are proved from predecessor invariants and noninterference. Retain the unavailable production footprint until approval; then integrate execution and close arbitrary finite batches, regions, and scheduled choice in the approved [dependency order](INTERNAL-COMMUTATION-PROPOSAL.md#selected-final-closure-account).
