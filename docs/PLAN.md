...
Next action: repair the selected-join dependency gap identified by the [local-control preparation prerequisite](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite), then integrate token-producing local control batches with complete sibling preparation, canonical token multiplicity, and accepted publication. The approved [canonical token-storage checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-token-storage-checkpoint) supplies the insertion/removal laws, while the [place-wide token-owner census checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#place-wide-token-owner-census-checkpoint) supplies the dependency atom for token-owner selection. Regions, scheduled choice, and final closure remain open; the [cold library memory correction](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction) retains the unchanged 3 GiB ceiling.
