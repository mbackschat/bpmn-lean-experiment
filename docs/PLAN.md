...
1. **In progress: remove Product 2's single-node scale boundary.** The complete PostgreSQL persistence graph, exact migration application through epoch 8, Audit source-to-sink completeness, generic bounded lease kernel, and all eleven candidate and lease-fenced recovery families are Green without adding a database to the ordinary package loop. Complete the append-only freshness reads, then compose the recovery worker and shared-mode API and close concurrent multi-replica evidence. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.
...
**The next implementation boundary is append-only projection freshness.** Add database-clock completion facts to committed execution and occurrence heads, then serve exact-population metrics and per-instance reads from one PostgreSQL statement without changing bodies or authorization order. Do not add a metrics generation table or a twelfth recovery family. Then compose the worker and shared API. Review target `c9cf16e` and correction audit `e03240c` bind this boundary.

Item 1 is **in progress** and unblocked. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a` through `20e1e3f` implement the approved dependencies, runtime, Promise repositories, PostgreSQL artifacts and business repositories, exact Audit sinks, the six-owner migration application through ordinal 0008 and epoch 8, generic leases, all eleven candidate families, bounded Audit batches, and all eleven lease-fenced recovery families. Work and incident snapshots now allocate committed gap-free populations, build immutable bounded generations through one-Process observations, and prove complete age-bounded coverage in one PostgreSQL statement. Cross-backend evidence covers exact Unicode, gap-free allocation, canonical values, suffix-only projections, terminal monotonicity, concurrent retries, Audit completeness, lease reclaim, stale-token refusal, same-lease start dispatch, atomic subscriber delivery, terminal publication without host redispatch, stale-generation refusal, and freshness headers only on successful shared reads. Ordinary checkpoints remain database-free; PostgreSQL 18 uses named witnesses. Append-only freshness reads, both process compositions, and multi-replica closure are incomplete. No capacity claim exists.
