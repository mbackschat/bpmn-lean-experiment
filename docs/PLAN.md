...
Next action: validate and commit the [mutation-Worker startup correction](../packages/temporal-adapter/testkit/src/bypass-mutation.ts), then separately fix retained-mutation termination failure skipping Worker shutdown. Run clean integration before accepting the reviewed [regional/arming outcome](INTERNAL-COMMUTATION-PROPOSAL.md#regionalarming-outcome). Retain all attempts in the [cost comparison](CAPSULE-COST-LEDGER.md#regionalarming-outcome-preparation-2026-09-21). The [completeness guard is deferred](PROCESS-ASSESSMENT-LEDGER.md#finding-63). Remaining pairs/batching stay open; E1/E2 change no RC scope.
