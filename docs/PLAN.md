...
9. **Focused refinement green; differential closure next — Service Task Activity capsule:** checked source, shared contracts, Lean, the independent TypeScript core, transport identities, Activity execution, retry reconciliation, typed exhaustion, Worker replacement, bypass detection, and focused replay are green under the owner-approved [Service Task effect proposal](capsules/SERVICE-TASK-EFFECT-PROPOSAL.md). The next implementation step is the eighth answer-free scenario, ordinary CIB runner path, retained evidence, and complete differential matrix.
