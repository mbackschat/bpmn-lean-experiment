...
1. **In progress: remove Product 2's single-node scale boundary.** The complete PostgreSQL persistence graph, exact migration application, Audit source-to-sink completeness, and generic bounded lease kernel are Green without adding a database to the ordinary package loop. Add the eleven domain-owned recovery families one bounded group at a time, beginning with confirmed registration, direct start, Schedule, and Message Start; then compose the recovery worker, shared-mode API, freshness-bounded projection reads, and concurrent multi-replica closure evidence. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.
...
**The exact next implementation boundary is the first domain recovery group.** Add bounded candidate discovery and single-item reconciliation for confirmed registration delivery, direct start, Schedule, and Message Start. Correct the terminal-publication omission so a `started` Schedule or `accepted` Message Start remains recoverable after downstream confirmed-publication failure. Keep host work outside database transactions, preserve no-redispatch and exact content binding, and do not yet compose the worker or shared API. Review target `c9cf16e` and correction audit `e03240c` bind the repositories, artifact store, repair families, request-time fan-out, projection freshness, audit completeness, and same-database publication invariant.

Item 1 is **in progress** and blocked on nothing. On 2026-08-16 `./scripts/doctor.sh verify` passed with the pinned toolchain and verified OMG corpus. Commits `e658d3a`, `5deb00d`, `58cacb7`, `78c59a4`, `8c1e4eb`, `d8c610e`, `087fe33`, `cb11e90`, and `c43ce17` implement the dependency set, bounded runtime, Promise-only repositories, PostgreSQL artifacts and all twelve business repositories, exact Audit sinks, the explicit six-owner migration application, schema epoch 6, and the generic recovery lease kernel. Cross-backend evidence covers exact Unicode including U+0000, rollback-safe gap-free allocation, canonical values, suffix-only projections, terminal monotonicity, exact concurrent retries, strict audit completeness, two-runtime disjoint claims, database-clock reclaim, and stale-token zero-callback behavior. Ordinary package and platform checkpoints remain database-free; PostgreSQL 18 runs only through named witnesses. Domain recovery composition, shared API composition, freshness-backed reads, and multi-replica closure remain incomplete. No capacity claim exists.
