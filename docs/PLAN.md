...
The M5 flow-node occurrence metrics increment is implemented, closure-reviewed, evidence-closed, and graduated. Its Product 1 publication, Product 2 projection and aggregation, and local 1280/1600 functional UI lane are green. The active checkpoint is now the curated executable BPMN model corpus. Its MVP suite contains 22 retained business-purpose-shaped project models that cover all 25 executable BPMN element or semantic variants registered by the production pipeline, plus seven external candidates across six deduplicated CIB Seven, OMG, and Betsy families. One model is catalog-ready: the exact User Task assignment/form model now has a production-preview Chromium journey through deployment, start, claim-gated work, completion, Operations History, and Work audit. Product 2 also exposes the exact package version, restrictions, and separately classified CIB evidence through the About capability table.
...
1. **In progress: Evolve the curated executable BPMN model corpus before selecting the next semantic family.** The manifest and generated report bind 22 retained business-purpose-shaped project models covering all 25 executable element or semantic variants in the registered pipeline, plus seven exact external files across six CIB Seven, OMG, and Betsy clone families. The guard derives support from pipeline XML and fails unless the canonical capability/restriction catalog and retained-model union match it exactly; the Product 2 About page consumes that same catalog. The retained suite passes the existing Lean, semantic-core, selected CIB, Temporal, replay, and mutation pipeline; the external tranche verifies Git revisions or archive and entry digests, reruns exact compiler admission, and ranks clone-family-deduplicated gaps without retaining external bytes. The exact assignment/form model is catalog-ready through a CIB-preflighted production-preview Chromium journey. The exact parallel model remains engine-only because its metadata-free User Tasks are intentionally inoperable in Product 2. The owner-approved [parallel User Task metadata composition proposal](capsules/PARALLEL-USER-TASK-METADATA-COMPOSITION-PROPOSAL.md) has reached its first private green semantic checkpoint and is paused at the required independent checkpoint review before CIB, differential, live Temporal, or Product 2 registration. For the next semantic-family decision, assess Script Task execution's two-family practical reach against the co-equal high-risk semantic families named below; prevalence alone does not select it. No model may enter the browser catalog before its exact advertised workflow passes locally. The report never overrides BPMN dependencies, creates a combined conformance percentage, or turns an external engine into semantic authority.
...
**Next action: obtain the required context-cold semantic-checkpoint verdict for the committed first green implementation of the owner-approved [parallel User Task metadata composition proposal](capsules/PARALLEL-USER-TASK-METADATA-COMPOSITION-PROPOSAL.md).** The exact source and private successor profile, strict checked-source and Semantic Process admission, proved Lean composition owner, independent semantic-core evidence, empty-only Process Start rule, and passive-pair host-capability witness are implemented. The target-bound full verifier passed every lane before its final differential pipeline; that pipeline and one isolated rerun reached zero failed assertions but hit the existing aggregate timeout while CIB Maven exceeded its soft targets under external workstation contention. Keep the existing budgets and carry both exact records into review. Do not add combined CIB evidence, profile or scenario registration, full differential or live Temporal closure, or the Product 2 catalog journey until that verdict approves the checkpoint. In the engine lane, classify Script Task execution against BPMN, CIB, deterministic sandboxing, data, security, and Temporal requirements before deciding whether its two-family prevalence justifies a proposal. The remaining operator-history and audit-export increments stay ordered after this corpus decision gate.

Item 1 is **at its combined-profile and next-family research boundary**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and the current M5 observation increments are closed through the graduated [committed execution publication](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) and [flow-node occurrence metrics](capsules/FLOW-NODE-OCCURRENCE-METRICS-SPEC.md) specifications. The exact manifest, 22 retained models covering all 25 registered executable element or semantic variants, seven external candidates across six clone families, strict Git/archive provenance guard, generated rankings, living capability guard, Product 2 About disclosure, and first exact production browser journey are implemented. Product 2 readiness is one, with 21 retained models still explicitly excluded from the browser catalog.

**Latest evidence:** `doctor.sh research` passed with external provenance ready. The complete corpus gate passed 13/13 in 17.4 seconds across 22 retained models and seven external admission candidates. The web package passed 115/115, and the production-built 1280/1600 UI-quality lane passed 56/56 in 35.4 seconds. The new checkpoint's focused Lean build, 31 semantic-core tests, 42 BPMN-source tests, and two host-admission tests are green. Playwright remains outside semantic verification.
