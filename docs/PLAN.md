...
Next action: establish canonical scope-occurrence and remaining activation-counter storage before scope/region batch preparation. Independent production review [approved correction `34e38132`](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt), and the clean complete gate passes at `9fddbaca`. Replace the remaining Lean prepend writers, extend both collection-order predicates, and preserve existing proof consumers. The separating oracles compare exact stored scope and counter values across distinct-key insertion orders.
