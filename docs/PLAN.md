...
The next semantic-family selection is additionally gated by the executable model corpus increment in [Ordered work](#ordered-work). This is roadmap infrastructure rather than another BPMN feature: it must establish which representative whole models the current engine and platform can actually run before corpus prevalence is allowed to influence the order of the extensions above.
...
**Last verified commands:** the complete local macOS `./scripts/verify.sh` gate is green for the current policy change. The three ordered components of `test:pre-push:ui` are also green locally: `test:platform-operations-checkpoint`, `test:showcase:types`, and `test:ui-quality`, with the blocking browser lane passing 72/72 cases at 1600 and 1280 pixels. The latest hosted Ubuntu/macOS semantic verification is green at `577df49`; its Product 2 UI-quality run exposed the now-retired pixel-comparison failure class. Playwright remains outside semantic verification.
