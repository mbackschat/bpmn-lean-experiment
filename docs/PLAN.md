...
Next action: independently review [Complete/Return runtime preservation](INTERNAL-COMMUTATION-PROPOSAL.md#complete-and-return-runtime-preservation) and its retained Activity-owner correction, alongside clean complete integration. The complete Lean/core gate, both-target controls, and fixed-3-GiB checks pass. After acceptance, audit preparation-frame gaps against existing fixed-mask and publication laws before adding machinery. [Error/Terminate preservation](INTERNAL-COMMUTATION-PROPOSAL.md#error-and-terminate-runtime-preservation) is approved at `0f4e8a51`. E1/E2 change no RC scope.
