...
1. **In progress — implement M1's preserve-only admission:** [the proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md) is owner-approved as of 2026-08-08. Implemented and registered: the recursive classifier, the one-executable-entry rule, the preserved set of D2, reference-target admission, the foreign-attribute rule, D5's non-interference evidence with its seeded defect, and the profile artifact with its scenario, pipeline case, and live example. Still owed: **D3's per-element diagnostics** and the structural requirement-ledger rows. The semantic-checkpoint review returned `approve-with-required-edits`; its correction audit is the next stage boundary.
...
**Next action: add D3's per-element admission diagnostics.** The classifier, the one-executable-entry rule, the non-interference evidence, and the profile registration landed at `3903d2a`, `73780a6`, `d8ec4ec`, and the registration commit. A rejected source still yields one diagnostic for the whole compilation, which is the last required M1 item besides the structural requirement-ledger rows.

**Blockers: the semantic-checkpoint review is paused mid-flight** against target `2b1a133`, which fails `./scripts/verify.sh` on the live-example guard the registration then closed. Resume it against the registration commit before closure. No owner is near its ceiling: `checked-process-compiler.ts` holds 350 nonblank lines of headroom after its extraction, the new classifier 330, and `workflow-implementation.ts` 50.
