...
Next action: establish complete regional preparation and pair commutation for `returnProcess`, `completeScope`, `throwError`, and `terminateScope` under the [regional account](INTERNAL-COMMUTATION-PROPOSAL.md#prepared-transition-and-region-footprints). First prove agreement between predecessor-derived occurrence regions and actual removal closures, including directed Calls, attached handlers, bounded completion, and preserved high-water records. Group the shared closure, preparation, preservation, and pair laws into one checkpoint before production batching/publication integration. Retain the snapshot-declaration exclusion, predicate coverage, and 3 GiB ceiling. The [scope-creation batch integration](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite) and [Call pairing correction](capsules/CALL-ACTIVITY-SPEC.md#independent-cold-review-receipt) are independently approved.
