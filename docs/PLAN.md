...
**Timer Start has an independently approved semantic checkpoint and a registered product profile.** Target `7ac0307` received required edits, and correction `ba3bbf8` closed its IL-contract and XSD-regression findings. Commit `8aa0cc3` atomically registered the standards-only profile, answer-free scenario, differential case, and runnable example. The live Schedule witness found that pinned Temporal assigns an execution Workflow ID by appending the nominal due time to the configured semantic-instance-derived base. The capsule now proposes using the service-returned execution identity, but that material host-contract correction must receive a new cold review and fresh owner approval before live evidence resumes. Product 2 scheduling remains outside this capsule.
...
1. **Next: close the approved Timer Start Event capsule after resolving its Schedule execution-identity contract.** Submit the configured-base versus service-returned execution-ID correction for cold review and owner approval, then complete the test-owned one-action Temporal Schedule witness with pre-Schedule refusal, post-Schedule action-integrity discrimination, Worker absence, history inspection, and replay. Registration is complete at `8aa0cc3`. Product 2 schedule management remains excluded.
...
**Next action: review the material Schedule execution-identity correction in the [Timer Start Event proposal](capsules/TIMER-START-EVENT-PROPOSAL.md).** Registration is complete at `8aa0cc3`. The live witness must remain paused until a cold reviewer and the owner approve consuming the service-returned execution Workflow ID while retaining the exact semantic-instance-derived configured base. After approval, finish only that witness and the capsule closure. Do not add Product 2 scheduling, recurrence, calendar forms, activation policy, or public schedule management.

**The only blocker is the required review and owner decision on the corrected Schedule execution-identity contract.** The semantic checkpoint lifecycle gate is closed, registration is green, and no technical or environmental blocker remains. The earlier approval of concrete `@temporalio/client@1.21.0` Product 2 reachability and the no-umbrella Temporal subsystem package layout remains binding.

**Last verified commands:** the Timer Start checkpoint passes semantic-core 180/180, BPMN source 392/392, contracts 44/44, focused Temporal identity/host admission 19/19, strict harness types, focused Lean conformance and experiment targets across 82 jobs, source hygiene 30/30, and the documentation/review guard 45/45. Its target-bound complete verifier at clean checkpoint target `7ac0307` passed in 179.89 seconds with log SHA-256 `811534c10fbe1fabca67009c95a80feeaa671c4de26d5d8fc4ce539415e59617` and replayed 39 live histories. Correction `ba3bbf8` additionally passes the 28 focused document, verification-entrypoint, and XML-validator tests plus direct pinned OMG validation of both mandatory BPMN inputs. The mandatory Linux/arm64 admission audit passed with Lean 4.31.0 under one CPU, a 3 GiB memory-and-swap ceiling with effective swap disabled, and no OOM: 44.64 seconds wall, 2,668,168 KiB maximum RSS, and 2,980,679,680 bytes cgroup peak. The complete platform M1 gate passed in 14.53 seconds, the registered M1 showcase passed in 9.11 seconds, and the infrastructure gate passed 220/220 in 38.18 seconds.
