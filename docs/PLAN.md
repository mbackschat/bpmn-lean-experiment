...
1. **Next: implement the owner-approved M2 cyclic-control-flow proposal.** The owner approved the independently reviewed proposal on 2026-08-10. The first implementation commit creates the immutable baseline catalog from exact baseline `7529150`, before extraction or semantic changes; the second is the separately gated behavior-preserving checked-source graph-admission extraction. Only then begin semantic Red/Green implementation of the one root-scope User Task Sequence Flow cycle, identity-only Exclusive Merge derived from checked Sequence Flows, and profile-gated resumption-cut admission. Keep the decided-fixture admission lane under the recorded build ceiling.
...
**Next action: create and commit the non-updatable baseline preservation artifact required by the owner-approved [resumption-bounded cyclic control flow proposal](capsules/CYCLIC-CONTROL-FLOW-PROPOSAL.md), generated from exact baseline `7529150` before extraction or semantic changes.** Context-cold proposal target `31dd294` and final owner-authorized correction audit target `d5e7bb9` have no open findings. After the baseline artifact, land the separately gated behavior-preserving checked-source graph-admission extraction before semantic Red. The mandatory decided-fixture review is closed by the proof-lane split and its recorded CPU and resident-memory figures. M1 remains closed through the public API and UI over real Temporal hosting. The A12 product boundary remains closed, its stable contract is active, and its proposal is archived.

**No owner blocker.** The owner approved the independently reviewed cyclic-control-flow proposal on 2026-08-10. The earlier approval of concrete `@temporalio/client@1.21.0` Product 2 reachability and the no-umbrella Temporal subsystem package layout remains binding.
