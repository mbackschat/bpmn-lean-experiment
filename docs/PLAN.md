...
**Status: in progress.** The independently reviewed [committed execution publication proposal](capsules/COMMITTED-EXECUTION-PUBLICATION-PROPOSAL.md) and its nine selected decisions are owner-approved. The Product 1 semantic checkpoint now implements exact evaluator-root traces, independent current positions, strict wire and canonical bytes, an atomic Workflow publication Query, a representation-free client, an opaque-locator engine API, and exact Lean/TypeScript parity. Independent checkpoint review is next. Product 2 projection, live retention/replay evidence, History, Diagram, and export remain open.
