...
The unresolved current owner decision is whether to approve the eight selections in [the CreateDocument data and mapping proposal](capsules/CREATE-DOCUMENT-DATA-PROPOSAL.md): a distinct CIB Seven `2.0.0` A12 profile; exact unchanged source with profile-supplied protocol and source-supplied handler; string-only Process and Activity-local variables; literal input and simple local-reference output mapping; committed effect arguments and typed local patch; content-bound argument/result identities; success-only transaction refinement with no rollback claim; and license-safe separation of a project-authored mandatory fixture from the exact external target gate. No implementation is authorized by the assessment.
...
11. **Completed assessment; owner decision required — CIB target alignment and variable/expression proposal:** [the target baseline](research/CIB-SEVEN-A12-BASELINE-RESEARCH.md) preserves CIB Seven `2.0.0` and `2.2.0` as distinct accounts, identifies the exact source-identical successful `CreateDocument` seam and the broader non-equivalence boundary, and records the temporary `2.0.0` executable result. [The proposal](capsules/CREATE-DOCUMENT-DATA-PROPOSAL.md) specifies the smallest string variable/effect-patch, literal/simple-reference mapping, committed argument, success-only synchronous-transaction refinement, and license-safe external-source contract. Do not begin implementation before owner approval. Typed `BpmnError` follows immediately if approved.
...
The first incomplete ordered item is owner review of [the CreateDocument data and mapping proposal](capsules/CREATE-DOCUMENT-DATA-PROPOSAL.md). Do not implement it without approval of its eight requested decisions. If approved, begin with the distinct CIB Seven `2.0.0` profile/evidence identity and exact external-source admission boundary, then close the string value, scope, mapping, patch, and success-only refinement account through the normal red/green capsule order. If rejected or revised, update the proposal rather than improvising a source rewrite or merged profile. The missing materialized Workflows-enabled full-stack-template variant remains an integration input. D1 remains unapproved and behind this product-forced work. Keep C2 frozen under the non-extension and reopening rules in [the experiment record](experiments/CHECKED-SOURCE-RELATION-EXPERIMENT.md).
