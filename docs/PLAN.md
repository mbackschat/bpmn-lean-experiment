...
2. **Blocked on M5 closure: execute the Temporal scalability-readiness roadmap, starting with Product 2.** This is the first post-MVP architecture program and takes precedence over broader platform feature expansion. Its first increment must remove Product 2's larger horizontal-scaling blocker: replace process-local synchronous SQLite ownership with a horizontally shared transactional persistence architecture; replace request-time sequential N+1 Temporal Query aggregation with incremental, bounded, partition-owned background projections and indexed materialized read models; make publication ingestion proportional to the new suffix instead of rebuilding and rewriting retained history; replace startup-wide scans with checkpointed, leased, bounded recovery; and prove that at least two interchangeable Product 2 replicas can serve and mutate shared state without duplicate ownership or global backlog-drain readiness. The next increment hardens Product 1 for long-lived execution through actual encoded-payload and Event History budgets, scale telemetry, a quiescent observation-transparent Continue-As-New contract, compact cross-Run state and deduplication, append-only publication authority, content-addressed Program delivery, and explicit Worker fleet, queue, rollout, and tenant-isolation policy. Only after those architectures and metrics exist may representative infrastructure tests establish throughput, latency, replay, failover, backlog-recovery, and scaling claims. The work begins with an independently reviewed scalability proposal that binds workload envelopes, SLOs, deployment class, exact current ceilings, failure behavior, and exit criteria; single-machine benchmarks are not acceptance evidence.
...
Item 2 is **blocked on ordered-work item 1 closing M5**. When M5 closes, item 2 becomes the exact resume point before any broader post-MVP platform feature expansion; its first deliverable is the reviewed Product 2 shared-persistence and incremental-projection architecture proposal.
