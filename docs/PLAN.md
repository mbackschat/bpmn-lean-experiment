...
1. **In progress: remove Product 2's single-node scale boundary.** The PostgreSQL runtime and checksum-bound migration seam is green without adding a database to the ordinary package loop. Continue the owner-approved [shared persistence and projection proposal](BPM-PLATFORM-SHARED-PERSISTENCE-AND-PROJECTION-PROPOSAL.md) with one atomic asynchronous repository contract, local SQLite and shared PostgreSQL adapters, exact-byte admitted-source storage, bounded background recovery, monotonic suffix projections, and projection-backed reads. Preserve fail-closed completeness and exact retry semantics, and close with concurrent multi-replica correctness evidence before making any performance or capacity claim.
...
**The exact next implementation boundary is the atomic Promise conversion of all twelve repository ports, their SQLite implementations, every producer and consumer, fakes, tests, and server composition.** Do not retain parallel synchronous and asynchronous service paths. Continue with one bounded, disjoint implementation lane at a time under the delegated implementation protocol. Review target `c9cf16e` and correction audit `e03240c` bind every current SQLite repository, the exact admitted-source artifact store, lifecycle repair family, request-time Temporal Query fan-out, projection freshness rule, audit source-to-sink completeness rule, and same-database publication invariant.

Item 1 is **in progress** and blocked on nothing. On 2026-08-16 `./scripts/doctor.sh verify` passed with Node 24.18.0, pnpm 11.20.0, Java 21, Lean 4.31.0/Lake 5.0, and the verified OMG corpus. The approved `pg@8.22.0`, `@types/pg@8.20.0`, and `node-pg-migrate@9.0.0` dependencies, bounded runtime, checksum-bound migration command, database-free package tests, and isolated real PostgreSQL 18 runtime witness are implemented. The business repositories and every later Horizon 1 boundary remain unchanged. No distributed benchmark or production capacity claim exists.
