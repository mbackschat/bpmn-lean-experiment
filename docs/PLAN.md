...
**Two MIT products are now separated.** The BPMN execution engine is implemented across twenty-plus closed semantic capsules; the [BPM platform](BPM-PLATFORM-PROPOSAL.md) is owner-approved as a contract, has its [modular-monolith architecture](ARCHITECTURE.md), guarded package dependency direction, narrow compilation gateway, and exact content-addressed artifact store. It has no end-to-end product capability yet. [PROJECT-DESIGN.md](PROJECT-DESIGN.md#product-division) owns the division and [IMPLEMENTATION-MAP.md](IMPLEMENTATION-MAP.md) owns the exact implemented and absent boundary for both.
...
**Status: in progress.** The engine admission slice, narrow compilation gateway, and exact artifact byte store are implemented. The composed deployment workflow, public API, versioning, diagram rendering, start boundary, and React client are absent, so the milestone has not reached its exit gate.
...
**Next action: implement ordered-work item 1, the open M1 platform increments.** The narrow compilation gateway and content-addressed artifact store are implemented. Continue at the definitions module with durable per-process version ordinals and exact deployment binding, then expose that capability through the public HTTP API. The A12 product boundary is closed, its stable contract is active, and its proposal is archived.

**Blockers: none for the definitions and HTTP increments.** Explicit owner approval is still required before adding the selected React stack or a BPMN viewer dependency; no UI dependency has been added implicitly.
