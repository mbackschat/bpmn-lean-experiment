...
13. **Next independent cleanup — substantial directly executed TypeScript harnesses:** after the Service Task capsule commit, migrate the substantial `.mjs` harnesses without mixing the change into semantic work. Proper Node typing requires an explicit direct dependency decision before implementation; do not rely on a transitive `@types/node`.
...
Commit the completed Service Task capsule after its full gate, then take the requested substantial `.mjs`-to-direct-`.ts` harness migration as the next independent cleanup. Require direct Node typings and a no-emit strict harness type-check; do not rely on transitive declarations. If the owner supplies the target-solution checkout meanwhile, record it in [SOURCES.md](SOURCES.md) and run the read-only inventory. Keep D1 behind the migration-critical work. Keep C2 frozen under the non-extension and reopening rules in [the experiment record](experiments/CHECKED-SOURCE-RELATION-EXPERIMENT.md); do not extend it with new capsules or silently treat the unresolved theorem as permanently waived.
