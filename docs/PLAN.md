...
**Next action: implement live Temporal retention/replay evidence and the fail-closed Product 2 projection, History, Diagram, and exact JSON export for the approved [M5 E1 committed execution publication](capsules/COMMITTED-EXECUTION-PUBLICATION-PROPOSAL.md) checkpoint.** The E1 implementation will not itself close M5; the remaining frequency/duration, operator-history, and audit-export increments stay ordered after it.

Item 1 is **implementation-stage and in progress**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md). M5 E1 now has an independently approved Product 1 public contract, TypeScript and Lean trace/position roots, deterministic Workflow accumulator and Query, strict client, engine API, and cross-target parity oracle. Product 2 projection/storage, live Temporal retention/replay evidence, UI surfaces, and export remain absent. Event History and state differencing remain forbidden fact sources.

**Last verified commands:** the complete `./scripts/verify.sh` repository gate is green at approved checkpoint correction target `6f6ab9dd424de304842b03164e954b279d359492`, including Lean, semantic core, source-compiled Lean/TypeScript publication parity, protocol/schema/canonical bytes, Workflow Query, client/API, CIB, differential, live Temporal, replay, and the complete four-target pipeline. Playwright remains in its separate Product 2 workflow.
