...
- Subscription closure integration. Command: `./scripts/pnpm.sh run test:pre-push:verify`. Status: `exit 0`. Date: `2026-09-24`. Commit: `b7eefa969235974a20d07d91b04288db2cc3d842`. The clean receipt `/tmp/bpmn-subscription-closure-clean-integration-runtime-rerun` is independently asserted; all 86 registered cases agree and 97 live histories replay. The [closure review](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-SPEC.md#independent-cold-review-receipt) is approved. The [cost record](CAPSULE-COST-LEDGER.md#subscription-closure-cost-2026-09-24) retains the earlier native V8 crash and unchanged successful rerun.
...
Next action: classify Compensation scheduling against its selected checked-source admission before public registration. The [ordinary snapshot arming and complete pipeline](capsules/COMPENSATION-TRIGGER-HANDLER-PROPOSAL.md#public-registration-prerequisites) are accepted. Inspect exact topology, lowering and reachable command frontiers; settle each obligation with the required admission argument and widening counterexample. Preserve the known unequal raw Lean/TypeScript admission domains. The diagnostic `/tmp/bpmn-compensation-rc-frontier-diagnostic` inspected all three forward task orders and found only ordinary Start arming as a multiple-operation frontier; finite enumeration alone proves no exclusion.

Accepted: [subscription closure](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-SPEC.md#independent-cold-review-receipt), including the [bounded scheduling proof domain](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-SPEC.md#scheduling-proof-domain-amendment), [Timer-task arming](INTERNAL-COMMUTATION-PROPOSAL.md#timer-task-arming-outcome), [standalone Activity-data arming](INTERNAL-COMMUTATION-PROPOSAL.md#standalone-activity-data-arming-outcome) and [exact Merge/private scheduling](INTERNAL-COMMUTATION-PROPOSAL.md#exact-merge-frontier-outcome). No further proof infrastructure or generic family-pair expansion is selected. Compensation/Transaction dependencies and regional-trial closure remain open.
