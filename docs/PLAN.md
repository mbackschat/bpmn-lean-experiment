...
1. **In progress: complete the open M1 platform increments.** The public definition API and HTTP-only React definition workspace now expose deployment, listing, diagnostics, exact source retrieval, and viewer-only diagram rendering, and required headless Chromium acceptance exercises an unseen runtime-created source. Add the narrowed definition-version start boundary and registered showcase. Close M1 only when that exit gate proves an externally supplied file is admitted, stored, versioned, rendered, and started while an unsupported file is rejected before Workflow start with its element identity.
...
**Next action: continue ordered-work item 1 by adding the narrowed definition-version start boundary.** The Temporal adapter is now an executable six-package subsystem with no production umbrella export; its protocol, client, Workflow, Worker, runner, and testkit dependency closures are guard-enforced. The public raw-XML deployment API, exact-source reads, Node server composition root, React definition workspace, licensed viewer-only `bpmn-js` adapter, and required headless Chromium acceptance are implemented. Playwright and its browser binary are development/CI-only. The A12 product boundary is closed, its stable contract is active, and its proposal is archived.

**No owner blocker.** The owner approved the concrete `@temporalio/client@1.21.0` Product 2 reachability, the resulting dependency-budget increase, and the no-umbrella Temporal subsystem package layout on 2026-08-10.

**Last verified command:** the prebuilt live Temporal runtime suite passed 157/157 in 59.28 seconds after the six-package split. The release-bound differential pipeline passed 4/4, the complete M1 platform gate passed, the complete infrastructure gate passed 211/211, the platform dependency-policy gate passed 15/15, `./scripts/doctor.sh verify` accepted all six package and cache owners, and the Temporal package-boundary guard passed 4/4. The existing M1 Chromium case admits and renders runtime-created third-party source, advances changed source to version 2, and rejects an unsupported element without advancing the catalog. The last complete engine gate remains `/usr/bin/time -p env CI=true ./scripts/verify.sh`, exit 0 in 149.71 seconds on commit `adf757b`, with output SHA-256 `c7a3a47f18c27163a723900e403da718eb6982e84089c754401ce913c7efd6f8`.
