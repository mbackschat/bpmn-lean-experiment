...
The active checkpoint is the M5 flow-node occurrence metrics increment. Its semantic checkpoint and correction audit are approved, Product 1 publication and Product 2 projection are green, and the local 1280/1600 functional UI lane is green. Closure documentation, review, and the complete integration gate remain before the curated executable model corpus becomes the next ordered item.
...
1. **In progress: implement the owner-approved bounded M5 flow-node occurrence metrics increment.** The source-grounded Product 2 preflight over current CIB Seven Cockpit documentation, the pinned pristine source, and an independent Camunda Optimize comparison is recorded in the [UI/UX research](research/BPM-PLATFORM-UI-UX-INFORMATION-ARCHITECTURE-RESEARCH.md#pattern-11-flow-node-metrics-require-occurrence-facts-not-transition-counts). The owner-approved and independently reviewed [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md) selects an exact Product 1 occurrence lifecycle and deterministic engine commit time before Product 2 frequency or duration. Review target `2503464` and final same-reviewer correction audit `5a75af9` are approved. Do not infer duration from Temporal Event History, Product 2 ingestion time, or semantic logical time. The closure-reviewed [E1 specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) remains the only committed semantic-history and current-position authority while completed operator history and audit export stay later M5 increments.
2. **Scheduled: establish a curated executable BPMN model corpus before selecting the next semantic family.** Research the pinned CIB Seven `2.2.0` source and tests, the external BPMN 2.0.2 examples, and other registered corpora for representative process patterns without copying protected source or treating any example as semantic authority. Retain only project-authored or licence-compatible BPMN bytes; keep external-only candidates as provenance, digest, and classification records. The first maintained tranche must give every model a stable identity, source and licence class, clone family, BPMN construct and reusable-mechanism inventory, selected profile, deterministic command schedule, admission result, semantic-core result, Temporal result, optional CIB relation, and Product 2 readiness. Local gates must execute every model through every claimed layer, and no model may appear in the browser catalog unless its advertised start and relevant workspace surfaces pass the same production-backed acceptance path locally. A deduplicated mechanism report must then identify which currently unsupported mechanisms block the most independent whole-model families. That report informs the next proposal but never overrides BPMN normative dependencies, creates a combined conformance percentage, or turns CIB behavior into BPMN meaning.
...
Corpus prevalence is counted by independently reviewed clone family and reusable BPMN mechanism, not by physical file or XML element occurrence alone. The roadmap report must publish both the number of independent model families blocked by a mechanism and the mechanism's semantic dependencies, implementation cost, Temporal feasibility, and standards value. The next semantic capsule is selected from that evidence under the durable ordering rule in [PROJECT-DESIGN.md](PROJECT-DESIGN.md#cib-seven-220-breadth-ordering); a frequent construct does not bypass its prerequisites or approval lifecycle.
...
6. a generated roadmap report ranks unimplemented reusable mechanisms without changing the BPMN, CIB, and platform coverage denominators.
...
**Next action: finish the production-backed browser and closure evidence for the independently approved [flow-node occurrence metrics increment](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md), then execute ordered item 2 before selecting another semantic family.** Semantic-checkpoint review target `33ffa62` and same-reviewer correction audit `4aa081b` are approved. Product 1 and the Product 2 contracts, projection, aggregation, and routes are green; the local 1280/1600 functional UI lane is green, and closure documentation, review, and the complete integration gate remain. The executable model corpus then becomes the required source of whole-model blocker evidence for the next standards proposal. The remaining operator-history and audit-export increments stay ordered after the current M5 capsule.

Item 1 is **at its green Product 2 UI checkpoint after the approved semantic checkpoint**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and M5 E1 is closed through the graduated [committed execution publication specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md). The occurrence lifecycle, strict additive wire, replay-stable command-batch time, immutable Workflow accumulator, Product 1 client, Product 2 projection and aggregate, and the 1280/1600 functional UI are green. Closure review remains the governing pause before item 2. Event History, state differencing, Product 2 ingestion time, and semantic logical time remain forbidden substitutes for an exact public wall-clock duration fact.

**Last verified commands:** the optimized local feedback components are green on macOS. Infrastructure passed 287/287 in 41.9 seconds with bounded file concurrency two; the one-build Product 1 TypeScript closure completed in 3.2 seconds; semantic core passed 251/251, BPMN source 466/466, differential 36/36, and Temporal 208/208 in 58.8 seconds with bounded file concurrency three. Both pinned CIB release lanes passed inside their 60-second gate, and the complete four-target pipeline passed 5/5 with 52 retained history replays. Product 2's one-build platform checkpoint completed in 28.7 seconds, showcase type compatibility in 5.5 seconds, and the local functional UI lane in 29.8 seconds. The latest hosted Ubuntu/macOS semantic verification is green at `577df49`; its Product 2 UI-quality run exposed the now-retired pixel-comparison failure class. Playwright remains outside semantic verification.
