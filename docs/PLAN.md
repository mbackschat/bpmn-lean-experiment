...
**Next action: narrow the broad Lean imports that unnecessarily rebuild proof-heavy conformance modules.** Keep this import-boundary refactor semantically separate, enforce it with a source guard, and measure it through one root-owned serialized build. The parallel-metadata checkpoint correction is approved; after the import refactor, proceed to the proposal's combined CIB, differential, live Temporal, and Product 2 closure lanes. In the engine lane, classify Script Task execution against BPMN, CIB, deterministic sandboxing, data, security, and Temporal requirements before deciding whether its two-family prevalence justifies a proposal. The remaining operator-history and audit-export increments stay ordered after this corpus decision gate.
