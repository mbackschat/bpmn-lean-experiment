...
**Product 2 definition scheduling is implementation- and evidence-complete, pending governed closure review.** Accepted compilation publishes the exact Timer Start identity and duration; the platform persists that capability with exact definition bytes, exposes a strict one-shot schedule API and HTTP-only web panel, reserves immutable semantic and private host identities before network work, reconciles response loss and cancellation races through handle-free Schedule operations, and wires startup recovery into the production server. The live M2 witness proves exact-version retention after later deployment, platform restart, Worker replacement, opaque execution addressing, cancellation, exhaustion, history, and replay. Headless Chromium proves the same public path through the production UI. M1 remains an independent regression floor.
...
**Status: in progress.** The resumption-bounded cyclic-control-flow, Message Start Event, Timer Start Event, Terminate End Event, and [configured Task extension](capsules/CONFIGURED-GENERIC-TASK-SPEC.md) increments are implemented, closure-reviewed, evidence-closed, and graduated. The M2 engine boundary is complete. Platform definition scheduling for Timer Start is implementation- and evidence-complete and awaits governed closure review. Published message ingress and instance search remain.
...
1. **Active: close [M2 definition scheduling](BPM-PLATFORM-DEFINITION-SCHEDULING-PROPOSAL.md) for registered Timer Start.** Implementation, live Temporal evidence, cancellation and recovery discriminators, and browser acceptance are complete. Run the complete gate, retain cost and process reflection, and submit the governed closure review. Published Message Start routing follows because it requires an independent delivery, retry, and fanout contract. Instance search remains after that because its durable cross-instance index must be designed with the platform read-model boundary rather than inferred from engine Event History.
...
**Latest complete gate: 2026-08-11, exit 0 at `e1f81ca`.** All **42** registered cases reached agreement across their declared targets, all 42 seeded semantic mutations produced their required disagreement, all 18 retained-CIB evidence comparisons agreed, and all 44 live histories passed. Its 27391.305625ms warm pipeline total is **correctness evidence only**, not a baseline: the owner reported other processes consuming substantial CPU during the run.
...
**Next action: submit the implementation- and evidence-complete [Product 2 definition-scheduling proposal](BPM-PLATFORM-DEFINITION-SCHEDULING-PROPOSAL.md) to governed closure review.** First run the complete repository gate and bind its exact result to the immutable closure target. Preserve exact definition-version binding, keep service execution identity opaque, and exclude recurrence, silent latest-version retargeting, broker/message routing, instance search, and new BPMN semantics from this platform increment. After closure approval, select published Message Start ingress as the next M2 platform increment.
...
**Last verified commands:** `env CI=true ./scripts/pnpm.sh run test:platform-m1` passed the definition-scheduling checkpoint in 15.94 seconds, including the strict public contract, engine gateway, exact SQLite lifecycle, HTTP/server composition, web-client contract projection, platform harness, licence, and dependency-policy gates. `/usr/bin/time -p env CI=true ./scripts/verify.sh` passed at immutable configured Task closure target `e1f81ca` in 313.91 seconds with output SHA-256 `30b7773e68114858b0d2c0248e2e58f1c7df42f369f22d857012f86c5065b04d`; all 42 registered cases, 42 seeded mutations, 18 retained-CIB comparisons, and 44 live histories passed. The warm correction audit approved `8959a0c`. The mandatory Linux/arm64 Lean admission audit and registered M1 showcase remain green.
