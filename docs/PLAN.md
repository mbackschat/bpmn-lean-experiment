...
Next action: audit remaining Complete/Return aggregate obligations against their existing position, quiescent-removal, reference, and publication proofs, then compose preservation for the actual prepared successors as the next bounded deliverable. Derive terminal and retained Compensation cases from existing declaration/selection facts; reassess any substantial prerequisite or representation change. [Error/Terminate runtime preservation](INTERNAL-COMMUTATION-PROPOSAL.md#error-and-terminate-runtime-preservation) is independently approved at `0f4e8a51` with clean integration. E1/E2 change no RC scope.
