...
Next action: implement the [approved subscription preparation certificate](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#scheduling-proof-domain-amendment), then close its enumerated proof obligations within the renewed effort bound. Preserve existing theorem guarantees and the older TypeScript raw bounded-scope path. Source-frontier witnesses support capability claims; quantified assurance also covers valid intermediate predecessors. The [subscription checkpoint](capsules/REPEATABLE-EVENT-SUBSCRIPTIONS-PROPOSAL.md#review-questions-and-implementation-obligations) still requires retained-body proof integration, complete verification, resource qualification, and checkpoint review before hosting or registration. Snapshot arming and pipeline correspondence are accepted. Preserve the bounded checkpoint and regional trial.
