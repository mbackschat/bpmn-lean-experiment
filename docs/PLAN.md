...
| The `CreateDocument` and boundary-Error slices, their `2.0.0` profiles, and their exact source readers | Preserve the legacy generation byte-for-byte as optional adoption evidence, replace every production decision with the neutral profiles and data-only overlay contract in the owner-approved [A12 boundary proposal](A12-ADD-ON-BOUNDARY-PROPOSAL.md), and do not repeat the shape for further A12 models. |
...
1. **In progress: correct the A12 product boundary without discarding the implemented slices.** The behavior-preserving dispatch registry is implemented and guarded by complete-result equality across all four current paths. The [A12 add-on boundary proposal](A12-ADD-ON-BOUNDARY-PROPOSAL.md) is independently reviewed and owner-approved: retain mapped-effect and boundary-Error mechanisms in product-neutral engine profiles, move exact A12 identities, business literals, fixtures, and evidence behind the optional adoption boundary, and provide only a data-only source-overlay seam for a future separate A12-owned add-on. The earlier CreateDocument-only expanded-name task is subsumed by that overlay contract and must not extend the A12-shaped production reader first.
...
**Next action: implement the first red product-boundary and overlay-contract guards from the owner-approved [A12 add-on boundary proposal](A12-ADD-ON-BOUNDARY-PROPOSAL.md), then replace the two A12-shaped production slices atomically across the wire, source, Lean, core, Temporal, and evidence lanes.** Preserve the immutable legacy generation at target `02330ad` and do not add or extend an A12-specific production path. The M1 engine admission slice is closed, while the complete M1 showcase remains in progress until ordered-work item 2 supplies and verifies its platform increments.

**Blockers: none.** The mandatory context-cold proposal review and owner approval are recorded. Ordered-work item 1 now advances through red/green implementation, a semantic checkpoint review at the first complete green cross-language target, and closure review.
