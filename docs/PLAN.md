...
1. **In progress: implement the M4 Service Task incident and retry semantic checkpoint.** The M4 inventory selects three separately governed stages: Service Task incident/retry semantics, incident-scoped root-Process cancellation, then Product 2 incident operations. The owner-approved [stage-one proposal](capsules/SERVICE-TASK-INCIDENT-RETRY-PROPOSAL.md) defines one typed technical failure, one literal-generation incident, one exact retry of the same effect occurrence, exact CIB failed-job mapping, a proved Lean lane, and a Temporal preflight. Implement the wire/schema, TypeScript semantic core, and Lean account, then obtain the required semantic-checkpoint review before CIB, differential, and live Temporal closure work.
...
**Next action: implement the first green M4 Service Task incident and retry semantic checkpoint across wire/schema, TypeScript, and Lean.** M3 is closed end to end: the semantic and human-work contract is graduated through correction audit `23892a5`, diagram presentation integrity is approved through `83de531`, the reusable Product 2 information architecture and UI design are approved through `83de531` and `afc4b63` respectively, and the maintained browser walkthrough exposes the complete owner-review path.

Item 1 is **in progress**. Context-cold proposal review of `56b9398` returned `approve with required edits`; the same reviewer closed the bounded corrections at `43228e3`, and the owner approved implementation on 2026-08-13. The selected account preserves existing bare semantic Activity payloads, adds one unwrapped technical-failure arm, moves the full effect wait into an identity-bound incident that blocks scope completion and counts as a retry resumption surface, and permits one exact retry without a generation-two semantic state. Packaged CIB research at `ee6c244` proves the enabled/disabled incident discriminator, same-job reset, raw incident replacement, and later success; the selected semantic mapping stops after the first reset. Playwright remains a Product 2 harness outside every Product 1 feedback loop.
