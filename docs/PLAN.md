...
1. **In progress — implement M1's preserve-only admission:** [the proposal](PRESERVE-ONLY-ADMISSION-PROPOSAL.md) is owner-approved as of 2026-08-08. Implemented and registered: the recursive classifier, the one-executable-entry rule, the preserved set of D2, reference-target admission, the foreign-attribute rule, D5's non-interference evidence with its seeded defect, the profile artifact with its scenario, pipeline case, and live example, **D3's per-element diagnostics over the classification lane**, and the five `BPMN-STRUCT-` rows in [the requirement ledger](BPMN-REQUIREMENT-LEDGER.md#reviewed-requirements), each `supported` on its own evidence while the operational requirement it names stays `unsupported`. Every engineering item the proposal requires is now landed. The semantic-checkpoint review returned `approve-with-required-edits` and its corrections were audited and owner-accepted; the closure review is the only remaining stage boundary, after which this item and the proposal's `-PROPOSAL` suffix both retire.
...
**Latest complete gate: 2026-08-08, exit 0 on the working tree carrying M1's complete engineering scope, over baseline `42439b6`.** All **35** registered cases reached agreement across their declared targets, all 35 seeded semantic mutations produced their required disagreement, and all 18 retained-CIB evidence comparisons agreed. Its 20398.663ms warm total reported the soft target and is **correctness evidence only**, not a baseline: the run shared a host with earlier gates in the same session. The preceding clean-tree run was `9e08b4a` on 2026-08-07 at 34 cases and 20406.710ms, also contended.
...
**Blockers: none.** The semantic-checkpoint review closed against `2deb802` with its corrections audited at `ab29717` and `9923b02` and the owner accepting `c915628`; [the receipt](PRESERVE-ONLY-ADMISSION-PROPOSAL.md#independent-cold-review-receipt) owns those targets. No owner is near its ceiling: `preserved-element-classification.ts` holds 173 nonblank lines of headroom, `checked-process-compiler.ts` 226, the new `admission-diagnostics.ts` 368, and `workflow-implementation.ts` 50.

**Last verified command:** `./scripts/verify.sh`, exit 0, on the working tree carrying M1's complete engineering scope over baseline `42439b6`, with 35 agreements and 35 seeded mutations detected. Its timing is contended and is not a baseline; see [the baselines above](#last-verified-baseline).
