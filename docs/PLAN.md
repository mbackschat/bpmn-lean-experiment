...
1. **In progress: implement M4 incident-scoped cancellation.** Context-cold proposal target `d03f528` received `APPROVE WITH REQUIRED EDITS`; the same reviewer approved bounded correction `4505dbf` with no retained finding. The owner approved the proposal and all eight decisions on 2026-08-13. Implement the first green wire/runtime/receipt checkpoint, obtain its conditional semantic review, then complete the registered CIB, Lean, TypeScript, Temporal, and differential evidence. Product 2 incident operations remain Stage 3.
