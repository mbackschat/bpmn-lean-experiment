...
1. **In progress: remove Product 2's single-node scale boundary.** The PostgreSQL runtime, checksum-bound migration seam, and atomic Promise-only repository conversion are Green without adding a database to the ordinary package loop. Continue shared persistence as four separately Green domain-owned vertical slices in this order: Definitions plus exact admitted-source artifacts; Operate projections and incident actions; Work state and its audit outbox; Audit stores and exact source-to-sink completeness heads. Each slice owns its migration, PostgreSQL adapter, reusable SQLite/PostgreSQL behavioral contract, focused database-free package gate, and separate real-PostgreSQL gate. Only then add the bounded lease mechanism one recovery family at a time, compose the shared-mode API and recovery worker, replace request-time fleet fan-out with freshness-bounded projection reads, and close with concurrent multi-replica correctness evidence. Preserve fail-closed completeness and exact retry semantics throughout; make no performance or capacity claim.
...
**The exact next implementation boundary is the Definitions plus exact admitted-source artifact vertical slice.** Add only its migration, PostgreSQL adapters, reusable SQLite/PostgreSQL contracts, database-free gates, and separate real-PostgreSQL gates. Preserve artifact-before-metadata publication, missing-byte refusal, gap-free process-local versions, canonical Human Task catalogs, generated-DI identity and bytes, and lifecycle CAS distinctions. Do not begin Operate, Work, Audit, recovery, or shared composition in this slice. Continue with one bounded implementation lane at a time under the delegated implementation protocol. Review target `c9cf16e` and correction audit `e03240c` bind the current repositories, artifact store, repair families, request-time fan-out, projection freshness, audit completeness, and same-database publication invariant.

Item 1 is **in progress** and blocked on nothing. On 2026-08-16 `./scripts/doctor.sh verify` passed with Node 24.18.0, pnpm 11.20.0, Java 21, Lean 4.31.0/Lake 5.0, and the verified OMG corpus. The approved `pg@8.22.0`, `@types/pg@8.20.0`, and `node-pg-migrate@9.0.0` dependencies, bounded runtime, checksum-bound migration command, database-free package tests, and isolated real PostgreSQL 18 runtime witness are implemented. All twelve business repository owners, their SQLite adapters, services, routes, fakes, tests, and server composition now use one Promise-only contract; the complete Product 2 operations checkpoint and 312-test infrastructure gate are Green. Business PostgreSQL adapters and every later Horizon 1 boundary remain absent. No distributed benchmark or production capacity claim exists.
