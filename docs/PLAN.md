...
1. **In progress: implement M6 structured Human Work.** Implement the approved [structured Human Work proposal](BPM-PLATFORM-STRUCTURED-HUMAN-WORK-PROPOSAL.md) through its first green semantic checkpoint, independent checkpoint review, Product 2 catalog and form lanes, retained real-world model, complete user journey, and closure evidence. M6 keeps BPMN User Task lifecycle and the optional opaque Rendering hook separate from a Product 2-owned, Zod-backed form catalog, adds generic non-negative safe-integer and string-list process values, and reuses only the existing `CIB-EXT-0011` candidate-group relation in a new assignment-only passive metadata shape for Product 2 claim authorization. CIB Seven forms remain research inspiration and the old bounded M3 profiles only; they do not define the new semantic contract.
...
**Next action: complete ordered-work item 1 by implementing the first green semantic checkpoint of the approved M6 [structured Human Work proposal](BPM-PLATFORM-STRUCTURED-HUMAN-WORK-PROPOSAL.md).** The checkpoint must widen the generic value wire to non-negative safe integers and ordered string lists, admit the new profile and assignment-only passive metadata shape, preserve opaque bounded BPMN Rendering content, and close the Lean, TypeScript, schema, CIB, differential, and Temporal refinement evidence required before Product 2 form implementation proceeds.
...
**Latest evidence:** the M6 proposal target `d116ece` received a `fork-turns-none` context-cold review; corrections `e1366e6` and `da96485` closed the value-domain, ceiling, Rendering opacity, canonicalization, BPMN classification, and complete owner-routing findings, and the same reviewer approved the final correction. The source-grounded M6 research inspected the pinned CIB Seven form and Task APIs plus current CIB Seven, Camunda 8, and Flowable documentation. The approved scalability proposal remains the first post-MVP scale-out roadmap, and the prior M5, bundle, web, browser, and complete-gate evidence remains unchanged.
