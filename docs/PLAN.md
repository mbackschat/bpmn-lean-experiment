...
Next action: complete immutable calibration, independent checkpoint review, and repository verification for Lean [scope-creation preparation](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite). Its constructor, symbolic refinement/refusal laws, and bounded kernel witnesses are implemented. Selection and retained-patch refinement remain independently approved at `3b19a96d`. Then close validity, frame, commutation, accepted-publication correspondence, and production scope/region batching. Preserve exact complete-state equality, high-water records, and the distinction between collection algebra and transition preservation.
