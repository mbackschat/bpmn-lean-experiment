...
`env CI=true /opt/homebrew/bin/timeout 60 ./scripts/pnpm.sh run test:temporal` passed on 2026-07-26 after the restart-ordering correction and production-lifecycle probe. Five focused tests passed in 4.01 seconds, including Worker replacement, retained accepted-Update lookup after Workflow closure, refusal of a distinct late Update and Workflow-ID reuse, identity separation, and replay.
...
11. **In progress — decide the production Temporal lifecycle before expanding BPMN scope:** the bounded experiment now supplies Worker-restart, accepted-result retrieval, distinct late-command refusal, Workflow-ID non-reuse, replay, cleanup, and identity-separation evidence. Compare the permanent entity, durable router/entity composition, and closed-result boundary; record the recommended typed contract for owner approval before changing production Workflow behavior or admitting another semantic capsule.
...
Resume at ordered work item 11. The [production-lifecycle experiment](experiments/TEMPORAL-PRODUCTION-LIFECYCLE-EXPERIMENT.md) establishes the pinned platform facts and recommends a closed-Workflow/retained-result boundary as the smallest account. Draft the exact typed result union, command-retry lookup order, terminal result shape, Workflow-ID policy, retention limit, accepted-handler drain rule, race witness, and re-open conditions in a `-PROPOSAL` for owner approval. Do not change production Workflow lifetime until that approval.
