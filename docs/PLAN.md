...
**Authorized next action after checkpoint review:** assess the CIB Seven `2.0.0` target baseline separately from the current `2.2.0` profiles, then produce an owner-facing proposal for the smallest typed variable, input/output mapping, and deterministic expression contract covering one maintained A12 Workflows path. Do not implement the target profile, variable system, expression evaluator, Java Worker, façade, or template integration without the applicable owner decision and dependency approval.
...
11. **Next — CIB target alignment and variable/expression proposal:** preserve CIB Seven `2.0.0` and `2.2.0` as distinct accounts while assessing the used A12 subset, then specify the smallest typed variable/effect-patch, input/output mapping, deterministic expression, and synchronous-transaction classification needed to admit the maintained `CreateDocument.bpmn` product example unchanged. Use the four production `BpmnError` delegates to place typed faults and boundary errors immediately after that data contract. Do not begin implementation before owner approval.
...
The first incomplete ordered item is the target-backed CIB baseline and variable/expression assessment. Begin by comparing the A12-used CIB Seven `2.0.0` surface with the current `2.2.0` oracle without merging their profiles, then draft the smallest typed variable, input/output mapping, deterministic expression, effect-patch, and synchronous-transaction classification needed to admit `CreateDocument.bpmn` unchanged. Record the missing materialized Workflows-enabled full-stack-template variant as an integration input, not as an excuse to infer its wiring. D1 remains unapproved and behind this product-forced work. Keep C2 frozen under the non-extension and reopening rules in [the experiment record](experiments/CHECKED-SOURCE-RELATION-EXPERIMENT.md); do not extend it with new capsules or silently treat the unresolved theorem as permanently waived.
