...
Next action: review the [local-control proof checkpoint](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite) while running its clean complete gate. Complete preparation, mixed-pair frames, arbitrary finite prefix validity, exact execution/publication commutation, kernel discriminators, and [immutable bounded calibration](CAPSULE-COST-LEDGER.md#finite-mixed-local-control-calibration) are implemented. After approval, remove the classifier's trace-proof import dependency before production integration. The [Inclusive pairing repair](capsules/INCLUSIVE-GATEWAY-SPEC.md#lean-assurance-lane), token-patch validity, and selected-record algebra retain checkpoint approval.
