...
- The approved User Task interaction contract now has a versioned draft profile and neutral scenario. The pinned CIB runner observes the task name through public `TaskService` queries, projects structured semantic occurrence `(Instance_1, UserTask_Approve, 1)`, completes that exact occurrence, rejects the same element with activation `2` without state change, and cleans both runs.
...
1. Continue the approved [User Task discovery/completion capsule](USER-TASK-DISCOVERY-COMPLETION-CAPSULE.md) in the same fast lane: the versioned neutral scenario and CIB probe are complete; next add the Lean transition, useful laws, and checked element-only non-law, then executable IR/semantic core → Temporal Query/Update mechanics → differential, refinement, replay, and seeded-mutation evidence.
2. Keep Search Attributes, Visibility-based global discovery, and a production task inbox outside that capsule; exact Query by known Workflow ID is the approved discovery surface.
3. Add a second concrete compiler consumer before generalizing the bounded CMOF manifest or executable IR schema; keep full metamodel generation, non-UTF-8 decoding, source locations, extensions, DI, and export outside the first slice.
4. Keep advanced boundary Event, Event Sub-Process, and multi-instance PVM models in the diagnostic research lane until their semantic capsules are approved; the first read-only sequential-model projection answered the M0 topology question.
5. Before adding an auxiliary formal tool, identify a concrete question and seeded defect, then time-box the smallest candidate experiment from [TLA-AND-BISIMULATION-RESEARCH.md](TLA-AND-BISIMULATION-RESEARCH.md); no formal-method spike is currently scheduled.
6. Expand the BPMN requirement ledger, MIWG ingestion coverage, and CIB assertion/fixture extraction one semantic capsule at a time.
...
- Last verified command: the complete `./scripts/verify.sh` gate passed after the ingestion change, including exact CMOF fact checks, source/compiler negative guards, exact four-target agreement, the classified seeded disagreement, two isolated Temporal executions, CIB cleanup, and new plus retained replay.
- Current state: the content-addressed BPMN fixture, exact-source importer, bounded CMOF manifest, sequential compiler, two draft spike profiles and neutral scenarios, lifecycle plus task-instance CIB calibration, production Lean lifecycle interpreter and proofs, dependency-free IR-consuming TypeScript lifecycle semantic core, deterministic IR-consuming Temporal Workflow host, pure comparator, live refinement equality, retained replay, diagnostic PVM projection, cleanup checks, provenance, and required phase timings are green.
- Next implementation target: add the User Task interaction identity, projection, exact completion, wrong-activation state-preservation theorem, and element-only checked non-law to Lean; no Search Attribute, inbox, actor, assignment, variable, or form semantics is approved.
