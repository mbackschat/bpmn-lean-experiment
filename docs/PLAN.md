...
The M5 flow-node occurrence metrics increment is implemented, closure-reviewed, evidence-closed, and graduated. Its Product 1 publication, Product 2 projection and aggregation, and local 1280/1600 functional UI lane are green. The active checkpoint is now the curated executable BPMN model corpus. Its first exact tranche contains five retained executable project models and seven external candidates across six deduplicated CIB Seven, OMG, and Betsy families, with a strict manifest, generated blocker ranking, and a five-model production pipeline green. One model is catalog-ready: the exact User Task assignment/form model now has a production-preview Chromium journey through deployment, start, claim-gated work, completion, Operations History, and Work audit.
...
Item 1 is **at its combined-profile and next-family research boundary**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and the current M5 observation increments are closed through the graduated [committed execution publication](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) and [flow-node occurrence metrics](capsules/FLOW-NODE-OCCURRENCE-METRICS-SPEC.md) specifications. The exact manifest, five retained models, seven external candidates across six clone families, strict Git/archive provenance guard, generated rankings, five-model production pipeline, and first exact production browser journey are implemented. Product 2 readiness is one, with four retained models still explicitly excluded from the browser catalog.

**Last verified commands:** `doctor.sh research` passed with external provenance ready. The corpus gate passed 11/11 in 13.0 seconds across five retained models and seven external admission candidates. M3 live integration passed 2/2 in 8.6 seconds. Its production-preview Chromium set passed 3/3 in 9.7 seconds against the local production composition, including the 1.9-second corpus journey and response-loss journey. The focused CI, documentation, source-hygiene, and feedback-policy gate passed 79/79 in 7.2 seconds. Playwright remains outside semantic verification. The last pre-policy hosted Ubuntu and macOS semantic verification was green at `577df49`; GitHub verification is Ubuntu-only from this policy change onward.
