...
| `INTERNAL-COMMUTATION` | `active` | Close all operation and RuntimeState classifications, arbitrary finite independent frontiers, region footprints, canonical publication order, explicit scheduled choice, and Temporal admission. | [Internal Commutation proposal](INTERNAL-COMMUTATION-PROPOSAL.md) |
...
- Lean scope-creation selection and retained-patch refinement. Command: `./scripts/lake.sh test`. Status: `exit 0`. Date: `2026-09-18`. Working-tree base: `2ce2eb5e`. All 371 library jobs pass; the new kernel witness also passes under the unchanged 3 GiB ceiling with zero pressure events and swap. Immutable resource binding, independent checkpoint review, and complete repository verification remain pending.
- TypeScript scope-creation preparation checkpoint. Command: `./scripts/pnpm.sh run test:pre-push`. Status: `exit 0`. Date: `2026-09-18`. Commit: `fc5fc6044a4c89b017026b8804ddb9e8a23b3d27`. The [review receipt](INTERNAL-COMMUTATION-PROPOSAL.md#independent-cold-review-receipt) records approval after whitespace-only correction `d8661a20`, the separately asserted clean complete gate, and its output digest. All 72 pipeline cases agree. Its 566.03-second elapsed time is correctness evidence, not a performance baseline.
...
Next action: bind immutable resource evidence and complete the independent review and repository gate for Lean [scope-creation selection and retained-patch refinement](INTERNAL-COMMUTATION-PROPOSAL.md#scope-creation-preparation-prerequisite). Then implement complete preparation, validity, frame, commutation, publication, and production scope/region batching. The [canonical scope and counter-storage prerequisite](INTERNAL-COMMUTATION-PROPOSAL.md#canonical-scope-and-counter-storage-checkpoint) remains independently approved and completely verified at `da9657e0`. Preserve exact complete-state equality, high-water records, and the distinction between collection algebra and transition preservation.

The [TypeScript and Lean mixed batch integration](INTERNAL-COMMUTATION-PROPOSAL.md#local-control-preparation-prerequisite) is implemented with canonical publication and whole-command rollback; the complete 886-test semantic-core gate and bounded Lean production witness pass. Restored stashes `f35bdd6b0424b2e3d5d06f101eb822bd285ab29e` and `76a2b017a630d23203ded482227382003f296f0b` remain retained. Keep token-unit multiplicity distinct from dependency-set membership. Regions, scheduled choice, and final closure remain open. Preserve the [3 GiB ceiling](CAPSULE-COST-LEDGER.md#package-wide-theorem-elaboration-memory-correction).
