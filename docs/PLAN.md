...
1. Prepare the User Task discovery/completion capsule proposal: identify the normative lifecycle clauses and pinned CIB witnesses, define the task identity and public observation contract, and compare Update versus Signal, Search Attribute projection, and production inbox boundaries without implementing an unapproved choice.
2. Once that capsule is approved, close the same fast lane in order: neutral scenario and CIB probe → Lean transition and law/non-law → executable IR extension and pure semantic core → Temporal adapter mechanics → differential, refinement, replay, and seeded-mutation evidence.
...
- Current state: the content-addressed BPMN fixture, exact-source importer, bounded CMOF manifest, sequential compiler, draft spike profile, calibrated CIB trace, production Lean interpreter and proofs, dependency-free IR-consuming TypeScript semantic core, deterministic IR-consuming Temporal Workflow host, pure comparator, live refinement equality, retained replay, diagnostic PVM projection, cleanup checks, provenance, and required phase timings are green.
- Next implementation target: write the User Task discovery/completion capsule proposal and its separating witnesses; no new lifecycle, Update/Signal, Search Attribute, or inbox semantics is approved yet.
