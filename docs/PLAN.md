...
1. **In progress: implement the M4 Product 2 incident-operations workspace.** The owner-approved [incident-operations proposal](BPM-PLATFORM-INCIDENT-OPERATIONS-PROPOSAL.md) selects exact private locator retention plus retrospective startup bootstrap, one dedicated current-incident Query, all-or-error Operate aggregation, configured operator authorization, durable content-bound Retry/Cancel actions without platform priority, separate paged incident audit, and an Operations collection/detail workspace. Checkpoint `6e2827b` plus correction `c941516` is independently approved; Product 2 persistence, actions, audit, HTTP, UI, and browser evidence are next.
...
**Next action: implement M4 Stage 3 Product 2 incident operations from approved checkpoint `6e2827b` and correction `c941516`.** Add confirmed-publication bootstrap and locator retention, all-or-error Operate aggregation, configured authorization, durable content-bound Retry/Cancel actions, separate incident audit, strict HTTP routes, the Operations workspace, and browser evidence without deriving an engine fact.

Item 1 is **in progress after the approved conditional semantic checkpoint**. Stages 1 and 2 are graduated. The checkpoint reads the canonical stable-state observation rather than reconstructing Retry/Cancel, rejects nonempty effect arguments and unsupported incident-state interactions, preserves direct/Message/Schedule locator bytes, corroborates terminal status with the exact retained receipt, and proves that the Query creates no Event History event while both histories replay. Correction `c941516` keeps generic Workflow-address minting and decoding private to Product 1. Product 2 persistence, authorization, actions, audit, HTTP, UI, and Playwright remain absent and are the active work.
