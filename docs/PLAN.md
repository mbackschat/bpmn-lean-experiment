...
**Next action: review the draft [parallel User Task metadata composition proposal](capsules/PARALLEL-USER-TASK-METADATA-COMPOSITION-PROPOSAL.md) at the combined-profile approval boundary before selecting another semantic family.** The existing contracts confirm that no second general catalog model works honestly today: Product 2 hides metadata-free tasks, ordinary showcase Workers fail closed on undeclared effects, and cosmetic sequential clones would fake breadth. The draft selects one separately authored parallel review model and one bounded profile that composes the approved two-branch fork/join account with the approved passive assignment/form publication. Its acceptance is a two-claim/two-completion production browser journey without reinterpreting the metadata-free fixture, and it explicitly refuses to claim arbitrary Process-data order independence. An immutable context-cold proposal review and owner approval are required before implementation. In the engine lane, classify Script Task execution against BPMN, CIB, deterministic sandboxing, data, security, and Temporal requirements before deciding whether its two-family prevalence justifies a proposal. The remaining operator-history and audit-export increments stay ordered after this corpus decision gate.
