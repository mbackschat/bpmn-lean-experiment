...
**The first A12 product-boundary checkpoint was rejected and its material proposal correction is independently approved.** Built-in profiles, scenarios, source readers, Lean, the semantic core, Temporal, CIB evidence, and product examples use product-neutral mapped-success and mapped-boundary-Error identities, but the shared Semantic Process schema still contains legacy A12 boundary IDs, the frozen export omits two retained-evidence dependencies, and the original payload-free Service Task byte-preservation promise conflicted with the mandatory `sourceOverlay` wire field. The reviewed correction preserves the atomic wire contract and narrows that promise to exact profile/BPMN bytes plus exactly bounded scenario/evidence changes. Owner approval is required before implementation resumes.
...
1. **In progress: correct the A12 product boundary without discarding the implemented slices.** Context-cold review rejected checkpoint `59bcefc` because a shared schema still encodes A12 boundary IDs, the neutrality and frozen-dependency inventories are incomplete, and the proposal's payload-free Service Task byte guarantee contradicts its mandatory wire replacement. The smallest coherent proposal correction is independently approved and awaits owner approval. After approval, fix the schema and guards at their shared roots, complete the frozen dependency closure, add the exact preservation oracle, and submit a new cold checkpoint. Closure documentation, cost, the complete gate, and closure review remain after checkpoint approval.
