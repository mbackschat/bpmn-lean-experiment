...
1. **In progress: review and select the next bounded M5 frequency and duration increment.** The source-grounded Product 2 preflight over current CIB Seven Cockpit documentation, the pinned pristine source, and an independent Camunda Optimize comparison is recorded in the [UI/UX research](research/BPM-PLATFORM-UI-UX-INFORMATION-ARCHITECTURE-RESEARCH.md#pattern-11-flow-node-metrics-require-occurrence-facts-not-transition-counts). The draft [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md) selects an exact Product 1 occurrence lifecycle and deterministic engine commit time before Product 2 frequency or duration. It is not approved or implemented. Do not infer duration from Temporal Event History, Product 2 ingestion time, or semantic logical time. The closure-reviewed [E1 specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md) remains the only committed semantic-history and current-position authority while completed operator history and audit export stay later M5 increments.
...
**Next action: complete context-cold review of the draft [flow-node occurrence metrics proposal](capsules/FLOW-NODE-OCCURRENCE-METRICS-PROPOSAL.md), integrate any required corrections, then request owner approval of its nine selected decisions.** No implementation begins before that approval. The remaining operator-history and audit-export increments stay ordered after it.

Item 1 is **preimplementation proposal review and contract selection in progress**. M4 is closed through the graduated [incident-operations specification](BPM-PLATFORM-INCIDENT-OPERATIONS-SPEC.md), and M5 E1 is closed through the graduated [committed execution publication specification](capsules/COMMITTED-EXECUTION-PUBLICATION-SPEC.md). The next proposal remains unapproved. Event History, state differencing, Product 2 ingestion time, and semantic logical time remain forbidden substitutes for an exact public wall-clock duration fact.
