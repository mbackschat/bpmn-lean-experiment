...
Next action: finish the public Compensation correction audit with the original reviewer and run clean committed integration. Exact terminal classification and registration-owner reconciliation are implemented. The affected Temporal package passes all component, concurrent, hosted and serial checks; infrastructure passes. The separately committed publication-poll correction retains page progress without changing deadlines. Earlier quiet-host subscription and Multi-Instance failures remain unexplained; retain those receipts and the overloaded-host/native-crash evidence in [the cost ledger](CAPSULE-COST-LEDGER.md#public-compensation-registration-2026-09-24), without claiming a runtime fix. Product 2 failed-Process support and bounded Transaction cancellation follow registration acceptance; no new proof infrastructure is selected.
