...
**Owner-approved on 2026-08-16 after independent proposal review and one correction round; implementation is in progress and closure review has not started.** The shared PostgreSQL runtime, checksum migration command, exact artifact storage, all twelve business-repository adapters, both Audit sinks, generic bounded recovery lease kernel, four Definitions recovery families, Operate and Work candidate discovery, and bounded Audit delivery are implemented with separate database-free and PostgreSQL 18 evidence. The remaining single-item handlers, recovery-worker composition, freshness-bounded reads, multi-replica closure evidence, and closure review are incomplete. This proposal selects the focused Product 2 architecture required by Horizon 1 of the owner-approved [Temporal BPMN execution scalability roadmap](TEMPORAL-BPMN-EXECUTION-SCALABILITY-PROPOSAL.md#horizon-1-remove-product-2s-single-node-scale-boundary). It changes no BPMN meaning, semantic profile, Product 1 Workflow behavior, public semantic observation, or current scalability claim.
