...
This lifecycle closes the M2 orphan-registration gap, not the direct-start route's caller-idempotency gap. If the direct-start response is lost, the accepted host instance still converges into Operate and Work, but the caller has no resource identity with which to retrieve that response. A later direct-start request intentionally creates a distinct semantic Process identity. Making direct start retry-transparent would require a separate public idempotency contract.
