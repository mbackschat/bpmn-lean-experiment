...
**Status:** Stage 1 completed; Stage 2 retained graph validation; Stage 2b closed declarative tail decomposition; Stage 2c closed whole-process graph coverage at 229 new nonblank lines; Stage 2d independently accepted after required gate and saturation-cost amendments at 127 new or materially rewritten nonblank lines; revised Stage 3a independently accepted and post-review cleanup closes at 276 new or materially rewritten nonblank lines; later Stage 3 work remains unapproved; full preservation remains unresolved and not adopted
...
[CheckedSourceFrontier.lean](../../BpmnSemantics/Experiments/CheckedSourceFrontier.lean) owns revised Stage 3a's graph-derived single-token frontier account. It bridges incoming and outgoing Sequence Flow identifiers to the full checked Flow lists, proves reusable distinct-key and order-independent `filterMap` isolation facts, extracts identifier distinctness and per-node arity from `sourceGraphWellFormed`, and proves `enabledTransitionsAtSingleToken` without a parser premise, selector-result premise, or node-order constraint. [CheckedSourceFrontierConformance.lean](../../BpmnSemantics/Experiments/CheckedSourceFrontierConformance.lean) kernel-checks that the retained parallel graph enables exactly its fork at the incoming frontier and enables nothing at a half-ready join. The focused main imports both modules. Default verification builds and executes that main, and an infrastructure guard locks both commands and both imports so the retained Boolean Stage 2 discriminators as well as the kernel-checked Stage 3a witnesses remain in the default lane.
...
The owner reopened the experiment on 2026-07-28 for Stage 1 and approved Stage 2 on 2026-07-29 under its 500-line ceiling. Independent review triggered the decomposition stop while accepting graph validation. The owner then approved Stage 2b under 250 lines; it stopped before fuel-complete reachability and independent review accepted the retained result. The owner approved Stage 2c under a separately anchored 230-line ceiling and Stage 2d under 150 lines on 2026-07-29. Stage 2c completed at 229 new nonblank lines. Independent review accepted Stage 2d with two required amendments: make its proof target part of default verification and hoist the reached set out of the per-edge certificate predicate. The amended stage closes at 127 new or materially rewritten nonblank lines with saturation-certified completeness, the fuel-one three-cycle discriminator, and all seven accepted program witnesses retained. Vertex-count fuel adequacy is deferred to optional Stage 2e as a no-false-rejection theorem. Independent review then returned the original Stage 3 bundle as unaffordable and dependency-inverted. The owner authorized only revised Stage 3a under 300 lines from commit `bb6b149`; independent review accepted it at 277 lines with no blocker, and the post-review cleanup closes at 276. Stage 3b, Stage 3c, Stage 3d, direct Timer/effect source semantics, and production source-admission widening remain unauthorized.
