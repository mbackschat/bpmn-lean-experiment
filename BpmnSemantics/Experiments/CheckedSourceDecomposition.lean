...
def parseFrom (source : CheckedProcess) :
  ...
  | fuel + 1, current =>
      ...
      | some (.dataInputUserTask ..)
      | some (.dataInputOutputUserTask ..)
      | some (.dataOutputUserTask ..)
      ...
      | some (.payloadMessageCatchEvent ..)
      | some (.correlatedPayloadMessageCatchEvent ..)
      | some (.receiveTask ..)
      | some (.configuredTask ..)
...
def composedNodeSurfaceValid : CheckedNode → Bool
  ...
  | .dataInputUserTask .. => false
  | .dataInputOutputUserTask .. => false
