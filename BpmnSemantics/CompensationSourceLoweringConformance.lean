...
/-! # Compensation source lowering checkpoint

Independent checked and Program fixtures for the owner-approved source checkpoint. The runtime
Compensation relation is deliberately not exercised here: this owner locks only strict decoding,
checked admission, canonical lowering, and complete cross-artifact definition binding.
-/
...
private def mutateRetentionLimits (records bytes : Nat) : Program :=
...
private def mutateSnapshotLimits (records bytes : Nat) : Program :=
...
private def mutateExecutionLimits (triggers handlers bytes : Nat) : Program :=
  match expectedProgram.compensationExecution with
  | none => expectedProgram
  | some declaration =>
      let updated :=
...
private def compensationAcceptedAs (json : Lean.Json)
    (expected : CheckedCompensation) : Bool :=
  match decodeCheckedCompensation json with
  | .ok actual => decide (actual = expected)
  | .error _ => false

private def compensationRejected (json : Lean.Json) : Bool :=
  match decodeCheckedCompensation json with
  | .ok _ => false
  | .error _ => true

private def checkedAccepted (json : Lean.Json) : Bool :=
...
/-- The complete checked artifact admits and lowers to the independently authored Program. -/
theorem exact_checked_source_admits_and_lowers :
    checkedWellFormed checkedProcess = true ∧
      lowerCheckedProcess checkedProcess = expectedProgram ∧
      programWellFormed expectedProgram = true ∧
      programProfileCapabilitiesValid expectedProgram = true ∧
      definitionBindingValid checkedProcess expectedProgram = true := by
  decide +kernel

/-- The checkpoint keeps Process start empty and old checked/Program declarations physically absent. -/
theorem empty_start_domain_and_old_omission_are_preserved :
    processDataBindingsAdmitted compensationSourceCheckpointProfileId .processStart [] = true ∧
      processDataBindingsAdmitted compensationSourceCheckpointProfileId .processStart
        [{ name := "unexpected", value := .string "value" }] = false ∧
      oldCheckedProcess.compensation = none ∧
      (lowerCheckedProcess oldCheckedProcess).compensationActivityRetention = none ∧
      (lowerCheckedProcess oldCheckedProcess).compensationEventSubProcessSnapshots = none ∧
      (lowerCheckedProcess oldCheckedProcess).compensationExecution = none := by
  decide +kernel

/-- Strict decoding accepts the exact declaration and preserves old physical omission. -/
theorem strict_decoder_accepts_exact_and_preserves_old_omission :
    compensationAcceptedAs compensationJson checkedCompensation = true ∧
      checkedAccepted checkedJsonWithCompensation = true ∧
      checkedAccepted oldCheckedJson = true := by
  decide +kernel

/-- Strict decoding rejects present null, an extra key, and both closed-discriminator drifts. -/
theorem strict_decoder_rejects_null_extra_and_discriminator_drift :
    compensationRejected .null = true ∧
      checkedAccepted (checkedJsonWithCompensation .null) = false ∧
      compensationRejected (compensationJson (includeExtra := true)) = true ∧
      compensationRejected (compensationJson (bodyKind := "otherEffect")) = true ∧
      compensationRejected (compensationJson (dependencyReason := "association")) = true := by
  decide +kernel

/-- Every fixed retention, snapshot, and execution limit is decoder-enforced. -/
theorem strict_decoder_rejects_every_fixed_limit_drift :
    compensationRejected (compensationJson (retentionRecords := 3)) = true ∧
      compensationRejected (compensationJson (retentionBytes := 4097)) = true ∧
      compensationRejected (compensationJson (snapshotRecords := 2)) = true ∧
      compensationRejected (compensationJson (snapshotBytes := 8193)) = true ∧
      compensationRejected (compensationJson (executionTriggers := 2)) = true ∧
      compensationRejected (compensationJson (executionHandlers := 4)) = true ∧
      compensationRejected (compensationJson (executionBytes := 20481)) = true := by
  decide +kernel

/-- Plausible endpoint and dependency reconstructions remain valid Programs but fail exact binding. -/
theorem binding_rejects_swapped_endpoint_and_reversed_dependency :
    programWellFormed swappedRestoredBindingProgram = true ∧
      definitionBindingValid checkedProcess swappedRestoredBindingProgram = false ∧
      programWellFormed reversedDependencyProgram = true ∧
      definitionBindingValid checkedProcess reversedDependencyProgram = false := by
  decide +kernel

/-- Every retained body, trigger, dormant-scope, and limit mutation fails the complete equality. -/
theorem binding_rejects_every_other_retained_mutation :
    definitionBindingValid checkedProcess substitutedEffectProgram = false ∧
      definitionBindingValid checkedProcess triggerFlowProgram = false ∧
      definitionBindingValid checkedProcess triggerScopeProgram = false ∧
      definitionBindingValid checkedProcess executableDormantScopeProgram = false ∧
      definitionBindingValid checkedProcess (mutateRetentionLimits 3 4096) = false ∧
      definitionBindingValid checkedProcess (mutateRetentionLimits 2 4097) = false ∧
      definitionBindingValid checkedProcess (mutateSnapshotLimits 2 8192) = false ∧
      definitionBindingValid checkedProcess (mutateSnapshotLimits 1 8193) = false ∧
      definitionBindingValid checkedProcess (mutateExecutionLimits 2 3 20480) = false ∧
      definitionBindingValid checkedProcess (mutateExecutionLimits 1 4 20480) = false ∧
