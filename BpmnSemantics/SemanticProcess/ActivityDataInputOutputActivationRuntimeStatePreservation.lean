...
/-! # Activity-data activation runtime-state preservation

Activating a data User Task inserts one joined wait, Activity record, and Activity-local
...
/-- ADINPUT-SCOPE-01, ADOUTPUT-ATOMIC-01, and ADIO-SCOPE-01 use the same unbounded task/Activity lifetime. Local bindings do not alter its identity or controller frames. -/
theorem unboundedTaskActivityInsertion_preserves_runtimeStateWellFormed
    (program : Program) (expectedInstanceId : SemanticId) (before : RuntimeState)
    (instanceId : SemanticId) (owner : ScopeOccurrenceId)
    (input output : ControlPlaceId) (taskId : TaskDefinitionId) (taskName : Option String)
    (bindings : List VariableBinding) (operation : SemanticOperation)
    ...
    (declaration : UnboundedUserTaskWaitDeclaration
      { processInstanceId := instanceId, owner, task := { id := taskId, name := taskName },
        activation := activationCount before taskId + 1, output } operation)
    (inputMember : input ∈ operationControlPlaces operation) :
    runtimeStateWellFormed program expectedInstanceId
      { before with
        tokens := removeToken before.tokens input owner
        waits := insertUserTaskWait
          { processInstanceId := instanceId, owner, task := { id := taskId, name := taskName },
            activation := activationCount before taskId + 1, output } before.waits
        activations := setActivationCount before.activations taskId (activationCount before taskId + 1)
        activityOccurrences := insertActivityOccurrence
          (dataInputOutputActivityRecord before instanceId owner taskId) before.activityOccurrences
        activityActivations := setActivationCount before.activityActivations taskId
          (activityActivationCount before taskId + 1)
        variables := addActivityOccurrenceVariableScope before.variables
          (dataInputOutputActivityOwner before instanceId taskId) bindings } = true := by
  let insertedWait : UserTaskWait :=
    { processInstanceId := instanceId, owner, task := { id := taskId, name := taskName },
      activation := activationCount before taskId + 1, output }
  ...
  let successor : RuntimeState :=
    { before with
      ...
      activityActivations := setActivationCount before.activityActivations taskId record.activation
      variables := addActivityOccurrenceVariableScope before.variables activityOwner bindings }
  change runtimeStateWellFormed program expectedInstanceId successor = true
  have sameScope : operationControlPlacesShareOwner operation = true := by
    cases declaration <;> rfl
  have declaresKey : operationDeclaresWaitKey operation (userTaskWaitDeclarationKey taskId) = true := by
    cases declaration <;> simp [operationDeclaresWaitKey, operationWaitDeclarationKeys,
      userTaskWaitDeclarationKey]
  ...
  obtain ⟨operationScope, _, operationOwner, inputOwner, _⟩ :=
    programGraphWellFormed_operationControlPlaceScope program operation input
      ...
      declared sameScope inputMember
  ...
  have incidentsAfter : effectIncidentAssociationsValid successor = true := by
    have activityFrame (effectId : EffectOccurrenceId) :
      ...
      change (insertActivityVariableScope
        { owner := .activityOccurrence activityOwner, bindings }
  ...
  have declarer : declaredByExactlyOneOwnedOperation program
    ...
    rw [userTaskWaitDeclarers_eq_keyFilter,
      programWellFormed_waitDeclarer program operation _ structural declared
        declaresKey]
    simp [declaredByExactlyOneOwnedOperation, operationOwningScope?,
  ...
  have claimsAfter : activityBodyClaimsUnique successor.activityOccurrences = true := by
    apply activityBodyClaimsUnique_insertActivityOccurrence _ _ _ claims
    rw [List.all_eq_true]
    intro existing member
    apply activityBodyClaimsDisjoint_userTask_of_not_mem record existing
      { processInstanceId := instanceId, elementId := ⟨taskId.value⟩,
        activation := activationCount before taskId + 1 }
    intro claimed
    obtain ⟨candidate, candidateMem, names⟩ := activityBodyTaskClaim_has_live_wait before
      existing _ bodies member claimed
    have taskBounds := bounds
    simp only [runtimeStateIdentityBound, Bool.and_eq_true] at taskBounds
    have bounded := List.all_eq_true.mp taskBounds.1.1 candidate candidateMem
  ...
  have disjoint : ∀ candidate ∈ program.operations,
    ...
    all_goals
      ...
      cases declaration <;> simp at conflict
...
/-- The composed activation retains its exact transition guarantee through the shared lifetime insertion. -/
  ...
  case activate =>
      ...
      | some owner =>
          ...
          | some binding =>
              ...
              · simp at success
              · cases success
                exact unboundedTaskActivityInsertion_preserves_runtimeStateWellFormed program
                  expectedInstanceId before instanceId owner input output taskId taskName
                  ...
                  (.awaitDataInputOutputUserTask id origin input output taskId taskName directInput
                    directOutput) wellFormed running selected declared
                  (.dataInputOutput id origin input directInput directOutput rfl rfl) (by left)
