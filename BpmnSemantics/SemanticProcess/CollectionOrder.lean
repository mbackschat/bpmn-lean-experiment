...
theorem all_occursOnce_erase [BEq α] [LawfulBEq α] (same : α → α → Bool)
...
theorem all_occursOnce_filter (same : α → α → Bool)
...
theorem orderedBy_erase [BEq α] [LawfulBEq α] (before : α → α → Bool)
    (compose : ∀ a b c, before b a = false → before c b = false → before c a = false)
