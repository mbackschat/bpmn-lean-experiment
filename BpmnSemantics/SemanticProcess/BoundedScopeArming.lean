...
def armScopeDeadline (state : RuntimeState) (owner : ScopeOccurrenceId)
    (origin : BpmnElementOrigin) (child : ScopeOccurrenceId)
  ...
  let deadlineId : OccurrenceId :=
    { processInstanceId := owner.processInstanceId
      ...
      activation }
  -- The Activity counter is keyed by the BPMN element, as in the TypeScript arming account.
  -- Definition-scope identifiers occupy another namespace and can equal an unrelated task ID;
  -- the 2026-09-22 bounded-entry reassessment records two valid-state counterexamples.
  ...
  let activityActivation := activityActivationCount state { value := origin.elementId.value } + 1
  { state with
    ...
    activityOccurrences := insertActivityOccurrence
      { processInstanceId := owner.processInstanceId
        activityElementId := { value := origin.elementId.value }
    ...
    activityActivations := setActivationCount state.activityActivations
      ⟨origin.elementId.value⟩ activityActivation
...
def armBoundedScopeState? (state : RuntimeState) (origin : BpmnElementOrigin)
  ...
  pure (armScopeDeadline entered parent origin child boundaryTimer)

/-- Atomic declarative arming relation with explicit parent ownership, the shared scope entry, and the exact resulting state. -/
inductive BoundedScopeArmingStep : RuntimeState → BpmnElementOrigin → ControlPlaceId → ControlPlaceId →
    DefinitionScopeId → BoundaryTimerArm → RuntimeState → Prop where
  | arm (before : RuntimeState) (origin : BpmnElementOrigin) (input childEntry : ControlPlaceId)
      (childScopeId : DefinitionScopeId) (boundaryTimer : BoundaryTimerArm)
      ...
      BoundedScopeArmingStep before origin input childEntry childScopeId boundaryTimer
        (armScopeDeadline entered parent origin child boundaryTimer)
...
theorem armBoundedScopeState_sound (before after : RuntimeState)
    ...
    (success : armBoundedScopeState? before origin input childEntry childScopeId
      boundaryTimer = some after) :
    BoundedScopeArmingStep before origin input childEntry childScopeId boundaryTimer after := by
  ...
  | some parent =>
      ...
      | some entered =>
          ...
          | some childId =>
              ...
              cases success
              exact .arm before origin input childEntry childScopeId boundaryTimer parent entered
...
theorem armBoundedScope_adds_one_deadline (state : RuntimeState)
    ...
    (armScopeDeadline state owner origin child boundaryTimer).timerWaits.length =
...
theorem armBoundedScope_records_one_occurrence (state : RuntimeState)
    (owner : ScopeOccurrenceId) (origin : BpmnElementOrigin)
    (child : ScopeOccurrenceId) (boundaryTimer : BoundaryTimerArm) :
    (armScopeDeadline state owner origin child boundaryTimer).activityOccurrences.length =
...
theorem armScopeDeadline_issues_fresh_activity (state : RuntimeState)
    ...
    activityIdentityIssuingDiscipline state
      (armScopeDeadline state owner origin child boundaryTimer) = true := by
