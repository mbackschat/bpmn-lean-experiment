...
theorem deliverCorrelatedPayloadMessage_preserves_runtimeStateWellFormed
  ...
  | commit candidate wait projected rechecked selected ordinary unattached =>
      ...
      have legacy : program.identity.semanticProfile ≠ repeatableSubscriptionCheckpointProfileId := by
        rw [profile]
        simp [messageKeyCorrelationProfileId, repeatableSubscriptionCheckpointProfileId]
      ...
      have orderAfter : canonicalCollectionOrder settled = true := by
        ...
        refine ⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨orderedBy_addToken _ wait.output wait.owner tokenOrder,
          activityCounterOrder⟩, taskOrder⟩, activationOrder⟩, ?_⟩, timerOrder⟩,
