...
- The [Message key-correlation checkpoint source](message-key-correlation/README.md) supplies only the exact settlement-confirmation BPMN graph for source/checked/IL and semantic-checkpoint evidence. It is not a registered target scenario before independent checkpoint approval.
