...
- The [Message key-correlation population scenarios](message-key-correlation/README.md) supply one exact settlement-confirmation graph, a same-local-id second definition, and answer-free unique, zero, ambiguous, and cross-definition schedules through a separate closed population contract. Lean, the core, and Temporal are selected explicitly; CIB is explicitly absent.
...
- The [interrupting Activity boundary Message scenarios](activity-boundary-message/README.md) order exact task completion and payload-free Message delivery first in turn, then submit the losing stale stimulus while the selected follow-on task keeps the Process open. Both cases compare Lean, the core, and Temporal with no CIB target.
