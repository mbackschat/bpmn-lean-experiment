...
- The [interrupting Activity boundary Message checkpoint source](activity-boundary-message/README.md) supplies one omission-only, payload-free, operation-addressed Message handler on a User Task. It remains a source/admission fixture with no registered scenario or Temporal lane until semantic-checkpoint approval.
