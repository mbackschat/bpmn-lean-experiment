...
This directory contains one exact BPMN 2.0.2 invoice-review source and three answer-free scenarios for the [Activity data-input mediation specification](../../docs/capsules/ACTIVITY-DATA-INPUT-MEDIATION-SPEC.md). The User Task declares one required scalar `DataInput`, one `InputSet`, one empty `OutputSet`, and one direct `DataInputAssociation` from the Process-owned `Property_ReviewContext`.
