...
This directory contains the exact admitted two-Process BPMN source and one answer-free schedule for the [bounded Call Activity proposal](../../docs/capsules/CALL-ACTIVITY-PROPOSAL.md). The caller invokes the called Process, the scenario completes `CalledTask` with the derived called semantic Process-instance ID, then completes `CallerTask` with the original caller/root ID.
