...
- [cibseven-evidence.json](cibseven-evidence.json) separately retains the expected canonical CIB result, producer environment, projection version, and exact scenario digest.
...
The retained evidence trace is calibrated by the pinned CIB Seven M0.2 oracle. It records successful deployment, committed start, one active User Task with one enabled completion stimulus, committed completion, and completed Process status at unchanged logical time. The Java verifier parses the evidence into the typed canonical vocabulary and compares it with two isolated engine runs without passing the answer to the runner.
