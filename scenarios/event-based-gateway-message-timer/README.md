...
This directory contains one exact BPMN 2.0.2 source and two answer-free scenarios for the [Event-Based Gateway proposal](../../docs/capsules/EVENT-BASED-GATEWAY-PROPOSAL.md). The Message-wins scenario orders the exact operation-addressed delivery before User Task completion. The Timer-wins scenario orders the exact `PT1S` firing before its User Task completion. The target inputs contain no expected result or winner annotation.
